...
<?
	...
	$s = "SELECT prefijosucursal,cliente,folio,fechaguia,fechafactura,fechavenc,diasvencidos,alcorriente,c1a15dias,c16a30dias,
		c31a60dias,may60dias,sobrepeso,saldo,factura,contrarecibo,idcliente,tcargo 
		FROM (
		...
		SUM(ge.texcedente) AS sobrepeso,SUM($if) AS saldo,0 AS factura,IFNULL(ge.acuserecibo,0)AS contrarecibo,temp.idcliente,'' AS tcargo
		FROM guiasempresariales ge 
		...
		WHERE ge.tipopago='CREDITO' AND (ge.tipoguia='CONSIGNACION' OR (ge.tipoguia='PREPAGADA' AND (ge.texcedente>0 OR ge.tseguro>0))) 
		AND ISNULL(ge.factura) $folioCliente $andsucursal1 GROUP BY ge.id
		UNION
		...
		SUM(gv.texcedente) AS sobrepeso,SUM(gv.total) AS saldo,0 AS factura,IFNULL(gv.acuserecibo,0)AS contrarecibo,temp.idcliente,
		IF(IF(gv.tipoflete=0,gv.idsucursalorigen,gv.idsucursaldestino)!=pg.sucursalacobrar,'TRAS','') tcargo 
		...
		WHERE gv.condicionpago=1 AND ISNULL(gv.factura) AND gv.estado!='CANCELADO' $folioCliente $andsucursal2 GROUP BY gv.id
		UNION
		...
		DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))<16,
		(IFNULL(f.total,0) + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)),0)) AS c1a15dias,
		SUM(IF(DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))>15 AND
		DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))<31,
		(IFNULL(f.total,0) + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)),0)) AS c16a30dias,
		SUM(IF(DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))>30 AND
		DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))<61,
		(IFNULL(f.total,0) + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)),0)) AS c31a60dias,
		...
		IFNULL(f.otrosmontofacturar,0)),0)) AS may60dias,SUM(f.excedente) AS sobrepeso,
		SUM(IFNULL(f.total,0) + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)) AS saldo,
		IFNULL(f.folio,'')AS factura,0 AS contrarecibo,temp.idcliente,'' AS tcargo
		FROM facturacion f 
		INNER JOIN catalogosucursal cs ON f.idsucursal=cs.id
		INNER JOIN tmp_clientes$x temp ON f.cliente=temp.idcliente
		WHERE f.credito='SI' AND ISNULL(f.fechacancelacion) AND f.estadocobranza <> 'C' AND f.tipoguia='empresarial' $andsucursal3 $folioCliente  GROUP BY f.folio
		UNION
		SELECT cs.prefijo AS prefijosucursal,temp.ncliente AS cliente,fd.folio,gv.fecha AS fechaguia,f.fecha AS fechafactura,
		IFNULL(ADDDATE(gv.fecha,INTERVAL temp.dcredito DAY),'') AS fechavenc,
		IF(DATEDIFF(CURRENT_DATE(),ADDDATE(gv.fecha,INTERVAL temp.dcredito DAY))>0,
		DATEDIFF(CURRENT_DATE(),ADDDATE(gv.fecha,INTERVAL temp.dcredito DAY)),0) AS diasvencidos,
		SUM(IF(DATEDIFF(CURRENT_DATE(),ADDDATE(gv.fecha,INTERVAL IFNULL(temp.dcredito,0) DAY))<=0,fd.total,0)) AS alcorriente,
		SUM(IF(DATEDIFF(CURRENT_DATE(),ADDDATE(gv.fecha,INTERVAL temp.dcredito DAY))>0 AND
		DATEDIFF(CURRENT_DATE(),ADDDATE(gv.fecha,INTERVAL temp.dcredito DAY))<16,fd.total,0)) AS c1a15dias,
		SUM(IF(DATEDIFF(CURRENT_DATE(),ADDDATE(gv.fecha,INTERVAL temp.dcredito DAY))>15 AND
		DATEDIFF(CURRENT_DATE(),ADDDATE(gv.fecha,INTERVAL temp.dcredito DAY))<31,fd.total,0)) AS c16a30dias,
		SUM(IF(DATEDIFF(CURRENT_DATE(),ADDDATE(gv.fecha,INTERVAL temp.dcredito DAY))>30 AND
		DATEDIFF(CURRENT_DATE(),ADDDATE(gv.fecha,INTERVAL temp.dcredito DAY))<61,fd.total,0)) AS c31a60dias,
		SUM(IF(DATEDIFF(CURRENT_DATE(),ADDDATE(gv.fecha,INTERVAL IFNULL(temp.dcredito,1) DAY))>60,fd.total,0)) AS may60dias,
		SUM(fd.excedente) AS sobrepeso,SUM(fd.total) AS saldo,IFNULL(f.folio,'')AS factura,0 AS contrarecibo,temp.idcliente,'' AS tcargo 
		FROM facturacion f 
		...
		GROUP BY fd.folio)t ORDER BY cliente,idcliente,fechaguia ";
	$r  = mysql_query($s,$l) or die($s);
?>
	<table width="1211">
    	<tr>
        	<td colspan="16" style="font-weight:bold; font-size:16">TITULO: ANTIGUEDAD DE SALDOS</td>
        </tr>
    	<tr>
    	  <td width="86" style="font-weight:bold; border:#000 solid 1px">SUCURSAL</td>
		  <td width="98" style="font-weight:bold; border:#000 solid 1px">GUIA/FACTURA</td>
    	  <td width="77" style="font-weight:bold; border:#000 solid 1px">FECHA</td>
		  <td width="103" style="font-weight:bold; border:#000 solid 1px">FECHA FACT</td>
    	  <td width="103" style="font-weight:bold; border:#000 solid 1px">FECHA VTO.</td>
		  ...
		  <td width="87" style="font-weight:bold; border:#000 solid 1px">SOBREPESO</td>
    	  <td width="87" style="font-weight:bold; border:#000 solid 1px">SALDO</td>
    	  <td width="60" style="font-weight:bold; border:#000 solid 1px">FACTURA</td>
    	  <td width="98" style="font-weight:bold; border:#000 solid 1px">CONTRARECIBO</td>
		  <td width="71" style="font-weight:bold; border:#000 solid 1px">TRASPASO CARGO</td>
      </tr>
      	<?
		$arre = array();
		$arr = 0;
		...
		while($f = mysql_fetch_array($r)){
			...
			if($cliente!=cambio_texto($f[cliente]) || $nocliente!=cambio_texto($f[idcliente])){
				if($cliente!=""){
				?>
				<tr>
				  <td style="border-bottom: 1px #000 solid; border-left:1px #000 solid; font-weight:bold">TOTAL <?=cambio_texto($cliente)?></td>
				  <td style="border-bottom: 1px #000 solid; border-left:1px #000 solid; font-weight:bold"></td>
				  <td style="border-bottom: 1px #000 solid; border-left:1px #000 solid; font-weight:bold"></td>
				  <td style="border-bottom: 1px #000 solid; border-left:1px #000 solid; font-weight:bold"></td>
				  <td style="border-bottom: 1px #000 solid; border-left:1px #000 solid; font-weight:bold"></td>
				  ...
				  <td style="border-bottom: 1px #000 solid; font-weight:bold"><?='$ '.number_format($sobrepeso,2)?></td>
              ...
              <tr>
              	  <td style="border-top:#000 1px solid; border-left:#000 1px solid">&nbsp;</td>
			  ...
			  <?
				...
				$nocliente=cambio_texto($f[idcliente]);
				?>
              <tr>
                  <td style="border-top:#000 1px solid; border-left:#000 1px solid;font-weight:bold"><?=cambio_texto($f[cliente])?></td>
				  <td style="border-top:#000 1px solid">&nbsp;</td>
			...
			<tr>
				  ...
				  <td><?=$f[5]?></td>
				  <td><?=$f[6]?></td>
				  <td><?='$ '.number_format($f[7],2)?></td>
				  <td><?='$ '.number_format($f[8],2)?></td>
				  <td><?='$ '.number_format($f[9],2)?></td>
				  <td><?='$ '.number_format($f[10],2)?></td>
				  <td><?='$ '.number_format($f[11],2)?></td>
				  <td><?='$ '.number_format($f[12],2)?></td>
				  <td><?='$ '.number_format($f[13],2)?></td>
				  <td><?=$f[14]?></td>
				  <td><?=$f[15]?></td>
				  <td style=" border-right:1px solid #000"><?=$f[17]?></td>
			  </tr>
			<?
				$var_alco  	+= $f[7];
				$var_115   	+= $f[8];
