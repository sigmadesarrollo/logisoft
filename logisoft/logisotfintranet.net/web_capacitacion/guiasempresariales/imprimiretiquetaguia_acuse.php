...
<?
	if($_GET[tipo]==1){//GUIAS DE VENTANILLA
		...
		$s = "SELECT
		ge.id, gr.numerorastreo, DATE_FORMAT(ge.fecha, '%d/%m/%Y') AS fecha, 
		DATE_FORMAT(ADDDATE(ge.fecha, INTERVAL IF(ge.ocurre=0, entregaead/24, entregaocurre/24) DAY), '%d/%m/%y') AS estimado,
		IF(ge.ocurre=0,'EAD','OCURRE') AS tipoentrega,
		csd.prefijo AS destino, cso.prefijo AS origen,
		DATE_FORMAT(ge.fecha, '%d/%m/%Y') AS fecha,
		ge.totalpaquetes, ge.totalvolumen, totalpeso, 
		ge.tflete, ge.trecoleccion, ge.tcostoead, ge.tseguro,
		ge.tcombustible, ge.subtotal, ge.tiva, ge.ivaretenido, 
		ge.ttotaldescuento, ge.totros, ge.texcedente, ge.tcombustible,
		ge.total, ge.valordeclarado, DATE_FORMAT(ge.hora_registro, '%H:%i') AS hora,
		ge.idremitente, 
		...
		INNER JOIN catalogosucursal AS csd ON ge.idsucursaldestino = csd.id
		INNER JOIN catalogosucursal AS cso ON ge.idsucursalorigen = cso.id
		INNER JOIN catalogodestino AS cd ON ge.iddestino = cd.id
		LEFT JOIN guia_rastreo AS gr ON ge.id = gr.noguia
		INNER JOIN catalogocliente AS ccr ON ge.idremitente = ccr.id
		LEFT JOIN direccion AS dr ON ge.iddireccionremitente = dr.id
		INNER JOIN catalogocliente AS ccd ON ge.iddestinatario = ccd.id
		LEFT JOIN direccion AS dd ON ge.iddirecciondestinatario = dd.id
		WHERE ge.id='$_GET[codigo]'";
...
<tr>
  ...
  <td height="57" valign="top" class="texto_bold2" align="CENTER">
  	<img src="imagen.php" width="70px" height="70px" />
  </td>
</tr>
</table>
<p>&nbsp;</p>
