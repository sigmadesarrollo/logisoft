<?	session_start();
	...
	if($_GET[accion]==1){//OBTENER DATOS GENERALES -MODULOQUEJASDAÑOSFALTANTES.PHP
		...
		if($_GET[tipo]=="grabar"){
			...
			$sqlnuevo="INSERT INTO moduloquejasdanosfaltantes (folio, fecharegistro, estado, idsucursal,nguia,idcliente,remitentecliente, nombre, observaciones, relacionembarque, copiafactura,confirmacopiafactura, comentariogerente,confirmacomentariogerente, cartareclamacion, reportedanosyfaltantes,confirmareportedanosyfaltantes, idresponsable,quejas,fechaposible, idusuario, usuario, fecha)	VALUES
		(NULL, CURRENT_DATE, 'REVISION COMITE', '".$_SESSION[IDSUCURSAL]."',
		 '".$_GET[guia]."','".$_GET['cliente']."' ,'".$_GET['remitente_cliente']."',
		 UCASE('".$_GET[nombre]."'), UCASE('".$_GET[observaciones]."'), '".$_GET[relacionembarque]."', '".$_GET[copiafactura]."','".$_GET[confirmacopiafactura]."','".$_GET[comentariogerente]."','".$_GET[confirmacomentariogerente]."' , '".$_GET[cartareclamacion]."', '".$_GET[reportedanosyfaltantes]."','".$_GET[confirmareportedanosyfaltantes]."', '".$_GET[responsable]."','QUEJAS DAÑOS Y FALTANTES','".cambiaf_a_mysql($fecha)."','".$_SESSION[IDUSUARIO]."','".$_SESSION[NOMBREUSUARIO]."',CURRENT_DATE)";
		...
		mq.idsucursal,cs.descripcion AS sucursal,mq.nguia,g.estado AS estadoguia,mq.idcliente,
		...
		INNER JOIN (SELECT id,estado FROM  guiasempresariales 
		UNION SELECT id,estado FROM guiasventanilla) AS g ON mq.nguia = g.id
