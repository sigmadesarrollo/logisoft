<?	session_start();
	...
	if($_GET[accion] == "0"){//BORARR TABLA LIQUIDACION COBRANZA DETALLE
		...
		$registros = array();
		$row = ObtenerFolio('liquidacioncobranza','webpmm');
		...
		WHERE rc.folio = ".$_GET[folio]." AND rc.sucursal=".$_GET[sucursal]." AND NOT EXISTS (SELECT * FROM liquidacioncobranza lq WHERE rc.folio=lq.foliocobranza AND lq.estado='LIQUIDADO')";
		//echo $s;
		$r = mysql_query($s,$l) or die($s);
		$registros = array();
		...
		$s = "SELECT rd.cliente, rd.guia, DATE_FORMAT(rd.fechaguia,'%d/%m/%Y') AS fecha,
		DATE_FORMAT(rd.fechavencimiento,'%d/%m/%Y') AS fechavencimiento, rd.factura,
		rd.importe, rd.saldo AS saldoactual,
		...
		INNER JOIN relacioncobranzadetalle rd ON rc.folio = rd.relacioncobranza
		...
		WHERE rc.folio = ".$_GET[folio]." AND rc.sucursal=".$_GET[sucursal]." GROUP BY rd.guia"; 
		$r = mysql_query($s,$l) or die($s);
		$registros = array();
			while($f = mysql_fetch_object($r)){				
				$registros[] = $f;
		...
		$s = "INSERT INTO liquidacioncobranzadetalle_tmp SELECT 0 AS id,
		".$_GET[folio]." AS foliorelacion, rd.cliente, rd.guia, rd.fechaguia,
		rd.fechavencimiento, rd.factura, rd.importe, rd.saldo AS saldoactual,
		IF(rcon.fecharegistro IS NULL,'NO','SI') AS revision,
		/*IF(rcon.fecharegistro IS NULL,'NO',rcon.fecharegistro) AS revision,*/
		'NO' AS cobrar,		
		...
		CURRENT_TIMESTAMP as fecha,0 as motivo FROM relacioncobranza rc
		INNER JOIN relacioncobranzadetalle rd ON rc.folio = rd.relacioncobranza
		LEFT JOIN registrodecontrarecibos rcon ON rd.factura = rcon.factura
		LEFT JOIN registroscompromisos rcom ON rd.factura = rcom.factura
		WHERE rc.folio = ".$_GET[folio]." group by rd.guia";
		mysql_query($s,$l) or die($s);
		...
		(estado,fechaliquidacion,sucursal,foliocobranza,cobrador,cantidadentregada,diferencia,idusuario,fecha) VALUES
		(".(($_GET[estado]!="")? "'LIQUIDADO'" : "'GUARDADO'" ).",'".cambiaf_a_mysql($arr[0])."',
		".$arr[1].",".$arr[2].",".$arr[3].",'".$_GET[cantidadentregada]."',
		'".(($_GET[diferencia] < 0)?0:$_GET[diferencia])."', ".$_SESSION[IDUSUARIO].",CURRENT_TIMESTAMP())";
		$r = mysql_query($s,$l) or die($s);
		$folio = mysql_insert_id();
		
		$s = "UPDATE relacioncobranza set aplicada = 'S' where folio = ".$arr[2];
		mysql_query($s,$l) or die($s);
		
		$s = "INSERT INTO liquidacioncobranzadetalle SELECT 0 AS id, ".$folio." AS folioliquidacion,
		...
		idusuario, fecha,motivo FROM liquidacioncobranzadetalle_tmp
		...
		diferencia = '".(($_GET[diferencia] < 0)?0:$_GET[diferencia])."' WHERE folio = ".$_GET[folio]."";
		$r = mysql_query($s,$l) or die($s);
		...
		if($cant>0){
			...
			mysql_query($s,$l) or die($s);
			$s = "INSERT INTO liquidacioncobranzadetalle SELECT 0 AS id, ".$_GET[folio]." AS folioliquidacion,
			foliorelacion, cliente, guia, fechaguia, fechavencimiento,
			factura, importe, saldoactual, revision, cobrar, contrarecibo,
			compromiso, efectivo, cheque, banco, ncheque, tarjeta,transferencia,notacredito,nnotacredito,
			idusuario, CURDATE(),motivo FROM liquidacioncobranzadetalle_tmp
			WHERE idusuario = ".$_SESSION[IDUSUARIO]."";
			mysql_query($s,$l) or die($s);
		}
			...
			ld.tarjeta,ld.transferencia,ld.cheque,ld.ncheque,ld.banco,ld.notacredito,ld.nnotacredito,lc.sucursal,
			lc.idusuario,lc.fecha,ld.cliente FROM liquidacioncobranza lc
			INNER JOIN liquidacioncobranzadetalle ld ON lc.folio=ld.folioliquidacion 
			WHERE lc.folio=".$_GET[folio]." and ld.cobrar='SI' GROUP BY ld.factura";
		...
		WHERE lc.folio=".$_GET[folio]." and lcd.cobrar='SI' GROUP BY lcd.guia,lcd.factura";
		$r = mysql_query($sql,$l) or die($sql);
		...
		while ($f = mysql_fetch_object($r)){
				$s="UPDATE pagoguias
					...
					WHERE folio='$f->factura'";
					mysql_query($s,$l) or die($s);
					
					$sq="UPDATE pagoguias SET pagado='S',fechapago=CURRENT_DATE,usuariocobro='".$_SESSION[IDUSUARIO]."',
					sucursalcobro='".$_SESSION[IDSUCURSAL]."' WHERE guia='$f->factura' and tipo='FACT'";		
	...
	arr[4] = tarjeta; arr[5] = transferencia;*/
		...
		WHERE idusuario=".$_SESSION[IDUSUARIO]." GROUP BY guia";
		$r = mysql_query($sql,$l) or die($sql);
		$registros = array();
			if(mysql_num_rows($r)>0){
				while($f = mysql_fetch_object($r)){
		...
		WHERE factura=".$_GET[factura]." AND idusuario=".$_GET[usuario]."";
		$r = mysql_query($s,$l) or die($s);
		$registros = array();
			while($f = mysql_fetch_object($r)){
				$registros[] = $f;
