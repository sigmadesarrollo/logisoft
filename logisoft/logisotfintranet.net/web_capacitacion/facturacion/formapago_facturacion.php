<?	session_start();
	...
	$l = Conectarse("webpmm");	$sql = mysql_query("SELECT idcliente FROM generacionconvenio WHERE idcliente='".$_GET[cliente]."'",$l);
	...
	function validarTotales(){
		...
		if(u.total.value != u.acumulado.value){
			...
			alerta("Faltan "+(Math.round(comparacion*100)/100).toLocaleString().replace("-","")+" pesos para alcanzar el total","¡Atencion!","efectivo");	
				...
				up.conformapago.value			= "SI";
				parent.VentanaModal.cerrar();
				parent.guardarFacturar();
			}
		}else{
			up = parent.document.all;	
			if (cambio>0){
    ...
    <table width="405" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
          ...
          <td height="147"><br><table width="362" border="0" align="center" cellpadding="0" cellspacing="0">
                  <tr>
                    ...
                    <td colspan="3"><input tabindex="0" onKeyPress="return Numeros(event)" onKeyDown="return tabular(event)" name="total" type="text" id="total" class="Tablas"  style="font:tahoma; font-size:9px; background:#FFFF99; text-align:right" readonly="true" value="$ <?=number_format($_GET[total],2,".",","); ?>" size="15"   ></td>
                  </tr>
                  <tr>
