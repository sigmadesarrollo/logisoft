<?
	...
	if($_GET[accion] == 1){//PRINCIPAL CLIENTES
		$s = "SELECT t2.sucursal,CONCAT('#%',IFNULL(SUM(IF(t1.estadoconvenio='ACTIVADO' AND YEAR(t1.fecha)='".$_GET[fecha]."',1,0)),0)) AS vigentes,
		CONCAT('#%',IFNULL(SUM(IF(t1.estadoconvenio='EXPIRADO' AND YEAR(t1.fecha)='".$_GET[fecha]."',1,0)),0)) AS vencidos,
		CONCAT('#%',IFNULL(SUM(IF((t1.estadoconvenio='EXPIRADO' OR t1.estadoconvenio='ACTIVADO') AND YEAR(t1.fecha)='".$_GET[fecha]."',1,0)),0)) total, 
		...
		WHERE IF(tipoventa <> 'GUIA VENTANILLA',YEAR(fechafacturacion)='".$_GET[fecha]."', YEAR(fecharealizacion)='".$_GET[fecha]."') AND activo='S' AND
		(NOT ISNULL(convenio) AND convenio!=0) GROUP BY sucursal) AS t2 ON t1.sucursal=t2.idsucorigen
	   ...
	   <Row ss:Index="5" ss:Height="13.5">
		...
		$s = "SELECT (t0.prefijo) AS prefijosucursal,SUM(IFNULL(t1.tfacturado,0)) AS facturado,
		...
		GROUP BY idsucorigen) t1 ON t0.id=t1.idsucorigen WHERE t1.tfacturado>0 OR t1.tnofacturado>0 GROUP BY prefijosucursal ORDER BY prefijosucursal";	
	   ...
	   <Row ss:Height="13.5">
		...
		(NOT ISNULL(convenio) AND convenio!=0) AND (NOT ISNULL(factura)) GROUP BY prefijosucursal ORDER BY prefijosucursal";
		$r = mysql_query($s,$l) or die($s);		
		$filas = mysql_num_rows($r);
	   ...
	   <Row ss:Index="5" ss:Height="13.5">
		...
		FROM generacionconvenio WHERE YEAR(fecha)= '".$_GET[fecha]."' AND estadoconvenio!='AUTORIZADO' AND 
		...
		)t INNER JOIN catalogosucursal cs ON t.sucursalacobrar=cs.id GROUP BY sucursalacobrar ORDER BY prefijo";
		$r = mysql_query($s,$l) or die($s);
		$filas = mysql_num_rows($r);
	   ...
	   <Row ss:Index="5" ss:Height="13.5">
		...
		$s = "SELECT cs.prefijo AS prefijosucursal,CONCAT('#',temp.idcliente),temp.cliente,CONCAT('#',sge.id) AS nventa,
		CONCAT('#',IFNULL(sge.cantidad,0)) cantidad,CONCAT(sge.desdefolio,'-',sge.hastafolio) pfolios,sge.fecha,
		IFNULL(SUM(sge.subtotal),0) flete,t.excedente AS sobrepeso,t.seguro AS costoseguro,t.costoead AS costoead,SUM(sge.total) total
	  ...
	  $s = "DROP TABLE tmp_convenio$x;";
		...
		$s = "SELECT cs.prefijo AS prefijosucursal,CONCAT('#',tc.idcliente),tc.cliente,SUM(ge.tflete) porfacturar,
		SUM(IFNULL(ge.texcedente,0)) sobrepeso,SUM(IFNULL(ge.tseguro,0)) valordeclarado,SUM(IFNULL(ge.tcostoead,0)) costoead,
		SUM(ge.total) total FROM guiasempresariales ge INNER JOIN pagoguias pg ON ge.id = pg.guia
		...
		AND NOT ISNULL(ge.idremitente) AND NOT ISNULL(ge.total) AND ge.total!=0 GROUP BY tc.idcliente ORDER BY cliente ";
		$r = mysql_query($s,$l) or die($s);		
		$filas = mysql_num_rows($r);
	   ...
	   <Row ss:Index="5" ss:Height="13.5">
		...
		$s = "SELECT (prefijoorigen) AS prefijosucursal,CONCAT('#',t2.idcliente),CONCAT(t2.nombre,' ',t2.apaterno,' ',t2.amaterno) cliente,
		...
		SUM(IF(t1.tipoventa='GUIA VENTANILLA',total,0)) AS normales, 
		SUM(IF(t1.tipoventa='SOLICITUD DE FOLIOS',total,0)) AS prepagadas, 
		SUM(IF(t1.tipoventa='GUIA EMPRESARIAL' AND t1.tipoempresarial='CONSIGNACION',total,0)) AS consignacion,
		SUM(IF(t1.tipoventa='FACTURA OTROS' OR t1.tipoventa='FACTURA EXCEDENTE',total,0)) AS otros,
		SUM(IF(t1.tipoventa='GUIA VENTANILLA',total,0) + IF(t1.tipoventa='SOLICITUD DE FOLIOS',total,0) +
		IF(t1.tipoventa='GUIA EMPRESARIAL' AND t1.tipoempresarial='CONSIGNACION',total,0) + 
		IF(t1.tipoventa='FACTURA OTROS' OR t1.tipoventa='FACTURA EXCEDENTE',total,0)) AS total
		...
		GROUP BY t1.idcliente ORDER BY prefijosucursal";
		$r = mysql_query($s,$l) or die($s);	
		$filas = mysql_num_rows($r);
	   ...
	   <Row ss:Index="5" ss:Height="13.5">
		...
		WHERE YEAR(fecha)= '".$_GET[fecha]."' AND estadoconvenio!='AUTORIZADO' AND estadoconvenio!='IMPRESO' AND estadoconvenio!='NO ACTIVADO' 
		GROUP BY idcliente;";
		mysql_query($s,$l) or die($s); 
		//agregar el convenio y la sucursal
		$s = "UPDATE tmp_convenio temp INNER JOIN generacionconvenio gc ON temp.folio=gc.folio
