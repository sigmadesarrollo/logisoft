<?	session_start();
	...
	$s = "DELETE FROM cobranza60dias_tmp WHERE idusuario = ".$_SESSION[IDUSUARIO]."";
	mysql_query($s,$l) or die($s);
	
		$s = "INSERT INTO cobranza60dias_tmp(id, factura, fechaemision, cliente, nombre,
		importe, tipofactura, fechavencimiento, diasatraso, contrarecibo, diapago, idusuario, idsucursal)																																																															  		SELECT 0 AS id, f.folio AS factura,gv.fecha AS fechaemision, f.cliente,
		...
		".(($_SESSION[IDSUCURSAL]!=1)? " AND f.idsucursal = ".$_SESSION[IDSUCURSAL]."":"")." GROUP BY f.folio 
	UNION
		SELECT 0 AS id, f.folio AS factura,gv.fecha AS fechaemision, f.cliente,
...
<script>
	...
	function obtenerDetalle(){
		consultaTexto("resTabla1","consultasAlertas.php?accion=26&contador="+u.pag1_contador.value
		+"&s="+Math.random());
	}
	...
	function paginacion(movimiento){
		switch(movimiento){
			case 'primero':
				consultaTexto("resTabla1","consultasAlertas.php?accion=26&contador=0&s="+Math.random());
				break;
			case 'adelante':
				if(u.pag1_adelante.value==1){
					consultaTexto("resTabla1","consultasAlertas.php?accion=26&contador="+(parseFloat(u.pag1_contador.value)+1)					
					+"&s="+Math.random());
				}
				break;
			case 'atras':
			...
			case 'ultimo':
				...
				consultaTexto("resTabla1","consultasAlertas.php?accion=26&contador="+contador
				+"&s="+Math.random());
				break;
		}
	}
	...
	function imprimirReporte(){
		if(tabla1.getRecordCount()>0){
			if(document.URL.indexOf("web_capacitacionPruebas/")>-1){		
				var v_dir = "https://www.pmmintranet.net/web_capacitacionPruebas/alerta/";
			}else if(document.URL.indexOf("web_capacitacion/")>-1){
				var v_dir = "https://www.pmmintranet.net/web_capacitacion/alerta/";
			}else if(document.URL.indexOf("web_pruebas/")>-1){
				var v_dir = "https://www.pmmintranet.net/web_pruebas/alerta/";
			}
			window.open(v_dir+"generarExcelCobranza60.php");
		}else{
...
<form id="form1" name="form1" method="post" action="">
  <table width="550" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
    <tr>
      <td class="FondoTabla">COBRANZA 60 DIAS </td>
    </tr>
    <tr>
