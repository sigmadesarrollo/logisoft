...
<script>
	...
	tabla1.setAttributes({
		...
		campos:[
			...
			{nombre:"IMPORTE", medida:60, alineacion:"right", tipo:"moneda", datos:"importe"},
			{nombre:"F. CANCELACION", medida:85, alineacion:"center", datos:"fechacancelacion"},
			{nombre:"MOT. CANCELACION", medida:100, alineacion:"left", datos:"motivo"},
			{nombre:"USUARIO CANCELACION", medida:120, alineacion:"left", datos:"usuario"}/*,
			{nombre:"CANCELACION AUTORIZADA", medida:120, alineacion:"center", datos:"cancelacionautorizada"}*/
		],
		filasInicial:20,
		alto:250,
		seleccion:true,
	...
	function obtenerDetalle(){		
		consultaTexto("resTabla1","consultasAlertas.php?accion=9&contador="+u.pag1_contador.value
		+"&s="+Math.random());
	}
	...
	function paginacion(movimiento){
		switch(movimiento){
			case 'primero':
				consultaTexto("resTabla1","consultasAlertas.php?accion=9&contador=0&s="+Math.random());
				break;
			case 'adelante':
				if(u.pag1_adelante.value==1){
					consultaTexto("resTabla1","consultasAlertas.php?accion=9&contador="+(parseFloat(u.pag1_contador.value)+1)					
					+"&s="+Math.random());
				}
				break;
			case 'atras':
			...
			case 'ultimo':
				...
				consultaTexto("resTabla1","consultasAlertas.php?accion=9&contador="+contador
				+"&s="+Math.random());
				break;
		}
	}
