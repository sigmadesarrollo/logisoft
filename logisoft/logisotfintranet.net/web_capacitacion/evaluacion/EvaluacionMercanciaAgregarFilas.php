...
<script>
	...
	function Validar(){
		<?php
			$s="SELECT numeroconcepto FROM configuradorgeneral;";
			$r = mysql_query($s,$link) or die(mysql_error($link).$s);		
			$f = mysql_fetch_object($r);
		?>
		if(parent.tabla1.getRecordCount() < <?=$f->numeroconcepto; ?>){
		...
		if(document.getElementById('cantidad').value==""){
			...
			mens.show("A","Solo puede registrar <?=$f->numeroconcepto; ?> articulos","¡Atención!");	
	}
