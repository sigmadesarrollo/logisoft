	...
	$fecha=cambiaf_a_mysql($fecha);
		if($registros>0){
			...
			for($folioii;$folioii<=$folioff;$folioii++){
				...
				WHERE folio='".$E['evaluacion']."'",$link)or die("Error en la linea ".__LINE__);		

				$sql_del=mysql_query("DELETE FROM guiasempresariales 
				WHERE id='".$i.str_pad($folioii,9,"0",STR_PAD_LEFT).$f."'",$link)or die("Error en la linea ".__LINE__);

				$sql_del_det=mysql_query("DELETE FROM guiasempresariales_unidades 
...
function ValidarFolio(){
	...
	consulta("mostrarValidarFolio","liberacionguiasempresariales_con.php?folioi="+u.folioi.value+"&accion="+2+"&foliof="+u.foliof.value+"&id="+u.venta.value+"&prepagada="+u.prepagada.value+"&sid="+Math.random());
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td colspan="5"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="13%">F. Inicial: </td>
            <td width="20%"><span style="width:85px"><span class="Tablas">
              <input name="folioi" type="text" class="Tablas" id="folioi" style="width:50px;" value="<?=$folioi ?>" />
            </span></span></td>
            <td width="11%"><span style="width:35px">F. Final:</span></td>
            <td width="16%"><span style="width:85px"><span class="Tablas">
              <input name="foliof" type="text" class="Tablas" id="foliof" style="width:50px;" value="<?=$foliof ?>" />
            </span></span></td>
            <td width="19%"><div class="ebtn_aceptar" onclick="ValidarFolio();"></div></td>
            <td width="2%">&nbsp;</td>
            <td width="19%"><div class="ebtn_eliminar" onclick="EliminarFila()"></div></td>
            </tr>
          <tr>
