...
<script>
	...
	function mostrarDatos(datos){
		...
		if(objeto!=0){
			...
			mens.show('A','Error al cargar datos principales','¡Atención!');
		}
		u.destino.focus();
...
consultaTexto("mostrarTodo","recoleccion_conj.php?accion=6&folio="+u.folio_hidden.value+"&idsucursal="+u.idsucursal2.value+"&valor="+Math.random());		
	...
	function validar(){
		...
		if(u.h1.value =="00" || u.h3.value =="00"){
			mens.show('A','Debe capturar Horario','¡Atención!', ((u.h1.value=="")? 'h1' : 'h3' ));
			return false;
		}		
	...
	function confirmarCancelar(){
		if(document.all.fecha=='<?=date('d/m/Y')?>'){
			...
			mens.show('A','Solo puede cancelar recolecciones del dia actual','¡Atención!');
	}
	...
	function obtenerDatosDestino(datos){
		...
		if(objeto[0].dia==1){
			...
			mens.show('A','El destino no hace recolección el dia '+u.fecha.value,'¡Atención!','destino');		
		}*/
	}
        ...
        <tr>
          <td colspan="4"><table width="447" border="0" align="center" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td width="85"><div id="d_transmitir" class="ebtn_transmitir" onClick="mens.show('C','¿Esta seguro de Transmitir la Recolección?', '', '', 'transmitir();');"></div></td>
                <td width="85"><div id="d_realizado" class="ebtn_realizado" onClick="mens.show('C','¿Esta seguro que desea Realizar la Recolección?', '', '', 'realizar();');"></div></td>
                <td width="85"><div id="d_cancelado" class="ebtn_cancelado" onClick="mens.show('C','¿Esta seguro de Cancelar la Recolección?', '', '', 'confirmarCancelar();');"></div></td>
                <td width="96"><div id="d_reprogramado" class="ebtn_reprogramado" onClick="mens.show('C','¿Esta seguro de Reprogramar la Recolección?', '', '', 'confirmarReprogramacion();');"></div></td>
