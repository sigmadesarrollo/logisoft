...
<?
	...
	$s = "SELECT prefijosucursal,cliente,folio,fechaguia,fechafactura,fechavenc,diasvencidos,alcorriente,c1a15dias,c16a30dias,
		...
		IF(IF(gv.tipoflete=0,gv.idsucursalorigen,gv.idsucursaldestino)!=pg.sucursalacobrar,'SI','') tcargo 
		...
		DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))<16,(IFNULL(f.total,0) + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)),0)) 
		AS c1a15dias,
		SUM(IF(DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))>15 AND
		DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))<31,(IFNULL(f.total,0) + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)),0)) 
		AS c16a30dias,
		SUM(IF(DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))>30 AND
		DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))<61,(IFNULL(f.total,0) + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)),0)) 
		AS c31a60dias,
