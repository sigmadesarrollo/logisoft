...
<script>
	...
	function mostrarVentas(datos){
		try{
			...
			mens.show("A","Error " + datos,"¡Atención!","sucursal");
			return false;
		}
		...
		u.comead2.value			= obj.totales.comisionead;
		u.sobrepeso2.value		= obj.totales.sobrepeso;
	...
	function mostrarRecibido(datos){
		...
		u.comead3.value			= obj.totales.comisionead;
	...
	function confirmaGuardar(){
		...
		+"&fechainicio="+u.fechainicio.value+"&fechafin="+u.fechafin.value+"&folio="+u.folio.value+"&val="+Math.random());
	}
	...
	function obtenerFolio2(folio){
		u.folio_oculto.value = folio;
		sucursal=u.sucursal_hidden.value;
		if(u.sucursal_hidden.value!=''){
			consultaTexto("mostrarFolio","consultas.php?accion=6&folio="+folio+"&sucursal="+sucursal);
		}else{
			mens.show("A","Seleccione una concesión","¡Atención!");
		}
	}
			...
			<td>
			  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td width="99"><label>
                      <input name="folio" type="text" id="folio" class="Tablas" style="width:80px" onkeypress="if(event.keyCode==13){obtenerFolio2(this.value);}" />
                    </label></td>
