 ...
 <Worksheet ss:Name="Hoja1">';
	...
	if($_GET[accion]==1){//REPORTE INGRESOS
		...
		FROM ( 
			...
			IFNULL(SUM(fp.notacredito),0) nc FROM formapago fp
	...
	$s="SELECT nombresucursal,DATE_FORMAT(fecha,'%d/%m/%Y')AS fecha,guia,cliente,importe,CONCAT('C-',caja) caja
		FROM (
			(SELECT cs.prefijo AS nombresucursal,DATE_FORMAT(gv.fecha,'%d/%m/%Y')AS fecha,gv.id AS guia,
		...
		UNION
			(SELECT cs.prefijo AS nombresucursal,DATE_FORMAT(f.fecha,'%d/%m/%Y')AS fecha,CONCAT('FA-',f.folio) AS guia,
...
$s="SELECT nombresucursal,DATE_FORMAT(fecha,'%d/%m/%Y')AS fecha,guia,cliente,importe,CONCAT('C-',caja) caja
	...
	UNION 
		(SELECT cs.prefijo AS nombresucursal,fp.fecha,gv.id AS guia,
