<?
	...
	if($_GET[accion]==1){ // optener cliente abonocliente.php
		...
		$s = "DELETE FROM abonodecliente_detalle_tmp WHERE usuario = $_SESSION[IDUSUARIO]";
		mysql_query($s,$l) or die($s);
		$s = "DELETE FROM abonodecliente_detalle_ori WHERE usuario = $_SESSION[IDUSUARIO]";
		mysql_query($s,$l) or die($s);
		
		$s="INSERT INTO abonodecliente_detalle_tmp (folios, fecha, fechavencimiento,factura,total,usuario)
		SELECT GROUP_CONCAT(folio) , fecha, fecha_venc, factura, saldo, $_SESSION[IDUSUARIO]
		FROM (
			(SELECT fd.folio, f.fecha, 
			DATE_ADD(f.fecha, INTERVAL cc.diascredito DAY) AS fecha_venc,
			f.folio factura, (f.total + f.otrosmontofacturar + f.sobmontoafacturar) AS saldo
			FROM facturacion f
			...
			DATE_ADD(f.fecha, INTERVAL cc.diascredito DAY) AS fecha_venc,
			f.folio, (f.total + f.otrosmontofacturar + f.sobmontoafacturar) AS saldo
			FROM facturacion f
		...
		select folios, fecha, fechavencimiento,factura,total,usuario from abonodecliente_detalle_tmp where usuario = $_SESSION[IDUSUARIO]";
		mysql_query($s,$l) or die($s);
		
				$s = "select tmp.*,DATE_FORMAT(fecha,'%d/%m/%Y') fecha, 
				DATE_FORMAT(fechavencimiento,'%d/%m/%Y') fechavencimiento, 
				1 aplicacion from abonodecliente_detalle_tmp tmp where usuario = $_SESSION[IDUSUARIO]";
				$registros = array();
				$r = mysql_query($s,$l) or die($s);
	...
	if($_GET[accion] == 2){
		//agrupar
		$s = "DELETE FROM abonodecliente_detalle_tmp WHERE factura IN($_GET[facturas]) and usuario = $_SESSION[IDUSUARIO];";
		mysql_query($s,$l) or die($s);
		
		$s = "INSERT INTO abonodecliente_detalle_tmp (folios, fecha, fechavencimiento,factura,total,usuario)
		SELECT GROUP_CONCAT(folios), fecha, fechavencimiento,GROUP_CONCAT(factura),sum(total),usuario 
		FROM abonodecliente_detalle_ori WHERE factura IN($_GET[facturas]) and usuario = $_SESSION[IDUSUARIO]";
		mysql_query($s,$l) or die($s);
	...
	if($_GET[accion] == 3){
		//desagrupar
		$s = "DELETE FROM abonodecliente_detalle_tmp WHERE factura ='$_GET[facturas]' and usuario = $_SESSION[IDUSUARIO];";
		mysql_query($s,$l) or die($s);
	...
	if($_GET[accion] == 4){
		...
		banco='$row[2]', nocheque='$row[3]' WHERE factura = '$_GET[factura]' and usuario = $_SESSION[IDUSUARIO]";
		mysql_query($s,$l) or die($s);
	...
	if($_GET[accion] == 5){
		...
		banco='0', nocheque='0' WHERE factura = '$_GET[factura]' and usuario = $_SESSION[IDUSUARIO]";
		mysql_query($s,$l) or die($s);
	...
	if($_GET[accion]==7){
		$s = "INSERT INTO abonodecliente (folio,fecharegistro,idsucursal,
			...
			WHERE usuario = '$_SESSION[IDUSUARIO]' and totalpagado>0";
			mysql_query($s,$l) or die($s);
			...
			while($fmd = mysql_fetch_object($rmd)){
				...
				total='$fmd->totalpagado',efectivo='$fmd->efectivo',
