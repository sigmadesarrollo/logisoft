    ...
    <Cell ss:StyleID="s22"/>
 	...
 	if ($_GET[accion]==1){
		...
		FROM(
			...
			AND gv.estado!='CANCELADO' AND gc.estadoconvenio='ACTIVADO'
			".(($_GET[sucursal]==1)?"":" AND gc.sucursal = $_GET[sucursal]")." GROUP BY gc.vendedor
		UNION
		...
		UNION
			...
			AND ISNULL(ge.factura) AND gc.estadoconvenio='ACTIVADO'
			".(($_GET[sucursal]==1)?"":" AND gc.sucursal = $_GET[sucursal]")." GROUP BY gc.vendedor
		UNION
