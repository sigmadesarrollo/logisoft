<?
	...
	if($_GET[accion] == 1){
		...
		if(mysql_num_rows($r)>0){
			...
			<datos>
				...
				<creditoactivado>".cambio_texto($creditoactivado)."</creditoactivado>
	...
	if($_GET[accion] == 2){
		...
		$s = "(SELECT gv.id, 'NORMAL' AS tipoguia, gv.evaluacion, 
		DATE_FORMAT(gv.fecha, '%d/%m/%Y') AS fecha, gv.fechaentrega, gv.factura, 
		...
		'G' AS tipo FROM guiasventanilla AS gv
		INNER JOIN pagoguias AS pg ON gv.id = pg.guia
		WHERE ISNULL(factura) AND gv.condicionpago = $_GET[tipoguia] AND pg.pagado <> 'C'
		AND gv.fecha BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql($_GET[fechafin])."')
		UNION
		(SELECT ge.id, 'CONSIGNACION' AS tipoguia, ge.evaluacion, DATE_FORMAT(ge.fecha, '%d/%m/%Y') AS fecha,
		ge.fechaentrega, ge.factura, ge.estado, ge.tipoflete, ge.ocurre, ge.idsucursalorigen, 
		...
		WHERE (ge.tipoguia <> 'PREPAGADA') AND ISNULL(factura) AND
		ge.tipopago = ".(($_GET[tipoguia]==1)?"'CREDITO'":"'CONTADO'")." AND pg.pagado <> 'C'
		AND ge.fecha BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql($_GET[fechafin])."')
		UNION
		(SELECT sge.id, 'PREPAGADA' AS tipoguia, 0 AS evaluacion, DATE_FORMAT(sge.fecha, '%d/%m/%Y') AS fecha,
		'' AS fechaentrega, sge.factura, 'SOLICIDADA' AS estado, '' AS tipoflete, '' AS ocurre, '' AS idsucursalorigen, 
		...
		sge.usuario, sge.fecha, '' AS hora_registro, DATE_FORMAT(CURRENT_DATE, '%d/%m/%Y') AS fechaactual, 'S' AS tipo
		FROM solicitudguiasempresariales AS sge
		WHERE sge.prepagada = 'SI' AND  (ISNULL(factura) OR factura=0) AND sge.estado = 'GUARDADA' 
		AND sge.condicionpago = ".(($_GET[tipoguia]==1)?"'CREDITO'":"'CONTADO'")."
		AND sge.fecha BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql($_GET[fechafin])."')";
	...
	if($_GET[accion] == 3){
		...
		if($_GET[valorcontado]!="0" && $_GET[valorcontado]!=""){
			...
			if($_GET[credito]=="SI"){
				...
				ncheque='$_GET[ncheque]',banco='$_GET[banco]',notacredito='$_GET[nc]', cliente = '$_GET[cliente]',
		...
		if($_GET[otrosmontofacturar]!="" && $_GET[otrosmontofacturar]!="0"){
		...
		$paraciclo = split(',',$_GET[foliosguias2]);
		if($_GET[foliosguias2]!=""){
			for($i=0; $i<count($paraciclo); $i++){
	...
	if($_GET[accion] == 7){		
		...
		ge.usuario, ge.fecha_registro, ge.hora_registro, DATE_FORMAT(CURRENT_DATE, '%d/%m/%Y') AS fechaactual
		FROM guiasempresariales AS ge
		WHERE (NOT ISNULL(ge.valordeclarado) OR ge.texcedente>0) AND ge.tipoguia='PREPAGADA'
		AND ISNULL(factura) AND ge.tipopago = ".(($_GET[tipoguia]==1)?"'CREDITO'":"'CONTADO'")."
		AND ge.idsucursalorigen = ".$_SESSION[IDSUCURSAL]."
		AND ge.fecha BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql($_GET[fechafin])."'";
	...
	if($_GET[accion] == 12){//FACTURACION GENERAL CRITERIOS
		$s = "SELECT ge.id, ge.evaluacion, DATE_FORMAT(ge.fecha, '%d/%m/%Y') AS fecha, ge.fechaentrega, ge.factura, 
		ge.estado, ge.tipoflete, ge.ocurre, ge.idsucursalorigen, 
		ge.idsucursalorigen,ge.idsucursaldestino,ge.entregaocurre, 
		if(NOT ISNULL(ge.valordeclarado) and ge.texcedente>0,'EXCEDENTE/VALOR DECLARADO',
