<?	session_start();
	...
	if($_GET[accion] == 1){
		...
		if($f->ocurre==1 && ($f->estado == 'ENTREGADA' || $f->estado == 'POR ENTREGAR')){
			$s = "SELECT entregaocurre, sucursal FROM entregasocurre_detalle WHERE guia = '".$row[1]."'";
			$rm = mysql_query($s,$l) or die($s);
			$fm = mysql_fetch_object($rm);
			$folioocurre = $fm->entregaocurre;
			
			$s = "SELECT * FROM entregasocurre_detalle WHERE entregaocurre = '$folioocurre' AND sucursal = '$_SESSION[IDSUCURSAL]'";
			$rm = mysql_query($s,$l) or die($s);
			
			if(mysql_num_rows($rm)>1){
				die("Esta guia es ocurre y fue entregada contras guias y afecta en el corte, no puede ser ");
			}
			
			$s = "DELETE FROM entregasocurre WHERE id = '$folioocurre'";
			mysql_query($s,$l) or die($s);
			
			$s = "DELETE FROM entregasocurre_detalle WHERE entregaocurre = '$folioocurre' AND sucursal = '$_SESSION[IDSUCURSAL]'";
			mysql_query($s,$l) or die($s);
			
			$s = "DELETE FROM formapago WHERE procedencia = 'O' AND guia = '$folioocurre' AND sucursal = '$_SESSION[IDSUCURSAL]'";
			mysql_query($s,$l) or die($s);			
		...
		$s = "UPDATE pagoguias SET credito = 'SI', pagado = 'N',usuariocobro = NULL, sucursalcobro = NULL, fechapago = NULL
		WHERE guia = '".$row[1]."'";
		mysql_query($s,$l) or die($s);
		#---
		...
		#-- seguimiento guias y historial de movimientos
		$s ="INSERT INTO seguimiento_guias (guia,ubicacion,estado,fecha,hora,usuario) 
		VALUES('".$row[1]."','".$_SESSION[IDSUCURSAL]."','TRASPASO CREDITO',CURRENT_DATE,CURRENT_TIME,'".$_SESSION[IDUSUARIO]."')";
		mysql_query($s,$l) or die($s);
		$s ="INSERT INTO historialmovimientos (modulo,folio,estado,idusuario,fechamodificacion) 
		VALUES('guiasventanilla','".$row[1]."','TRASPASO CREDITO','".$_SESSION[IDUSUARIO]."',CURRENT_TIMESTAMP)";
		mysql_query($s,$l) or die($s);
