<?
	...
	if($_GET['accion']==1){
		...
		$s = "insert into tmp_sucursales".$_SESSION[IDUSUARIO]."
		...
		WHERE idusuario = '$_GET[idpagina]' AND sucursal = ".$_SESSION[IDSUCURSAL]."";
		mysql_query($s,$l) or die($s);
		//echo $s."<br>";
		...
		if(mysql_num_rows($r)>0){
			...
			AND catalogorutadetalle.sucursal = '$_SESSION[IDSUCURSAL]'  AND bitacorasalida.unidad = '$_GET[unidad]'";
			//echo $s."<br>";
			$r = mysql_query($s,$l) or die($s);
			if(mysql_num_rows($r)>0){
				$inner1 = " INNER JOIN programacionrecepciondiaria as pr on '$_GET[unidad]' = pr.unidad 
				and pr.hrllegada <> '00:00:00' and pr.hrsalida = '00:00:00' ";
			}else{
				$inner1 = " ";
			}
		}
		...
		gvu.paquete, CONCAT(gvu.paquete,' de ', gvu.depaquetes), gvu.estado, 'N', gvu.peso, '$_GET[idpagina]',
		'$_SESSION[IDSUCURSAL]'
		...
		geu.paquete, CONCAT(geu.paquete,' de ', geu.depaquetes), geu.estado, 'N', geu.peso, '$_GET[idpagina]',
		'$_SESSION[IDSUCURSAL]'
		...
		FROM embarques_tmp
		where subido = 'N' and idusuario = '$_GET[idpagina]' AND sucursal = ".$_SESSION[IDSUCURSAL]."
		GROUP BY guia
		order by destino";
		//echo $s."<br>";
		$r = mysql_query($s,$l) or die($s);
		...
		$s = "SELECT sector, guia, origen, destino, sum(peso) as peso
		FROM embarques_tmp
		where subido = 'S' and idusuario = $_GET[idpagina] AND sucursal = ".$_SESSION[IDSUCURSAL]."
		GROUP BY guia
		order by destino";
		$r = mysql_query($s,$l) or die($s);
		$arre = array();
	...
	if($_GET[accion]==2){
		...
		FROM embarques_tmp 
		WHERE guia='$_GET[folio]' and subido='N' and idusuario = $_GET[idpagina] 
	...
	if($_GET[accion]==3){
		$s = "SELECT registro,paquete,codigobarra as codigobarras,estado,guia,peso
		FROM embarques_tmp 
		WHERE guia='$_GET[folio]' and subido='S' and idusuario = $_GET[idpagina]
		AND sucursal = ".$_SESSION[IDSUCURSAL]."";
		$r = mysql_query($s,$l) or die($s);
		$arre = array();
		while($f = mysql_fetch_object($r)){
	...
	if($_GET[accion]==4){
		...
		WHERE guia = '$_GET[folio]' and idusuario = $_GET[idpagina]
		AND sucursal = ".$_SESSION[IDSUCURSAL]."";
		mysql_query($s,$l) or die($s);
		$s = "SELECT ifnull(SUM(peso),0) AS pesototal 
		FROM embarques_tmp 
		WHERE idusuario = $_GET[idpagina] and subido = 'S' AND sucursal = ".$_SESSION[IDSUCURSAL]."";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
	...
	if($_GET[accion]==5){
		$s = "UPDATE embarques_tmp SET subido = 'S' 
		WHERE guia = '$_GET[folio]' and registro = $_GET[registro] and idusuario = $_GET[idpagina]
		AND sucursal = ".$_SESSION[IDSUCURSAL]."";
		mysql_query($s,$l) or die($s);
		$s = "SELECT ifnull(SUM(peso),0) AS pesototal 
		FROM embarques_tmp 
	...
	if($_GET[accion]==8){
		...
		(select guia, subido 
		from embarques_tmp
		where subido = 'S' and idusuario = '$_GET[idpagina]' AND sucursal = ".$_SESSION[IDSUCURSAL].") as t1
		inner join 
		(select guia, subido 
		from embarques_tmp
		where subido = 'N' and idusuario = '$_GET[idpagina]' AND sucursal = ".$_SESSION[IDSUCURSAL].") as t2 
		on t1.guia = t2.guia
		group by t1.guia";*/
		
		$s = "SELECT guia, GROUP_CONCAT(subido) subidos
		FROM embarques_tmp
		WHERE idusuario = '110217032112' AND sucursal = 45
		GROUP BY guia
		HAVING subidos LIKE '%N%S%' OR subidos LIKE '%S%N%'";
		$r = mysql_query($s,$l) or die($s);
		if(mysql_num_rows($r)>0){
			$arre = array();
			while($f = mysql_fetch_object($r)){
	...
	if($_GET[accion]==9){
		...
		#se cambio para poder embarcar varias veces y se puso en la salida de la unidad
		...
		WHERE idusuario = $_GET[idpagina] and rg.subido = 'S'
		AND sucursal = ".$_SESSION[IDSUCURSAL]." 
		GROUP BY rg.guia";
		//echo "$s<br><br>";
		mysql_query($s,$l) or die($s);
		...
		FROM embarques_tmp WHERE idusuario=$_GET[idpagina]
		...
		WHERE e.idusuario = '$_GET[idpagina]' AND e.subido = 'S' AND sucursal = ".$_SESSION[IDSUCURSAL]."
		GROUP BY guia";
		mysql_query($s,$l) or die($s);
		...
		WHERE embarques_tmp.idusuario = $_GET[idpagina] and embarques_tmp.subido = 'S'
		...
		$s = "SELECT COUNT(*) cantidad FROM embarquedemercanciadetalle WHERE idembarque=$fidx->folio AND sucursal=".$_SESSION[IDSUCURSAL].";";
		$r = mysql_query($s,$l) or die($s); 
		$f = mysql_fetch_object($r);
		...
		$s = "CREATE TEMPORARY TABLE `temp_mereem` (
		  `cantidad` BIGINT(21) DEFAULT '0',
		  `sucursal` DOUBLE DEFAULT '0'
		) ENGINE=MYISAM DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
		mysql_query($s,$l) OR die(mysql_error($l).$s);
		$s = "INSERT INTO temp_mereem 
		SELECT  COUNT(*) cantidad,'$_SESSION[IDSUCURSAL]' FROM embarquedemercanciadetalle WHERE idembarque=$fidx->folio	GROUP BY sucursal;";
		mysql_query($s,$l) or die($s); 
		$s = "UPDATE lasalertas l INNER JOIN temp_mereem t ON l.sucursal=t.sucursal SET l.mereem=l.mereem-t.cantidad;";
		mysql_query($s,$l) OR die(mysql_error($l).$s);
	...
	if($_GET[accion]==10){
		...
		//$f->destino = cambio_texto($dest->sucursal);
		
		$datosembarque = str_replace("null",'""',json_encode($f));
		
		$s = "DELETE FROM embarques_tmp WHERE idusuario = '$_GET[idpagina]' AND sucursal=".$_SESSION[IDSUCURSAL]."";
		mysql_query($s,$l) or die($s);
		
		$s = "INSERT INTO embarques_tmp
		SELECT idunidad, sector, guia, tipoguia, origen, destino, codigobarra, 
		registro, paquete, estado, 'S', peso, '$_GET[idpagina]', sucursal
