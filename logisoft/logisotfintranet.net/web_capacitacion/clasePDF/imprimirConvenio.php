<?php
	...
	$pdf->addText(110,722,24,'<b>Paquetería y Mensajería en Movimiento</b>');
	...
	if($f->valordeclarado!=""){
		...
		$pdf->ezText("<b>\nDATOS DEL CONVENIO PARA VALOR DECLARADO</b>",12,array('justification'=>'left'));
		$pdf->setColor(.25,.25,.25);
		$pdf->ezText("SI EL CLIENTE DECLARA VALOR, SE COBRARA $ ".number_format($f->valordeclarado,2,".",",")." POR CADA $ ".number_format($f->porcada,2,".",",")." QUE HAYAN SIDO DECLARADOS, HASTA UN LIMITE DE $ ".number_format($f->limite,2,".",",").". SI SE EXCEDE ESTE LIMITE, POR CADA $ ".number_format($f->porcada,2,".",",")." FUERA DEL LIMITE ESTABLECIDO SE COBRARA $ ".number_format($f->costoextra,2,".",","),10,array('justification'=>'left'));
	}
