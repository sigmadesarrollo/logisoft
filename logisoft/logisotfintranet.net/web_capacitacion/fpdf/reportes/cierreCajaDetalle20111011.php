<?	//die("DISCULPE LAS MOLESTIAS EN ESTE MOMENTO EL REPORTE SE ENCUENTRA FUERA DE SERVICIO");
	...
	CONCAT_WS(' ', cc.nombre,cc.paterno,cc.materno) AS cliente,
	cs.prefijo AS destino, sum(se.total) AS importe, se.idusuario,
	...
	AND se.condicionpago='CONTADO' AND f.idsucursal = ".$_GET[sucursal]." AND se.factura<>0 
	GROUP BY f.folio ORDER BY se.idusuario";	
	obtenerDatos($criterioprepagadascontado,$pdf,"FACTURACION DE VENTAS GUIAS PREPAGADAS CONTADO","SI");
	...
	WHERE se.prepagada='SI' AND se.estado='GUARDADA' AND f.facturaestado='GUARDADO' AND 
	se.fecha BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql(((!empty($_GET[fechafin]))?$_GET[fechafin]:$_GET[fechainicio]))."'
	AND se.condicionpago='CREDITO' AND f.idsucursal = ".$_GET[sucursal]." AND se.factura<>0 
	GROUP BY f.folio ORDER BY se.idusuario";	
	obtenerDatos($criterioprepagadascredito,$pdf,"FACTURACION DE VENTAS GUIAS PREPAGADAS CREDITO","SI");
	...
	obtenerDatos($criterioconsignacioncontado,$pdf,"FACTURACION DE GUIAS CONSIGNACION CONTADO","SI");	
	...
	AND f.fecha BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql(((!empty($_GET[fechafin]))?$_GET[fechafin]:$_GET[fechainicio]))."'
	GROUP BY f.folio ORDER BY f.idusuario";
	obtenerDatos($criterioconsignacioncredito,$pdf,"FACTURACION DE GUIAS CONSIGNACION CREDITO","SI");
	...
	GROUP BY f.folio ORDER BY f.idusuario";
	obtenerDatos($criteriofacturacionsobrepesocontado,$pdf,"FACTURACION DE SOBREPESO CONTADO","SI");
	...
	INNER JOIN abonodecliente_facturas af ON a.id = af.folioabono
	...
	obtenerDatos($criterioingresoguiasforaneacobrarcontadoentregadas,$pdf,"GUIAS FORANEAS COBRAR-CONTADO ENTREGADAS","","SI");
