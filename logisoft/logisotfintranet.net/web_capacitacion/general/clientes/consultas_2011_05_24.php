<?	session_start();
	...
	if($_GET[accion]==1){//PRINCIPAL CLIENTES
		...
		INNER JOIN( SELECT idsucorigen,total 
		FROM reportes_ventas 
		WHERE IF(tipoventa <> 'GUIA VENTANILLA',YEAR(fechafacturacion)='".$_GET[fecha]."', YEAR(fecharealizacion)='".$_GET[fecha]."') 
		AND activo='S' AND (NOT ISNULL(convenio) AND convenio!=0) GROUP BY idsucorigen) t1 ON cs.id=t1.idsucorigen 
		WHERE t1.total>0 GROUP BY cs.id";
		$r = mysql_query($s,$l) or die($s);
		$totalregistros = mysql_num_rows($r);
		
		$s = "SELECT FORMAT(SUM(IF(NOT ISNULL(factura),total,0)),2) AS facturado,FORMAT(SUM(IF(ISNULL(factura),total,0)),2) AS nofacturado, 
		FORMAT(SUM(total),2) total
		FROM reportes_ventas 
		WHERE IF(tipoventa <> 'GUIA VENTANILLA',YEAR(fechafacturacion)='".$_GET[fecha]."', YEAR(fecharealizacion)='".$_GET[fecha]."') 
		AND activo='S' AND (NOT ISNULL(convenio) AND convenio!=0)";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
		$totales = json_encode($f);
		...
		INNER JOIN(SELECT idsucorigen,SUM(IF(NOT ISNULL(factura),total,0)) AS tfacturado, SUM(IF(ISNULL(factura),total,0)) AS tnofacturado 
		FROM reportes_ventas 
		WHERE IF(tipoventa <> 'GUIA VENTANILLA',YEAR(fechafacturacion)='".$_GET[fecha]."', YEAR(fecharealizacion)='".$_GET[fecha]."') 
		AND activo='S' AND (NOT ISNULL(convenio) AND convenio!=0)GROUP BY idsucorigen) t1 ON t0.id=t1.idsucorigen 
		...
		FROM reportes_ventas	
		WHERE IF(tipoventa <> 'GUIA VENTANILLA',YEAR(fechafacturacion)='".$_GET[fecha]."', YEAR(fecharealizacion)='".$_GET[fecha]."') 
		AND activo='S' AND (NOT ISNULL(convenio) AND convenio!=0) AND (NOT ISNULL(factura)) GROUP BY idsucorigen";
		$r = mysql_query($s,$l) or die($s);
		$totalregistros = mysql_num_rows($r);
		...
		FROM reportes_ventas 
		WHERE IF(tipoventa <> 'GUIA VENTANILLA',YEAR(fechafacturacion)='".$_GET[fecha]."', YEAR(fecharealizacion)='".$_GET[fecha]."') 
		AND activo='S' AND (NOT ISNULL(convenio) AND convenio!=0) AND (NOT ISNULL(factura)) GROUP BY idsucursal ORDER BY prefijosucursal $limite";
		$r = mysql_query($s,$l) or die($s);
		$arr = array();
		while($f = mysql_fetch_object($r)){
			$f->prefijosucursal = cambio_texto($f->prefijosucursal);
		...
		WHERE (ISNULL(ge.factura) OR ge.factura=0) AND YEAR(ge.fecha)='".$_GET[fecha]."' GROUP BY tc.idsucursal )t ";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
		$totales = json_encode($f);
		...
		FROM guiasempresariales ge INNER JOIN tmp_convenio tc ON ge.idremitente=tc.idcliente
		WHERE (ISNULL(ge.factura) OR ge.factura=0) AND YEAR(ge.fecha)='".$_GET[fecha]."' 
		AND NOT ISNULL(ge.total) AND ge.total!=0 GROUP BY tc.idsucursal
		...
		UNION
		SELECT tc.idsucursal AS sucursalacobrar FROM guiasempresariales ge INNER JOIN tmp_convenio tc ON ge.idremitente=tc.idcliente
