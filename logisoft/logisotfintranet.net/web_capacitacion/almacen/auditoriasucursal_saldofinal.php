...
<script>
	...
	tabla1.setAttributes({
		...
		campos:[
			...
			{nombre:"PAGO", medida:80, alineacion:"right", datos:"pago"},
			{nombre:"ESTATUS",medida:70, alineacion:"left", datos:"status"}
		],
		filasInicial:30,
		alto:250,
		seleccion:true,
	...
	function obtenerDetalle(){
		consultaTexto("resTabla1","auditoriasucursalnuevo_con.php?sucursalseleccionada=<?=$_GET[sucursalseleccionada]?>&accion=8<?=(($_GET[folio]!="")?"&folio=$_GET[folio]":"")?>&contador="+u.pag1_contador.value
		+"&s="+Math.random());
	}
	...
	function paginacion(movimiento){
		switch(movimiento){
			case 'primero':
				consultaTexto("resTabla1","auditoriasucursalnuevo_con.php?sucursalseleccionada=<?=$_GET[sucursalseleccionada]?>&accion=8&contador=0&s="+Math.random());
				break;
			case 'adelante':
				if(u.pag1_adelante.value==1){
					consultaTexto("resTabla1","auditoriasucursalnuevo_con.php?sucursalseleccionada=<?=$_GET[sucursalseleccionada]?>&accion=8&contador="+(parseFloat(u.pag1_contador.value)+1)					
					+"&s="+Math.random());
				}
				break;
			case 'atras':
			...
			case 'ultimo':
				...
				consultaTexto("resTabla1","auditoriasucursalnuevo_con.php?sucursalseleccionada=<?=$_GET[sucursalseleccionada]?>&accion=8&contador="+contador
				+"&s="+Math.random());
				break;
		}
	}
...
<form id="form1" name="form1" method="post" action="">
  <table border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
    <tr>
      <td class="FondoTabla">SALDO FINAL</td>
    </tr>
    <tr>
