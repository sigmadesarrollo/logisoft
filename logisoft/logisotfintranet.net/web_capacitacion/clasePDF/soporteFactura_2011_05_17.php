<?php
	...
	#detallado superior
	$s = "SELECT fd.*,SUBSTRING(CONCAT(f.nombrecliente,' ',f.apellidopaternocliente,' ',f.apellidomaternocliente),1,25) cliente,gv.totalpaquetes,
		DATE_FORMAT(fd.fecha,'%d/%m/%Y') AS fecha
		FROM facturadetalle fd
		INNER JOIN facturacion f ON fd.factura=f.folio
		INNER JOIN guiasventanilla gv ON fd.folio=gv.id
		WHERE fd.factura = '$_GET[folio]'
		UNION
		SELECT fd.*,SUBSTRING(CONCAT(f.nombrecliente,' ',f.apellidopaternocliente,' ',f.apellidomaternocliente),1,25) cliente,ge.totalpaquetes,
		DATE_FORMAT(fd.fecha,'%d/%m/%Y') AS fecha
		FROM facturadetalle fd
		INNER JOIN facturacion f ON fd.factura=f.folio
	...
	if($registros>0){
		...
		$col = array('0'=>"Folio",'1'=>'Remitente','2'=>'#Paqs','3'=>'Fecha','4'=>'Flete','5'=>'Desc','6'=>'Exced','7'=>'EAD',
		...
		'2'=>array('justification'=>'center'),'3'=>array('justification'=>'right'),'4'=>array('justification'=>'right'),
