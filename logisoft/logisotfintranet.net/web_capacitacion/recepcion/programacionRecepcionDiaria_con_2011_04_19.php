<? session_start();
	...
	if($_GET[accion]==1){//OBTENER UNIDAD
		...
		$rr = mysql_query($s,$l) or postError($s);
		...
		$ultsucursal = $ff->sucursal;		
			...
			$d = mysql_query($s,$l) or postError($s);
		 	...
		 	if($df->hrsalida!='00:00:00' && $df->sucursal==$ultsucursal){
				...
				$ru = mysql_query($s,$l) or postError($s);
		...
		$sq = mysql_query($s, $l) or postError(mysql_error($l).$s);		
		...
		SET p.hrllegada=".$arr[1].", 
		p.hrsalida=".$arr[2].", p.fechallegada=CURRENT_DATE, 
		p.recibio=CONCAT_WS(' ',ce.nombre,ce.apellidopaterno, ce.apellidomaterno)
		WHERE folio=".$arr[0]."";
		$r = mysql_query($s, $l) or postError(mysql_error($l).$s);		 
		...
		$rx = mysql_query($s,$l) or postError($s."<BR>".mysql_error($l));
		...
		if($arr[1]!="'00:00:00'" && $arr[2]=="'00:00:00'"){
			...
			WHERE guiaventanilla_unidades.unidad = '$_GET[unidad]' AND guiaventanilla_unidades.proceso = 'EN TRANSITO'
			AND guiasventanilla.estado like '%EN TRANSITO%'
			$and1";
			//echo "$s<br><br>";
			...
			WHERE guiasempresariales_unidades.unidad = '$_GET[unidad]' AND guiasempresariales_unidades.proceso = 'EN TRANSITO'
			AND guiasempresariales.estado like '%EN TRANSITO%'
			$and2";			
			//echo "$s<br><br>";
			mysql_query($s,$l) or postError($s."<BR>".mysql_error($l));
			...
			$rd = mysql_query($s,$l) or postError($s."<BR>".mysql_error($l));
		...
		WHERE bs.unidad ='".$_GET[unidad]."' AND d.sucursal=".$_SESSION[IDSUCURSAL]." AND bs.status = 0 AND bs.cancelada=0";
		$rx = mysql_query($s,$l) or postError($s."<BR>".mysql_error($l));
		$fx = mysql_fetch_object($rx);
		if($fx->tipo==3){
			...
			WHERE ruta = '$fx->ruta' AND cd.sucursal <> '$_SESSION[IDSUCURSAL]'";
			mysql_query($s,$l) or postError($s);
			
			$s = "SELECT cd.sucursalestransbordo
			FROM catalogorutadetalle cd 
			WHERE ruta = '$fx->ruta' AND cd.sucursal <> '$_SESSION[IDSUCURSAL]' AND sucursalestransbordo<>''";
			$rsa = mysql_query($s,$l) or postError($s);
			...
			INNER JOIN sucursales_anteriores".$_SESSION[IDUSUARIO]." sa ON gv.idsucursaldestino = sa.idsucursal
			SET gu.unidad = NULL
			WHERE gu.unidad = '$_GET[unidad]' and gv.estado = 'POR RECIBIR'";
			mysql_query($s,$l) or postError($s);
			...
			$s = "SELECT gu.idguia
			FROM guiaventanilla_unidades gu
			WHERE gu.unidad = '$_GET[unidad]' and gu.proceso = 'POR RECIBIR' AND gu.ubicacion = '$_SESSION[IDSUCURSAL]' 
			UNION
			SELECT gue.idguia
			FROM guiasempresariales_unidades gue
			WHERE gue.unidad = '$_GET[unidad]' and gue.proceso = 'POR RECIBIR' AND gue.ubicacion = '$_SESSION[IDSUCURSAL]'";
			$rge = mysql_query($s,$l) or postError($s);
			if(mysql_num_rows($rge)<1){
				$s = "UPDATE bitacorasalida SET status=1 WHERE unidad=UCASE('".$_GET[unidad]."') AND status=0";
				mysql_query($s,$l) or postError($s."<BR>".mysql_error($l));
				
				$s = "update catalogoruta set enuso=0 where id = $fx->ruta";
				mysql_query($s,$l) or postError($s."<BR>".mysql_error($l));
		...
		$s = "UPDATE catalogounidad SET recepcionado = 'N'
		WHERE numeroeconomico = '$_GET[unidad]'";
		mysql_query($s,$l) or postError($s."<BR>".mysql_error($l));
		...
		$fe= mysql_query($s,$l) or postError($s);
		...
		$su= mysql_query($s,$l) or postError($s); $sucur = mysql_fetch_object($su);
		...
		$de= mysql_query($s,$l) or postError($s);	
		...
		INNER JOIN bitacorasalida bs ON pr.idbitacora = bs.folio
		WHERE pr.unidad = '".$_GET[unidad]."' 
		AND pr.sucursal = '".$_SESSION[IDSUCURSAL]."' AND bs.status = 0 and bs.cancelada=0";
		$r = mysql_query($s,$l) or postError($s);
		$f = mysql_fetch_object($r);
		...
		INNER JOIN catalogosucursal cs ON pr.sucursal = cs.id
		WHERE pr.unidad = '".$_GET[unidad]."'  AND pr.folio < '$f->folio'
		AND bs.status = 0 and bs.cancelada=0 AND pr.hrsalida = '00:00:00'";
		$r = mysql_query($s,$l) or postError($s);
