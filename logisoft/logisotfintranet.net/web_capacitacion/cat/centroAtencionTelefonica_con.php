...
/*	require_once '../swiftMailer/lib/swift_required.php';
	...
	if($_GET[accion]==1){//OBTENER DATOS GENERALES
		...
		if(mysql_num_rows($r)>0){
			...
			$s = "SELECT id FROM direccion WHERE codigo = '$f->cliente' AND colonia = '$f->colonia' 
			AND calle = '$f->calle' AND numero = '$f->numero' AND municipio = '$f->municipio'";
			...
			$f->iddireccion = cambio_texto($fd->id);
		...
		observaciones,responsable,supervisor,idusuario,fecha,folioactividad,iddireccion)
		VALUES
		...
		".$_SESSION[IDUSUARIO].",CURRENT_TIMESTAMP(),'".$folioactividad."','".$row[15]."')";
		...
		responsable=".$row[11].",supervisor=".$row[12].", iddireccion = '".$row[15]."'
	...
	INNER JOIN direccion d ON s.cliente = d.codigo AND s.iddireccion = d.id
	WHERE s.folio = ".$folio."";
	$r = mysql_query($s,$l) or die($s);
	$f = mysql_fetch_object($r);
...
Ejecutivo de servicio asignado: '.$f->responsable.'</p>
  ...
  &nbsp;<br />
	...
	$s = "SELECT UCASE(CONCAT_WS(' ',eti_nombre1,eti_nombre2)) nombreempresa FROM configuradorgeneral";
	$rne = mysql_query($s,$l) or die($s);
	$fne = mysql_fetch_object($rne);
...
<table width="550" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="100" rowspan="6"><img src="http://files.pmmintranet.net/files/logopmm.gif" width="95" height="100" /></td>
    <td width="450" height="30" align="center"><strong>'.$fne->nombreempresa.'</strong></td>
  </tr>
  <tr>
    <td height="19" align="center"><strong>Centro de Atenci&oacute;n Telef&oacute;nica</strong></td>
  </tr>
  ...
  <tr>
    <td><strong>Estimado(a):</strong> '.$f->nombre.' </td>
  </tr>
  <tr>
    <td>Su Solicitud de Atenci&oacute;n fue registrada con los siguientes datos:</td>
  </tr>
...
Ejecutivo de servicio asignado: '.$f->responsable.'</p>
  ...
  &nbsp;<br />
	...
	if($_GET[accion]==15){
		...
		IFNULL(d.crucecalles,0) crucecalles, IFNULL(d.municipio,0) municipio,
		CONCAT_WS(' ',cc.nombre,cc.paterno,cc.materno) AS ncliente
		FROM solicitudtelefonica s
		INNER JOIN catalogocliente cc ON s.cliente = cc.id
		INNER JOIN direccion d ON s.cliente = d.codigo AND s.iddireccion = d.id
		WHERE s.folio = '$_GET[folio]'";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
		
		$valores = json_encode($f);
		echo "({
				'cliente':".str_replace('&#32;','',$valores).",
				'direcciones':[".str_replace('&#32;','',$valores)."]
			})";
	}
