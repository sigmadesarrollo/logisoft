<? session_start();
	...
	switch ($_GET[mostrar]){
		case "entregasocurre":
			...
			NOT EXISTS(SELECT * FROM entregasocurrealmacen eo 
			WHERE e.folio = eo.folioentregasocurre AND eo.sucursal = ".$_SESSION[IDSUCURSAL].")";
			break;
      ...
      <td colspan="2" class="Tablas" height="300px" valign="top"><table width="100%" border="0" align="center" class="Tablas">
		  ...
		  switch ($_GET[mostrar]){
			case "entregasocurre":
				...
				NOT EXISTS(SELECT * FROM entregasocurrealmacen eo 
				WHERE e.folio = eo.folioentregasocurre AND eo.sucursal = ".$_SESSION[IDSUCURSAL].") limit ".$st.','.$pp;
				break;
			case "entregasocurrealmacen":
