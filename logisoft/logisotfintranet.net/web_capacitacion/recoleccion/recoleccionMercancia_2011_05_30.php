...
<script>
	...
	function obtenerRecolecciones(datos){
		consultaTexto("obtenerTotal","recoleccion_conj.php?accion=5&tipo=0&sucursal="+u.idSucOrigen.value+"&valor="+Math.random());			
	}
	...
	function obtenerTotal(datos){
		...
		if(movioFecha=="SI"){
			...
			var sucursal = ((u.sucursal_hidden.value!="")? "&sucursal="+u.sucursal_hidden.value : "");
			consultaTexto("mostrarRecoleccion","recoleccion_conj.php?accion=5&tipo=1&inicio=0"+sucursal);
		}
	}
			...
			if(parseFloat(u.totaldes.value) <= "1"){
				...
				if(u.ultimo.value == "SI"){
					...
					if(movioFecha=="SI"){
						...
						+"&sucursal="+u.idSucOrigen.value+"&valor="+Math.random());
					}				
					...
					if(movioFecha=="SI"){
						...
						if(u.idSucOrigen.value == 1 || (u.idSucOrigen.value!=1 && u.sucursal_hidden.value!=u.idSucOrigen.value)){
							var sucursal = ((u.sucursal_hidden.value!="")? "&sucursal="+u.sucursal_hidden.value : "");
							consultaTexto("mostrarRecoleccion","recoleccion_conj.php?accion=5&tipo=1&inicio=0&sucursal="+sucursal+"&valor="+Math.random());
						}else{
							consultaTexto("mostrarRecoleccion","recoleccion_conj.php?accion=5&tipo=1&inicio=0&sucursal="+u.idSucOrigen.value+"&valor="+Math.random());
						}
					}
				}
			}else{
				...
				if(movioFecha=="SI"){
					...
					if(u.idSucOrigen.value == 1 || (u.idSucOrigen.value!=1 && u.sucursal_hidden.value!=u.idSucOrigen.value)){
						var sucursal = ((u.sucursal_hidden.value!="")? "&sucursal="+u.sucursal_hidden.value : "");
						consultaTexto("mostrarRecoleccion","recoleccion_conj.php?accion=5&tipo=1&inicio="+u.totaldes.value+"&sucursal="+sucursal+"&valor="+Math.random());
					}else{
						consultaTexto("mostrarRecoleccion","recoleccion_conj.php?accion=5&tipo=1&inicio="+u.totaldes.value+"&sucursal="+u.idSucOrigen.value+"&valor="+Math.random());
					}
				}
			}
		}else{
			...
			if(parseFloat(u.totaldes.value) > parseFloat(u.contadordes.value)){
				...
				if(movioFecha=="SI"){
					...
					if(u.idSucOrigen.value == 1 || (u.idSucOrigen.value!=1 && u.sucursal_hidden.value!=u.idSucOrigen.value)){
						...
						+"&inicio="+u.totaldes.value+"&valor="+Math.random());
					}else{
	...
	function obtenerPrimero(){
		...
		if(movioFecha=="SI"){
			...
			if(u.idSucOrigen.value == 1 || (u.idSucOrigen.value!=1 && u.sucursal_hidden.value!=u.idSucOrigen.value)){
				var sucursal = ((u.sucursal_hidden.value!="")? "&sucursal="+u.sucursal_hidden.value : "");
				consultaTexto("mostrarRecoleccion","recoleccion_conj.php?accion=5&tipo=1&inicio="+u.totaldes.value
				+"&sucursal="+sucursal+"&valor="+Math.random());
			}else{
				consultaTexto("mostrarRecoleccion","recoleccion_conj.php?accion=5&tipo=1&inicio="+u.totaldes.value
				+"&sucursal="+u.idSucOrigen.value+"&valor="+Math.random());
			}
	...
	function buscarFolio(folio){
		for(var i=0;i<tabla1.getRecordCount();i++){
			var arr = tabla1.getValuesFromField("folio",",");
		}
		arr = arr.split(",");
	...
	function devolverCliente(){
		if(u.cliente_hidden.value==""){
			...
			consultaTexto("obtenerTotal", "recoleccion_conj.php?accion=5&tipo=0&sucursal="+u.sucursal_hidden.value
			+"&cliente="+u.cliente_hidden.value+"&valor="+Math.random());
		}
	}	
	...
	function refrescar(){
		movioFecha = "";
		u.fecha.value = '<?=$fecha; ?>';
		consultaTexto("obtenerTotal","recoleccion_conj.php?tipo=0&accion=5"+((u.sucursal_hidden.value!="")? "&sucursal="+u.sucursal_hidden.value : "")+"&valor="+Math.random());
	}
...
<table width="800" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        <td>Folio:</td>
        <td width="242"><input name="folio" type="text" class="Tablas" onKeyPress="if(event.keyCode==13){buscarFolio(this.value);}" id="folio" style="width:80px" value="<?=$folio ?>"/>
          <img src="../img/Buscar_24.gif" width="24" height="23" align="absbottom"></td>
        </tr>
      <tr>
        <td width="76">Cliente:
        ...
        <td colspan="4"><input name="cliente" type="text" class="Tablas" id="cliente" style="width:250px" value="<?=$cliente ?>" onKeyUp="ajax_showOptions(this,'getCountriesByLetters',event,'ajax-list-cliente.php')" onKeyPress="if(event.keyCode==13){devolverCliente();}" />
          <span class="Tablas"><img src="../img/Buscar_24.gif" width="24" height="23" align="absbottom" style="cursor:pointer" onclick=            "abrirVentanaFija('../buscadores_generales/buscarClienteGen.php?funcion=obtenerClienteBusqueda', 625, 418, 'ventana', 'Busqueda')">&nbsp;</span></td>
      ...
      <tr>
        <td colspan="7" align="center"><p></p>          </td>
      </tr>
    </table></td></tr>
</table>
</form>
