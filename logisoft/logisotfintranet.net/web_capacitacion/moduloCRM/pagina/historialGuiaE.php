...
<script>
	...
	function generarHistorial(){
		...
		consultaTexto("obtenerTotal","consultas_crm.php?accion=4&tipo=0&cliente=<?=$_GET[cliente] ?>&fechainicio="+u.fechainicio.value+"&fechafin="+u.fechafin.value);
		}
	}
	function obtenerTotal(datos){
		...
		consultaTexto("mostrarDetalle","consultas_crm.php?accion=4&tipo=1&inicio=0&cliente=<?=$_GET[cliente] ?>&fechainicio="+u.fechainicio.value+"&fechafin="+u.fechafin.value);
	}
		...
		if(tipo == "atras"){
			...
			if(parseFloat(u.totaldes.value) <= "1"){
				...
				if(u.ultimo.value == "SI"){
					...
					u.ultimo.value = "";
					consultaTexto("mostrarDetalle","consultas_crm.php?accion=4&tipo=1&inicio="+u.totaldes.value+"&cliente=<?=$_GET[cliente] ?>&fechainicio="+u.fechainicio.value+"&fechafin="+u.fechafin.value);
				}else{
					u.d_atrasdes.style.visibility = "hidden";
					u.primero.style.visibility = "hidden";
					consultaTexto("mostrarDetalle","consultas_crm.php?accion=4&tipo=1&inicio=0&cliente=<?=$_GET[cliente] ?>&fechainicio="+u.fechainicio.value+"&fechafin="+u.fechafin.value);
				...
				consultaTexto("mostrarDetalle","consultas_crm.php?accion=4&tipo=1&cliente=<?=$_GET[cliente] ?>&inicio="+u.totaldes.value+"&fechainicio="+u.fechainicio.value+"&fechafin="+u.fechafin.value);
			}
		}else{
			cont++;
	...
	function obtenerUltimo(){
		...
		u.d_sigdes.style.visibility = "hidden";
		u.d_ultimo.style.visibility = "hidden";
		consultaTexto("mostrarDetalle","consultas_crm.php?accion=5&tipo=1&cliente=<?=$_GET[cliente] ?>&fechainicio="+u.fechainicio.value+"&fechafin="+u.fechafin.value);
	}
	
	function obtenerDetalle(){
		var obj = tabla1.getSelectedRow();
		abrirVentanaFija('detalleHistorialGuiaE.php?guia='+obj.guia, 600, 250, 'ventana', 'Seguimiento Guía Detalle');
	}
