<?	//die("DISCULPE LAS MOLESTIAS EN ESTE MOMENTO EL REPORTE SE ENCUENTRA FUERA DE SERVICIO");
	...
	INNER JOIN catalogocliente cc ON f.cliente = cc.id
	LEFT JOIN catalogoempleado ce ON f.idusuario = ce.id
	WHERE f.idsucursal = ".$_GET[sucursal]." AND (ISNULL(f.tipofactura) OR f.tipofactura='NORMAL') AND f.credito = 'NO' AND f.otrosmontofacturar > 0
	AND date(f.fecha) BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql(((!empty($_GET[fechafin]))?$_GET[fechafin]:$_GET[fechainicio]))."'
	...
	INNER JOIN catalogocliente cc ON f.cliente = cc.id
	LEFT JOIN catalogoempleado ce ON f.idusuario = ce.id
	WHERE f.idsucursal = ".$_GET[sucursal]." AND (ISNULL(f.tipofactura) OR f.tipofactura='NORMAL') AND f.credito = 'SI' AND f.otrosmontofacturar > 0
	AND date(f.fecha) BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql(((!empty($_GET[fechafin]))?$_GET[fechafin]:$_GET[fechainicio]))."'
	AND f.facturaestado!='CANCELADO'
	ORDER BY f.idusuario";
	...
	IFNULL(f.otrosmontofacturar,0) AS importe,a.idusuario, CONCAT_WS(' ',ce.nombre,ce.apellidopaterno,ce.apellidomaterno) empleado
	...
	AND f.otrosmontofacturar > 0 AND (f.total = 0 OR ISNULL(f.total)) 
	AND (f.sobmontoafacturar = 0 OR ISNULL(f.sobmontoafacturar))
	GROUP BY f.folio
	UNION
	SELECT CONCAT('Fact-',f.folio) AS guia, DATE_FORMAT(a.fecharegistro,'%d/%m/%Y') AS fecha,
	CONCAT_WS(' ', cc.nombre,cc.paterno,cc.materno) AS cliente, cs.prefijo AS destino,
	...
	obtenerDatos($criterioingresocobranzacredito,$pdf,"INGRESOS DE COBRANZA PAQUETERIA","");
