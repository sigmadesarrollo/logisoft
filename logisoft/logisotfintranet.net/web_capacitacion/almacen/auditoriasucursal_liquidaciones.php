...
<script>
	...
	tabla1.setAttributes({
		...
		campos:[
			...
			{nombre:"EAD",medida:60, alineacion:"right", tipo:"moneda", datos:"EAD"},
			{nombre:"RAD", medida:60, alineacion:"right", tipo:"moneda", datos:"RAD"},
			{nombre:"SEGURO", medida:60, alineacion:"right", tipo:"moneda", datos:"seguro"},
			{nombre:"OTROS", medida:60, alineacion:"right", tipo:"moneda", datos:"otros"},
			{nombre:"EXCED", medida:60, alineacion:"right", tipo:"moneda", datos:"excedente"},
			{nombre:"COMB", medida:60, alineacion:"right", tipo:"moneda", datos:"combustible"},
			{nombre:"IVA RET", medida:60, alineacion:"right", tipo:"moneda", datos:"ivaretenido"},
	...
	function obtenerDetalle(){
		consultaTexto("resTabla1","auditoriasucursalnuevo_con.php?sucursalseleccionada=<?=$_GET[sucursalseleccionada]?>&accion=3<?=(($_GET[folio]!="")?"&folio=$_GET[folio]":"")?>&contador="+u.pag1_contador.value
		+"&s="+Math.random());
	}
	...
	function paginacion(movimiento){
		switch(movimiento){
			case 'primero':
				consultaTexto("resTabla1","auditoriasucursalnuevo_con.php?sucursalseleccionada=<?=$_GET[sucursalseleccionada]?>&accion=3&contador=0&s="+Math.random());
				break;
			case 'adelante':
				if(u.pag1_adelante.value==1){
					consultaTexto("resTabla1","auditoriasucursalnuevo_con.php?sucursalseleccionada=<?=$_GET[sucursalseleccionada]?>&accion=3&contador="+(parseFloat(u.pag1_contador.value)+1)					
					+"&s="+Math.random());
				}
				break;
			case 'atras':
			...
			case 'ultimo':
				...
				consultaTexto("resTabla1","auditoriasucursalnuevo_con.php?sucursalseleccionada=<?=$_GET[sucursalseleccionada]?>&accion=3&contador="+contador
				+"&s="+Math.random());
				break;
		}
	}
...
<form id="form1" name="form1" method="post" action="">
  <table border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
    <tr>
      <td class="FondoTabla">LIQUIDACIONES</td>
    </tr>
    <tr>
