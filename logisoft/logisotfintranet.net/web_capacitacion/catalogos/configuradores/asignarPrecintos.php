<?	session_start();	
	...
	$s = "SELECT MAX(foliofinal) AS foliofinal FROM configuradorfoliosprecintos";
...
<script>
	...
	function obtenerCantidadPrecintos(datos){
		consultaTexto("obtenerCantidad","configuradorFoliosPrecintos_con.php?accion=4");
	}
	...
	function pedirPrecintos(){
		...
		if(parseInt(u.precintos.value) < parseInt(u.cantidad.value)){
			alerta('No cuenta con Precintos suficientes en Existencia','¡Atención!','ffinal');
			return false;
		}
		
		/*if(parseFloat(u.h_final.value) >= parseFloat(u.finicial.value)){
		...
		consultaTexto("mostrarPrecintos","configuradorFoliosPrecintos_con.php?accion=2&cantidad="+u.cantidad.value
		+"&finicial="+u.finicial.value+"&ffinal="+u.ffinal.value+"&s="+Math.random());
		
		/*if(u.h_final.value != "000000000"){
	...
	function guardar(){
		<?=$cpermiso->verificarPermiso(313,$_SESSION[IDUSUARIO]);?>
		if(u.sucursal.value == 0){
			...
			alerta('No cuenta con Precintos suficientes en Existencia','¡Atención!','finicial');
		}else if(tabla1.getRecordCount()==0){
			alerta('Debe capturar por lo menos una Asignación de Precintos','¡Atención!','finicial');
		}else{
	...
	function confirmarRegistro(){
		...
		consultaTexto("registroPrecinto","configuradorFoliosPrecintos_con.php?accion=3&cantidad="+u.cantidad.value
	...
	function registroPrecinto(datos){
		if(datos.indexOf("ok")>-1){
			if(datos.indexOf("faltaron")>-1){
				alerta3('No cuenta con Precintos suficientes en Existencia','¡Atención!');
			}else{
				var row = datos.split(",");
	...
	function obtenerHistorialPrecintos(sucursal){
		consultaTexto("mostrarHistorialPrecintos","configuradorFoliosPrecintos_con.php?accion=5&sucursal="+sucursal
		+"&s"+Math.random());
	}
	...
	function validarInicial(valor){
		if(u.h_inicial.value == "000000000" && parseFloat(u.finicial.value) < parseFloat(u.h_folioinicial.value)){
			alerta('Los Folios de Precintos deben empezar en '+u.h_folioinicial.value,'¡Atención!','finicial');
			return false;
		}
		if(u.h_inicial.value == "000000000" && parseFloat(u.finicial.value) > parseFloat(u.h_folioinicial.value)){
			alerta('Los Folios de Precintos deben de empezar con el folio '+u.h_folioinicial.value,'¡Atención!','finicial');
			return false;
		}
		u.ffinal.focus();
	}
...
<form name="form1" method="post" action="">
  ...
  <table width="450" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
    ...
    <tr>
      ...
      <td class="FondoTabla">ASIGNACI&Oacute;N DE PRECINTOS</td>
