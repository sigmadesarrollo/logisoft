...
<script>
	...
	function limpiarTodo(){
		...
		bloquearObjeto(ubi.legal,false);
//		bloquearObjeto(ubi.compromiso,false);
		limpiarCliente();
		...
		accionSucursalesVentanilla(false);
		accionSucursalesEmpresarial(false);
	...
	function nuevosFolios(datos){
		...
		ubi.fecha.innerHTML=objeto.fecha;
		ubi.vigencia.innerHTML=objeto.fechalimite;
	}
...
//		bloquearObjeto(ubi.compromiso, true);
	...
	function respPedirPropuesta(datos){
		...
		ubi.fecha.innerHTML 			= objeto[0].propuesta[0].fecha;
		ubi.vigencia.innerHTML			= objeto[0].propuesta[0].vigencia ;
		...
		ubi.compromiso.value 			= objeto[0].propuesta[0].compromisomes;
		ubi.ckncliente.checked			=(objeto[0].propuesta[0].ncliente==1)?true:false;
		ubi.ckncliente.disabled=true;		
		
		if(ubi.radiogrupo[2].checked==true){
			ubi.descuentoflete.readOnly=true;
		}
		...
		if(objeto[0].propuesta[0].consignacioncaja==1){
			...
			if(objeto[0].propuesta[0].motivo!=''){
				alerta3(objeto[0].propuesta[0].motivo,"MOTIVO DE NO AUTORIZADA")
			}
	}
	...
	function agregarServicio(objeto){
		...
		accionSucursalesVentanilla(verificarServicios('V'));
	}
	...
	function agregarServicio2(objeto){
		...
		accionSucursalesEmpresarial(verificarServicios('E'));
	}
	...
	function guardarPropuesta(){		
		...
		if(ubi.nombresucursal.value=="0"){
			alerta("Por seleccione la sucursal", "¡Atención!","nombresucursal");
			return false;
		}
		
		if(ubi.legal.value==""){
			alerta("Por proporcione el representante legal", "¡Atención!","nombresucursal");
			return false;
		}
		...
		if(ubi.radiogrupo[0].checked==false && ubi.radiogrupo[1].checked==false && ubi.radiogrupo[2].checked==false &&
		ubi.radiobutton[0].checked==false && ubi.radiobutton[1].checked==false && ubi.radiobutton[2].checked==false &&
		ubi.checkprepagadas.checked==false){
			alerta("Por favor seleccione una opción del convenio", "¡Atención!","vendedor");
			return false;
		}
		...
		if((ubi.radiogrupo[1].checked && ubi.descuentoflete.value=="") || (ubi.radiogrupo[2].checked && ubi.descuentoflete.value=="")){
			seleccionarTab(0);
			alerta("Por favor ingrese el descuento sobre flete", "¡Atención!","descuentoflete");
			return false;
		}			
		
		if((ubi.radiobutton[1].checked && ubi.consignaciondes.value=="") || (ubi.radiobutton[2].checked && ubi.consignaciondes.value=="")){
			seleccionarTab(2);
			alerta("Por favor ingrese el descuento de consignación", "¡Atención!","consignaciondes");
			return false;
		}
			...
			if(ubi.compromiso.disabled==false && ubi.compromiso.value == ""){
				alerta("Por favor ingrese el compromiso mensual","¡Atención!","compromiso");
				return false;
			}
			
			var folio 					= ubi.folio.value;
			var fecha 					= ubi.fecha.innerHTML;
			var vigencia 				= ubi.vigencia.innerHTML;
			...
			var compromiso				= ubi.compromiso.value;
			var pestado 				= "";
			var esrenovacion			= ubi.esrenovacion.value;
			var provienedefolio 		= ubi.provienedefolio.value;
			
			
			var calle 					= ubi.calle.value;
			var numero 					= ubi.numero.value;
			var cp 						= ubi.cp.value;
			var colonia 				= ubi.colonia.value;
			var poblacion 				= ubi.poblacion.value;
			var municipio 				= ubi.municipio.value;
			var estado 					= ubi.estado.value;
			var pais 					= ubi.pais.value;
			var celular 				= ubi.celular.value;
			var telefono 				= ubi.telefono.value;
			var email 					= ubi.email.value;
			...
			var nclient 				= (ubi.ckncliente.checked)?"1":"0";
					
			document.all.btnguardar.style.visibility = 'hidden';
			...
			+"&porcada="+porcada+"&costoextra="+costoextra+"&legal="+legal+"&compromiso="+compromiso
			+"&calle="+calle+"&numero="+numero+"&cp="+cp+"&colonia="+colonia
			+"&poblacion="+poblacion+"&municipio="+municipio+"&estadodir="+estado+"&pais="+pais
			+"&celular="+celular+"&telefono="+telefono+"&email="+email+"&ncliente="+nclient
			+"&random="+Math.random());
	}
	function resGuardarPropuesta(res){
		...
		if(res.indexOf("guardo")>-1){
			...
			if(document.all.btnguardar){
				document.all.btnguardar.style.visibility = 'visible';
			}
		}
	}
	...
	function darEstadoPropuesta(estado){
		...
		if(ubi.motivonoautorizar.value == '' && estado == 'NO AUTORIZADA'){
			abrirVentanaFija('motivonoautorizar.php?estado=abierto', 400, 350, 'ventana', 'Motivos de no autorizacion')
		}else{
			var calle 					= ubi.calle.value;
			var numero 					= ubi.numero.value;
			var cp 						= ubi.cp.value;
			...
			var poblacion 				= ubi.poblacion.value;
			var municipio 				= ubi.municipio.value;
			var estadodir				= ubi.estado.value;
			var pais 					= ubi.pais.value;
			var celular 				= ubi.celular.value;
			var telefono 				= ubi.telefono.value;
			var email 					= ubi.email.value;
			...
			consultaTexto("resDarEstadoP","propuestaconvenio_con.php?accion=8&estado="+estado+"&folio="+ubi.folio.value
				+'&motivonoautorizar='+ubi.motivonoautorizar.value+"&calle="+calle+"&numero="+numero+"&cp="+cp+"&colonia="+colonia
				+"&poblacion="+poblacion+"&municipio="+municipio+"&estadodir="+estadodir+"&pais="+pais+"&celular="+celular
				+"&telefono="+telefono+"&email="+email+"&compromiso="+ubi.compromiso.value+"&ncliente="+nclient);
		}
	}
	...
	function guardarPropuesta2(){
		...
		+"&consignacioncaja="+consignacioncaja+"&consignaciondescuento="+consignaciondescuento
		+"&consignaciondescantidad="+consignaciondescantidad+"&preciokgexcedente="+preciokgexcedente
		+"&valordeclarado="+valordeclarado+"&limite="+limite+"&ncliente="+nclient
		+"&porcada="+porcada+"&costoextra="+costoextra+"&legal="+legal+"&compromiso="+compromiso+"&random="+Math.random());
	...
	function accionSucursalesVentanilla(valor){
		valor =! valor;
		ubi.chk_sucead1.disabled = valor;
		ubi.sucursalesead1.disabled = valor;
		ubi.sucursalesead1_sel.disabled = valor;
		if(valor==true){
			ubi.chk_sucead1.checked = false;
			ubi.sucursalesead1_sel.options.length=0;
		}
	...
	function accionSucursalesEmpresarial(valor){
		valor =! valor;
		ubi.chk_sucead2.disabled = valor;
		ubi.sucursalesead3.disabled = valor;
		ubi.sucursalesead3_sel.disabled = valor;
		if(valor==true){
			ubi.chk_sucead2.checked = false;
			ubi.sucursalesead3_sel.options.length=0;
		}
	...
	function verificarServicios(tipo){
		if(tipo == 'V'){
			for(var i=0; i< tablaservicios.getRecordCount(); i++){
				if(document.all['tabladeservicios_Servicio'][i].value.toUpperCase() == "E.A.D." || document.all['tabladeservicios_Servicio'][i].value.toUpperCase() == "RECOLECCION")
					return true;
			}
		}else{
			for(var i=0; i< tablaservicios2.getRecordCount(); i++){
				if(document.all['tabladeservicios2_Servicio'][i].value.toUpperCase() == "E.A.D." || document.all['tabladeservicios2_Servicio'][i].value.toUpperCase() == "RECOLECCION")
					return true;
			}
		}
		return false;
	...
	function imprimirPropuesta(){
		if(document.URL.indexOf("web/")>-1){		
			window.open("https://www.pmmintranet.net/web/fpdf/reportes/propuesta.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+ubi.folio.value);			
		}else if(document.URL.indexOf("web_capacitacion/")>-1){
			window.open("https://www.pmmintranet.net/web_capacitacion/fpdf/reportes/propuesta.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+ubi.folio.value);			
		}else if(document.URL.indexOf("web_pruebas/")>-1){
			window.open("https://www.pmmintranet.net/web_pruebas/fpdf/reportes/propuesta.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+ubi.folio.value);
		}
	}
</script>
          ...
          <div class="dvTable clearfix" style="width:250px; color:#FFFFFF;">
            <div class="c1">Folio</div>
              ...
              <input type="button" id="search3" value=" " class="srch-btn" title="Buscar" style="float:right" onClick="abrirVentanaFija('../buscadores_generales/buscarPropuestaConvenioGen.php?pestado=TODOS&funcion=solicitarPropuestaConvenio', 630, 450, 'ventana', 'Propuestas pendientes de Aceptacion');"/></div>
                <div class="c1" style="height:15px;">Fecha</div>
				<div class="c2" id="fecha" style="height:15px;"><?=date('d/m/Y') ?></div>
                <div class="c1" style="height:15px;">Estado</div>
                <div class="c2" id="estadopropuesta" style="height:15px;"></div>
                <div class="c1" style="height:15px;">Vigencia</div>
                <div class="c2" id="vigencia" style="height:15px;"></div>
                <div class="c1" style="height:15px;">Sucursal</div>
                <div class="c2" style="height:15px;">                
                  <select name="nombresucursal" id="nombresucursal" style="width:145px">
                    <option value="0">.::Seleccione::.</option>
					<?
					$s = "select cs.descripcion as sdesc, cs.id as idsuc from catalogosucursal as cs where id <> 1 order by descripcion asc";
					$r = mysql_query($s,$l) or die($s);
					while($f = mysql_fetch_object($r)){
					
					?>
					<option value="<?=$f->idsuc?>"><?=utf8_decode($f->sdesc)?></option>
					<?
						}
					?>
                  </select>
            ...
            <div class="c2"><input name="vendedor" type="text" class="text" id="vendedor" style="width:120px;" onkeypress="if(event.keyCode==13){pedirEmpleado(this.value)}else{return solonumeros(event)}"/>
              <input name="buscarvendedor" type="button" class="srch-btn" id="search3" style="float:right" title="Buscar" value=" " onClick="abrirVentanaFija('../buscadores_generales/buscarEmpleado.php?funcion=pedirEmpleado&empleadodefault=1&tipo=29&sucursal='+ubi.nombresucursal.value, 625, 550, 'ventana', 'Busqueda')"/>
                ...
                <div class="c2" style="width:200px; padding-left:50px; font-size:12px; height:12px; overflow:hidden" id="vendedorb"></div>
...
Cliente</span></td>
                          <td width="41%"><input name="personamoral" id="personamoral" type="radio" value="SI" checked onClick="document.all.personamoral_valor.value = 'SI'; limpiarCliente()" />
Persona Moral
  <input name="personamoral" id="personamoral" type="radio" value="NO" onClick="document.all.personamoral_valor.value = 'NO'; limpiarCliente()" /> <input type="hidden" name="personamoral_valor" value="SI">
Persona F&iacute;sica</td>
					...
					<tr>
					  ...
					  <th width="166"><input name="prospecto" type="text" class="text" id="prospecto" style="width:120px;" onKeyPress="if(event.keyCode==13){if(ubi.clienterdo[0].checked){pedirProspecto(this.value)}else{pedirCliente(this.value)}}else{return solonumeros(event)}"/>
					  <input name="buscarprospecto" type="button" class="srch-btn" id="search" title="Buscar" value=" " onClick="mostrarBuscador();"/></th>
					  <th width="76">&nbsp;</th>
					  <th width="185">Cliente Nuevo<input name="ckncliente" type="checkbox" value="" />
					  <input name="esrenovar" type="hidden" id="esrenovar"><input type="hidden" name="esrenovacion" value="" />
        			  <input type="hidden" name="provienedefolio" value="" /><input type="hidden" name="motivonoautorizar" value="" />
			        </th>
				  </tr>
					<tr>
					  <th>Nick</th>
					...
					<tr>
					  <th>Calle</th>
					  <th id="celda_des_calle"><input name="calle" type="text" class="text" id="calle" style="width:120px; height:9px; font-size:9px; margin:0px;" /></th>
					  <th>Número</th>
					  <th><input name="numero" type="text" class="text" id="numero" style="width:120px; height:9px; font-size:9px; margin:0px;" /></th>
				  </tr>
					<tr>
					  <th>CP</th>
					  <th><input name="cp" type="text" class="text" id="cp" style="width:120px; height:9px; font-size:9px; margin:0px;" /></th>
					  <th>Colonia</th>
					  <th><input name="colonia" type="text" class="text" id="colonia" style="width:120px; height:9px; font-size:9px; margin:0px;" /></th>
				  </tr>
					<tr>
					  <th>Población</th>
					  <th><input name="poblacion" type="text" class="text" id="poblacion" style="width:120px; height:9px; font-size:9px; margin:0px;" /></th>
					  <th>Mun./Deleg</th>
					  <th><input name="municipio" type="text" class="text" id="municipio" style="width:120px; height:9px; font-size:9px; margin:0px;" /></th>
				  </tr>
					<tr>
					  <th>Estado</th>
					  <th><input name="estado" type="text" class="text" id="estado" style="width:120px; height:9px; font-size:9px; margin:0px;" /></th>
					  <th>País</th>
					  <th><input name="pais" type="text" class="text" id="pais" style="width:120px; height:9px; font-size:9px; margin:0px;" /></th>
				  </tr>
					<tr>
					  <th>Celular</th>
					  <th><input name="celular" type="text" class="text" id="celular" style="width:120px; height:9px; font-size:9px; margin:0px;" /></th>
					  <th>Teléfono</th>
					  <th><input name="telefono" type="text" class="text" id="telefono" style="width:120px; height:9px; font-size:9px; margin:0px;" /></th>
				  </tr>
					<tr>
					  <th>Email</th>
					  <th><input name="email" type="text" class="text" id="email" style="width:120px; height:9px; font-size:9px; margin:0px;" /></th>
					  <th>Repte. Legal</th>
				  ...
				  <tr>
					...
					<tr>
					  <th>Costo</th>
					  <th><input name="valordeclarado" id="valordeclarado" type="text" class="text" style="width:120px; height:9px; font-size:9px; margin:0px;" onKeyPress="if(event.keyCode==13){ this.value = convertirMoneda(this.value);document.all.limite.focus();}else{return tiposMoneda(event,this.value)}" readonly="readonly" /></th>
					  <th>L&iacute;mite</th>
					  <th><input name="limite" id="limite" class="text" style="width:120px; height:9px; font-size:9px; margin:0px;" onKeyPress="if(event.keyCode==13){document.all.porcada.focus();}else{return solonumeros2(event)}" readonly="readonly"/></th>
				  </tr>
					<tr>
					  <th>Por cada</th>
					  <th><input name="porcada" id="porcada" class="text" style="width:120px; height:9px; font-size:9px; margin:0px;" onKeyPress="if(event.keyCode==13){document.all.costoextra.focus();}else{return solonumeros2(event)}" readonly="readonly" /></th>
					  <th>Costo Extra</th>
					  <th><input name="costoextra" id="costoextra" type="text" class="text" style="width:120px; height:9px; font-size:9px; margin:0px;" onKeyPress="if(event.keyCode==13){this.value = convertirMoneda(this.value);document.all.radiogrupo[0].focus();}else{return tiposMoneda(event,this.value)}" readonly="readonly"/></th>
				  </tr>
				</tbody>
				</table>
          </div>
          ...
          <tr>
            ...
            onClick="ubi.div_descripcion.innerHTML = conveniodescripcion; ubi.descuentoflete.value=''; ubi.descuentoflete.readOnly=true; ubi.descuentoflete.style.backgroundColor = '#FFFF99'; document.all.boton_agregarpreciokg.style.visibility = ((this.checked==true)?'hidden':'visible');  if(!this.checked){ubi.div_preciokg.innerHTML = conveniopesokg; ubi.tienedatoskg.value=0} bloquearObjeto(ubi.boton_agregarcajapaquete,true); ubi.tienedatoskg.value = 0;  ubi.tienedatoskg_excedio.value = 0; ubi.tienedatosprecio.value = 0; ubi.tienedatosprecio_excedio.value = 0; " onDblClick="desahabilitarNormal('1')" />-->
			<input type="radio" name="radiogrupo" value="checkbox" onClick="ubi.div_descripcion.innerHTML = conveniodescripcion; ubi.descuentoflete.value=''; ubi.descuentoflete.readOnly=true; ubi.descuentoflete.style.backgroundColor = '#FFFF99'; document.getElementById('boton_agregarpreciokg').style.visibility = 'visible'; document.getElementById('boton_agregarcajapaquete').style.visibility = 'hidden'; if(!this.checked){ubi.div_preciokg.innerHTML = conveniopesokg; ubi.tienedatoskg.value=0} bloquearObjeto(ubi.boton_agregarcajapaquete,true); ubi.tienedatoskg.value = 0;  ubi.tienedatoskg_excedio.value = 0; ubi.tienedatosprecio.value = 0; ubi.tienedatosprecio_excedio.value = 0;"  onDblClick="desahabilitarNormal('1')" />
            Precio por KG</td>
            <td colspan="2"><input type="radio" name="radiogrupo" value="checkbox" 
             onclick=" ubi.div_preciokg.innerHTML = conveniopesokg; ubi.descuentoflete.value=''; document.all.boton_agregarcajapaquete.style.visibility = 'visible'; if(!this.checked){ubi.div_descripcion.innerHTML = conveniodescripcion; ubi.tienedatosprecio.value=0}else{ubi.descuentoflete.style.backgroundColor = ''; ubi.descuentoflete.readOnly=false;}bloquearObjeto(ubi.boton_agregarpreciokg,true);document.getElementById('boton_agregarpreciokg').style.visibility = 'hidden';" ondblclick="desahabilitarNormal('2')"/>
            Precio por Caja/Paquete </td>
            <td width="208"><input type="radio" name="radiogrupo" onclick="ubi.descuentoflete.readOnly=false; ubi.descuentoflete.style.backgroundColor = ''; ubi.div_preciokg.innerHTML = conveniopesokg; ubi.div_descripcion.innerHTML = conveniodescripcion; bloquearObjeto(ubi.boton_agregarpreciokg,true);	bloquearObjeto(ubi.boton_agregarcajapaquete,true); document.getElementById('boton_agregarpreciokg').style.visibility = 'hidden'; document.getElementById('boton_agregarcajapaquete').style.visibility = 'hidden';" ondblclick="desahabilitarNormal('3')" />
            <?
            ...
            <td width="121"><input name="descuentoflete" type="text" class="text" id="descuentoflete" onkeypress="return solonumeros(event)" style="width:100px;font-size:11px; margin:0px;" readonly="readonly" onblur="if(document.all['radiogrupo'][0].checked==false){ if(this.value > '<?=$f->desmaximopermitido?>'){alerta3('El descuento maximo permitido es de <?=$f->desmaximopermitido?>','&iexcl;Atenci&oacute;n!'); this.value = '<?=$f->desmaximopermitido?>';} }" /></td>
      </tr>
	  ...
	  <input name="boton_agregarpreciokg" id="boton_agregarpreciokg" style="visibility:hidden" type="button" class="button" onclick="abrirVentanaFija('conveniopreciokg.php?tienedatoskg='+document.all.tienedatoskg.value, 620, 418, 'ventana', 'Precio por KG')" value="Agregar" />
        ...
        <td width="320" align="center" style="font-size:20"><table width="283" border="0" align="left" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td colspan="11"><table width="266" border="0" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <input type="checkbox" name="chk_sucead1" disabled="disabled" onClick="todos(ubi.sucursalesead1, ubi.sucursalesead1_sel, this.checked, 'SUCONVENIO')">Todos
            	 <select name="sucursalesead1" style="width:200px" class="Tablas" disabled="disabled" onChange="insertarServicio(this, this.value, document.all.sucursalesead1_sel, 'La Sucursal', 'SUCONVENIO')")>
                 <option value=""></option>
              	<? 
          ...
          <tr>
            <td colspan="11"><div align="center">
                 <select name="sucursalesead1_sel" disabled="disabled" size="7" style="width:265px" onDblClick="borrarServicio(this, 'SUCONVENIO')">
              </select>
            </div></td>
          </tr>
        </table></td>
		  ...
		  <td><table width="100%" border="0" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td width="181"><span class="Tablas">
				...
				$f = mysql_fetch_object($r);
			?>
                <input name="consignaciondes" type="text" class="text" id="consignaciondes" style="width:100px;font-size:11px; margin:0px;" onkeypress="return solonumeros(event)" readonly="readonly" value="<?=$consignaciondes ?>" onblur="if(document.all['radiogrupo'][0].checked==false){ if(this.value><?=$f->desmaximopermitido?>){alerta3('El descuento maximo permitido es de <?=$f->desmaximopermitido?>','&iexcl;Atenci&oacute;n!'); this.value = '<?=$f->desmaximopermitido?>';} }" />
              </span></td>
...
<tr>
		...
		<tr>
		    <td>
			<input name="botonagregarservicio2" id="botonagregarservicio2" type="button" class="button" onclick="abrirVentanaFija('servicios.php?tipo=CONSIGNACION&fagregar=agregarServicio2&fborrar=borrarServicios2&fmodificar=modificarServicios2&limpiar='+ubi.servicios2.value, 400, 350, 'ventana', 'Servicios')" value="Agregar" />
        ...
        <td width="320" align="center" style="font-size:20"><table width="283" border="0" align="left" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td colspan="11"><table width="266" border="0" cellpadding="0" cellspacing="0">
                <tr>
                  <td colspan="12" style="text-align:center; color:#FFFFFF">
                  <input type="checkbox" name="chk_sucead2" disabled="disabled" onClick="todos(ubi.sucursalesead3, ubi.sucursalesead3_sel, this.checked, 'SUCONSIGNACION2');">Todos
                  <select name="sucursalesead3" style="width:200px" class="Tablas" disabled="disabled" onChange="insertarServicio(this, this.value, document.all.sucursalesead3_sel, 'La Sucursal', 'SUCONSIGNACION2')")>
                      <option value=""></option>
                      <? 
					$s = "select * from catalogosucursal where id > 1 order by descripcion asc";
					$r = mysql_query($s,$l) or die($s);
          ...
          <tr>
            <td colspan="11"><div align="center">
                <select name="sucursalesead3_sel" size="7" disabled="disabled" style="width:265px" onDblClick="borrarServicio(this, 'SUCONSIGNACION2')">
                </select>
            </div></td>
          </tr>
        </table></td>
