...
<script>
	...
	tabla1.setAttributes({
		...
		campos:[
			...
			{nombre:"SUCURSALDESCRIPCION", medida:4, tipo:"oculto",  alineacion:"center",  datos:"sucursaldescripcion"}
	...
	function mostrarSucursal(datos){
		if(datos!="0"){
			...
			u.sucursal_hidden.value = obj[0].id;
			u.rtipo[0].focus();
		}else{
			alerta("El codigo de Sucursal no existe","¡Atencion!","sucursal");
		}
	}
	function obtenerSucursal(id,prefijo,descripcion){
		u.sucursal_hidden.value  = id;
		u.sucursal.value = prefijo;
	...
	v_suc = tabla1.getValuesFromField("sucursal",",");
		if(u.modifico.value==""){
			if(v_suc!=""){
				if(v_suc.indexOf(u.sucursal.value)>-1){
		...
		obj.idsucursal	= u.sucursal_hidden.value;
		obj.sucursal	= u.sucursal.value;
		obj.sucursaldescripcion	= u.sucursalb.value;
	...
	function ModificarFila(){
		if(tabla1.getValSelFromField('sucursal','SUCURSAL')!=""){
			...
			if(obj.tipo == 1){
				...
				u.sucursalb.value	= obj.sucursaldescripcion;
				u.sucursal_hidden.value	= obj.idsucursal;
...
D </label>
        ...
        <tr>
          ...
          <td colspan="3"><span class="Tablas"><img src="../../img/Buscar_24.gif" width="24" height="23" align="absbottom" style="cursor:pointer" title="Buscar Sucursal" onclick="abrirVentanaFija('catalogosrutas_Buscar.php?tipo=2', 600, 500, 'ventana', 'Busqueda')" /></span><span class="Tablas">&nbsp;&nbsp;&nbsp;&nbsp;
            ...
            <input name="sucursal_hidden" type="hidden" id="sucursal_hidden" size="5" />
          </span></td>
          <td>&nbsp;</td>
        </tr>
        <tr>
