<?
	...
	if ($_GET[tipo]=="1"){
		/*
		SELECT rv.idcliente cliente, rv.nombrecliente nombre, rv.destino, rv.folio guia, rv.total importe
		...
		AND ISNULL(rv.convenio) AND rv.activo='S' AND cs.prefijo = '".$_GET[sucursal]."' AND rv.tipoventa='GUIA VENTANILLA'
		GROUP BY cs.id*/
		$sql = "SELECT rv.idcliente cliente, rv.nombrecliente nombre, rv.destino, rv.folio guia, sum(rv.total) importe
		FROM reportes_ventas rv
		INNER JOIN catalogosucursal cs ON IF(NOT ISNULL(rv.sucursalfacturo),rv.sucursalfacturo=cs.descripcion,rv.sucursalrealizo=cs.id)
		WHERE rv.fecharealizacion BETWEEN '".cambiaf_a_mysql($_GET[fechaini])."' AND '".cambiaf_a_mysql($_GET[fechafin])."'
		AND NOT ISNULL(rv.convenio) AND rv.activo='S' AND cs.prefijo = '".$_GET[sucursal]."' and tipopago = 'CONTADO'
		AND rv.tipoflete = 'PAGADA'
		GROUP BY cs.id
		LIMIT 0,30";
		...
		$sql = "select rv.idcliente cliente, rv.nombrecliente nombre, IF(rv.tipoventa='FACTURA OTROS',rv.tipoventa,rv.destino) destino, 
		IF(rv.tipoventa='FACTURA OTROS',CONCAT('FACT-',rv.folio),rv.folio) guia, sum(rv.total) importe
		FROM reportes_ventas rv
		INNER JOIN catalogosucursal cs ON IF(NOT ISNULL(rv.sucursalfacturo),rv.sucursalfacturo=cs.descripcion,rv.sucursalrealizo=cs.id)
		WHERE rv.fecharealizacion BETWEEN '".cambiaf_a_mysql($_GET[fechaini])."' AND '".cambiaf_a_mysql($_GET[fechafin])."'
		AND ISNULL(rv.convenio) AND rv.activo='S' AND cs.prefijo = '".$_GET[sucursal]."' AND (rv.tipoventa='GUIA VENTANILLA' OR rv.tipoventa='FACTURA OTROS')
		GROUP BY cs.id
		LIMIT 0,30";
	}
