<?	
	...
	ROUND(
			SUM(IF(rcd.tipo='V' AND rcd.condicion='PAGADA-CONTADO',rcd.totalgral,0)) +
			SUM(IF(rcd.tipo='V' AND rcd.condicion='PAGADA-CREDITO',rcd.totalgral,0)) +
			SUM(IF(rcd.tipo='R' AND rcd.condicion='POR COBRAR-CONTADO',rcd.totalgral,0)) +
			SUM(IF(rcd.tipo='R' AND rcd.condicion='POR COBRAR-CREDITO',rcd.totalgral,0)) 
			  ,2)total,
	ROUND(SUM(rcd.totalcom),2)importe,
	ROUND((
		SUM(IF(rcd.tipo='V' AND rcd.condicion='PAGADA-CONTADO',rcd.totalgral,0)) +
		SUM(IF(rcd.tipo='V' AND rcd.condicion='PAGADA-CREDITO',rcd.totalgral,0)) +
		SUM(IF(rcd.tipo='R' AND rcd.condicion='POR COBRAR-CONTADO',rcd.totalgral,0)) +
		SUM(IF(rcd.tipo='R' AND rcd.condicion='POR COBRAR-CREDITO',rcd.totalgral,0))
	)-SUM(rcd.totalcom),2)liquidar
