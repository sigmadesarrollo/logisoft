<?	session_start();
	...
	if($_GET[accion]==1){//OBTENER ESTADO DE CUENTA	
		...
		WHERE estado = 'ACTIVA' AND pagado = 'N' AND folio<>0 $andsucursal $folioCliente /* and folio not like '%Z' */ and folio >0
		AND (tipo='V' OR (tipo='E' AND NOT ISNULL(factura)))";
		$r = mysql_query($s,$l) or die($s);
		$totalregistros = mysql_num_rows($r);
		...
		$andsucursal $folioCliente /* and folio not like '%Z' */ and folio >0
		AND (rc.tipo='V' OR (rc.tipo='E' AND NOT ISNULL(rc.factura)))
		group by rc.folio
		$limite";
		$r = mysql_query($s,$l) or die($s);
		$ar = array();
