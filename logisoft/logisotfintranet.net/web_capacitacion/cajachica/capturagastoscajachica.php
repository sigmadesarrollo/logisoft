      ...
      <tr>
        <td><table width="532" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td width="100"><span class="Tablas">
              <input name="folio" type="text" class="Tablas" id="folio" style="width:100px;" value="<?=$_POST['folio'] == "" ? $folio : $_POST['folio'] ?>" onkeypress="if(event.keyCode=='13'){ObtenerFolio(this.value);}" />
            </span></td>
        ...
        <td><table width="534" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td width="114"><span class="Tablas">
              ...
              value="<?=$_POST['unidad'] == "" ? "" : $_POST['unidad'] ?>" onkeypress="if(event.keyCode=='13'){CargarUnidad(this.value);}" />
            </span></td>
            <td>
...
<script>	
	...
	function CargarUnidad(unidad){
		consultaTexto("ObtenerUnidad2","cajachica_con.php?accion=5&unidad="+unidad+"&sucursal="+<?=$_SESSION[IDSUCURSAL]?>);
	}
	
	function ObtenerUnidad2(datos){
		if(datos.indexOf("no encontro")<0){
			var obj = eval(convertirValoresJson(datos));
			document.getElementById('idunidad').value=obj[0].id;
			document.getElementById('unidad').value=obj[0].numeroeconomico;
		}else{
			alerta3("La unidad no es valida","¡Atención!");
