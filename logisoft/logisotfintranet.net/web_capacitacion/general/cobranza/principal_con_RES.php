<? session_start();
	...
	if ($_GET[accion]==1){
$sql="(SELECT tabla.sucursal,cs.prefijo as nombresucursal,ifnull(cl.contador,0)AS carteracredito,SUM(tabla.carteravigente) AS carteravigente,SUM(tabla.carteramorosa)AS carteramorosa,(IFNULL(SUM(tabla.carteravigente),0)+IFNULL(SUM(tabla.carteramorosa),0))AS carteratotal 
FROM (
	...
	IFNULL(CASE 
		WHEN '" .cambiaf_a_mysql($_GET[fecha])."'<DATE_ADD(pg.fechacreo,INTERVAL sc.diascredito+1 DAY) THEN SUM(pg.total)
	...
	GROUP BY pg.sucursalacobrar,pg.guia
)tabla 
INNER JOIN catalogosucursal cs on tabla.sucursal=cs.id 
LEFT JOIN 
(
	SELECT sucursal,count(*)as contador from(
		SELECT pg.sucursalacobrar AS sucursal,pg.cliente 
		FROM pagoguias pg
		INNER JOIN solicitudcredito sc ON pg.cliente=sc.cliente
		WHERE sc.estado='ACTIVADO' AND sc.fechaactivacion<='" .cambiaf_a_mysql($_GET[fecha])."' and pg.pagado='N'
		GROUP BY pg.sucursalacobrar,pg.cliente
	)Tabla 
	group by sucursal
)cl on tabla.sucursal=cl.sucursal
GROUP BY tabla.sucursal)
union
(SELECT tabla.sucursal,cs.prefijo as nombresucursal,ifnull(cl.contador,0)AS carteracredito,
SUM(tabla.carteravigente) AS carteravigente,SUM(tabla.carteramorosa)AS carteramorosa,
(IFNULL(SUM(tabla.carteravigente),0)+IFNULL(SUM(tabla.carteramorosa),0))AS carteratotal 
FROM (
	SELECT f.idsucursal AS sucursal,f.cliente,SUM(f.total+f.sobmontoafacturar+f.otrosmontofacturar) AS total,
	DATE_ADD(f.fecha,INTERVAL sc.diascredito DAY)AS fechavencimiento,
	IFNULL(CASE 
		WHEN '" .cambiaf_a_mysql($_GET[fecha])."'<DATE_ADD(f.fecha,INTERVAL sc.diascredito+1 DAY) THEN SUM(f.total+f.sobmontoafacturar+f.otrosmontofacturar)
	END,0) AS carteravigente,
	IFNULL(CASE 
	WHEN '" .cambiaf_a_mysql($_GET[fecha])."'>=DATE_ADD(f.fecha,INTERVAL sc.diascredito+1 DAY) THEN SUM(f.total+f.sobmontoafacturar+f.otrosmontofacturar)
	END,0) AS carteramorosa
	...
	WHERE sc.estado='ACTIVADO' AND f.credito='SI' AND
	sc.fechaactivacion<='" .cambiaf_a_mysql($_GET[fecha])."' AND f.estadocobranza<>'C'
	GROUP BY f.idsucursal, f.cliente
)tabla 
INNER JOIN catalogosucursal cs on tabla.sucursal=cs.id 
LEFT JOIN 
(
	SELECT sucursal,count(*)as contador from(
		SELECT f.idsucursal AS sucursal,f.cliente 
		FROM facturacion f
		INNER JOIN solicitudcredito sc ON f.cliente=sc.cliente
		WHERE sc.estado='ACTIVADO'  AND f.credito='SI'
		AND sc.fechaactivacion<='" .cambiaf_a_mysql($_GET[fecha])."' and f.estadocobranza<>'C'
		GROUP BY f.idsucursal, f.cliente
	)Tabla 
	group by sucursal
)cl on tabla.sucursal=cl.sucursal
...
(
	/*GUIAS EMPRESARIALES*/
	SELECT gv.fecha,cs.prefijo AS sucursal,gv.id AS referenciacargo,0 AS referenciaabono, 
	gv.total AS cargo, 0 AS abono,0 AS saldo,'Guia Crédito Foraneo' AS descripcion FROM guiasempresariales gv
	...
	AND */ ISNULL(gv.factura) AND cc.id='" .$_GET[cliente]. "'
UNION
	/*GUIAS EMPRESARIALES*/
	...
	INNER JOIN catalogosucursal cs ON cs.id= pg.sucursalacobrar	
	WHERE /*gv.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."'  
	AND */ gv.factura<>0 AND f.cliente='" .$_GET[cliente]. "'
	GROUP BY f.folio
UNION
	/*GUIAS VENTANILLA*/
	SELECT gv.fecha,cs.prefijo AS sucursal,gv.id AS referenciacargo,0 AS referenciaabono, 
	gv.total AS cargo, 0 AS abono,0 AS saldo,'Guia Crédito Foraneo' AS descripcion FROM guiasventanilla gv
	INNER JOIN pagoguias pg ON gv.id=pg.guia
	INNER JOIN catalogosucursal cs ON cs.id= pg.sucursalacobrar	
	INNER JOIN catalogocliente cc ON cc.id= pg.cliente
	WHERE /*gv.fecha	BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."'  
...
UNION
	...
	CONCAT('PAGO',' ',IF(a.efectivo>0,'EFECTIVO',' '),',',IF(a.banco>0,'BANCO',' '),',',IF(a.cheque>0,
	'CHEQUE',' '),',',IF(a.tarjeta>0,'TARJETA',' '),',',IF(a.transferencia>0,'TRANSFERENCIA',' '))AS descripcion 
	FROM abonodecliente a
	INNER JOIN catalogosucursal cs ON a.idsucursal=cs.id 
	WHERE /*a.fecharegistro BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."'  AND */ a.idcliente='" .$_GET[cliente]. "'
	GROUP BY a.factura
UNION
	...
	CONCAT('PAGO',' ',IF(fp.efectivo>0,'EFECTIVO',' '),',',IF(fp.banco>0,'BANCO',' '),',',IF(fp.cheque>0,'CHEQUE',' '),',',IF(fp.tarjeta>0,'TARJETA',' '),',',IF(fp.transferencia>0,'TRANSFERENCIA',' '))AS descripcion FROM formapago fp
	INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id 
	INNER JOIN pagoguias pg ON fp.guia=pg.guia
	WHERE /*fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' AND */ fp.procedencia='G' AND pg.cliente='" .$_GET[cliente]. "'
	GROUP BY fp.guia
UNION
	...
	CONCAT('PAGO',' ',IF(lcd.efectivo>0,'EFECTIVO',' '),',',IF(lcd.banco>0,'BANCO',' '),',',
	IF(lcd.cheque>0,'CHEQUE',' '),',',IF(lcd.tarjeta>0,'TARJETA',' '),',',
	IF(lcd.transferencia>0,'TRANSFERENCIA',' '))AS descripcion 
	...
	WHERE /*lc.fechaliquidacion BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."'  
	AND */ lcd.cliente='" .$_GET[cliente]. "' AND lcd.cobrar='SI'
	GROUP BY lcd.factura
UNION
	/*CANCELACION GUIAS VENTANILLA*/
	SELECT gv.fecha,cs.prefijo AS sucursal,0 AS referenciacargo,
	CONCAT('CANCELACION-','',gv.id)AS referenciaabono,0 AS cargo, gv.total AS abono,
	0 AS saldo,'Cancelacion de Guia' AS descripcion FROM guiasventanilla gv
	INNER JOIN pagoguias pg ON gv.id=pg.guia
	INNER JOIN catalogosucursal cs ON cs.id= pg.sucursalacobrar	
	INNER JOIN catalogocliente cc ON cc.id= pg.cliente
	WHERE /*gv.fecha	BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' 
	AND */ ISNULL(gv.factura) AND cc.id='" .$_GET[cliente]. "' AND gv.estado='CANCELADO'
UNION
...
FROM (
	...
	WHERE /*pg.fechacreo BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' AND*/ pg.pagado='N'
	AND ISNULL(gv.factura) AND sc.estado='ACTIVADO' $concatenacion2 $concatenacion
UNION
...
UNION
	...
	ON f.folio=cr.factura
	WHERE /*pg.fechacreo BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' AND */ pg.pagado='N'
	AND sc.estado='ACTIVADO' AND gv.factura<>0 $concatenacion2 $concatenacion
	GROUP BY f.folio)Tabla ORDER BY clavecliente,fecha LIMIT ".$_GET[inicio].",30";
	$r=mysql_query($sql,$l)or die($sql); 
...
(
	...
	INNER JOIN catalogosucursal cs ON cs.id= pg.sucursalacobrar	
	INNER JOIN catalogocliente cc ON cc.id= pg.cliente
	WHERE gv.fecha	BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."'  AND ISNULL(gv.factura) AND cc.id='" .$_GET[cliente]. "'
UNION
	/*GUIAS EMPRESARIALES*/
	SELECT gv.fecha,cs.prefijo AS sucursal,CONCAT('FAC-','',f.folio) AS referenciacargo,0 AS referenciaabono, 
	f.total AS cargo, 0 AS abono,0 AS saldo,'Guia Crédito Foraneo' AS descripcion FROM facturacion f
...
UNION
	...
	0 AS abono,0 AS saldo,'Guia Crédito Foraneo' AS descripcion FROM guiasventanilla gv
	INNER JOIN pagoguias pg ON gv.id=pg.guia
	INNER JOIN catalogosucursal cs ON cs.id= pg.sucursalacobrar	
	INNER JOIN catalogocliente cc ON cc.id= pg.cliente
	WHERE gv.fecha	BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."'  
...
UNION
	...
	INNER JOIN pagoguias pg ON gv.id=pg.guia
	INNER JOIN catalogosucursal cs ON cs.id= pg.sucursalacobrar	
	WHERE gv.fecha	BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."'  AND gv.factura<>0 AND f.cliente='" .$_GET[cliente]. "'
	GROUP BY f.folio	
UNION
	/*ABONOS CLIENTE*/
	...
	CONCAT('PAGO',' ',IF(a.efectivo>0,'EFECTIVO',' '),',',IF(a.banco>0,'BANCO',' '),',',IF(a.cheque>0,'CHEQUE',' '),',',IF(a.tarjeta>0,'TARJETA',' '),',',IF(a.transferencia>0,'TRANSFERENCIA',' '))AS descripcion FROM abonodecliente a
	INNER JOIN catalogosucursal cs ON a.idsucursal=cs.id 
	WHERE a.fecharegistro BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."'  AND a.idcliente='" .$_GET[cliente]. "'
	GROUP BY a.factura
UNION
	...
	INNER JOIN liquidacioncobranzadetalle lcd ON lc.folio=lcd.folioliquidacion
	INNER JOIN catalogosucursal cs ON lc.sucursal=cs.id
	WHERE lc.fechaliquidacion BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."'  AND  lcd.cliente='" .$_GET[cliente]. "' AND lcd.cobrar='SI'
	GROUP BY lcd.factura
UNION
	/*CANCELACION GUIAS VENTANILLA*/
	...
	INNER JOIN catalogosucursal cs ON cs.id= pg.sucursalacobrar	
	INNER JOIN catalogocliente cc ON cc.id= pg.cliente
	WHERE gv.fecha	BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."'  AND ISNULL(gv.factura) AND cc.id='" .$_GET[cliente]. "' AND gv.estado='CANCELADO'
UNION
	/*CANCELACION GUIAS EMPRESARIALES*/
	SELECT gv.fecha,cs.prefijo AS sucursal,0 AS referenciacargo,CONCAT('CANCELACION-','',gv.id)AS referenciaabono,0 AS cargo, gv.total AS abono,0 AS saldo,'Cancelacion de Guia' AS descripcion FROM guiasempresariales gv
	INNER JOIN pagoguias pg ON gv.id=pg.guia
