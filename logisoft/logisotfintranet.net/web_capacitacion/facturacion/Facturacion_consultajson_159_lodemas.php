<?
	...
	$_POST[sucorigen] = 2;
	$_POST[cliente] = 159;
	$_POST[nombrecliente] = "COMERCIAL D PORTENIS SA DE CV";
	$_POST[rfc] = "CDP9501269M5";
	$_POST[credito] = "SI";
	$_POST[calle] = "Carretera Internacional";
	$_POST[numero] = "Nte Km 1205 INT-A";
	$_POST[sustitutode] = "";
	$_POST[codigopostal] = "82128";
	$_POST[colonia] = "INFONAVIT PLAYAS";
	$_POST[crucecalles] = "";
	$_POST[poblacion] = "MAZATLAN";
	$_POST[municipio] = "MAZATLAN";
	$_POST[estado] = "SINALOA";
	$_POST[pais] = "";
	$_POST[telefono] = "";
	$_POST[fax] = "";
	
	/*
	(iddireccionremitente not in(51119,
				51109,
				51113,
				51114) or iddirecciondestinatario not in (51119,
				51109,
				51113,
				51114))
	*/
	...
	$s = "SELECT 
	COUNT(ge.id) cantidad,
	SUM(ge.tflete) flete, SUM(ge.ttotaldescuento) descuento, SUM(ge.tcostoead) ead,
	SUM(ge.trecoleccion) recoleccion, SUM(ge.tseguro) seguro, SUM(ge.totros) otros,
	SUM(ge.texcedente) excedente, SUM(ge.tcombustible) combustible, SUM(ge.subtotal) subtotal,
	SUM(ge.tiva) iva, SUM(ge.ivaretenido) ivaretenido, SUM(ge.total) total
	FROM guiasempresariales ge
	WHERE idremitente = 159 AND DATE(fecha) < CURRENT_DATE and ge.tipoguia = 'CONSIGNACION'
	AND ISNULL(factura) AND 
	id NOT LIKE '%Z'";
	$r = mysql_query($s,$l) or die($s);
	$f = mysql_fetch_object($r);
	
	$_POST[guiasempresa] = $f->cantidad;
 	$_POST[flete] = $f->flete;
	$_POST[totaldescuento] = $f->descuento;
	$_POST[excedente] = $f->excedente;
	$_POST[ead] = $f->ead;
	$_POST[recoleccion] = $f->recoleccion;
	$_POST[seguro] = $f->seguro;
	$_POST[combustible] = $f->combustible;
	$_POST[otros] = $f->otros;
	$_POST[subtotal] = $f->subtotal;
	$_POST[iva] = $f->iva;
	$_POST[ivaretenido] = $f->ivaretenido;
	$_POST[total] = $f->total;
		...
		tipoguia='empresarial',idsucursal='$_POST[sucorigen]', cliente = '$_POST[cliente]',
		...
		echo "el folio de la factura es -> ".$nfact;
		...
		# de la facturacion
		$arre_cliente['informacion']["municipio"] = "MAZATLAN";
		$arre_cliente['informacion']["state"] = "SINALOA";
		...
		*/
		...
		if($_POST[valorcontado]!="0" && $_POST[valorcontado]!=""){
			...
			if($_POST[credito]=="SI"){
				...
				fechacreo = CURRENT_DATE, usuariocreo = $_SESSION[IDUSUARIO], sucursalcreo = $_POST[sucorigen], 
				cliente = '$_POST[cliente]', credito='SI',
				sucursalacobrar = '$_POST[sucorigen]', pagado='N'";
				...
				nnotacredito='$_POST[nc_folio]',sucursal='$_POST[sucorigen]',usuario='$_SESSION[IDUSUARIO]',fecha=current_date";
				mysql_query(str_replace("''","null",$s),$l) or die($s);
				
				
				$s = "INSERT INTO pagoguias SET guia = '$nfact', tipo='FACT', total='$_POST[valorcontado]', 
				fechacreo = CURRENT_DATE, usuariocreo = $_SESSION[IDUSUARIO], sucursalcreo = $_POST[sucorigen], 
				cliente = '$_POST[cliente]',	sucursalacobrar = '$_POST[sucorigen]', pagado='S', 
				fechapago = CURRENT_DATE, usuariocobro = $_SESSION[IDUSUARIO], sucursalcobro = $_POST[sucorigen]";
				$r = @mysql_query(str_replace("''","null",$s),$l) or die($s);
			}
		}
		...
		SET factura = $nfact 
		WHERE DATE(fecha) < CURRENT_DATE and tipoguia = 'CONSIGNACION'
		AND ISNULL(factura) AND idremitente = 159 AND 
		id NOT LIKE '%Z'";
		mysql_query($s,$l) or die($s);
		//echo $s;
		...
		".$_SESSION[IDUSUARIO]." AS idusuario, CURRENT_TIMESTAMP
		FROM guiasempresariales AS ge 
		WHERE factura in($nfact) and ge.tipoguia = 'CONSIGNACION'";
		mysql_query($s,$l) or die($s);
...
sge.id, 'PREPAGADA' AS tipoguia, DATE_FORMAT(sge.fecha, '%d/%m/%Y') AS fecha, 
		...
		*/
		...
		if($_POST[tipoguiac]=='empresarial'){
			$s = "call proc_RegistroAuditorias('LF','$nfact',$_POST[sucorigen])";
			$d = mysql_query($s, $l);
		}
		...
		$s = "CALL proc_VentasVsPresupuesto('FACTURA','$nfact',$_POST[sucorigen]);";
		$r = @mysql_query($s,$l) or die($s);
		
		if($_POST[tipoguiac]=='empresarial'){
			$s = "CALL proc_RegistroFranquiciasConceciones('facturacion', $nfact, ".$_POST[sucorigen].", 0);";
			$r = @mysql_query($s,$l) or die($s);
		}
