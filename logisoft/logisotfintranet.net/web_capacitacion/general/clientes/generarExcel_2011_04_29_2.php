<?
	...
	if($_GET[accion] == 1){//PRINCIPAL CLIENTES
		...
		$s = "SELECT t2.sucursal,IFNULL(SUM(IF(t1.estado ='ACTIVADO' AND YEAR(t1.fecha)='".$_GET[fecha]."',1,0)),0) AS vigentes,
		...
		FORMAT(t2.timporte,2) importe,t1.sucursal as idsucursal 
	  ...
	  $s = "DROP TABLE tmp_convenio$x;";
	   ...
	   <Row ss:Index="5" ss:Height="13.5">
		...
		FORMAT(SUM(IF(rv.tipoventa='GUIA VENTANILLA',total,0)),2) AS normales, 
		FORMAT(SUM(IF(rv.tipoventa='SOLICITUD DE FOLIOS',total,0)),2) AS prepagadas, 
		...
		IF(rv.tipoventa='FACTURA OTROS' OR rv.tipoventa='FACTURA EXCEDENTE',total,0)),2) AS total,rv.idsucorigen
