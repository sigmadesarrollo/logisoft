<?	session_start();
	...
	if($_GET[accion]=="0"){//OBTENER SUCURSAL
		...
		$s = "SELECT * FROM guiasventanilla gv WHERE gv.estado = 'AUTORIZACION PARA CANCELAR' 
		...
		".(($_SESSION[IDSUCURSAL]!=1)? " AND gv.idsucursalorigen=".$_SESSION[IDSUCURSAL]."" : "")." $limite";
		$r = mysql_query($s,$l) or die($s);
		$arr = array();			
	...
	$rx = mysql_query($s,$l) or die($s);
		...
		$s = "SELECT * FROM moduloquejasdanosfaltantes
		WHERE ".(($_SESSION[IDSUCURSAL]!=1)? " idsucursal = ".$_SESSION[IDSUCURSAL]."":"")."";
		$r = mysql_query($s,$l) or die($s);
		$totalregistros = mysql_num_rows($r);		
		
		$totales = 0;
		
		/*$s = "SELECT t.guia, t.fecha, t.remitente, t.destinatario,t.destino, rd.folio AS foliodano, 
		...
		INNER JOIN catalogocliente de ON ge.iddestinatario = de.id) t ON rd.nguia = t.guia
		...
		INNER JOIN embarquedemercancia em ON emd.idembarque = em.folio AND emd.sucursal = em.idsucursal
		WHERE ".(($_SESSION[IDSUCURSAL]!=1)? " rd.idsucursal = ".$_SESSION[IDSUCURSAL]."":"")."";*/
		$s = "SELECT gv.id AS guia, DATE_FORMAT(gv.fecha,'%d/%m/%Y') AS fecha, 
		CONCAT_WS(' ',re.nombre,r.paterno,re.materno) AS remitente,
		CONCAT_WS(' ',de.nombre,de.paterno,de.materno) AS destinatario,
		...
		INNER JOIN catalogocliente de ON gv.iddestinatario = de.id
		LEFT JOIN catalogosector cs ON gv.sector = cs.id
		WHERE gv.estado = 'ALMACEN ORIGEN' AND SUBSTRING(gv.id,1,3)<>'888'    
		".(($_SESSION[IDSUCURSAL]!=1)? " AND gv.idsucursalorigen = ".$_SESSION[IDSUCURSAL]."":"")."		
		UNION
		...
		e.guia, e.observaciones, cs.prefijo AS sucursal,
		IF(e.personarequireead<>'',e.personarequireead,IF(e.opcion2=0,CONCAT_WS(' ',re.nombre,re.paterno,re.materno),CONCAT_WS(' ',de.nombre,de.paterno,de.materno))) AS persona,
