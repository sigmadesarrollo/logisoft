<?	session_start();
	...
	if($_GET[accion] == 1){
		...
		VALUES('".$row[1]."','".$_SESSION[IDSUCURSAL]."','SOLICITUD TRASPASO CARGO',CURRENT_DATE,CURRENT_TIME,'".$_SESSION[IDUSUARIO]."')";
		mysql_query($s,$l) or die($s);
		$s ="INSERT INTO historialmovimientos (modulo,folio,estado,idusuario,fechamodificacion) 
		VALUES('guiasventanilla','".$row[1]."','SOLICITUD TRASPASO CARGO','".$_SESSION[IDUSUARIO]."',CURRENT_TIMESTAMP)";
		mysql_query($s,$l) or die($s);
		...
		$s = "SELECT gv.id as guia, IFNULL(gv.total,0) AS importe, cd.descripcion AS destino, cs.descripcion AS origen,
		...
		INNER JOIN catalogocliente de ON gv.iddestinatario = de.id
		LEFT JOIN historialdetraspaso ht ON gv.id = ht.guia
		WHERE gv.condicionpago=1 AND gv.id='".$_GET[guia]."'
		AND (ht.aceptada!=1 or isnull(ht.aceptada)) AND isnull(gv.factura)
		AND ((gv.tipoflete=0 AND gv.idsucursalorigen = '$_SESSION[IDSUCURSAL]' AND isnull(gv.factura)) OR 
		...
		LEFT JOIN historialdetraspaso ht ON ge.id = ht.guia
		WHERE ge.tipopago='CREDITO' AND ge.id='".$_GET[guia]."'
		and tipoguia = 'CONSIGNACION' AND isnull(ge.factura)
		AND ((ge.tipoflete='PAGADA' AND ge.idsucursalorigen = '$_SESSION[IDSUCURSAL]' AND isnull(ge.factura)) OR 
