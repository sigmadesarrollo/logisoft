<?	session_start();
	...
	if($_GET[accion]==1){
		$s="SELECT cs.descripcion, cd.subdestinos, ifnull(cd.costoead,0) as costoead, cd.notificacion, cd.notificaciones 
		...
		$contenido = $_GET[contenido];
			$s = "INSERT INTO evaluacionmercanciadetalle_tmp 
			...
			('".$arr[0]."', '".$arr[1]."', '".cambio_texto($_GET[contenido])."', '".$arr[2]."', '".$arr[3]."', 
			 ...
			 ".$_SESSION[IDUSUARIO].",'".$_SESSION[NOMBREUSUARIO]."','".$fechan."',".$_SESSION[IDSUCURSAL].")";
		...
		$s = "DELETE FROM evaluacionmercanciadetalle_tmp 
		...
		SELECT null AS id, ".$fx->folio." AS evaluacion, cantidad, descripcion, contenido, peso, largo, ancho, alto, 
		volumen, pesototal, pesounit, idusuario, usuario, fecha, sucursal FROM evaluacionmercanciadetalle_tmp
		WHERE idusuario=".$_SESSION[IDUSUARIO]." AND sucursal = ".$_SESSION[IDSUCURSAL]."";
		...
		$row = split(",",$_GET[arre]);
		$s = "UPDATE evaluacionmercanciadetalle_tmp SET cantidad=".$row[0].", descripcion=".$row[1].",
		contenido='".cambio_texto($_GET[contenido])."', peso=".$row[2].", largo=".$row[3].", ancho=".$row[4].",
		alto=".$row[5].", volumen=".$row[6].", pesototal=".$row[7].",
		pesounit=".$row[8]." 
		WHERE idusuario=".$_SESSION[IDUSUARIO]." AND fecha='".$_GET[fecha]."' AND sucursal = ".$_SESSION[IDSUCURSAL]."";
		//echo $s;
		$r = mysql_query($s,$l) or die($s);
