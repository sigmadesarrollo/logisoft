<?	session_start();
	...
	if($_GET['accion']==1){// VALIDAR INICIO DIA
		$s = "SELECT * FROM iniciodia WHERE fechainiciodia='".$f."' AND sucursal = ".$_SESSION[IDSUCURSAL]."";
	...
	$s = "SELECT 
	CASE (SELECT COUNT(id) FROM iniciocaja WHERE usuariocaja='".$_GET['idusuario']."' AND sucursal=".$_GET[sucursal]." AND fechainiciocaja='".$f."')
	WHEN 0 THEN 1 ELSE (SELECT COUNT(id) FROM cierrecaja WHERE
	iniciocaja=(SELECT MAX(id) FROM iniciocaja) AND usuariocaja='".$_GET['idusuario']."' AND sucursal=".$_GET[sucursal].")
	END AS validar";
	
		$sql = mysql_query("SELECT * FROM iniciocaja 
		WHERE usuariocaja='".$_GET['idusuario']."' AND sucursal=".$_GET[sucursal]." AND fechainiciocaja='".$f."'",$link);		
...
WHERE i.idusuario='".$_GET['idusuario']."' AND c.iniciodia=(SELECT iniciodia FROM cierredia WHERE iniciodia=i.id AND idusuario='".$_GET['idusuario']."')";
		$r = mysql_query($s,$link) or die("error en linea ".__LINE__);
		if(mysql_num_rows($r)>0){
		$f = mysql_fetch_object($r); $cant = mysql_num_rows($r);
			$xml = "<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
	...
	$day = mysql_query("SELECT DAYOFWEEK('".$fu."') AS dia",$link);
		...
		$fu = cambiaf_a_mysql($_GET['fechacierrecaja']);	
		$s = "SELECT tipocierre FROM cierrecaja WHERE fechacierre='".$fu."' AND usuariocaja='".$_GET['idusuario']."'";
		$r = mysql_query($s,$link) or die("error en linea ".__LINE__);
		if(mysql_num_rows($r)>0){
		$f = mysql_fetch_object($r); $cant = mysql_num_rows($r);
			$xml = "<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
