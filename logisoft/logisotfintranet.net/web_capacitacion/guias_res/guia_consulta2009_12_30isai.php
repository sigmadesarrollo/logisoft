<?
	...
	if($_GET[accion] == 4){
		...
		sector=ifNull((SELECT cs.id
			...
			and catalogosector.idsucursal = '$_SESSION[IDSUCURSAL]'
			LIMIT 1),0),
		...
		".(($_GET[condicionpago]==0)?", pagado='S', fechapago = CURRENT_DATE, usuariocobro = $_SESSION[IDUSUARIO], sucursalcobro = $_SESSION[IDSUCURSAL]":"");
		$r = @mysql_query(str_replace("''","null",$s),$l) or die("<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
				<datos>
				<guardado>0</guardado>
				<consulta>$s</consulta>
	...
	if($_GET[accion] == 7){
		...
		$s = "UPDATE pagoguias SET pagado = 'C' WHERE guia = '$_GET[folio]', fechacancelacion=current_date";
		$r = mysql_query($s,$l) or die("<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
				<datos>
				<guardado>0</guardado>
				<guardado>$s</guardado>
