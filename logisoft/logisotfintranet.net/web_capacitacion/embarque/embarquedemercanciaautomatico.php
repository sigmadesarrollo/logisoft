<? 	session_start();
	...
	//Folio
	$sql=mysql_query("SELECT CONCAT('$prefijo','-',LPAD((SELECT ifnull(MAX(folio),0)+1 FROM embarquedemercancia 
	WHERE idsucursal=".$_SESSION[IDSUCURSAL]."),5,'0'))",$link);
	...
	function moverAlaDerecha(){
		if(tabla1.getRecordCount()==""){
			...
			if(tabla1.getSelectedIdRow()!=""){
				consultaTexto("resMovADerC","embarquedemercanciaautomatica_con.php?accion=4&folio="+tabla1.getSelectedRow().guia+"&sid="+Math.random());
			}else if(tabla3.getSelectedIdRow()!=""){
				consultaTexto("resMovADerU","embarquedemercanciaautomatica_con.php?accion=5&folio="+tabla3.getSelectedRow().guia
	...
	function moverAlaizquierda(){
		if(tabla2.getRecordCount()==""){
			...
			if(tabla2.getSelectedIdRow()!=""){
				consultaTexto("resMovAIzqC","embarquedemercanciaautomatica_con.php?accion=6&folio="+tabla2.getSelectedRow().guia+"&sid="+Math.random());
			}else if(tabla4.getSelectedIdRow()!=""){
				consultaTexto("resMovAIzqU","embarquedemercanciaautomatica_con.php?accion=7&folio="+tabla4.getSelectedRow().guia
...
/*******************/
	function ObtDetalleIzq(){
		...
		if(tabla1.getValSelFromField('guia','No_GUIA')!=""){
			consultaTexto("mostrarIzqAbajo","embarquedemercanciaautomatica_con.php?accion=2&folio="+tabla1.getValSelFromField('guia','No_GUIA')+"&sid="+Math.random());
		}
	}
...
/****************/
	function ObtDetalleDer(){
		...
		if(tabla2.getValSelFromField('guia','No_GUIA')!=""){
			consultaTexto("mostrarDerAbajo","embarquedemercanciaautomatica_con.php?accion=3&folio="+tabla2.getValSelFromField('guia','No_GUIA')+"&sid="+Math.random());
		}
	}
...
/********************/
	function CargarGuias(){
		consultaTexto("mostrarCargarGuias","embarquedemercanciaautomatica_con.php?accion=1&unidad="+document.all.unidad.value+"&sid="+Math.random());
	}
	function mostrarCargarGuias(datos){
		try{
			...
			var guias = objeto.derecha;
			var pesox = 0;
			
			for(var i=0; i<guias.length; i++){
				pesox += parseFloat(guias[i].peso);
			}
			
			barra.setValue(parseFloat(pesox));
		}catch(e){
			alerta3(datos,"");
		}
	}
...
/********************/
	...
	function limpiarDatos(){
		...
		//consultaTexto("ponerIniciado","embarquedemercanciaautomatica_con.php?accion=11");
	}
	...
	function guardarValores(){
		<?=$cpermiso->verificarPermiso(307,$_SESSION[IDUSUARIO]);?>;
		if(u.ruta.value==""){
		...
		if(tabla2.getRecordCount()>0 && u.guardado.value == 0){
			consultaTexto("resValidar","embarquedemercanciaautomatica_con.php?accion=8&mathrand="+Math.random());
		}else{
	...
	function guardarFinal(){
		...
		var v_fecha		= fechahora(v_fecha);
					consultaTexto("resGuardar","embarquedemercanciaautomatica_con.php?accion=9&unidad="+unidad
					 +"&ruta="+ruta+"&tipoembarque=AUTOMATICO&folios="+tabla2.getValuesFromField('guia')+"&foliobitacora="+foliobitacora+"&mathrand="+Math.random()+"&fechahora="+v_fecha);
	}
	...
	function cargarEmbarque(folio){
		consultaTexto("resCargarEmbarque","embarquedemercanciaautomatica_con.php?accion=10&folio="+folio)
	}
	...
	function resCargarEmbarque(datos){
		...
		document.getElementById('d_imprimir').style.visibility = "visible";
	}
	
	function imprimirEmbarque(){
		if(u.guardado.value == 1){	
			var folio = u.folio.value.replace(/[a-zA-Z]/g,"");
			folio = folio.replace(/-/g,"");
			if(document.URL.indexOf("web_capacitacionPruebas/")>-1){		
				window.open("https://www.pmmintranet.net/web_capacitacionPruebas/fpdf/reportes/relacionEmbarque.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+folio+"&unidad="+u.unidad.value+"&fechaembarque="+u.fecha.value+"&destino="+u.destino.value);
			
			}else if(document.URL.indexOf("web_capacitacion/")>-1){
			window.open("https://www.pmmintranet.net/web_capacitacion/fpdf/reportes/relacionEmbarque.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+folio+"&unidad="+u.unidad.value+"&fechaembarque="+u.fecha.value+"&destino="+u.destino.value);
			
			}else if(document.URL.indexOf("web_pruebas/")>-1){
			window.open("https://www.pmmintranet.net/web_pruebas/fpdf/reportes/relacionEmbarque.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+folio+"&unidad="+u.unidad.value+"&fechaembarque="+u.fecha.value+"&destino="+u.destino.value);
			}		
		}else{
...
<table width="619" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
  <tr>
    <td width="619" class="FondoTabla Estilo4">EMBARQUE DE MERCANC&Iacute;A AUTOM&Aacute;TICO</td>
  </tr>
  <tr>
      ...
      <tr>
        <td><table width="618" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td width="58"><div class="ebtn_buscar" onclick="abrirVentanaFija('../buscadores_generales/buscarFoliosEmbarque.php?funcion=cargarEmbarque&tipo=2', 600, 450, 'ventana', 'Busqueda');"></div></td>
      ...
      <tr>
        <td><table width="618" border="0" align="center" cellpadding="0" cellspacing="0">
            ...
            <tr>
              ...
              <td width="24" align="center"><div id="aderecha" class="ebtn_adelante" onclick="moverAlaDerecha()" style="visibility:hidden"></div><BR /><BR /><div id="aizquierda" class="ebtn_atraz" onclick="moverAlaizquierda()" style="visibility:hidden"></div></td>
                  <td width="296">
