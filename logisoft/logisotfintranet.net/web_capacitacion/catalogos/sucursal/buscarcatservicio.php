...
echo "<script language='javascript' type='text/javascript'>document.location.href='../../../index.php';</script>";
	...
	$get = mysql_query('select count(*) from catalogoservicio');
        ...
        <tr>
          <td colspan="2" height="300px" valign="top">
              <table width="100%" border="0" align="center">
                <?		
		$get = mysql_query('select * from catalogoservicio limit '.$st.','.$pp,$link);
		while($row=@mysql_fetch_array($get)){
	?>
                <tr>
                  <td width="49" class="Tablas"><span style="cursor:pointer; color:#0000FF" onclick="window.parent.obtener('<?= $row['id'];?>','<?=htmlentities(strtoupper($row['descripcion']));?>','<?= $row['restringir'];?>');parent.VentanaModal.cerrar();">
                    <?= $row['id'];?></span></td>
