<?
	...
	//solicitar clientes
	if($_GET[accion] == 2){
		$s = "SELECT iddestinatario FROM guiasventanilla WHERE id='$_GET[guia]'";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
		$dest=$f->iddestinatario;
		$s = "SELECT personamoral FROM catalogocliente WHERE id='$f->iddestinatario'";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
		$persona1=$f->personamoral;
		
		if ($_GET[idcliente]!=''){
			$s = "SELECT personamoral FROM catalogocliente WHERE id='$_GET[idcliente]'";
			$r = mysql_query($s,$l) or die($s);
			$f = mysql_fetch_object($r);
			$persona2=$f->personamoral;
		}else{
			$persona2=$persona1;
			$_GET[idcliente]=$dest;
		}
			...
			if(mysql_num_rows($r)>0){
				...
				if(mysql_num_rows($rx)>0){
					...
					while($fx = mysql_fetch_object($rx)){
						$xml .= "<codigo>".$_GET[idcliente]."</codigo>
						";
	...
	if($_GET[accion] == 4){
		...
		$s = "UPDATE lasalertas SET gufoaupaca=0;";
		mysql_query($s,$l) OR die(mysql_error($l).$s);
		$s = "UPDATE lasalertas SET gufoaupaca=(SELECT SUM(IF(estado = 'SUSTITUCION',1,0)) tgufoaupaca FROM guiasventanilla_cs);";
		mysql_query($s,$l) OR die(mysql_error($l).$s);
	}
	...
	if($_GET[accion] == 5){
		...
		$s = "SELECT gv.id, gv.evaluacion,date_format(gv.fecha, '%d/%m/%Y') fecha,gv.fechaentrega,gv.factura,gv.tipoflete,gv.ocurre,gv.idsucursalorigen,
		gv.estado,IF(cd.subdestinos=1,CONCAT(csd.prefijo,' - ',cd.descripcion,':',cd.id),CONCAT(cd.descripcion,' - ',csd.prefijo,':',cd.id)) ndestino, 
		csd.descripcion as nsucdestino,gv.idsucursalorigen,gv.idsucursaldestino,iddestino,

		gv.condicionpago,gv.idremitente, iddireccionremitente,concat_ws(' ', ccr.nombre, ccr.paterno, ccr.materno) rncliente,ccr.rfc as rrfc,
		ccr.celular as rcelular,dr.calle as rcalle, dr.numero as rnumero, dr.cp as rcp, dr.colonia as rcolonia, 
		dr.poblacion as rpoblacion, dr.telefono as rtelefono,
		 
		gv.iddestinatario,iddirecciondestinatario,concat_ws(' ', ccd.nombre, ccd.paterno, ccd.materno) dncliente,ccd.rfc as drfc,ccd.celular as dcelular,
		dd.calle as dcalle, dd.numero as dnumero, dd.cp as dcp, dd.colonia as dcolonia,dd.poblacion as dpoblacion, dd.telefono as dtelefono,
		
		gv.entregaocurre,gv.entregaead, gv.restrinccion, gv.totalpaquetes, gv.totalpeso, 
		...
		if(mysql_num_rows($r)>0){
			...
			$xml = "<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
				...
				<iddirecciondestinatario>".cambio_texto($f->iddirecciondestinatario)."</iddirecciondestinatario>
				...
				if(mysql_num_rows($rx)>0){
					...
					while($fx = mysql_fetch_object($rx)){
						...
						'$fx->peso', '$fx->volumen', '$fx->importe', '$fx->excedente', '$fx->kgexcedente', $_GET[idpagina]";
