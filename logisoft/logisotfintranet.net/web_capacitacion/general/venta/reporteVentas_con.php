<?
	...
	if($_GET[accion]==1){
		...
		$s = "SELECT FORMAT(ifnull(SUM(IF(ISNULL(rv.convenio) or rv.convenio=0,0,rv.total)),0),2) AS convenio, 
		FORMAT(ifnull(SUM(IF(ISNULL(rv.convenio) or rv.convenio=0,rv.total,0)),0),2) AS sinconvenio,
		FORMAT(ifnull(SUM(rv.total),0),2) AS total
		FROM reportes_ventas rv
		INNER JOIN catalogosucursal cs ON 
		IF(rv.tipoventa <> 'GUIA VENTANILLA',
		rv.sucursalfacturo=cs.descripcion,
	...
	if($_GET[accion]==4){
		...
		/*totales de los registros*/
		$s = "SELECT FORMAT(SUM(rv.total),2) AS total
		FROM reportes_ventas rv
		WHERE
		IF(rv.tipoventa <> 'GUIA VENTANILLA',
		rv.prefijosucfacturo='".$_GET[sucursal]."',
	...
	if($_GET[accion]==5){
		...
		/*totales de los registros*/
		$s = "SELECT FORMAT(IFNULL(SUM(rv.total),0),2) AS total
		FROM reportes_ventas rv
		WHERE rv.fecharealizacion BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql($_GET[fechafin])."' AND
		IF(rv.tipoventa <> 'GUIA VENTANILLA',
		rv.prefijosucfacturo='".$_GET[sucursal]."',
	...
	if($_GET[accion]==8){
		...
		/*totales de los registros*/
		$s = "SELECT 
		FORMAT(SUM(IFNULL(rv.seguro,0)+IFNULL(rv.excedente,0) +
	...
	if($_GET[accion]==9){
		...
		/*totales de los registros*/
		$s = "SELECT FORMAT(SUM(rv.total),2) total
		FROM solicitudguiasempresariales sg
		INNER JOIN reportes_ventas rv ON rv.folio BETWEEN sg.desdefolio AND sg.hastafolio 
		WHERE sg.prepagada<>'SI' and 
		IF(rv.tipoventa <> 'GUIA VENTANILLA',
