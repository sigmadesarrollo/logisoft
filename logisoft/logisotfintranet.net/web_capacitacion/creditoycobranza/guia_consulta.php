<?
	...
	if($_GET[accion] == 2){
		...
		if($f->diascredito>0){
			$s = "SELECT IFNULL(SUM(total),0) AS totalvencido FROM pagoguias WHERE pagado = 'N' AND cliente = $_GET[idcliente] AND
	...
	if($_GET[accion] == 4){
		...
		if($_GET[condicionpago]==1){
			...
			#LAS GUIAS SE HARAN AL CONSULTARSE
		...
		/*$s = "call proc_RegistroFranquiciasConceciones('ventanilla','$newfolio')";
		$r = @mysql_query(str_replace("''","null",$s),$l) or die("<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
				<datos>
				<guardado>0</guardado>
				<consulta>".str_replace("''","null",$s)."</consulta>
	...
	if($_GET[accion] == 7){
		...
		$s = "SELECT * FROM guiasventanilla WHERE id='$_GET[folio]' and condicionpago=0";
		$r = mysql_query($s,$l) or die($s);
		if(mysql_num_rows($r)>0){
