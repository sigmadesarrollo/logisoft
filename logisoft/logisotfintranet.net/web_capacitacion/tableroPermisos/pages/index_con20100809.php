	...
	if($_GET[accion]==1){
		...
		for($i=0;$i<count($arreids);$i++){
			...
			if(($arreids[$i]==3 || $arreids[$i]==14 || $arreids[$i]==35 || $arreids[$i]==31) && !ereg(",".$arreids[$i].",",$losides)){
				...
				$s .= "(SELECT COUNT(*) FROM propuestaconvenio WHERE estadopropuesta LIKE '%AUTORIZADA%'
									  AND NOT EXISTS(SELECT * FROM generacionconvenio WHERE propuesta = propuestaconvenio.folio)) prau";
			}
			...
			if(($arreids[$i]==48 || $arreids[$i]==93) && !ereg(",".$arreids[$i].",",$losides)){
				...
				$s .= "(SELECT SUM(total)  FROM (
					 (SELECT COUNT(*) total FROM guiasventanilla WHERE estado <> 'ENTREGADA' AND estado <> 'CANCELADO' AND
			...
			if($arreids[$i]==58 && !ereg(",".$arreids[$i].",",$losides)){
				$losides .= "58,";
				$s .= ($s != "select ")?",":"";
				$s .= "(SELECT COUNT(*) FROM sobrantes) guveso";
			}
			...
			if(($arreids[$i]==105 || $arreids[$i]==243) && !ereg(",".$arreids[$i].",",$losides)){
				$losides .= "105,243,";
				$s .= ($s != "select ")?",":"";
				$s .= "(SELECT count(*) FROM guiasventanilla_cs WHERE sucursal = $_SESSION[IDSUCURSAL] AND estado = 'SUSTITUCION') gufoaupaca";
			}
			...
			if($arreids[$i]==139 && !ereg(",".$arreids[$i].",",$losides)){
				...
				".(($_SESSION[IDSUCURSAL]!=1)? " AND idsucursal = $_SESSION[IDSUCURSAL]" : "").") faco";
			}
			...
			if($arreids[$i]==143 && !ereg(",".$arreids[$i].",",$losides)){
				...
				WHERE (DATEDIFF(CURDATE(),DATE_ADD(gv.fecha,INTERVAL cc.diascredito DAY)))>60 
				AND f.estado='CANCELADO' 
				".(($_SESSION[IDSUCURSAL]!="1")? " AND f.idsucursal=".$_SESSION[IDSUCURSAL]."" : "").") AS 60dias";
			}
		...
		$res = "[";
			   for($i=0;$i<count($arreids);$i++){
					...
					if($arreids[$i]==143){
						$res .= ($res!="[")?",":"";
						$res .= "{'campoid':'con".$arreids[$i]."','valor':'$f->60dias'}";
					}
