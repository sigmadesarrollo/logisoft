...
<script>
	...
	function respPedirConvenio(datos){
		...
		if(objeto.length){
			...
			//alert(objeto[0].propuesta[0].consignacioncaja);
			if(objeto[0].propuesta[0].consignacioncaja==1){
				//alerta3("generacionconvenio_con.php?accion=8&valor=4&idconvenio="+objeto[0].propuesta[0].folio);
	...
	function resGuardarConvenio(datosx){
		if(datosx.indexOf("guardo")>-1){
			...
			if(datosx.indexOf("La propuesta ya ha sido")>-1){
				alerta3(datosx,'¡Atención!');
			}else{
	...
	function imprimirActivado(){
		...
		if(ubi.estadoc.innerHTML == "ACTIVADO"){
			...
			if(document.URL.indexOf("web/")>-1){		
				window.open("https://www.pmmintranet.net/web/fpdf/reportes/convenio.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+ubi.folio.value);			
			}else if(document.URL.indexOf("web_capacitacion/")>-1){
				window.open("https://www.pmmintranet.net/web_capacitacion/fpdf/reportes/convenio.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+ubi.folio.value);			
			}else if(document.URL.indexOf("web_pruebas/")>-1){
				window.open("https://www.pmmintranet.net/web_pruebas/fpdf/reportes/convenio.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+ubi.folio.value);
			}
	...
	function resImprimir(datos){
		if(datos.indexOf("impreso")>-1){
			...
			window.open("../fpdf/reportes/convenio.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+ubi.folio.value);
	...
	function activarConvenio(){
		...
		document.getElementById('boton_activar').style.display = 'none';
	...
	function resConsultarSiActivado(datos){
		if(datos.indexOf("SI TIENE")>-1){
			alerta3("El cliente ya tiene convenio ACTIVADO con el folio "+datos.split(",")[1],"¡Atención!");
		}else{
...
Persona F&iacute;sica</td>
					...
					<tr>
					  ...
					  <th width="166"><input name="prospecto" type="text" class="text" id="prospecto" style="width:120px;" /></th>
					  <th width="76">&nbsp;</th>
        ...
        <input name="h_vencido" type="hidden" id="h_vencido" />
        <input name="h_yatiene" type="hidden" id="h_yatiene" /></th>
				  </tr>
					<tr>
					  <th>Nick</th>
...
<tr>
		...
		<div class="extra-data3">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">			  
			  ...
			  <tr>
				<td width="85%" style="text-align:right" ><input name="btnguardar" type="button" class="button" id="btnguardar"  onclick="guardarConvenio();" value="Guardar Convenio" /></td>
