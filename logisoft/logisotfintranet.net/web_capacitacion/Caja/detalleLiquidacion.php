<?	session_start();
	...
	if($_GET[tipo]=="ead"){		
		...
		$s = "SELECT guia, origen, destino, IFNULL(efectivo,0) AS efectivo, IFNULL(tarjeta,0) AS tarjeta, 
			IFNULL(cheque,0) AS cheque, IFNULL(transferencia,0) AS transferencia, 
			IFNULL(notacredito,0) AS notacredito 
			FROM (
				SELECT gv.id AS guia, ori.prefijo AS origen, des.prefijo AS destino, fp.efectivo, fp.tarjeta,
				fp.transferencia, fp.cheque, fp.notacredito
				FROM guiasventanilla gv
				INNER JOIN catalogosucursal ori ON gv.idsucursalorigen = ori.id
				INNER JOIN catalogosucursal des ON gv.idsucursalorigen = des.id
				INNER JOIN entregasocurre_detalle ed ON gv.id = ed.guia AND ed.sucursal = ".$_SESSION[IDSUCURSAL]."
				INNER JOIN entregasocurre eo ON ed.entregaocurre = eo.folio AND ed.sucursal = ".$_SESSION[IDSUCURSAL]."
				INNER JOIN formapago fp ON eo.folio = fp.guia AND fp.procedencia = 'O' 
				".((!empty($_GET[cambiafecha]))? " AND fp.fecha = '".cambiaf_a_mysql($_GET[fecha])."'" :" AND fp.fecha = CURRENT_DATE")."
				AND fp.usuario=".$_GET[empleado]."
				UNION
				SELECT ge.id AS guia, ori.prefijo AS origen, des.prefijo AS destino, fp.efectivo, fp.tarjeta,
				fp.transferencia, fp.cheque, fp.notacredito 
				FROM guiasempresariales ge
				INNER JOIN catalogosucursal ori ON ge.idsucursalorigen = ori.id
				INNER JOIN catalogosucursal des ON ge.idsucursalorigen = des.id
				INNER JOIN entregasocurre_detalle ed ON ge.id = ed.guia AND ed.sucursal = ".$_SESSION[IDSUCURSAL]."
				INNER JOIN entregasocurre eo ON ed.entregaocurre = eo.folio AND ed.sucursal = ".$_SESSION[IDSUCURSAL]."
				INNER JOIN formapago fp ON eo.folio = fp.guia AND fp.procedencia = 'O' 
				".((!empty($_GET[cambiafecha]))? " AND fp.fecha = '".cambiaf_a_mysql($_GET[fecha])."'" :" AND fp.fecha = CURRENT_DATE")."
				AND fp.usuario=".$_GET[empleado]."
