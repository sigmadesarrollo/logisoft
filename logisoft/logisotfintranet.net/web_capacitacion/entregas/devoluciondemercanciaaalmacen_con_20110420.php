<?	session_start();
	...
	if($_GET[accion]==3){
		...
		$s = "UPDATE guiasventanilla SET estado = 'ALMACEN DESTINO', entradasalida = 'ENTRADA'  WHERE id in($cadenas_folios2)";
		mysql_query($s,$l) or die($s);

		$s = "UPDATE guiasempresariales SET estado = 'ALMACEN DESTINO', entradasalida = 'ENTRADA'  WHERE id in($cadenas_folios2)";
		mysql_query($s,$l) or die($s);
