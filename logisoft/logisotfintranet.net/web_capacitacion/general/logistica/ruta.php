...
<script>
	...
	tabla1.setAttributes({
		...
		campos:[
			...
			{nombre:"TIEMPO RECORRIDO", medida:200, alineacion:"center", datos:"recorrido"},

			{nombre:"TIEMPO CARGA/DESCARGA", medida:150, alineacion:"center",  datos:"tiempocd"},					

			{nombre:"RECORRIDO", medida:110, alineacion:"center", datos:"recorrido2"}
	...
	function obtenerDetalle(){
		...
		consultaTexto("mostrarDetalle","logistica_con.php?accion=2&folio=<?=$_GET[folio]?>&fecha=<?=$_GET[fecha]?>&fecha2=<?=$_GET[fecha2]?>&valram="+Math.random());
	...
	function mostrarDetalle(datos){	
		...
		for(var i=0;i<objeto.length;i++){
			...
			obj.ruta		 	= objeto[i].descripcion;

			obj.recorrido 		= objeto[i].tiempor;

			obj.tiempocd	  	= objeto[i].desviaciones;

			obj.recorrido2 		= objeto[i].recorrido;
	...
	function tipoImpresion(valor){
		...
		if(valor=="Archivo"){			
			...
			window.open("http://www.pmmentuempresa.com/web/general/logistica/generarExcelLogistica.php?accion=2&titulo=DESCRIPCION DE RUTA&fecha=<?=$_GET[fecha] ?>&fecha2=<?=$_GET[fecha2] ?>&folio=<?=$_GET[folio]?>&valram="+Math.random());			
