...
<script>
	...
	function Validar(){		
		...
		if(document.getElementById('cantidad').value==""){
			...
			objeto.destino		= 	u.country_hidden.value;
			if(espesototal==0){
...
function devolverDestino(){
		...
		//	consultaTexto("mostrarDestino", "../evaluacion/evaluacionMercancia_con.php?accion=1&destino="+u.countryid.value);
			return false;
		}
...
var descdes = new Array(<?php echo $descdes; ?>);
        ...
        <tr bgcolor="dee3d5">
          ...
          <table width="330" border="0" align="center" cellpadding="0" cellspacing="0">
              <tr>
              	<td>Destino:</td>
              	<td colspan="4">
                <input name="country" type="text" class="Tablas" id="country" style="font-size:9px; text-transform:uppercase"  value="" size="35" maxlength="60" autocomplete="array:descdes" onKeyPress="if(event.keyCode==13){document.all.countryid.value=this.codigo; document.all.cantidad.focus();}" onkeydown="if(event.keyCode==9){document.all.countryid.value=this.codigo; }" onblur="if(this.value!=''){document.all.countryid.value=this.codigo;}" >
                <input type="hidden" id="country_hidden" name="countryid" value="<?=$_POST[countryid] ?>"></td>
              	</tr>
              <tr>
                <td width="71" class="Tablas">Cantidad:</td>
                <td colspan="4" class="Tablas"><label>
                  <input name="cantidad" type="text" class="Tablas" id="cantidad" onkeypress="if(event.keyCode==13){ if(this.value!='' && parseFloat(this.value)>1){this.value=1; mens.show('A','En las guias prepagadas solo se puede documentar un articulo','ATENCION')} document.all.descripcion.focus();} return solonumeros(event);" value="<?=$cantidad ?>" style="width:50px" maxlength="5" onblur="if(this.value!='' && parseFloat(this.value)>1){this.value=1; mens.show('A','En las guias prepagadas solo puede poner una cantidad','ATENCION')}" />
              ...
              <tr>
				...
				<input name="peso" type="text" class="Tablas" id="peso" onblur="CalcularUnitarioFoco()"  onkeypress="return SoloND(event,this.value)" onkeydown="CalcularUnitario(event); return tabular(event,this)" value="<?=$peso ?>" size="10" maxlength="15" <? if($_GET[espesototal]==1 && $peso==0){ echo "readonly=''"; } ?> /></td>
