...
<script>
	...
	function imprimirAuto(){
		...
		Etiqueta.Datos_Paquetes = document.getElementById('folioSeleccionado').innerHTML+"|"+u.fecha.value+"|"+u.totalpaquetes.value+" P.VOL: "+u.totalvolumen.value+" P. KG: "+u.totalpeso.value;
	...
	function bloquear(valor){
		...
		u.txtotrosAgregado.disabled = valor;
	...
	function cancelarGuia(){
		...
		if((u.estado.innerHTML=='ALMACEN DESTINO' && u.lstflete.value == 1) || u.sustitucionguia.innerHTML.indexOf('SUSTITUCION')>-1){
			...
			if("<?=$_SESSION[IDSUCURSAL]?>"!=u.idsucursalorigen.value && u.fechaactual.value != u.fecha.value){
				alerta3("Para hacer una cancelación local debe estar en la sucursal donde se registro la guia y debe ser la fecha actual","¡Atención!");
			}else{
	...
	function limpiar_remitente(){
		...
		u.idremitente_anterior.value = 0;
	...
	function mostrarRemitente(datos){
		...
		if(dcliente.cliente!="0"){
			...
			u.idremitente_anterior.value= dcliente.cliente.idcliente;
	...
	function validarDatos(){
		...
		if(u.bloquearocurre.value==1 && u.chocurre.value==1){
			...
			return false;
		}else if(parseFloat(u.t_txttotal.value.replace("$ ","").replace(/,/g,''))<=0){
			alerta3("El importe de la guia es 0, refresque la pagina, si el problema continua comuniquese con sistemas","¡Alerta!");	
			return false;
	...
	function registrarGuia(){
		...
		window.open("guia_consulta.php?accion=4&idpagina="+u.idpagina.value+"&evaluacion="+evaluacion
...
?>
  ...
  <input type="" name="creditodisponible" value="">
...
<div class="content" style="width:830px">
  ...
  <div class="remi-desti">
    <div class="remi"> <img src="../css_ne/style/arrow.jpg" style="position:absolute; left:260px;"/>
      ...
      <span>No. Cliente:<input type="hidden" name="rem_personamoral">
      <input type="hidden" name="idremitente_anterior" value="0">
      </span>
