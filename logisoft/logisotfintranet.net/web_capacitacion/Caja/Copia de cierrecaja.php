<?	session_start();
	...
	$s = mysql_query("SELECT SUM(efectivo) AS efectivo, SUM(tarjeta) AS tarjeta, SUM(trasferencia) AS transferencia, SUM(cheque) AS cheque, 0 AS retiros FROM guiasventanilla WHERE fecha='".$f."' AND idusuario='".$_SESSION[IDUSUARIO]."' AND idsucursalorigen='".$_SESSION[IDSUCURSAL]."'",$link);		
	...
	$fac= mysql_query("SELECT COUNT(*) AS factura FROM facturacion WHERE fecha='".$f."' AND facturaestado='CANCELADO' AND idusuario='".$_SESSION[IDUSUARIO]."' AND idsucursal='".$_SESSION[IDSUCURSAL]."'",$link);
...
var u = document.all;
	...
	alerta('Debe Iniciar Caja antes de Cerrar Caja','¡Atención!','efectivo');
		...
		}else if(parseFloat(u.efectivo.value)!=parseFloat(u.tefectivo.value)){
		alerta('Existen diferencias en Efectivo','¡Atención!','efectivo');		
		...
		}else if(parseFloat(u.tarjeta.value)!=parseFloat(u.ttarjeta.value)){
		alerta('Existen diferencias en Tarjeta','¡Atención!','tarjeta');		
...
}else if(parseFloat(u.transferencia.value)!=parseFloat(u.ttransferencia.value)){
	alerta('Existen diferencias en Transferencia','¡Atención!','transferencia');
		...
		}else if(parseFloat(u.cheque.value)!=parseFloat(u.tcheque.value)){
		alerta('Existen diferencias en Cheque','¡Atención!','cheque');		
		...
		}else if(parseFloat(u.retiros.value)!=parseFloat(u.tretiros.value)){
		alerta('Existen diferencias en Retiros','¡Atención!','retiros');
  ...
  <tr>
    <td><table width="400" align="center" cellpadding="0" cellspacing="0" >
      ...
      <tr>
        ...
        <td><input name="gcredito" type="text" class="Tablas" id="gcredito" onKeyPress="return Numeros(event)" onKeyDown="return tabular(event,this);" value="<?=$gcredito ?>" size="10" maxlength="5" style="text-align:right" /></td>
      </tr>
      <tr>
      ...
      <tr>
        <td colspan="4"><div align="center"><a href="../menu/webministator.php"><img src="../img/inicio_30.gif" width="29" height="33" border="0" align="absbottom"></a></div></td>
      </tr>
    </table></td>
  </tr>
</table>
