<?	session_start();
	...
	if($_GET['accion']==1){// VALIDAR INICIO DIA
		...
		$s = "SELECT 
		CASE (SELECT COUNT(id) FROM iniciocajaocurre WHERE usuariocaja='".$_GET['idusuario']."')
		WHEN 0 THEN 1 ELSE (SELECT COUNT(id) FROM cierrecajaocurre WHERE
		iniciocaja=(SELECT MAX(id) FROM iniciocajaocurre) AND usuariocaja='".$_GET['idusuario']."')
		END AS validar";
		$sql = mysql_query("SELECT * FROM iniciocajaocurre 
		WHERE usuariocaja='".$_GET['idusuario']."' AND fechainiciocaja='".$f."'",$link);
				
		if(mysql_num_rows($sql)>0){$existe = 1;}else{$existe = 0;}
		$r = mysql_query($s,$link) or die("error en linea ".__LINE__);
		if(mysql_num_rows($r)>0){
		$f = mysql_fetch_object($r); $cant = mysql_num_rows($r);
			$xml = "<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
