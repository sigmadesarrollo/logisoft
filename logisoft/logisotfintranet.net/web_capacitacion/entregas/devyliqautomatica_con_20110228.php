<?	session_start();
	...
	if($_GET[accion]==1){
		...
		$s = "SELECT MAX(folio) folioreparto FROM repartomercanciaead WHERE unidad = '$f->id' and liquidado = 0 and sucursal = '$_SESSION[IDSUCURSAL]'";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
		
		$s = "UPDATE devyliqautomatica SET reparto = $f->folioreparto WHERE unidad = '".$_GET[unidad]."' AND liquidada = 'N'";
		...
		WHERE r.folio=$folioreparto AND r.sucursal = ".$_SESSION[IDSUCURSAL];
		$r = mysql_query($s,$l) or die($s);
