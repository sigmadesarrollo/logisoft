<?	session_start();
	...
	WHERE id='".$_GET['empleado']."' and puesto between 47 and 48";
	$r = mysql_query($s,$link) or die("error en linea ".__LINE__);
		if(mysql_num_rows($r)>0){
			$f = mysql_fetch_object($r);
			$cant = mysql_num_rows($r);			
...
WHERE LG.folio='".$_GET['folio']."'";
	...
	INNER JOIN catalogoempleado e ON b.conductor1 = e.id 
	INNER JOIN catalogounidad u ON b.unidad = u.numeroeconomico
	WHERE b.folio='".$_GET['folio']."'  && b.preliquidaciondebitacora=0";
		$r = mysql_query($s,$link) or die("error en linea ".__LINE__);
		if(mysql_num_rows($r)>0){
			$f = mysql_fetch_object($r);
			$cant = mysql_num_rows($r);			
...
FROM catalogoempleado WHERE id='".$_GET['id']."'";
		...
		$s = "SELECT PRE.folio,PRE.fecha,PRE.afavorencontra,PRE.cantidad,
	...
	WHERE b.folio='".$_GET['folio']."'";
		...
		if(mysql_num_rows($r)>0){
			...
			$sqldetalle="select idconcepto,concepto,ifnull(cantidad,0)AS cantidad from comprobantedeliquidaciondebitacoradetalle 
where comprobantedeliquida='".$_GET['folio']."'";
