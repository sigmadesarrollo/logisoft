...
<script>
	...
	function obtenerFolioCobranzax(datos){
		...
		if(datos.indexOf("ok")>-1){
			...
			alerta3("Hubo un error al traer el detalle "+datos,"Atencin!");
		}
	}
	function obtenerFolioCobranza(e,folio){
		tecla = (u) ? e.keyCode : e.which ;
		if(tecla == 13 && folio!=""){
			consultaTexto("mostrarFolioCobranza","liquidacionCobranza_con.php?accion=2&folio="+folio+"&sucursal="+u.sucursal_hidden.value);
		}
	}
	function mostrarFolioCobranza(datos){
		...
		if(datos!=0){
			...
			alerta3("El folio de cobranza no existe o ya fue aplicado","Atencin!");
		}
	}
	...
	function guardar(){
		if(u.foliocobranza.value == ""){
			alerta("Debe capturar Folio Relacin Cobranza","Atencin!","foliocobranza");
			return false;
		}else if(u.cobrador.value == 0 || u.cobrador.options[u.cobrador.options.selectedIndex].text==""){
			alerta("Debe capturar Cobrador","Atencin!","cobrador");
			return false;
		}else{
	...
	function confirmarLiquidacion(datos){
		if(datos.indexOf("ok")>-1){
			...
			alerta3("Hubo un error al aplicar "+datos,"Atencin!");
		}
	}
	...
	function mostrarFolioLiquidacion(datos){
		...
		//u.diferencia.value			= obj[0].diferencia;
	...
	function ponerenRojolasrevisadas(){
		...
		for(var i=0; i<tabla1.getRecordCount();i++){	
			...
			if(u["detalle_COBRAR"][i].value=="SI"){
				total += parseFloat(u["detalle_IMPORTE"][i].value.replace("$ ","").replace(/,/,""));
			}
		}
	...
	function validarsitienemovimiento(){
		for(var i=0; i<tabla1.getRecordCount();i++){	
			if (u["detalle_REVISION"][i].value=="NO" && u["detalle_COBRAR"][i].value=="NO" && u["detalle_MOTIVO"][i].value=="0"){
				alerta3("No se ha registrado ningun movimiento a la factura: " +u["detalle_FACTURA"][i].value+ "","Atencin!");
				return false;
			}
		}
		...
		if (datos!=0 && datos!=""){
			...
			for(var i=0;i<objeto.length;i++){
				if (objeto[i].revision=='NO'){
					if (objeto[i].total==0){
						alerta3("Debe capturar la forma de pago de la factura: " +objeto[i].factura+ "","Atencin!");
						return false;
					}
				}
			}
	...
	function liquidar(){
	 if(u.accion.value == ""){
		if(u.foliocobranza.value == ""){
			...
			arr[2] = u.foliocobranza.value;
			arr[3] = u.cobrador.value;
			consultaTexto("registrarLiquidacionLiquidar","liquidacionCobranza_con.php?accion=6&estado=LIQUIDADO&arre="+arr+"&diferencia="+u.h_importe.value+"&cantidadentregada="+u.entregado.value.replace("$ ","").replace(/,/g,""));
		}
	 }else if(u.accion.value == "modificar"){
		u.d_guardar.style.visibility = "hidden";
		u.d_liquidar.style.visibility = "hidden";
		...
		u.estados.value = "LIQUIDADO";
	 	consultaTexto("registrarLiquidacionLiquidar","liquidacionCobranza_con.php?accion=7&folio="+u.folio.value+"&estado="+u.estado.value+"&diferencia="+u.h_importe.value+"&cantidadentregada="+u.entregado.value.replace("$ ","").replace(/,/g,""));
	 }
	}
	...
	function obtenerFactura(e,factura){
		...
		if (tecla == 13){
			if(tabla1.getRecordCount()>0 && factura!=""){
				...
				if(factura!=""){
					alerta("No existen datos en el detalle","Atencin!","foliocobranza");
				}
			}
		}
	}
          ...
          <td colspan="6"><table width="700" border="0" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td width="113"><span class="Tablas">
                <input name="importe" type="text" class="Tablas" id="importe" style="text-align:right;width:100px;background:#FFFF99" readonly=""/>
              </span></td>
          ...
          <td colspan="6"><div align="right">
            <table width="250" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td>&nbsp;</td>
                <td><div class="ebtn_Aplicar" id="d_liquidar" onClick="confirmar('Desea Aplicar el folio de liquidacin '+ document.all.folio.value+' ?', '', 'validarsitienemovimiento()', '')"></div></td>
                <td>&nbsp;</td>
