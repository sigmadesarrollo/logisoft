<?
	...
	if($_GET[accion] == 2){
		...
		$s = "(SELECT ge.id, 'CONSIGNACION' as tipoguia, ge.evaluacion, DATE_FORMAT(ge.fecha, '%d/%m/%Y') AS fecha, ge.fechaentrega, ge.factura, 
		...
		0 AS cod, 0 AS recoleccion, 0 AS observaciones, sge.subtotal-sge.combustible AS tflete, 0 AS tdescuento, 
	...
	if($_GET[accion] == 3){
		...
		$s = "INSERT INTO facturacion SET facturaestado = 'GUARDADO', tipofactura='NORMAL', idsucursal='$_SESSION[IDSUCURSAL]', cliente = '$_GET[cliente]',
		...
		totaldescuento = '$_GET[totaldescuento]',excedente = '$_GET[excedente]',ead = '$_GET[ead]',recoleccion = '$_GET[recoleccion]',
		...
		$s = "CALL proc_RegistroClientes('facturacion',".$_GET[cliente].",0,".$nfact.",0)";
		$r = @mysql_query($s,$l) or die("<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
			<datos>
			<guardado>0</guardado>
			<consulta>".str_replace("''","null",$s)."</consulta>
		...
		if((guiave=="emp" || $_GET[otrosimporte]>0 || $_GET[sobmontoafacturar]>0) && $_GET[credito]!="SI"){
			$s = "call proc_RegistroAuditorias('LF','$nfact',NULL)";
			$d = mysql_query($s, $l);
		}
	...
	if($_GET[accion] == 6){
		...
		FROM formapago WHERE procedencia = 'F'
		AND guia = '$_GET[folio]'";
		$r = mysql_query($s,$l) or die("<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
				<datos>
				<resultado>0</resultado>
		...
		if(mysql_num_rows($r)>0){
			...
			$xml = "<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
				...
				<fechapago>".cambio_texto(($f->credito=='NO')?$f->fechafactura:$facturaestado)."</fechapago>
