<?	header('Content-type: application/vnd.ms-excel');
	...
	IFNULL(f.otrosmontofacturar,0) AS importe,a.idusuario, CONCAT_WS(' ',ce.nombre,ce.apellidopaterno,ce.apellidomaterno) AS empleado
	...
	CONCAT_WS(' ', cc.nombre,cc.paterno,cc.materno) AS cliente, cs.prefijo AS destino,
	IFNULL(fd.total,0) AS importe,a.idusuario, CONCAT_WS(' ',ce.nombre,ce.apellidopaterno,ce.apellidomaterno) AS empleado
	FROM abonodecliente a
	INNER JOIN abonodecliente_facturas af ON a.id = af.folioabono AND a.idsucursal = af.sucursal
	INNER JOIN facturacion f ON af.factura = f.folio
	INNER JOIN facturadetalle fd ON f.folio = fd.factura
	...
	obtenerDatos($criterioingresocobranzacredito,"INGRESOS DE COBRANZA PAQUETERIA","",$l,$sucursalNombre);
...
/*						$f[0] = utf8_decode($f[0]);
					...
					for($i=0;$i<count($data);$i++){
						...
						echo '<td align="right">$'.number_format($data[$i][4],2).'</td>';
						echo '</tr>';				
						$usuario = $data[$i][5];
						$v_total += $data[$i][4];
						$v_empleado = $data[$i][6];
