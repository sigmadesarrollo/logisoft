...
<script>
	...
	function mostrarGuias(datos){
		if(datos.indexOf("SELECT")==-1){
			...
			u.nombre.value = objeto.cliente.nombre+" "+objeto.cliente.paterno+" "+objeto.cliente.materno;
			Total();
		}
	}
...
function Total(){
	...
	var total=0.0;	
	for(i=0;i<tabla1.getRecordCount();i++){
		if(document.all["tablalista_S"][i].checked==true && document.all["tablalista_TIPO_FLETE"][i].value!='PAGADO'){
			total +=parseFloat(document.all["tablalista_IMPORTE"][i].value.replace("$ "," "));
		}
	}
	if(!isNaN(total)){
		u.total.value=parseFloat(total).toFixed(2);
...
function perdirEntrega(folio){
	consultaTexto("MostrarPedirCliente", "entregaocurre_con.php?accion=3&folio="+folio+"&sucursal="+document.all.sucursal.value);
}

function MostrarPedirCliente(datos){
	var objeto = eval("("+convertirValoresJson(datos)+")");
	if(objeto.datoscliente.nombre1!=undefined){
		u.cliente.value 	= objeto.datoscliente.cliente;
		u.nombre.value 		= objeto.datoscliente.nombre;
		u.folio.value 		= objeto.datoscliente.folio;
		u.cliente.value 	= objeto.datoscliente.cliente;
		u.nombre.value 		= objeto.datoscliente.nombre;
		u.folio.value 		= objeto.datoscliente.folio;
	}
	tabla1.setJsonData(objeto.datostabla); 
	u.guardar.style.visibility = "hidden";
}
...
function mostrarformapago(){
		...
		u.nc.value		= 0;
		u.nc_folio.value= 0;
		if(u.total.value<1 || u.total.value==''){
			ejecutarSubmit();
		}else{
			abrirVentanaFija('formapago.php?total='+u.total.value+'&cliente='+u.cliente.value, 600, 400, 'ventana', 'Forma de Pago');
		}
  ...
  <tr>
    <td><table width="615" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <table width="615" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td width="25"><div class="ebtn_buscar" onclick="abrirVentanaFija('../buscadores_generales/buscarsucursal.php', 600, 550, 'ventana', 'Forma de Pago')"></div></td>
          </tr>
        </table>        </td>
      </tr>
      <tr>
        <td colspan="2" class="FondoTabla Estilo4">Obciones Busqueda </td>
      </tr>
      <tr>
        <td colspan="2"><table width="615" border="0" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td width="463"><div class="ebtn_buscar" onclick="abrirVentanaFija('../buscadores_generales/buscarGuiasEmpresariales_VentanillaGen.php?funcion=pedirGuiasPGuia&tipo=1', 600, 550, 'ventana', 'Buscar')"></div></td>
            </tr>
        </table></td>
      </tr>
      <tr>
        <td colspan="2"><table width="518" border="0" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td width="24" class="Tablas"><div class="ebtn_buscar" onclick="abrirVentanaFija('../buscadores_generales/buscarClienteGen.php?funcion=pedirGuiasPCliente', 600, 450, 'ventana', 'Buscar')"></div></td>
              <td width="51"><span class="Tablas">
                <label>Nombre</label>
                <label></label>
                &nbsp;</span></td>
              <td><span class="Tablas">
                <input name="nombre" type="text" class="Tablas" id="nombre" style="width:300px;background:#FFFF99" value="<?=$nombre ?>" readonly=""/>
              </span></td>
              <td width="84">&nbsp;</td>
            </tr>
        </table></td>
      ...
      <tr>
        ...
        <td><input name="precibe" style="width:250px" type="text" id="precibe" value="<?=$precibe ?>" /></td>
      </tr>
      <tr>
        <td width="116">Tipo Identificaci&oacute;n</td>
        <td width="499"><select name="identificacion" class="Tablas" style="width:160px; text-align:center" >
          ...
          No. Identificacion
          <input name="nidentificacion" type="text" style="width:80px" id="nidentificacion" value="<?=$nidentificacion ?>" /></td>
      </tr>
      ...
      <tr>
        <td colspan="2" align="center" id="celdabotones">
        	...
        	<table width="161">
       	    	<tr>
                	<td width="160"><table width="155" border="0">
                        <tr>
                          <td width="75" ><div id="guardar" class="ebtn_guardar" onclick="if(validarDatos()){ mostrarformapago();}" ></div></td>
