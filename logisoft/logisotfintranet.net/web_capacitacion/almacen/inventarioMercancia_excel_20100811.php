...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
	...
	$r = mysql_query($s,$l) or die($s); $f = mysql_fetch_object($r);
 	...
 	//$row[0]=sucursal, $row[1]=fechainicio, $row[2]=fechafin, $row[3]=estado
		...
		if($_GET[estado] == "EN REPARTO EAD"){
			...
			$condicionV=" (g.estado='ALMACEN DESTINO' OR g.estado='AUTORIZACION PARA SUSTITUIR' OR g.estado='POR RECIBIR') AND g.ocurre=0";
			$condicionE=" (ge.estado='ALMACEN DESTINO' OR ge.estado='AUTORIZACION PARA SUSTITUIR' OR ge.estado='POR RECIBIR') AND ge.ocurre=0";
		}else if($_GET[estado] == "OCURRE"){
			$condicionV=" (g.estado='ALMACEN DESTINO' OR g.estado='AUTORIZACION PARA SUSTITUIR' OR g.estado='POR RECIBIR') AND g.ocurre=1";
			$condicionE=" (ge.estado='ALMACEN DESTINO' OR ge.estado='AUTORIZACION PARA SUSTITUIR' OR ge.estado='POR RECIBIR') AND ge.ocurre=1";
		}else if($_GET[estado] == "ALMACEN TRANSBORDO"){		
			$condicionV = " g.estado = 'ALMACEN TRANSBORDO'";
			$condicionE = " ge.estado = 'ALMACEN TRANSBORDO'";
		}else if($_GET[estado] == "TODOS"){
 ...
 $xls .= '<Worksheet ss:Name="Hoja1">
  <Table ss:ExpandedColumnCount="9" ss:ExpandedRowCount="'.($filas+9).'" x:FullColumns="1"
   ...
   <Row>
    <Cell ss:Index="2" ss:MergeAcross="4" ss:StyleID="s26"><Data ss:Type="String">PAQUETERIA Y MENSAJERIA EN MOVIMIENTO</Data></Cell>
   </Row>
   <Row>
