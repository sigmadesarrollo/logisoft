...
cAutocomplete.complete = function( hEvent )
{
	...
	for( var nI = 0; nI < hArr.length; nI++ ){
		...
		if( nIdx == 0 && sMonth.length > sVal.length ){
			hElement.value = hArr[ nI ].split(":")[0];			
			hElement.codigo = hArr[ nI ].split(":")[1];
			if( hElement.createTextRange ){
				hRange = hElement.createTextRange()
				if(hArr[nI].indexOf(":")>-1){					
					if(hArr[ nI ].split(":")[0].substr( sVal.length )!=""){
						hRange.findText(hArr[nI].split(":")[0].substr(sVal.length) )
						hRange.select()
					}
				}else{
