<? 	session_start();
	...
	if($_GET[accion]==1){//OBTENER UNIDAD
		...
		$s = "SELECT r.folio, r.sucursal, r.estado, r.horario, CONCAT(c.nombre,' ',c.paterno,' ',c.materno) AS cliente, r.FolioCarta, 
		...
		".(($_GET['folio'] !="")? " AND r.folio ='$_GET[folio]'" : "")."
		ORDER BY (SUBSTRING(r.folio,1,4)*1) ASC, (SUBSTRING(r.folio,6)*1) ASC";
		if($_GET[tipo]=="0"){
			$r = mysql_query($s,$l) or die($s);
			...
			$r = mysql_query($s,$l) or die($s);
		 		while($f = mysql_fetch_object($r)){
					...
					r.fecharegistro=".(($_GET['fecha']=="")? "CURRENT_DATE()" : "'".cambiaf_a_mysql($_GET['fecha'])."'")."
					",$l) or die(mysql_error($l).$sc);
					...
					INNER JOIN catalogomotivos m ON r.motivo = m.id
					WHERE r.recoleccion='".$f->folio."' AND r.sucursal=".$_GET[sucursal]." AND 
					r.fecharegistro>=".(($_GET['fecha']=="")? "CURRENT_DATE()" : "'".cambiaf_a_mysql($_GET['fecha'])."'")."",$l);
			...
			$s = "SELECT SUM(IF(r.estado='CANCELADO' AND r.fecharecoleccion>CURRENT_DATE,1,0)) 'canceladas' ,
		SUM(IF(r.estado<>'CANCELADO' AND r.realizo='SI' AND r.fecharecoleccion>CURRENT_DATE,1,0)) 'realizadas',
		SUM(IF(r.estado<>'CANCELADO' AND r.realizo='NO' AND r.fecharecoleccion>CURRENT_DATE,1,0)) 'transmitidas'
		FROM recoleccion r
		INNER JOIN catalogocliente c ON r.cliente = c.id
		WHERE r.fecharegistro = 
		".(($_GET['fecha']=="")? "CURRENT_DATE()" : cambiaf_a_mysql($_GET[fecha]))."
		...
		".(($_GET['cliente'] !="")? " AND r.cliente =".$_GET['cliente']."" : "");
			...
			echo "[{'canceladas':'".$f->canceladas."', 'realizadas':'".$f->realizadas."', 'transmitidas':'$f->transmitidas'}]";
		}
	...
	if(mysql_num_rows($sc)>0){	
						...
						for($j=0;$j<count($dia);$j++){
							if($dia[$j]==$f->dia){
								...
								VALUES ('$folio',CURRENT_DATE,'NO TRANSMITIDO', $d->idsucursal,
								 $d->idorigen , $d->iddestino,$d->idcliente,UCASE('$dir->calle'), UCASE('$dir->numero'),
		...
		FROM recoleccion r
		INNER JOIN catalogocliente c ON r.cliente = c.id
		WHERE (r.fecharegistro ='".cambiaf_a_mysql($_GET[fecha])."' AND r.sucursal=".$_GET[sucursal]." 
	...
	}else if($_GET[accion]==18){//OBTENER Folio
		$row = ObtenerFolio('cartaporte','webpmm');
		echo $row[0];
	}
