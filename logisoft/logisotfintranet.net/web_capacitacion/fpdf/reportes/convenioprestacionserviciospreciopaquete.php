<?php
	...
	class pdf extends FPDF{
		...
		function Header(){
			...
			$this->Cell(70,10,'REPORTE:  CONVENIO DE PRESTACION DE SERVICIOS PRECIO POR PAQUETE 						FECHA IMPRESO:'.date('d/m/Y').'',0,0,'L');
			$this->Ln(5);
	...
	for($i=0;$i<count($data);$i++){
		...
		$pdf->MultiCell(0,5,'Entregas a Subdestinos		______',0,'J');
		...
		$pdf->MultiCell(0,5,'8.- El cliente dice que su consumo mensual estimado es de __________________, en base a este consumo La Prestadora de Servicios ofrece la siguiente forma de cobro para sus envíos:',0,'J');
		$pdf->MultiCell(0,5,'',0,'J');
		
		$pdf->MultiCell(0,5,'			- Precio por paquete: 50.00 hasta 30 kgs a una distancia de 200 kms,',0,'J');
		$pdf->MultiCell(0,5,'			Kg excedente 2.00',0,'J');
		$pdf->MultiCell(0,5,'			Bloque kilometraje excedente 5		(Cada 200 kms)',0,'L');
		$pdf->MultiCell(0,5,'			- Precio por tarima: de 800.00 hasta 350 kilogramos a una distancia de 200 kilometros',0,'J');
		$pdf->MultiCell(0,5,'			Kg excedente 2.00',0,'J');
		$pdf->MultiCell(0,5,'			Bloque Kilometraje excedente 5			(Cada 200 kms)',0,'J');
		$pdf->MultiCell(0,5,'',0,1,'L');
		
		$pdf->MultiCell(0,5,'			8.1.- Servicios para el cliente:',0,'J');
		$pdf->MultiCell(0,5,'					Recolección a Domicilio				(Sobre lo capturado el Grid)',0,'L');
		$pdf->MultiCell(0,5,'',0,'L');
		
		$pdf->MultiCell(0,5,'Cuando se realicen envíos con descripciones no contempladas en el cuadro anterior se aplicará el ______% de descuento sobre flete. Si el cliente requiere servicios no especificados o conveniados en este contrato se cobrará de acuerdo a las tarifas establecidas por La Prestadora de Servicios.',0,'J');
		
		$pdf->MultiCell(0,5,'',0,'J');
		
	$pdf->MultiCell(0,5,'9.- El Cliente acepta que para determinar el peso de sus envíos, se considerará el peso báscula o volumétrico (largo x alto x ancho), tomándose siempre el que resulte superior.',0,'L');
			$pdf->MultiCell(0,5,'',0,'J');
	$pdf->MultiCell(0,5,'10.- Si El Cliente solicita Guías Electrónicas se le asignarán un password, nip y los folios de las guías, los cuales podrá utilizar a través de nuestra página web, el uso del nip y los folios es responsabilidad de El Cliente.  La elaboración de una guía electrónica no implicará una responsabilidad de su contenido para la Prestadora de Servicios hasta que esta confirme que ha recibido las mercancías.',0,'J');
			$pdf->MultiCell(0,5,'',0,'J');
	$pdf->MultiCell(0,5,'11.- El Cliente solicita que se consideren las siguientes Restricciones en sus envíos.',0,'J');
				
	$pdf->MultiCell(0,5,'					No recibe por cobrar',0,'J');
	$pdf->MultiCell(0,5,'					No Asegura Mercancia',0,'J');
				$pdf->MultiCell(0,5,'',0,'J');
				$pdf->MultiCell(0,5,'',0,'J');
	$pdf->MultiCell(0,5,'12.- El Precio otorgado al cliente en este contrato es en base al consumo estimado prometido en el punto 8, de no cumplir con el consumo mensual La Prestadora de Servicios podrá dar por terminado este convenio en cualquier momento.',0,'J');
	$pdf->MultiCell(0,5,'',0,'J');
		$pdf->MultiCell(0,5,'13.- En caso de no declarar valor "La Prestadora de Servicios" se compromete a cubrir por daños o estravío, lo establecido en el contrato de adhesión en su cláusula décima segunda de la guía expedida.',0,'J');
		$pdf->MultiCell(0,5,'',0,'J');
		
		$pdf->MultiCell(0,5,'14.- A todos nuestros servicios le será agregado el 16% del impuesto al Valor Agregado.',0,'J');
		$pdf->MultiCell(0,5,'',0,'J');
		$pdf->MultiCell(0,5,'15.- Estos precios pueden variar sin previo aviso durante la vigencia de este contrato.',0,'J');
		$pdf->MultiCell(0,5,'',0,'J');
		$pdf->MultiCell(0,5,'16.- En caso de presentar inconformidad respecto a la interpretación, aplicación o ejecución de este contrato, las partes se someten a la competencia de los tribunales del fuero común y a las leyes que rigan en la Ciudad de Mazatlán, Sinaloa, renunciando expresamente a cualquier fuero que por su domicilio presente o futuro pudiera corresponder.',0,'J');
		...
		if($f->consignacioncaja==1 || $fx->c1>0 || $fx->c2>0 || $fx->c3>0){
