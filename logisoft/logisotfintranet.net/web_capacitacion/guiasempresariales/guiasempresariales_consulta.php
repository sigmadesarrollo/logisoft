<?
	...
	require_once("../clases/RollsBack.php");
	require_once("../../funciondeerrores.php");
	$l = Conectarse("webpmm");
	...
	if($_GET[accion] == 1){
		...
		cs.iva, cpo.descripcion as npobdes, cd.restringirporcobrar, if(cs.id=13 or cs.id=12,1,cd.subdestinos) subdestinos,
		...
		if(mysql_num_rows($r)>0){
			...
			$s = "SELECT gv.iddestinatario, 
			CONCAT(cc.nombre,' ',cc.paterno,' ',cc.materno) nombre, cc.rfc,
			gv.iddirecciondestinatario,
			d.calle, d.numero, d.colonia, d.cp, d.poblacion, d.telefono 
			FROM guiasventanillaclientes gv
			INNER JOIN direccion d ON gv.iddirecciondestinatario = d.id
			INNER JOIN catalogocliente cc ON gv.iddirecciondestinatario = cc.id
			WHERE gv.id = '$f->guiaempresarial'
			...
			if(mysql_num_rows($rmm)>0){
				...
				<dncliente>".strtoupper(cambio_texto($fff->nombre))."</dncliente>
				<drfc>".strtoupper(cambio_texto($fff->rfc))."</drfc>
				<iddirecciondestinatario>$fff->iddirecciondestinatario</iddirecciondestinatario>
				<dcalle>".strtoupper(cambio_texto($fff->calle))."</dcalle>
				<dnumero>".strtoupper(cambio_texto($fff->numero))."</dnumero>
				<dcp>".strtoupper(cambio_texto($fff->cp))."</dcp>
				<dpoblacion>".strtoupper(cambio_texto($fff->poblacion))."</dpoblacion>
				<dtelefono>".strtoupper(cambio_texto($fff->telefono))."</dtelefono>
				<dcolonia>".strtoupper(cambio_texto($fff->colonia))."</dcolonia>";
			}
			...
			INNER JOIN generacionconvenio AS gcn ON cc.id = gcn.idcliente 
	...
	if($_GET[accion] == 4){
		...
		$r = mysql_query($s,$l) or postError("<xml version=\"1.0\" standalone=\"yes\" encoding=\"utf-8\"> 
				<datos>
				<guardado>0</guardado>
				<consulta>$s</consulta>
				</datos>
		...
		$rb->prepararDatosGuiaE($_GET[newfolio], $_GET[evaluacion], $_SESSION[IDSUCURSAL], $_SESSION[IDUSUARIO], $_GET[bolsaempaque]);
		...
		evaluacion='$_GET[evaluacion]', fecha=current_date,iddireccionremitente='$_GET[iddireccionremitente]', 
		...
		$r = @mysql_query(str_replace("''","null",$s),$l) or $rb->rollBackGuiaempresarial(0,$s,$l);
		
		$verifG->verificaGuiaEmpresarial($_GET[newfolio]);
		
		$s = "update evaluacionmercancia set estado = 'ENGUIA' 
		where folio = $_GET[evaluacion] and sucursal = ".$_SESSION[IDSUCURSAL]."";
		mysql_query($s,$l) or $rb->rollBackGuiaempresarial(2,$s,$l);
		
		$s = "INSERT INTO guias_valorotros
		SET guia = '$_GET[newfolio]', otros='$_GET[nuevootros]', fecha=CURRENT_DATE";
		$r = @mysql_query($s,$l) or $rb->rollBackGuiaempresarial(3,$s,$l);
		...
		mysql_query($s,$l) or $rb->rollBackGuiaempresarial(4,$s,$l);
		...
		/*$s = "call proc_RegistroAuditorias('LGE','$_GET[newfolio]',$_SESSION[IDSUCURSAL])";
			mysql_query($s, $l) or postError("<xml version=\"1.0\" standalone=\"yes\" encoding=\"utf-8\"> 
				<datos>
				<guardado>0</guardado>
		...
		$r = @mysql_query($s,$l) or $rb->rollBackGuiaempresarial(5,$s,$l);
		...
		if(mysql_num_rows($r)<1){		
			...
			@mysql_query(str_replace("''","null",$s),$l) or $rb->rollBackGuiaempresarial(5,$s,$l);
		}
		...
		if($_GET[bolsaempaque]!="" && $_GET[bolsaempaque]!="0"){
			...
			$r = @mysql_query(str_replace("''","null",$s),$l) or $rb->rollBackGuiaempresarial(6,$s,$l);
		}
		...
		if($_GET[tipopago]=="CONTADO"){
			...
			@mysql_query(str_replace("''","null",$s),$l) or postError("<xml version=\"1.0\" standalone=\"yes\" encoding=\"utf-8\"> 
					<datos>
					<guardado>0</guardado>
					<consulta>$s</consulta>
					</datos>
		...
		if($_GET[tipoguia]!="PREPAGADAS"){
			...
			$r = @mysql_query(str_replace("''","null",$s),$l) or $rb->rollBackGuiaempresarial(7,$s,$l);
			
			$s = "call proc_RegistroVentas('VENTA_GUIAS_E','$_GET[newfolio]',0)";
			$r = @mysql_query($s,$l) or $rb->rollBackGuiaempresarial(8,$s,$l);
		}
		...
		/*if($_GET[tipopago]=="CREDITO"){
			...
			$r = @mysql_query(str_replace("''","null",$s),$l) or postError("<xml version=\"1.0\" standalone=\"yes\" encoding=\"utf-8\"> 
				<datos>
				<guardado>0</guardado>
				<consulta>".str_replace("''","null",$s)."</consulta>
				</datos>
		...
		WHERE idusuario = '$_GET[idpagina]'";
		$r = mysql_query($s,$l) or $rb->rollBackGuiaempresarial(9,$s,$l);
		if(mysql_num_rows($r)>1){
			...
			$s = "INSERT INTO guiasempresariales_detalle
			SELECT '$_GET[newfolio]', ed.cantidad, cd.descripcion, ed.contenido, ed.peso, ed.largo, ed.ancho, ed.alto,
		...
		$r = mysql_query($s,$l) or $rb->rollBackGuiaempresarial(10,$s,$l);
	...
	if($_GET[accion] == 5){
		...
		LEFT JOIN catalogocliente AS ccd ON gv.iddestinatario = ccd.id
		...
		if(mysql_num_rows($r)>0){
			...
			if($f->estado=='EN REPARTO EAD'){
				...
				if(mysql_num_rows($rre)>0){
					...
					$f->estado = 'ENTREGADA PENDIENTE LIQUIDAR';
	...
	if($_GET[accion] == 7){
		...
		$s = "SELECT idsucursaldestino FROM guiasempresariales WHERE folio=$_GET[folio];";
		$r = mysql_query($s,$l) or die($s); 
		$f = mysql_fetch_object($r);
		
		$s = "UPDATE lasalertas SET mereem=mereem-1 WHERE sucursal=$f->idsucursaldestino;";
		mysql_query($s,$l) OR die(mysql_error($l).$s);
		$s = "UPDATE lasalertas SET gupore=gupore-1 WHERE sucursal=$f->idsucursaldestino;";
