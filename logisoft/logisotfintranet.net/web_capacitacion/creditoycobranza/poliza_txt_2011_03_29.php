<?
	...
	$totalcargos = 0;
		...
		GROUP BY gv.idsucursaldestino, gv.id having not isnull(gv.id)	
		UNION
		...
		$s = "INSERT INTO movimientos
		(tipom,orden,guia,descripcion,cantidad,cuenta,prefijo)
		SELECT DISTINCT 1, 'E', f.folio, concat('LIQ. DEL $diainicioarriba AL $diafinarriba $mesarriba/".substr($dias[0],2,4)." ',cs.prefijo), SUM(IFNULL(f.sobseguro,0)) seguro, CONVERT(CONCAT_WS('',$inicuecom,cs.idsucursal,'000400') USING utf8) cuenta, cs.prefijo
		FROM facturacion f
		INNER JOIN catalogosucursal_tmp_".$var_itabla." cs ON f.idsucursal = cs.id
		WHERE f.idsucursal = $idsucursalorigen
		...
		UNION
		SELECT DISTINCT 1, 'G', f.folio, concat('LIQ. DEL $diainicioarriba AL $diafinarriba $mesarriba/".substr($dias[0],2,4)." ',cs.prefijo), SUM(IFNULL(f.sobexcedente,0)) adicional, 
		CONVERT(CONCAT_WS('',$inicuecom,cs.idsucursal,'000600') USING utf8) cuenta, cs.prefijo
		FROM facturacion f
		INNER JOIN catalogosucursal_tmp_".$var_itabla." cs ON f.idsucursal = cs.id
		WHERE f.idsucursal = $idsucursalorigen
		...
		UNION
		SELECT DISTINCT 0, 'CA',gv.id,concat(gv.id,' ',cs.prefijo), SUM(IFNULL(gv.tiva,0)) iva, 
		...
		$s = "select tipom, cuenta, descripcion, cantidad
		from movimientos
		where not isnull(guia) and orden = 'CA'
		order by orden";
		$r = mysql_query($s,$l) or die(mysql_error($l)."-".$s);
