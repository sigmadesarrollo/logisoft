<?
	...
	if($_GET['accion']==1){
		...
		INNER JOIN bitacorasalida AS bs ON catalogoruta.id = bs.ruta AND bs.folio = '$_GET[foliobitacora]'
		...
		WHERE bitacorasalida.folio = '$_GET[foliobitacora]' AND bitacorasalida.unidad = '$_GET[unidad]'
		...
		if(mysql_num_rows($r)>0){
			...
			INNER JOIN catalogorutadetalle ON catalogoruta.id = catalogorutadetalle.ruta
			INNER JOIN bitacorasalida ON catalogoruta.id = bitacorasalida.ruta
			WHERE bitacorasalida.folio = '$_GET[foliobitacora]' AND catalogorutadetalle.tipo<>1 
	...
	if($_GET[accion]==9){
		...
		SET guiaventanilla_unidades.proceso = 'POR RECIBIR', guiaventanilla_unidades.unidad='$_GET[unidad]',
		guiasventanilla.estado = 'EN TRANSITO', guiaventanilla_unidades.ubicacion = guiasventanilla.idsucursaldestino
		...
		SET guiasempresariales_unidades.proceso = 'POR RECIBIR', guiasempresariales_unidades.unidad='$_GET[unidad]',
		guiasempresariales.estado = 'EN TRANSITO', guiasempresariales_unidades.ubicacion = guiasempresariales.idsucursaldestino
		WHERE embarques_tmp.idusuario = $_GET[idpagina] and embarques_tmp.subido = 'S'
		and embarques_tmp.sucursal = ".$_SESSION[IDSUCURSAL]."";
		//echo "$s<br><br>";
		mysql_query($s,$l) or die($s."<BR>".mysql_error($l));
