...
<table width="500"  border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
    ...
    <tr>
      <td colspan="3" class="Tablas" height="300px" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="Tablas">
          <?		 
		$get =@mysql_query('SELECT guia, fecha, estado, iddestinatario, total FROM 
		(SELECT gv.id AS guia, DATE_FORMAT(gv.fecha,"%d/%m/%Y") AS fecha, gv.estado, gv.iddestinatario, gv.total
		FROM guiasventanilla gv
		INNER JOIN solicitudcredito sc ON gv.iddestinatario = sc.cliente
		WHERE gv.tipoflete=1 AND gv.condicionpago=0 AND gv.estado<>"ALMACEN ORIGEN" AND gv.estado<>"EN TRANSITO"
		AND (gv.estado="ALMACEN DESTINO" OR (gv.estado="ALMACEN DESTINO" AND gv.fechaentrega=CURRENT_DATE))
		AND ISNULL(gv.factura) AND gv.idsucursaldestino = "'.$_SESSION[IDSUCURSAL].'"
		UNION
		SELECT ge.id AS guia, DATE_FORMAT(ge.fecha,"%d/%m/%Y") AS fecha, ge.estado, ge.iddestinatario, ge.total
		FROM guiasempresariales ge
		INNER JOIN solicitudcredito sc ON ge.iddestinatario = sc.cliente		
		WHERE ge.tipoflete="POR COBRAR" AND ge.tipopago="CONTADO" AND ge.estado<>"ALMACEN ORIGEN" AND ge.estado<>"EN TRANSITO"
		AND (ge.estado="ALMACEN DESTINO" OR (ge.estado="ENTREGADA" AND ge.fechaentrega=CURRENT_DATE))
		AND ISNULL(ge.factura) AND ge.idsucursaldestino = "'.$_SESSION[IDSUCURSAL].'"
		) t limit '.$st.','.$pp,$link);
		while($row=@mysql_fetch_array($get)){
			$s = "SELECT IF(IFNULL(sc.montoautorizado,0) <= (IFNULL(SUM(pg.total),0) + $row[total]) OR cc.activado='NO','NO','SI') AS cambiable
			FROM solicitudcredito sc 
			INNER JOIN catalogocliente cc ON sc.cliente = cc.id 
			LEFT JOIN pagoguias pg ON sc.cliente = pg.cliente
			WHERE sc.cliente = $row[iddestinatario] AND pg.pagado = 'N'";
