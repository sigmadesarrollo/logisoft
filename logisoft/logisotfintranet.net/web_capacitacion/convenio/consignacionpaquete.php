<?
	...
	if($_POST[guardar_hidden]==1){
		$s = "delete from convenio_configurador_caja where idusuario = $_SESSION[IDUSUARIO] and tipo='CONSIGNACION' and isnull(idconvenio)";
		mysql_query($s,$l);
		...
		for($j=0; $j<$_POST[cantidadfilas]; $j++){
			...
			for($i=1; $i<$_POST[cantidadzonas]; $i++){
				...
				descripcion='".$_POST["descripcion"][$j]."', tipo='CONSIGNACION', 
		...
		$_POST[tienedatosprecioc]=1;
	}
	
	if($_GET[idconvenio]){
		
	}else{
		if($_GET[tienedatosprecioc]!=1 && $_POST[tienedatosprecioc]!= 1){
			$s = "delete from convenio_configurador_caja where idusuario = $_SESSION[IDUSUARIO] and tipo='CONSIGNACION' and isnull(idconvenio)";
			mysql_query($s,$l);
...
<script>
	...
	eval("page" + id + " = window.open(URL, '" + id + "', 'toolbar=1,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=530,height=380,left = 470,top = 200');");				
				...
				alerta2('Ya Se Encuentra Abierta la busqueda','¡Atención!','descripcions');
			}
		...
		if(u.descripcion_txt.value==""){
			...
			if(indice!=undefined){
				...
				if(indice==0 && tbody.rows.length==2){
					u.descripcion.value = u.descripcion_txt.value;
    ...
    <td><table width="547" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td colspan="6"><table width="545" border="0" cellpadding="0" cellspacing="0">
          <tr>
            <td width="57">Descripci&oacute;n</td>
            ...
            <td width="24"><img src="../img/Buscar_24.gif" width="24" height="23" align="absbottom" onClick="javascript:popUp('../evaluacion/buscar.php?tipo=descripcion')" style="cursor:pointer"></td>
            <td width="200"><input name="abierto" type="hidden" readonly=""></td>
            <td width="16">&nbsp;</td>
            <td width="63"><input type="hidden" name="tienedatosprecioc" 
            value="<?=($_GET[tienedatosprecioc]!="")?$_GET[tienedatosprecioc]:$_POST[tienedatosprecioc]?>"></td>
            <td width="10">&nbsp;</td>
            <td width="75">&nbsp;</td>
          </tr>
        </table></td>
      ...
      <tr>
        <td width="86">Zona</td>
        <td width="136">
		<select name="zona" style="width:100px" onkeypress="if(event.keyCode==13){u.preciocaja_txt.focus();}">
        ...
        <td width="123"><input name="preciocaja_txt" type="text" style="width:100px" value="" onKeyPress="if(event.keyCode==13){document.all.pesolimite_txt.focus();}"></td>
        <td width="43">&nbsp;</td>
        <td width="105">&nbsp;</td>
      </tr>
      <tr>
        <td width="86">Peso Limite</td>
        <td width="136"><input name="pesolimite_txt" type="text" style="width:100px" value="" onKeyPress="if(event.keyCode==13){u.precioexcedente_txt.focus();}else{return solonumeros(event);}"></td>
        <td width="124">Precio KG Exedente </td>
        <td width="123"><input name="precioexcedente_txt" type="text" style="width:100px" value="" onKeyPress="if(event.keyCode==13){agregar();}"></td>
        <td width="43">&nbsp;</td>
      ...
      <tr>
        <td colspan="6"><div id="detalle" style="width:547px; height:80px; overflow:auto" align="left">
              ...
              <table border="0" cellspacing="0" cellpadding="0" width="<?=$tamanotabla+265?>" id="tablaconveniopreciocaja">
                ...
                <?
					$s = "SELECT * FROM convenio_configurador_caja WHERE idusuario = $_SESSION[IDUSUARIO] and tipo='CONSIGNACION' and isnull(idconvenio) GROUP BY descripcion";
					$r = mysql_query($s,$l) or die($s);
                ...
                <tr>
                    ...
                    <? 
					$s = "SELECT * FROM convenio_configurador_caja WHERE idusuario = $_SESSION[IDUSUARIO] and tipo='CONSIGNACION' 
					and isnull(idconvenio) and descripcion = '$f[descripcion]' order by zona";
      ...
      <tr>
        <td colspan="6" align="center"><table width="272">
          <tr>
            <td width="131" align="center"><div class="ebtn_guardar" onClick="u.guardar_hidden.value=1; u.cantidadfilas.value=u.tablaconveniopreciocaja.rows.length-1; document.form1.submit()"></div>
...
<script>
	...
	if($_POST[guardar_hidden]==1){
	?> 
	parent.document.all.detalledescripcionc.innerHTML = document.all.detalle.innerHTML.replace(/visible/g,"hidden");
	parent.document.all.tienedatosprecioc.value = 1;
	parent.document.all.tienedatosprecioc_excedio.value = <?=$excedio?>;
