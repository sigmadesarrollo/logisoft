<?	session_start();
	...
	if($_GET[accion]==1){//OBTENER DETALLE
		...
		if($row[3]=="EN REPARTO EAD"){
			$condicionV=" g.estado='EN REPARTO EAD' and g.id not like '%Z'";
			$condicionE=" ge.estado='EN REPARTO EAD' and ge.id not like '%Z'";
		}else if($row[3]=="EAD"){
			...
			$condicionV=" (g.estado<>'CANCELADA' AND g.estado <>'CANCELADO' AND IF(g.fechaentrega > '".cambiaf_a_mysql($row[1])."',(g.estado = 'ENTREGADA' AND g.estado = 'ENTREGADO'), g.estado <>'ENTREGADA' 
AND g.estado <> 'ENTREGADO') and g.id not like '%Z')";
			$condicionE=" (ge.estado<>'CANCELADA' AND ge.estado <>'CANCELADO' and IF(ge.fechaentrega > '".cambiaf_a_mysql($row[1])."',(ge.estado = 'ENTREGADA' AND ge.estado = 'ENTREGADO'),ge.estado <>'ENTREGADA' 
AND ge.estado <> 'ENTREGADO') and ge.id not like '%Z')";
		}
		
		$fechaV = " AND g.fecha <= '".cambiaf_a_mysql($row[1])."' ";
		$fechaE = " AND ge.fecha <= '".cambiaf_a_mysql($row[1])."' ";
		...
		$s = "select * from (
			...
			WHERE g.id NOT LIKE '888%' AND ".(($row[0]!=1)? "g.idsucursaldestino='".$row[0]."' AND ": "")." $condicionV $fechaV
			GROUP BY g.id
			UNION 
			SELECT g.idsucursaldestino, cs.prefijo AS sucursal, g.id AS guia, 
			...
			LEFT JOIN guiaventanilla_detalle gd ON g.id = gd.idguia
			WHERE g.id LIKE '888%' AND ".(($row[0]!=1)? "g.idsucursaldestino='".$row[0]."' AND ": "")." $condicionV $fechaV
			GROUP BY g.id
			UNION
