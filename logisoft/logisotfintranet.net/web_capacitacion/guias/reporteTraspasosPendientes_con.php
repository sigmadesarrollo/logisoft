<?	session_start(); 
	...
	if($_GET[accion]==1){
		...
		foreach($guiasFolios as $folio){
			...
			VALUES($folio,'".$_SESSION[IDSUCURSAL]."','TRASPASO CARGO',CURRENT_DATE,CURRENT_TIME,'".$_SESSION[IDUSUARIO]."')";
			mysql_query($s,$l) or die($s);
			$s ="INSERT INTO historialmovimientos (modulo,folio,estado,idusuario,fechamodificacion) 
			VALUES('guiasventanilla',$folio,'TRASPASO CARGO','".$_SESSION[IDUSUARIO]."',CURRENT_TIMESTAMP)";
			mysql_query($s,$l) or die($s);
		}
		...
		foreach($guiasFolios as $folio){
			#-- seguimiento guias y historial de movimientos
			$s ="INSERT INTO seguimiento_guias (guia,ubicacion,estado,fecha,hora,usuario) 
			VALUES($folio,'".$_SESSION[IDSUCURSAL]."','TRASPASO NO AUTORIZADO',CURRENT_DATE,CURRENT_TIME,'".$_SESSION[IDUSUARIO]."')";
			mysql_query($s,$l) or die($s);
			$s ="INSERT INTO historialmovimientos (modulo,folio,estado,idusuario,fechamodificacion) 
			VALUES('guiasventanilla',$folio,'TRASPASO NO AUTORIZADO','".$_SESSION[IDUSUARIO]."',CURRENT_TIMESTAMP)";
			mysql_query($s,$l) or die($s);
		}
