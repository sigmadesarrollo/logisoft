<?
	...
	header("Content-Disposition: attachment; filename=concesiones_ventas_recibidas_Excel.xls");
...
<style>
	...
	<table width="1728" border="0" cellpadding="1" cellspacing="0">
    	...
    	<tr>
    	  ...
    	  <td colspan="12">CONCESIONES (VENTAS RECIBIDAS)</td>
   	  </tr>
    	<tr>
    	  <td colspan="2"></td>
    ...
    <?
		...
		$s = "SELECT (ge.id) guia,(ge.fecha) fechaguia,(ge.tflete) flete,(ge.tdescuento) descuento,((ge.tflete+ge.texcedente)-ge.tdescuento) fleteneto,
		(((ge.tflete+ge.texcedente)-ge.tdescuento)*(cs.recibido/100)) comision,(ge.trecoleccion) recoleccion,0 AS comisionrad,(ge.tcostoead) entrega, 
		(ge.tcostoead*(cs.porcead/100)) comisionead,(ge.texcedente) sobrepeso,ge.estado,CONCAT('PAGADA','-',IF(ge.tipopago='CONTADO','CONTADO','CREDITO')) condicion,
		((((ge.tflete+ge.texcedente)-ge.tdescuento)*(cs.recibido/100))+(ge.tcostoead*(cs.porcead/100))) total,(ge.total) tgral
		...
		UNION
		SELECT (gv.id) guia,(gv.fecha) fechaguia,(gv.tflete) flete,(gv.tdescuento) descuento,((gv.tflete+gv.texcedente)-gv.tdescuento) fleteneto,
		(((gv.tflete+gv.texcedente)-gv.tdescuento)*(cs.recibido/100)) comision,(gv.trecoleccion) recoleccion,0 AS comisionrad,(gv.tcostoead) entrega, 
		(gv.tcostoead*(cs.porcead/100)) comisionead,(gv.texcedente) sobrepeso,gv.estado,
		CONCAT(IF(gv.tipoflete=0,'PAGADA','POR COBRAR'),'-',IF(gv.condicionpago=0,'CONTADO','CREDITO')) condicion,
		((((gv.tflete+gv.texcedente)-gv.tdescuento)*(cs.recibido/100))+(gv.tcostoead*(cs.porcead/100))) total,(gv.total) tgral
		...
		UNION /*canceladas*/
		SELECT (gv.id) guia,(gv.fecha) fechaguia,(gv.tflete) flete,(gv.tdescuento) descuento,((gv.tflete+gv.texcedente)-gv.tdescuento) fleteneto,
		((((gv.tflete+gv.texcedente)-gv.tdescuento)*(cs.recibido/100))*-1) comision,(gv.trecoleccion) recoleccion,0 AS comisionrad,(gv.tcostoead) entrega, 
		((gv.tcostoead*(cs.porcead/100))*-1) comisionead,
		(gv.texcedente) sobrepeso,gv.estado,CONCAT(IF(gv.tipoflete=0,'PAGADA','POR COBRAR'),'-',IF(gv.condicionpago=0,'CONTADO','CREDITO')) condicion,
		(((((gv.tflete+gv.texcedente)-gv.tdescuento)*(cs.recibido/100))+(gv.tcostoead*(cs.porcead/100)))*-1) total,(gv.total) tgral
		FROM catalogosucursal cs INNER JOIN guiasventanilla gv  ON cs.id=gv.idsucursaldestino
