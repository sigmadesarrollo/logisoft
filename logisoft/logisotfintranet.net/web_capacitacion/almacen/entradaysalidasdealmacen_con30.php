<?
	...
	if($_GET[accion] == 1){
		...
		$s = "select guia, cantidad, tipoguia, date_format(fecha, '%d/%m/%Y') as fecha, 1 seleccion
		...
		if(mysql_num_rows($r)>0){
			...
			while($f = mysql_fetch_object($r)){
				...
				$f->cantidad = cambio_texto($f->cantidad);
				$f->fecha = cambio_texto($f->fecha);
				$f->tipoguia = cambio_texto($f->tipoguia);
				if($cont==30){
					echo "{'guia':'$f->guia','cantidad':'$f->cantidad','fecha':'$f->fecha','tipoguia':'$f->tipoguia'}";
					echo "]}";
					$cont = 0;
				}else{
					echo "{'guia':'$f->guia','cantidad':'$f->cantidad','fecha':'$f->fecha','tipoguia':'$f->tipoguia'}";
