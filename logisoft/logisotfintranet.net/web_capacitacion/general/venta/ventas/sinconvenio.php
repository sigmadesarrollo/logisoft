...
<script>
	...
	tabla1.setAttributes({
		...
		campos:[
			...
			{nombre:"CONTADO", medida:110, onDblClick:"obtenerContado", tipo:"moneda", alineacion:"center", datos:"contados"},
	...
	function mostrarDetalle(datos){
		if(datos.indexOf("no encontro")<0 && datos!=""){
			var obj = eval(convertirValoresJson(datos));
			var obje= new Object();
			for(var i=0;i<obj.length;i++){
				obje.contados	=	obj[i].contados;
				obje.credito	=	obj[i].credito;
				obje.cobcontado	=	obj[i].cobcontado;
				obje.cobcredito	=	obj[i].cobcredito;
				obje.sucursal	=	obj[i].sucursal;
				obje.total		=	parseFloat(obje.contados) + parseFloat(obje.credito) + parseFloat(obje.cobcontado) + parseFloat(obje.cobcredito); 
				tabla1.add(obje);
			}
		}else{
			var obj = new Object();
