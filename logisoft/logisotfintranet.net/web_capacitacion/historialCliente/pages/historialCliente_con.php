<?	//session_start();
	...
	if($_GET[accion]==1){
		...
		if(mysql_num_rows($r)>0){
			...
			(SELECT COUNT(*) FROM solicitudguiasempresariales s WHERE s.idcliente = ".$_GET[cliente]." AND
			NOT ISNULL(s.cantidad) AND YEAR(s.fecha)=YEAR(CURRENT_DATE)) tsolicitud,
		...
		$s = "SELECT s.id AS folio,IF(s.prepagada='SI','PREPAGADA','CONSIGNACION') AS preocon, 
		s.cantidad,IFNULL(cs.descripcion,'') AS sucursal,DATE_FORMAT(s.fecha,'%d/%m/%Y') AS fecha 
		FROM solicitudguiasempresariales s
		INNER JOIN catalogosucursal cs ON s.sucursalacobrar= cs.id
		WHERE s.idcliente = ".$_GET[cliente]." AND NOT ISNULL(s.cantidad) AND YEAR(s.fecha)=YEAR(CURRENT_DATE)
		ORDER BY s.fecha";
		$r = mysql_query($s,$l) or die($s);
		if(mysql_num_rows($r)>0){	
			$arr = array();
