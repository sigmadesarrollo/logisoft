...
<?
	...
	$s = "SELECT prefijosucursal,cliente,folio,fechaguia,fechafactura,fechavenc,diasvencidos,alcorriente,c1a15dias,c16a30dias,
		c31a60dias,may60dias,saldo,factura,contrarecibo,idcliente,tcargo 
		FROM (
		...
		SUM($if) AS saldo,0 AS factura,IFNULL(ge.acuserecibo,0)AS contrarecibo,temp.idcliente,'' AS tcargo 
		FROM guiasempresariales ge 
		...
		SUM(gv.total) AS saldo,0 AS factura,IFNULL(gv.acuserecibo,0)AS contrarecibo,temp.idcliente,
		IF(IF(gv.tipoflete=0,gv.idsucursalorigen,gv.idsucursaldestino)!=pg.sucursalacobrar,'SI','NO') tcargo 
		...
		DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))<16,(f.total + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)),0)) 
		AS c1a15dias,
		SUM(IF(DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))>15 AND
		DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))<31,(f.total + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)),0)) 
		AS c16a30dias,
		SUM(IF(DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))>30 AND
		DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL temp.dcredito DAY))<61,(f.total + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)),0)) 
		AS c31a60dias,
		SUM(IF(DATEDIFF(CURRENT_DATE(),ADDDATE(f.fecha,INTERVAL IFNULL(temp.dcredito,1) DAY))>60,(f.total + IFNULL(f.sobmontoafacturar,0) + 
		IFNULL(f.otrosmontofacturar,0)),0)) AS may60dias,
		SUM(f.total + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)) AS saldo,
		...
		SUM(fd.total) AS saldo,IFNULL(f.folio,'')AS factura,0 AS contrarecibo,temp.idcliente,'' AS tcargo 
		FROM facturacion f 
...
?>
	<table width="1211">
    	<tr>
        	<td colspan="15" style="font-weight:bold; font-size:16">TITULO: ANTIGUEDAD DE SALDOS</td>
        </tr>
    	<tr>
		  ...
		  <td width="108" style="font-weight:bold; border:#000 solid 1px">TRASPASO CARGO</td>
      </tr>
      	<?
		$arre = array();
		$arr = 0;
		...
		while($f = mysql_fetch_array($r)){
			...
			<tr>
				  ...
				  <td><?=$f[13]?></td>
				  <td><?=$f[14]?></td>
				  <td style=" border-right:1px solid #000"><?=$f[16]?></td>
			  </tr>
			<?
				$var_alco  	+= $f[7];
				$var_115   	+= $f[8];
