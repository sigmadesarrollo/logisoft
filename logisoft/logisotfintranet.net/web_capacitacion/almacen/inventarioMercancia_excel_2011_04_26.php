   ...
   <Font x:Family="Swiss" ss:Size="8"/>
	...
	$r = mysql_query($s,$l) or die($s); $f = mysql_fetch_object($r);
 	...
 	//$row[0]=sucursal, $row[1]=fechainicio, $row[2]=fechafin, $row[3]=estado
		...
		if($_GET[estado] == "EN REPARTO EAD"){
		...
		}else if($row[3] == "ALMACEN TRASBORDO"){		
			$condicionV = " g.estado = 'ALMACEN TRASBORDO' and g.id not like '%Z'";
			$condicionE = " ge.estado = 'ALMACEN TRASBORDO' and ge.id not like '%Z'";
		}else if($row[3] == "ALMACEN ORIGEN"){		
			$condicionV = " g.estado = 'ALMACEN ORIGEN' and g.id not like '%Z'";
			$condicionE = " ge.estado = 'ALMACEN ORIGEN' and ge.id not like '%Z'";
		}else if($row[3] == "EN TRANSITO"){		
			$condicionV = " g.estado = 'EN TRANSITO' and g.id not like '%Z'";
			$condicionE = " ge.estado = 'EN TRANSITO' and ge.id not like '%Z'";
		}else if($row[3] == "POR ENTREGAR"){		
			$condicionV = " g.estado = 'POR ENTREGAR' and g.id not like '%Z'";
			$condicionE = " ge.estado = 'POR ENTREGAR' and ge.id not like '%Z'";
		}else if($row[3] == "POR RECIBIR"){		
			$condicionV = " g.estado = 'POR RECIBIR' and g.id not like '%Z'";
			$condicionE = " ge.estado = 'POR RECIBIR' and ge.id not like '%Z'";
		}else if($_GET[estado] == "TODOS"){
			...
			$condicionV=" (g.estado<>'CANCELADA' AND g.estado <>'CANCELADO' AND IF(g.fechaentrega > '".cambiaf_a_mysql($_GET[fecha])."',(g.estado = 'ENTREGADA' AND g.estado = 'ENTREGADO'), g.estado <>'ENTREGADA' 
AND g.estado <> 'ENTREGADO') and g.id not like '%Z')";
			$condicionE=" (ge.estado<>'CANCELADA' AND ge.estado <>'CANCELADO' and IF(ge.fechaentrega > '".cambiaf_a_mysql($_GET[fecha])."',(ge.estado = 'ENTREGADA' AND ge.estado = 'ENTREGADO'),ge.estado <>'ENTREGADA' 
AND ge.estado <> 'ENTREGADO') and ge.id not like '%Z')";
		...
		contenido, importe, almacen from (
			...
			IF(g.estado='ALMACEN DESTINO' AND g.ocurre=1,'OCURRE',IF(g.estado='ALMACEN DESTINO' AND g.ocurre=0,'EAD',
			g.estado)) AS almacen, IF(g.tipoflete=0,'PAGADA','POR COBRAR') AS flete, IF(g.condicionpago=0,'CONTADO','CREDITO') AS pago
			FROM guiasventanilla g 
			LEFT JOIN catalogosucursal cs ON g.idsucursaldestino = cs.id 
			LEFT JOIN catalogocliente cc ON g.iddestinatario = cc.id 
			LEFT JOIN guiaventanilla_detalle gd ON g.id = gd.idguia
			WHERE g.id NOT LIKE '888%' AND ".(($_GET[sucursal]!=1)? "g.idsucursaldestino='".$_GET[sucursal]."' AND ": "")." $condicionV $fechaV
			GROUP BY g.id
			UNION 
			SELECT g.idsucursaldestino, cs.prefijo AS sucursal, g.id AS guia, 
			...
			LEFT JOIN guiaventanilla_detalle gd ON g.id = gd.idguia
			WHERE g.id LIKE '888%' AND ".(($_GET[sucursal]!=1)? "g.idsucursaldestino='".$_GET[sucursal]."' AND ": "")." $condicionV $fechaV
			GROUP BY g.id
			UNION 
			SELECT ge.idsucursaldestino, cs.prefijo AS sucursal, ge.id AS guia, 
			ge.iddestinatario AS nocliente, CONCAT(cc.nombre,' ',cc.paterno,' ',cc.materno) AS cliente,
			...
			IF(ge.estado='ALMACEN DESTINO' AND ge.ocurre=1,'OCURRE',IF(ge.estado='ALMACEN DESTINO' AND ge.ocurre=0,'EAD',
			ge.estado)) AS almacen, ge.tipoflete AS flete, ge.tipopago AS pago
			FROM guiasempresariales ge
			LEFT JOIN catalogosucursal cs ON ge.idsucursaldestino = cs.id 
			LEFT JOIN catalogocliente cc ON ge.iddestinatario = cc.id 
