<? 	session_start();
	...
	if(mysql_num_rows($r)==0){
		...
		WHERE DATEDIFF(CURRENT_DATE,cc.fecharegistro)>=365 AND cambiado = 'N'";
		mysql_query($s,$l) or die($s);
	}
...
<script>
		...
		function mostrarModulo(modulo){
			if(modulo==1){
				abrirVentanaFija('../../guias/localizadorGuia.php', 400, 400, 'ventana', 'Localizador Guías');				
			}else if(modulo==2){
        ...
        function mostrarPagina(pagina,script,nombre){
		...
		function resVerificacionInicio(res){
			...
			if(obj.sucursal!=1){
				if(var_pagina.indexOf("../../Caja/")>-1){
					...
					if(obj.cierrecajadefinitivo==1){
						alerta3("La caja del usuario actual ya ha sido cerrada definitivamente","&iexcl;Atencion!");
						ponerPestanas('pestana.php');
						return false;
					}
					
					if(obj.cierrecajaprincipal==1){
						alerta3("La caja principal ya ha sido cerrada","&iexcl;Atencion!");
						ponerPestanas('pestana.php');
						return false;
					}
				}
		...
		*/
		...
		function verNoticias(){
			asignarPagina("../../../pmmnews.pmmintranet.com?usuario=<?=$_SESSION['IDUSUARIO'] ?> ");
		}
...
<ul>
        ...
        Buscar guia: 
          <input type="text" value="" style="width:115px" onkeypress="if(event.keyCode==13){buscarlaguia(this.value)}" /></div>
    </div>
    	<div class="canvas">
     <div class="opt-menu"> 
     	<?
	...
	?>
     ...
     <div id="accordion">
			  ...
			  ?>
						...
						?>
                                  <li style="margin:0px;">
                                  <a href="#posicionPagina" onclick="mostrarPagina('<?=$fx->vinculo?>','<?=$fx->script?>', '<?=$f->grupo?> / <?=str_replace("()","",utf8_encode($fx->nombre))?>')" 
     ...
     <div class="content">
        ...
        <div class="breadcrumbs" id="div_ubicacion">Est&aacute;s en:</div>
        <ul class="tools">
