<?	
	...
	if($_GET['accion']==6){
		...
		WHERE kg.tipo = 'CONVENIO' AND gn.folio = $_GET[idconvenio] AND gn.idcliente=$_GET[cliente]	GROUP BY kg.zona";
		$r = mysql_query($s,$link) or die($s);				
		$tamanotabla = mysql_num_rows($r);
?>
        <table border="0" cellspacing="0" cellpadding="0" width="<?=$tamanotabla?>" style="background:#282828;">
	        <tr>
    	        <td height="16" width="45px" class="formato_columnas">&nbsp;</td>
                 <?
					$s = "SELECT kg.*, kg.kmi AS zoi, kg.kmf  AS zof FROM generacionconvenio gn
					INNER JOIN cconvenio_configurador_preciokg kg ON gn.folio = kg.idconvenio
					WHERE kg.tipo = 'CONVENIO' AND gn.folio = $_GET[idconvenio] AND gn.idcliente=$_GET[cliente] GROUP BY kg.zona";
					$r = mysql_query($s,$link) or die($s);
					$zona = 1;
				...
				?>
                 <td height="16" class="formato_columnas" align="center" style="width:45px;" >Zona
	                 <?=$zona?><br>
                     <?=$f->zoi?>-<?=$f->zof?></td>
                     <?
						$zona++;
            ...
            <tr>
            	<td  class="etiqueta" height="16" style="width:45px;" >Precio KG</td>
                <?				  	
					$s = "SELECT kg.zona, kg.kmi, kg.kmf, kg.valor FROM generacionconvenio gn
					INNER JOIN cconvenio_configurador_preciokg kg ON gn.folio = kg.idconvenio
					...
					while($f = mysql_fetch_object($r)){
				?>
                <td height="16" ><input type="text" readonly class="estilo_cajadesseleccion" style=" width:45px; text-align:right" 
				name="caja<?=$zona?>" value="<?=$f->valor?>" onDblClick="seleccionar('caja<?=$zona?>')"></td>
                <?	
					$zona++;
					}
				?>
	...
	<?
        ...
        <input name="descuento" type="text" class="Tablas" style="background:#FF9; width:50px" value="<?=$descuento ?>" readonly="true" />
			  <table border="0" cellspacing="0" cellpadding="0" style="background:#282828;" id="tablaconveniopreciocaja">
                <tr>
                  <td height="16" style="width:45px;" class="formato_columnas"></td>
                  <td height="16" style="width:90px;" class="formato_columnas">Descripcion</td>
        <?		 
			...
			while($f = mysql_fetch_object($r)){
		?>
		           <td height="16" style="width:45px;" class="formato_columnas" align="center" >Zona
                    <?=$zona?><br>
                    <?=$f->zoi?> - <?=$f->zof?></td>
                <?
					$zona++;
				...
				?>
					<td height="16" style="width:45px;" class="formato_columnas" align="right">Peso Limite</td>
					<td height="16" style="width:45px;" class="formato_columnas" align="right">Precio Excedente</td>
                </tr>
                <?
					$s = "SELECT c.descripcion, c.zona, c.kmi, c.kmf, c.precio, c.pesolimite, c.preciokgexcedente  
                ...
                <tr>
                    ...
                    <td><input type='text' readonly class='estilo_cajadesseleccion'
					  style='text-align:right; width:90px;' name='descripcion[]' id='descripcion' value='<?=$f[descripcion]?>'  
                ...
                <? 
					...
					?>
                    <td>
                      <input type='text' readonly class='estilo_cajadesseleccion' style='text-align:right; width:45px;' 
					  name='zona<?=$fx->zona?>[]' id='zona<?=$fx->zona?>' value='<?=$fx->precio?>' 
					  onDblClick='seleccionar(obtenerIndice(this,"zona<?=$fx->zona?>"),"zona<?=$fx->zona?>")'>
                    </td>
                    <? } ?>
                    <td><input type='text' readonly class='estilo_cajadesseleccion'
					  style='text-align:right; width:45px;' name='pesolimite[]' id='pesolimite' value='<?=$f["pesolimite"]?>' 
					  onDblClick='seleccionar(obtenerIndice(this,"pesolimite"),"zona1")'></td>
                    <td><input type='text' readonly class='estilo_cajadesseleccion' 
					  style='text-align:right; width:45px;' name='precioexcedente[]' id='precioexcedente' 
					  value='<?=$f["preciokgexcedente"]?>' onDblClick='seleccionar(obtenerIndice(this,"precioexcedente"),"zona1")'></td>
                </tr>
                <? } ?>
              </table>
		<?
		...
		if($_GET[valor]==3){
			...
			$tamanotabla = 55*mysql_num_rows($r);
		?>
              <table border="0" cellspacing="0" cellpadding="0" width="<?=$tamanotabla+55?>" style="background:#282828;">
                <tr>
                  <td height="16" width="55px"  class="formato_columnas">&nbsp;</td>
                  <?
				  $s = "SELECT kg.*, kg.kmi AS zoi, kg.kmf  AS zof
						...
						while($f = mysql_fetch_object($r)){
				  ?>
                  <td height="16" class="formato_columnas" width="45px" align="center" >Zona
                    <?=$zona?>
                    <br>
                    <?=$f->zoi?>
                    -
                ...
                <tr>
                  <td  class="formato_columnas" height="16" >Precio KG</td>
                  <?
						$s = "SELECT kg.zona, kg.kmi, kg.kmf, kg.valor FROM generacionconvenio gn
						INNER JOIN cconvenio_configurador_preciokg kg ON gn.folio = kg.idconvenio
                  ...
                  style="width:45px; text-align:right" name="caja<?=$zona?>" value="<?=$f->valor?>"
		...
		?>
            ...
            <input name="descuento2" type="text" class="Tablas" style="background:#FF9; width:50px" value="<?=$descuento2 ?>" readonly="true" />
              <table border="0" cellspacing="0" cellpadding="0" id="tablaconveniopreciocaja" style="background:#282828;">
                <tr>
                  <td height="16" width="45px"  class="formato_columnas"></td>
                  <td height="16" width="90px"  class="formato_columnas">Descripcion</td>
                  <?
				  $s = "SELECT c.*, c.kmi AS zoi, c.kmf AS zof FROM generacionconvenio gn
						INNER JOIN cconvenio_configurador_caja c ON gn.folio = c.idconvenio
						WHERE c.tipo = 'CONSIGNACION' AND gn.folio = $_GET[idconvenio]
				 ...
				 ?>
				  <td height="16" width="45px"  class="formato_columnas" align="right">Peso Limite</td>
				  <td height="16" width="45px"  class="formato_columnas" align="right">Precio Excedente</td>
                </tr>
                <?
					$s = "SELECT c.descripcion, c.zona, c.kmi, c.kmf, c.precio, c.pesolimite, c.preciokgexcedente  
					FROM generacionconvenio gn
                ...
                <tr>
                	<td>&nbsp;</td>
                    <td><input type='text' readonly class='estilo_cajadesseleccion'
					  style='width:90px; text-align:right' name='descripcion[]' id='descripcion' value='<?=$f[descripcion]?>'  
					  onDblClick='seleccionar(obtenerIndice(this,"descripcion"),"zona1")'></td>
                <? 
				...
				?>
                    <td>
					  ...
					  style='width:45px; text-align:right' name='zona<?=$fx->zona?>[]' id='zona<?=$fx->zona?>' value='<?=$fx->precio?>' 
					  onDblClick='seleccionar(obtenerIndice(this,"zona<?=$fx->zona?>"),"zona<?=$fx->zona?>")'>
                    </td>
                <? }?>
                    <td><input type='text' readonly class='estilo_cajadesseleccion'
					  style='width:45px; text-align:right' name='pesolimite[]' id='pesolimite' value='<?=$f["pesolimite"]?>' 
					  onDblClick='seleccionar(obtenerIndice(this,"pesolimite"),"zona1")'></td>
                    <td><input type='text' readonly class='estilo_cajadesseleccion' style='width:45px; text-align:right' 
						name='precioexcedente[]' id='precioexcedente' value='<?=$f["preciokgexcedente"]?>' 
