<?
	...
	$totalcargos = 0;
		...
		$s = "INSERT INTO movimientos
		(tipom,orden,guia,descripcion,cantidad,cuenta,prefijo)
		SELECT DISTINCT 1, 'CA',gv.id,concat('TRASPASO CARGO ',gv.id), gv.total, 
		CONVERT(CONCAT_WS('',$inicuetot,cs.idsucursal,'000100') USING utf8) cuenta, cs.prefijo
		FROM guiasventanilla gv
		INNER JOIN historialdetraspaso tc ON gv.id = tc.guia
		INNER JOIN catalogosucursal_tmp_".$var_itabla." cs ON tc.sucursalsolicita = cs.id
		WHERE tc.sucursalsolicita = $idsucursalorigen 
		AND DATE(tc.fechaaceptado) BETWEEN $fechainicio AND $fechafinal
		GROUP BY gv.id
		UNION
		SELECT DISTINCT 0, 'CA',gv.id,concat('TRASPASO CARGO ',gv.id), gv.total, 
		CONVERT(CONCAT_WS('',$inicuetot,cs.idsucursal,'000100') USING utf8) cuenta, cs.prefijo
		FROM guiasventanilla gv
		INNER JOIN historialdetraspaso tc ON gv.id = tc.guia
		INNER JOIN catalogosucursal_tmp_".$var_itabla." cs ON tc.sucursalacepta = cs.id
		WHERE tc.sucursalsolicita = $idsucursalorigen 
		AND DATE(tc.fechaaceptado) BETWEEN $fechainicio AND $fechafinal
		GROUP BY gv.id";
		mysql_query($s,$l) or die(mysql_error($l)."-".$s);
