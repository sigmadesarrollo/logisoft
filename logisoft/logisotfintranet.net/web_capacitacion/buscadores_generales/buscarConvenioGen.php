<? session_start();
	...
	if($_GET[accion]==1){
		...
		$l=Conectarse('webpmm');
		if($_GET[cestado]!="")
			$and = " and estadoconvenio = '$_GET[cestado]'";
		...
		if($_GET[campo]=="folio"){
			...
			$where = " where 1 = 1 ".(($_SESSION[IDSUCURSAL]!=1)? " AND sucursal = $_SESSION[IDSUCURSAL]" :"")."";
		
		$s = "SELECT folio, estadoconvenio as estado, CONCAT_WS(' ', nombre, apaterno, amaterno) AS nombrec, 
		DATE_FORMAT(fecha, '%d/%m/%Y') AS fecha, UCASE(SUBSTRING(tipoautorizacion,17)) AS tipoautorizacion FROM generacionconvenio
		$where $and ";		
...
<script>
	...
	function pedirFacturas(tipo,valor2){
		switch(tipo){
			case 1:
				consulta("mostrarFactura", "buscarConvenioGen.php?accion=1&campo=folio&valor="+valor2+"&vran="+Math.random());
				break;
			case 2:
				consulta("mostrarFactura", "buscarConvenioGen.php?accion=1&campo=nombre&valor="+valor2+"&vran="+Math.random());
				break;
		}
	}
...
<table width="550"  border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
	<tr>
      <td width="12%" class="FondoTabla">Folio</td>
      <td colspan="2" class="FondoTabla">Cliente</td>
<td colspan="2" class="FondoTabla">&nbsp;</td>
</tr>
    <tr>
      <td width="12%" class="FondoTabla"><input class="Tablas" name="buscarfactura" type="text" onKeyPress="if(event.keyCode==13){pedirFacturas(1,this.value)}" style="border:none; text-transform:uppercase; width:50px" /></td>
      ...
      <td width="17%" class="FondoTabla">Estado</td>
      <td width="10%" class="FondoTabla">Fecha</td>
      <td width="16%" class="FondoTabla">Autorización</td>
    </tr>
<tr>
