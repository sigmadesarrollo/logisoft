<?
	...
	if($_GET[accion] == 3){
		...
		$s = "update guiasventanilla set entradasalida='$DIO' where id in($losfolios)";
		mysql_query($s,$l) or die("$s<br>error en linea ".__LINE__);
			$xml = "<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
			<datos>
			<guardado>1</guardado>
		...
		$s = "update guiasempresariales set entradasalida='$DIO' where id in($losfolios)";
		mysql_query($s,$l) or die("$s<br>error en linea ".__LINE__);
			$xml = "<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
			<datos>
			<guardado>1</guardado>
		...
		SELECT NULL, e.guia,'$_SESSION[IDSUCURSAL]','',
