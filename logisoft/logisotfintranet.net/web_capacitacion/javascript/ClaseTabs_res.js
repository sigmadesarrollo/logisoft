...
function ClaseTabs(){
		...
		function iniciar(atributos){
			...
			this.obj+".seleccionar(0)'>Principal</td></tr></table></div></div>";
			
			//insertar delda movimiento
			var celda 	= fila.insertCell(fila.cells.length);
			celda.style.width="18px";
			celda.innerHTML = "<img src='"+this.rutaImagen+"/mini_derecha.gif' style='width:19px; height:18px; cursor:pointer' "+
			" onmouseover = '"+this.obj+".mover(7)' onmouseout='"+this.obj+".detener()' />";
		...
		function agregarIframe(indice,pagina){
			...
			capa.setAttribute('id',this.atributos.nombre+"_iframe_id"+indice); 
			document.body.appendChild(capa);
			capa = document.getElementById(this.atributos.nombre+"_iframe_id"+indice); 
			with(capa.style){
				position = "absolute";
				top = "0px";
				left = "0px";
			...
			capa.innerHTML="<table bordercolor='#016193' cellpadding='0' cellspacing='0' border='1'><tr><td>"+
			"<iframe frameborder=0 src='"+pagina+"' id=iframe_id"+indice+" style='width:"+
			(this.atributos.largo-2)+"px; height:"+(this.atributos.alto-2)+"px'></iframe></td></tr></table>"
		}
		...
		function agregarTabs(titulo, indice, pagina){
			...
			if(indice<this.tabulador){
				for (var i=indice; i<this.tabulador;i++){
					...
					document.body.removeChild(document.getElementById(objeto.nombre+"_iframe_id"+i));
				}
		...
		function seleccionar(tabs){
			...
			for(var i=0; i<this.tabulador; i++){
				if(tabs==i){
					...
					document.getElementById(objeto.nombre+"_iframe_id"+i).style.display="";
				}else{
					document.getElementById(objeto.nombre+"_contenedor_id"+i).className="tab_deseleccionado2";
					document.getElementById(objeto.nombre+"_iframe_id"+i).style.display="none";
				}
			}
		}
