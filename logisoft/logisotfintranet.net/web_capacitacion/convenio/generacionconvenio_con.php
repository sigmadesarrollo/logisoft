<?	session_start();
	...
	if($_GET[accion]==3){
		...
		if($_GET[tipoc]!="PRO"){
			...
			(id,personamoral,tipocliente,nombre,paterno,materno,rfc,email,celular,web,comision,usuario,fecha)
			SELECT null,personamoral,1,nombre,paterno,materno,rfc,email,celular,web,$f->comisiongeneral,
			...
			SELECT '',rfc,id,nombre,paterno,materno,'','0','0'
		...
		$s = "SELECT sucursal FROM propuestaconvenio WHERE folio=$_GET[idpropuesta]";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
		$s = "UPDATE lasalertas SET copeim=copeim+1 WHERE sucursal=$f->sucursal;";
		mysql_query($s,$l) OR die(mysql_error($l).$s);
	...
	if($_GET[accion]==4){
		...
		$s = "UPDATE lasalertas SET copeac=copeac+1;";
		mysql_query($s,$l) OR die(mysql_error($l).$s);
		$s = "SELECT sucursal FROM generacionconvenio WHERE folio=$_GET[folio]";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
		$s = "UPDATE lasalertas SET copeim=copeim-1 WHERE sucursal=$f->sucursal;";
		mysql_query($s,$l) OR die(mysql_error($l).$s);
	...
	if($_GET[accion]==9){
		...
		$s = "UPDATE lasalertas SET copeim=copeim+1;";
		mysql_query($s,$l) OR die(mysql_error($l).$s);
	...
	if($_GET[accion]==10){
		...
		$s = "UPDATE losclientes lc INNER JOIN generacionconvenio gc ON lc.id = gc.idcliente
		SET lc.convenio = 'NO' WHERE gc.folio = '$_GET[folio]'";
		mysql_query($s,$l) or die($s);
		
		$s = "CALL proc_RegistroClientes('cancelacionConvenio',0,".$_GET[folio].",0,0)";
		mysql_query($s,$l) or die($s);
		
		$s = "UPDATE lasalertas l INNER JOIN generacionconvenio gc ON '$_GET[folio]'=gc.folio
		SET copeau=copeau-1 WHERE l.sucursal= '$_SESSION[IDSUCURSAL]' AND 
		DATEDIFF(gc.vigencia,CURDATE()) <= (SELECT diasvencimientoconvenio FROM configuradorgeneral);";
		mysql_query($s,$l) OR die(mysql_error($l).$s);
		echo "cancelado";
	}
