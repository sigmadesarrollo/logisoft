	...
	<script>
		...
		function resVerificacionInicio(res){
			...
			if(pos == 2){
				...
				if(var_pagina != paginaframe && var_pagina.indexOf(paginaframe)>-1 && var_script!="" && paginaframe!=""){
					...
					/*if(document.all.modificando.value==1){
						...
						confirmar("Al cambiar de módulo se perderán los datos no guardados \n¿Deséa continuar?","¡Atención!","mostrarLaPagina()","");
		    ...
		    if(obj.iniciodia==1 && obj.iniciocaja==1){
				...
				if(obj.iniciodia==1 && obj.iniciocaja==0 && (var_pagina=="../../Caja/iniciocaja.php" || var_pagina=="../../Caja/cierrecaja.php")){
					...
					if(document.all.modificando.value==1){
						...
						if(var_pagina.indexOf(paginaframe)<0){
 ...
 <tr>
    ...
    <?
		...
		?>
			...
			<select name="sucursal" style="width:203px;" class="styled" alcambiar="cambiarSucursal(this.value); var pg = document.all.pagina.src;  document.all.pagina.src='';  document.all.pagina.src=pg;">
		...
		<?
			...
			echo strtoupper(cambio_texto($f->descripcion));
					...
					<div class="acc-content">
						...
						<table width="100%" class="<?=($cf)?'fila1':'fila2'; $cf=!$cf;?>" border="0" cellpadding="0" cellspacing="0">
							...
							<tr>
							  ...
							  <td align="left" onclick="mostrarPagina('<?=$fx->vinculo?>','<?=$fx->script?>')"><?=str_replace("()","(<a id='con$fx->id'></a>)",$fx->nombre)?></td>
