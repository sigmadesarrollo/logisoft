...
<script>
	...
	function imprimirDocumentos(valor,folio){
		if(valor!=6 && valor!=2  && valor!=4){
			...
			?>
			window.open("../guias/imprimiretiquetaguia_elegir.php?valor="+valor+"&folio="+folio+"&tipo=2&codigo="+folioimprimir,"1","width=3 height=3 left=2500 top=1500");
		}else{
	...
	function cambiarImpresora1_solo(valor,folio){
		...
		?>
		window.open("../guias/imprimiretiquetapaquete_elegir.php?valor="+valor+"&folio="+folio+"&tipo=2&codigo="+folioimprimir,"2","width=3 height=3 left=2500 top=1500");
	}
	...
	function imprimir(){
		if(document.all.estado.innerHTML=='ENTREGADA'){
			...
			?>
			window.open("../guias/imprimiretiquetaguia.php?tipo=2&codigo="+folioimprimir,"1","width=3 height=3 left=2500 top=1500");
		}
	}
	...
	function cambiarImpresora1(){
		...
		?>
		window.open("../guias/imprimiretiquetapaquete.php?tipo=2&codigo="+folioimprimir,"2","width=3 height=3 left=2500 top=1500");
	}
	...
	function obtenerMecanciaConvenio(datos){
		...
		if(valCon.validaPrecioPorCaja(0)>0){
			if(valCon.validaDescuentoSobreFlete(0)>0 && cantidad_con_x>0){
				var totalmasdesc = cantidad_con_x*100/(100-valCon.validaDescuentoSobreFlete(0));
				u.flete.value = "$ "+numcredvar((totalmasdesc + cantidad_sin_x).toLocaleString());
				u.t_txtdescuento1.value = valCon.validaDescuentoSobreFlete(0);
				u.t_txtdescuento2.value = ((valCon.validaDescuentoSobreFlete(0)/100)*totalmasdesc).toFixed(2);
			...
			if(valCon.validaDescuentoSobreFlete(0)>0){
				var totalmasdesc = importetotal*100/(100-valCon.validaDescuentoSobreFlete(0));
	...
	function devolverDatosEvaluacion(datos){
		...
		if(encon>0){
			...
			if(datos.getElementsByTagName('iddestinatario').item(0)!=null){
				devolverDestinatario(datos.getElementsByTagName('iddestinatario').item(0).firstChild.data);
			}else{
				u.idremitente.focus();
				solicitarDatosConv();
			}
