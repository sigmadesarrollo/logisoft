<?	session_start();
	...
	if($_GET['accion']==1){		
		...
		WHERE /*gv.idsucursaldestino = $_SESSION[IDSUCURSAL] and*/ gvu.unidad = '$_GET[unidad]'
		UNION
		...
		WHERE /*ge.idsucursaldestino = $_SESSION[IDSUCURSAL] and*/ geu.unidad = '$_GET[unidad]'";
	...
	if($_GET[accion]==9){
		/*
		se kito por ke no todo se baja
		...
		WHERE rg.guia IN ($_GET[folios]) AND rg.idusuario = $_SESSION[IDUSUARIO] and rg.subido = 'S'
	...
	if($_GET[accion]==13){
		$s = "SELECT gvu.id, gvu.unidad FROM guiasventanilla AS gv		
		INNER JOIN guiaventanilla_unidades AS gvu ON gv.id = gvu.idguia		
		WHERE gvu.idguia = UCASE('".$_GET[guia]."')/* AND gvu.ubicacion = $_SESSION[IDSUCURSAL]*/
		UNION
		SELECT geu.id, geu.unidad FROM guiasempresariales AS ge		
		INNER JOIN guiasempresariales_unidades AS geu ON ge.id = geu.idguia		
		WHERE geu.idguia = UCASE('".$_GET[guia]."') /*AND geu.ubicacion = $_SESSION[IDSUCURSAL]*/";
		$r = mysql_query($s,$l) or die($s);
		if(mysql_num_rows($r)==0){
			die("no existe");
		}else{
			...
			$s = "SELECT MAX(folio) AS bitacora FROM bitacorasalida WHERE unidad ='".$unidad."'";
			$r = mysql_query($s,$l) or die($s); $f = mysql_fetch_object($r);
			$bitacora = $f->bitacora;
			
			$s = "SELECT ruta FROM bitacorasalida WHERE folio ='".$bitacora."'";
			$r = mysql_query($s,$l) or die($s); $f = mysql_fetch_object($r);
			$ruta = $f->ruta;
			
			echo "({unidad:'$unidad', bitacora:'$bitacora', ruta:'$ruta'})";
		}
		
	}else if($_GET[accion]==14){
		$s = "SELECT MAX(folio) AS bitacora FROM bitacorasalida WHERE unidad ='".$_GET[unidad]."'";
		$r = mysql_query($s,$l) or die($s); $f = mysql_fetch_object($r);
		
		$s = "SELECT b.folio AS bitacora, b.ruta, r.descripcion AS desruta FROM bitacorasalida b
		INNER JOIN catalogoruta r ON b.ruta = r.id 
		WHERE b.folio='".$f->bitacora."'";
		$r = mysql_query($s,$l) or die($s);
		if(mysql_num_rows($r)>0){
			$f = mysql_fetch_object($r);
			$f->desruta = utf8_encode($f->desruta);
		...
		$s = "INSERT INTO recepcion_tmp
			...
			INNER JOIN catalogosucursal AS cs2 ON gv.idsucursaldestino = cs2.id
			WHERE gvu.idguia = UCASE('".$_GET[guia]."') /*AND gvu.ubicacion = $_SESSION[IDSUCURSAL] */
			AND NOT EXISTS(SELECT guia FROM recepcion_tmp tmp WHERE gv.id = tmp.guia)
			UNION
			SELECT geu.id, cse.descripcion, geu.idguia, 'EMPRESARIAL', cs.descripcion, cs2.descripcion, geu.codigobarras, 
			geu.paquete, CONCAT(geu.paquete,' de ', geu.depaquetes), ge.estado, 'N', '$_SESSION[IDUSUARIO]',
			'$_SESSION[IDSUCURSAL]'
			...
			INNER JOIN catalogosucursal AS cs2 ON ge.idsucursaldestino = cs2.id
			WHERE geu.idguia = UCASE('".$_GET[guia]."') /*AND geu.ubicacion = $_SESSION[IDSUCURSAL]*/
			AND NOT EXISTS(SELECT guia FROM recepcion_tmp tmp WHERE ge.id = tmp.guia)";
			mysql_query($s,$l) or die($s."<br>".mysql_error($l));
			
			$s = "UPDATE recepcion_tmp SET subido = 'S' WHERE codigobarra = UCASE('".$_GET[paquete]."')";
			mysql_query($s,$l) or die($s);
			...
			echo "({izquierda:$izquierda, derecha:$derecha})";
