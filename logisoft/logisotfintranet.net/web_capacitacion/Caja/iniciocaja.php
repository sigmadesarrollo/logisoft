...
<script>
	...
	function validar(){
		if(u.diacerrado.value==1){
			mens.show('A','No se puede iniciar caja por que el dia ha sido cerrado','¡Atención!');
			return false;
		}
		
		if(u.cierreprincipal.value==1){
			mens.show('A','No se puede iniciar caja por que la caja principal ha sido cerrada','¡Atención!');
			return false;
		}
		
		if(u.cierrecajadefinitivo.value==1){
			mens.show('A','No se puede iniciar caja por que la caja ha sido cerrada definitivamente','¡Atención!');
			return false;
		}
		
		if(u.inicio.value == "NO"){
			mens.show('A','No puede iniciar Caja sin iniciar Día','¡Atención!','fondo');
		}/*else if(u.existe.value==1){
		 	mens.show('A','El usuario no puede tener 2 cajas iniciadas','¡Atención!','fondo');
		}*/else if(u.iniciocaja.value==0){
			mens.show('A','La caja ya ha sido iniciada','¡Atención!','fondo');
		}else if(u.fondo.value==""){
			mens.show('A','Debe Capturar Fondo de Caja','¡Atención!','fondo');
		}else if(parseFloat(u.fondo.value) < 0){
			mens.show('A','El Fondo de Caja debe ser mayor a Cero','¡Atención!','fondo');		
		}else if(parseFloat(u.fondo.value) > parseFloat(u.fondocaja.value)){
			mens.show('A','El Fondo de caja NO debe ser mayor al fondo de caja configurado','¡Atención!','fondo');				
		}else{
