<?	session_start();
	...
	if($_GET[accion] == 1){
		...
		$s = "SELECT id,descripcion FROM catalogosucursal WHERE prefijo = '".$_GET[sucursal]."'";
		$r = mysql_query($s,$l) or die(mysql_error($l).$s);
		$registros = array();
		if(mysql_num_rows($r)>0){
			while($f = mysql_fetch_object($r)){
				$f->prefijo = cambio_texto($f->descripcion);
				$registros[] = $f;
			}
			echo str_replace('null','""',json_encode($registros));
		}else{
		...
		$f = mysql_fetch_object($r);
			...
			$s = "INSERT INTO catalogorutadetalletmp
			SELECT null AS id, tipo, diasalidas, sucursal, horasllegada, tiempodescarga, tiempocarga,
			...
			cs.prefijo AS sucursal, cs.descripcion as sucursaldescripcion, cd.fecha FROM catalogorutadetalletmp cd
			INNER JOIN catalogosucursal cs ON cd.sucursal = cs.id
			WHERE idusuario=".$_SESSION[IDUSUARIO]." ORDER BY cd.tipo";
			$su = mysql_query($s,$l) or die($s);
			$deta = array();
			while($g = mysql_fetch_object($su)){
				$g->sucursaldescripcion = cambio_texto($g->sucursaldescripcion);
				$deta[] = $g;
			}
