<?	session_start();
	...
	if($_GET[accion]==1){ //asignacionguiasempresariales.php
		...
		$s = "SELECT $montoautorizado - SUM(total) disponible FROM (
			SELECT SUM($if) total
			...
			AND ISNULL(ge.factura) AND tc.idcliente='".$_GET['idcliente']."'
		UNION
			SELECT SUM(gv.total) total
			FROM guiasventanilla gv INNER JOIN pagoguias pg ON gv.id = pg.guia
			WHERE gv.condicionpago=1 AND ISNULL(gv.factura) AND gv.estado!='CANCELADO' AND pg.cliente='".$_GET['idcliente']."'
		UNION
			SELECT SUM(IFNULL(f.total,0) + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)) total
			FROM facturacion f 
			WHERE f.credito='SI' AND ISNULL(f.fechacancelacion) AND f.estadocobranza <> 'C' AND f.tipoguia='empresarial' AND f.cliente='".$_GET['idcliente']."'
		UNION
		...
		$s = "select sucursal from generacionconvenio where idcliente = '$_GET[idcliente]' and estadoconvenio = 'ACTIVADO'";
		$rss = mysql_query($s,$l) or die($s);
		$fss = mysql_fetch_object($rss);
		$sucursal = $fss->sucursal;
		
		$s = "SELECT sd.idsucursal FROM solicitudcredito sc
		INNER JOIN solicitudcreditosucursaldetalle sd ON sc.folio = sd.solicitud
		WHERE sd.idsucursal IN(0,'$sucursal') AND sc.cliente = $_GET[idcliente] AND sc.estado = 'ACTIVADO'";
		$r = mysql_query($s,$l) or die($s);
		if(mysql_num_rows($r)>0){
			$creditable = "SI";
		}else{
	...
	if($_GET[accion]==4){ //asignacionguiasempresariales.php
		...
		apemat='$_GET[apemat]', prepagada='$_GET[prepagada]', idconvenio=$_GET[idconvenio],
