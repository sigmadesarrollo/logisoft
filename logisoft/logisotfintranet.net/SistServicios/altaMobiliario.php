...
<title>Sistema Mobiliario y Equipo</title>
<?php include("librerias.php"); ?>
<script>
	jQuery(document).ready(function(){
		jQuery("#lb3,#lb4,#lb5,#lb6").css("color","#900");
		...
		if('<?php echo $_GET[Folio]; ?>'!=''){
			jQuery.post("consultas.php",{accion:'Mob',eFolio:'<?php echo $_GET[Folio]; ?>',val:Math.random()},function(datos){				
		...
		jQuery("#btnGuardar").click(function(){
			if(jQuery("#eServicios").val()==""){
				jQuery("#lb3").text("* Campo requerido"); jQuery("#eServicios").focus(); return false;
			...
			if(jQuery("#cbEntrega").val()==""){
				jQuery("#lb6").text("* Campo requerido"); jQuery("#cbEntrega").focus(); return false;
			}else{ jQuery("#lb6").text("*"); }
			
			jQuery(this).attr("disabled",true);
			parent.bloquearPantalla(true);
			jQuery.ajax({type:"POST",url:"registrar.php",
				   data:"accion=Mob&"+$("form").serialize()+"&val="+Math.random(),
		...
		jQuery("#btnAutorizar,#btnNAutorizar").click(function(){
			...
			jQuery.post("registrar.php",{accion:"AutMob",hFolio:'<?php echo $_GET[Folio]; ?>',Autoriza:aut,val:Math.random()},function(datos){
				if(datos.indexOf("ok")>-1){
					alert("Los datos se guardar\u00f3n satisfactoriamente.");
	...
	function Limpiar(Bool){
		...
		if(Bool){
			jQuery("#lb3,#lb4,#lb5,#lb6").text("*");
			jQuery("#btnGuardar,#cbEntrega,#eFecha").attr("disabled",false);
			jQuery(":input[type=text], #eServicios").attr("readOnly",false);			
			jQuery.post("consultas.php",{accion:'',tabla:'mobiliario',val:Math.random()},function(datos){
...
<form id="form1" name="form1" method="post" action="">
	<table width="100%" border="0" cellpadding="3" cellspacing="1">
      <tr>
      	<td colspan="2">
        <span class="sLink" id="s0">Inicio</span> || <span class="sLink" id="s1">Mantenimiento Vehicular</span> || <span class="sLink" id="s2">Vehiculos Foraneos</span> || <span class="sLink" id="s3">Papeleria</span>        </td>
      </tr>
      <tr>
      	<td colspan="2">
        <span style="color:#006; font-size:25px; font-weight:bold">
        	Sistema Mobiliario y Equipo        </span><br />
      ...
      <tr>
        ...
        <td><textarea name="eServicios" id="eServicios" cols="45" rows="5"></textarea>
      ...
      <tr>
        ...
        <td>
        	<select id="cbEntrega" name="cbEntrega">
				...
				?>
            </select>&nbsp;D&iacute;a(s)<label id="lb6">*</label>        </td>
      </tr>
      <tr>
        <td>&nbsp;</td>
