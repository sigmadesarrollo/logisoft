<?	session_start();
	...
	$get = mysql_query('select count(*) from catalogopresupuesto');
  ...
  <table width="500"  border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
    ...
    <tr>
      <td colspan="2" class="Tablas" height="300px" valign="top"><table width="100%" border="0" align="center" class="Tablas">
          <?	
		$get = mysql_query('select cp.folio, cs.descripcion from catalogopresupuesto cp
		INNER JOIN catalogosucursal cs ON cp.sucursal = cs.id
		ORDER BY cs.descripcion limit '.$st.','.$pp,$link);		
...
<span onclick="parent.<?=$_GET[funcion] ?>('<?= $row[0];?>');parent.VentanaModal.cerrar();" style="cursor: pointer; color:#0000FF"><?= $row[0];?></span></td>
    ...
    <tr>
      <td colspan="2" align="center"><font color="#FF0000" size="-1"><? echo paginacion($total, $pp, $st, 'buscarPresupuesto.php?funcion=$_GET[funcion]&st='); ?></font></td>
    </tr>
  </table> 
</form>