<?	require('../fpdf.php');
	...
	class PDF extends FPDF{
		...
		function ImprovedTable($header,$data){
			...
			for($ii=0;$ii<count($data);$ii++){
				...
				$s = "SELECT * FROM (
					...
					AND gv.idsucursaldestino = ".$data[$ii]." AND gv.id NOT LIKE '888%'
					GROUP BY d.guia, gd.peso)
					UNION
					(SELECT gv.id AS guia, CONCAT_WS(' ',cc.nombre,cc.paterno,cc.materno) AS destinatario,
					CONCAT(gd.cantidad,' ',gd.descripcion,'/',gd.contenido) AS descripcion,
					gv.totalpeso, IF(gv.tipoflete=0,gv.total,0) AS pagado, IF(gv.tipoflete=1,gv.total,0) AS cobrar,
					...
					GROUP BY d.guia, gd.peso)
					UNION
					(SELECT gv.id AS guia, CONCAT_WS(' ',cc.nombre,cc.apellidopaterno,cc.apellidomaterno) AS destinatario,
					CONCAT(gd.cantidad,' ',gd.descripcion,'/',gd.contenido) AS descripcion,
					gv.totalpeso, IF(gv.tipoflete=0,gv.total,0) AS pagado, IF(gv.tipoflete=1,gv.total,0) AS cobrar,
					IF(gv.ocurre=0,'EAD','OCU') AS emb, gv.idsucursaldestino,
					IF(fe.guia IS NULL,'COM','FAL') AS incidencia ,gd.cantidad
					...
					INNER JOIN guiasventanilla gv ON d.guia = gv.id
					INNER JOIN guiaventanilla_detalle gd ON gv.id = gd.idguia
					INNER JOIN catalogoempleado cc ON gv.iddestinatario = cc.id
					LEFT JOIN embarquedemercancia_faltante fe ON gv.id = fe.guia AND d.sucursal = fe.sucursal
					WHERE d.idembarque = ".$_GET[folio]." AND d.sucursal = ".$_GET[sucursal]." 
					AND gv.idsucursaldestino = ".$data[$ii]." AND gv.id LIKE '888%'
					GROUP BY d.guia, gd.peso)
					UNION
					(SELECT gv.id AS guia, CONCAT_WS(' ',cc.nombre,cc.apellidopaterno,cc.apellidomaterno) AS destinatario,
					CONCAT(gd.cantidad,' ',gd.descripcion,'/',gd.contenido) AS descripcion,
		...
		function ImprimirHojaNoEmbarcadas($header,$destino){
			require_once('../../Conectar.php');
			$l = Conectarse('webpmm');
			...
			$s = "DROP TABLE IF EXISTS tmp_destinosEmbarque$valorNo";
			mysql_query($s,$l) or die($s);
			
			$s = "CREATE TABLE tmp_destinosEmbarque$valorNo (
				`iddestino` VARCHAR(25) COLLATE utf8_unicode_ci DEFAULT NULL,
				 KEY `iddestino` (`iddestino`)
			);";
			mysql_query($s,$l) or die($s);
			
			$s = "insert into tmp_destinosEmbarque$valorNo
			values $destinos";
			mysql_query($s,$l) or die($s);
			...
			$w = array(26,55,43,12,20,20,16);
				...
				$s = "SELECT * FROM (
					...
					CONCAT(gd.cantidad,' ',gd.descripcion,'/',gd.contenido) AS descripcion,
					gv.totalpeso, IF(gv.tipoflete=0,gv.total,0) AS pagado, IF(gv.tipoflete=1,gv.total,0) AS cobrar,
					DATE_FORMAT(gv.fecha, '%d/%m/%Y') AS emb, gv.idsucursaldestino,
					'' AS incidencia ,gd.cantidad
					FROM guiasventanilla gv
					INNER JOIN guiaventanilla_detalle gd ON gv.id = gd.idguia
					INNER JOIN guiaventanilla_unidades gu on gv.id = gu.idguia
					INNER JOIN catalogocliente cc ON gv.iddestinatario = cc.id
					INNER JOIN tmp_destinosEmbarque$valorNo te ON gv.idsucursaldestino = te.iddestino
					WHERE gv.id NOT LIKE '888%' AND (gv.estado = 'ALMACEN ORIGEN' OR gv.estado = 'ALMACEN TRASBORDO') 
					AND gu.ubicacion = $_GET[sucursal] AND gv.fecha >= ADDDATE(CURRENT_DATE, INTERVAL -5 DAY)
					GROUP BY gv.id)
					UNION
					(SELECT gv.id AS guia, CONCAT_WS(' ',cc.nombre,cc.paterno,cc.materno) AS destinatario,
					CONCAT(gd.cantidad,' ',gd.descripcion,'/',gd.contenido) AS descripcion,
					...
					INNER JOIN catalogoempleado cc ON gv.iddestinatario = cc.id
					INNER JOIN tmp_destinosEmbarque$valorNo te ON gv.idsucursaldestino = te.iddestino
					WHERE gv.id LIKE '888%' AND (gv.estado = 'ALMACEN ORIGEN' OR gv.estado = 'ALMACEN TRASBORDO') 
					AND gu.ubicacion = $_GET[sucursal] AND gv.fecha >= ADDDATE(CURRENT_DATE, INTERVAL -5 DAY)
					GROUP BY gv.id)
					UNION
					(SELECT gv.id AS guia, CONCAT_WS(' ',cc.nombre,cc.apellidopaterno,cc.apellidomaterno) AS destinatario,
				...
				if(mysql_num_rows($r)>0){
					...
					foreach($arr as $row){
						...
						if($cont>=27){
							...
							$this->Cell(array_sum($w),0,'','0');
							$this->addDestino("FALTANTES DE EMBARCAR",52);
							$this->addLeyenda("Los operadores abajo firmantes manifestamos haber recibido la mercancia amparada con las guias embarque registradas en la presente relacion,",235);
							$this->addLeyenda("por lo que nos obligamos hacer entrega de la misma en su destino, en las condiciones y cantidades que nos fueron entregadas.",240);
	...
	$header = array('GUIA','DESTINATARIO','DESC./CONT.','PESO','PAGADO','COBRAR','FECHA');
	
	$pdf->ImprimirHojaNoEmbarcadas($header,$_GET[destino]);
