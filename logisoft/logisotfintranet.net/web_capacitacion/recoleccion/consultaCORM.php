...
/*	$s = "SELECT b.fechabitacora, b.conductor1, b.conductor2, b.conductor3,
	...
	CONCAT(e2.nombre,' ',e2.apellidopaterno,' ',e2.apellidomaterno) AS nombre3,b.ruta, min(cd.sucursal) as origen,max(cd.sucursal) as destino,
    (select s.descripcion from catalogosucursal s where s.id = min(cd.sucursal)) as nomorigen,  
    (select s.descripcion from catalogosucursal s where s.id = max(cd.sucursal)) as nomDestino 
