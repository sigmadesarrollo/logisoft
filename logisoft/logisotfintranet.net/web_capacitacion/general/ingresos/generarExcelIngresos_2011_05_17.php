...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="Hoja1">';
	...
	$s="SELECT nombresucursal,CONCAT('#',guia) guia,fecha,contado,cobranza,entregadas,(contado+cobranza+entregadas) total,0 AS depositado,
	(contado+cobranza+entregadas) saldo  FROM ( /* cobranza */
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.guia,fp.fecha,0 AS contado,fp.total AS cobranza,0 AS entregadas 
		FROM formapago fp
		INNER JOIN liquidacioncobranzadetalle lcd ON fp.guia=lcd.factura
		INNER JOIN liquidacioncobranza lc ON lcd.folioliquidacion=lc.folio
		INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id		
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND fp.procedencia='C' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.guia
	UNION 
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,gv.id AS guia,gv.fecha,0 AS contado,
		gv.total AS cobranza,0 AS entregadas
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturadetalle fd ON a.factura=fd.factura
		...
		AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY gv.id
	UNION 
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,ge.id AS guia,ge.fecha,0 AS contado,
		ge.total AS cobranza,0 AS entregadas
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturadetalle fd ON a.factura=fd.factura
		...
		AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY ge.id
	UNION 
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,sge.id AS guia,sge.fecha,0 AS contado,
		sge.total AS cobranza,0 AS entregadas
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturadetalle fd ON a.factura=fd.factura
		...
		AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY sge.id
	UNION 
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,f.folio AS guia,f.fecha,0 AS contado,
		f.otrosmontofacturar AS cobranza,0 AS entregadas
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturacion f ON a.factura=f.folio
	...
	UNION 
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,f.folio AS guia,f.fecha,0 AS contado,
		f.sobmontoafacturar AS cobranza,0 AS entregadas
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturacion f ON a.factura=f.folio
		...
		AND f.sobmontoafacturar>0 AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.id
	UNION	/* contado */
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,gv.id AS guia,gv.fecha,gv.total AS contado,
		0 AS cobranza,0 AS entregadas 
		FROM formapago fp
		INNER JOIN guiasventanilla gv ON fp.guia=gv.id
		INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND (fp.procedencia='G' AND fp.tipo='V') AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY gv.id
	UNION
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,f.folio AS guia,f.fecha,fp.total AS contado,
		0 AS cobranza,0 AS entregadas 
		FROM formapago fp 
		INNER JOIN facturacion f ON fp.guia=f.folio
		INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND fp.procedencia='F' AND f.facturaestado='GUARDADO' AND f.credito='NO' AND cs.id='".$_GET[sucursal]."' GROUP BY f.folio
	UNION	/* entregadas */
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.guia,fp.fecha,0 AS contado,
		0 AS cobranza,fp.total AS entregadas 
		FROM formapago fp INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id		
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' AND '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND fp.procedencia='M' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.guia
	UNION
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,gv.id AS guia,gv.fecha,0 AS contado,
		0 AS cobranza,fp.total AS entregadas
		FROM entregasocurre eo 
		INNER JOIN entregasocurre_detalle eod ON eo.folio=eod.entregaocurre  AND eo.idsucursal = eod.sucursal
		INNER JOIN guiasventanilla gv ON eod.guia=gv.id
		INNER JOIN catalogosucursal cs ON eo.idsucursal=cs.id
		INNER JOIN formapago fp ON eo.folio=fp.guia AND eo.idsucursal = fp.sucursal
		WHERE eo.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' AND '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND fp.procedencia='O' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' 
		AND gv.tipoflete=1 AND gv.condicionpago=0 AND (gv.estado='POR ENTREGAR' OR gv.estado='ENTREGADA') GROUP BY fp.guia	
	)t GROUP BY guia ORDER BY fecha";
		$r = mysql_query($s,$l) or die($s);
