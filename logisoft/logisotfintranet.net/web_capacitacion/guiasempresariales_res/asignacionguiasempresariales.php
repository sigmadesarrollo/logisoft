...
<script>
	...
	var botonesnuevo = '<table width="174" align="center"><tr>'
			...
			+'<td><div class="ebtn_nuevo" id="btnnuevo" onclick="confirmar(\'¿Desea limpiar los datos?\',\'¡Atencion!\',\'limpiar()\');"></div></td>'
	...
	function mostrarCliente(datos){
		...
		if(objeto[0].datoscliente[0].id!=undefined){
			...
			if(objeto[0].datoscliente[0].desactivado=="SI"){
				alerta3("El crédito del Cliente esta desactivado","¡Atención!");
				u.creditodesactivado.value = "SI";
				u.lstpago.value = "0";
			}
			
			if(objeto[0].datoscliente[0].tienecredito=="SI"){
				u.lstpago.value = 1;
			}
	...
	function guardarSolicitud(){		
		if(u.guardada.value==""){				
			...
			if(u.creditodesactivado.value == "SI" && u.lstpago.value=="1"){
				u.lstpago.value = "0";
				alerta("El crédito del cliente #"+u.nocliente.value+" esta Bloqueado y no se le puede vender a Crédito","¡Atención!","lstpago");				
				return false;
			}else if(u.lstpago.value=="1" && parseFloat(u.creditodisponible.value) < parseFloat(u.total.value.replace("$ ","").replace(/,/g,""))){
				alerta("El cliente #"+u.nocliente.value+" no cuenta con suficiente crédito para realizar la compra","¡Atención!","lstpago");				
				return false;
			}
			...
			consultaTexto("resGuardar","solicitudguiasempresariales_con.php?accion=4&idcliente="+idcliente+"&condicionpago="+condicionpago+"&nombre="+nombre+"&apepat="+apepat+"&apemat="+apemat+"&idconvenio="+idconvenio+"&prepagada="+prepagada+"&cantidad="+cantidad+"&desdefolio="+desdefolio+"&hastafolio="+hastafolio+"&subtotal="+subtotal+"&combustible="+combustible+"&iva="+iva+"&ivar="+ivar+"&total="+total+'&folio='+u.idsolicitud.value);						  
        ...
        <td colspan="5"><table width="542" border="0">
          ...
          <tr>
            ...
            <table width="206" border="0" cellpadding="0" cellspacing="0">
            	<tr>
                	<td width="48">Folio</td>
                    <td width="110"><input name="folio" type="text" class="Tablas" id="folio" style="width:80px;background:#FFFF99" value="<?=$f->folio ?>" /></td>
                    <td width="48"><div class="ebtn_buscar" onclick="abrirVentanaFija('../buscadores_generales/buscarSolicitudGuiasGen.php?funcion=pedirSolicitud', 625, 550, 'ventana', 'Buscar Solicitud')"></div></td>
                </tr>
            	<tr>
            	  <td>Fecha</td>
            	...
            	<tr>
            	  <td>Factura</td>
            	  <td>
            	    <input name="factura" type="text" class="Tablas" id="fecha2" style="width:100px;background:#FFFF99; text-align:right" value="" readonly=""/>
            	  </td>
            	  <td>&nbsp;</td>
          	  </tr>
            </table>
        ...
        <td colspan="5" align="center" id="celdabotones">
        	<table width="283" align="center">
       	    <tr>
                   ...
                   <? } ?>
                    <td>
                      <div class="ebtn_nuevo" id="btnnuevo" onclick="confirmar('&iquest;Desea limpiar los datos?','&iexcl;Atencion!','limpiar()');"></div>
                    </td>
                </tr>
            </table>        </td>
      </tr>
