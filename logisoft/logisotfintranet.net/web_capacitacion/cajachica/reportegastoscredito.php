...
<?
	...
	function guardardatos($arrayids)
	{
		...
		$s = "SELECT IF(ISNULL(MAX(folio) + 1), 1, MAX(folio) + 1) FROM foliosgastoscajachica WHERE keytipopagoindex = '3'
			  AND keysucursal = '".$_POST['idsucursal']."'";//1009
		$sq = mysql_query($s) or die($s);
		mysql_num_rows($sq) > 0 ? $foliomayor = mysql_result($sq, 0) : $foliomayor = 0;
		
		$folioaux = $foliomayor - 1;
		$s = "SELECT generada FROM foliosgastoscajachica WHERE keytipopagoindex = '3' AND folio = '".$folioaux."'";
		$sq = mysql_query($s) or die($s);
		...
		if($generada == 'N') 
		{
			$foliomayor--;
			$sIni = 'UPDATE ';
			$sFin = ' WHERE keytipopagoindex = "3" AND folio = "'.$foliomayor.'"';
			$autorizado = "N";
		}
		else
		{
	...
	if($_POST['generar_datos'])
	{
		...
		//Pago Proveedor = 3
		$s = "SELECT IF(ISNULL(MAX(folio) + 1), 1, MAX(folio) + 1) FROM foliosgastoscajachica WHERE keytipopagoindex = '3' AND keysucursal = '".$idsucursalactual."'";//1009-2
		$sq = mysql_query($s) or die($s);
		...
		$s = "SELECT generada FROM foliosgastoscajachica WHERE keytipopagoindex = '3' AND folio = '".$folioaux."'";
		$sq = mysql_query($s) or die($s);
		mysql_num_rows($sq) > 0 ? $generada = mysql_result($sq, 0) : $generada = '';//1009-2
		
		if($generada == 'N') 
		{
			$folio--;
			$sIni = 'UPDATE ';
			$sFin = ' WHERE keytipopagoindex = "3" AND folio = "'.$folio.'"';
			$autorizado = "N";
		}
		else
		{
	...
	{
		$s = "SELECT IF(ISNULL(MAX(folio) + 1), 1, MAX(folio) + 1) FROM foliosgastoscajachica WHERE keytipopagoindex = '3' AND keysucursal = '".$idsucursalactual."'";
		$sq = mysql_query($s) or die($s);
		$folio = mysql_result($sq, 0);
...
<table width="614" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
  <tr>
    <td width="619" class="FondoTabla Estilo4">REPORTE GASTOS CR&Eacute;DITO</td>
  </tr>
				...
				if($idsucursalactual > 0 || $_POST['consultarfolio'] > 0)
				{
					...
					if($_POST['consultarfolio'] > 0 && $_POST['foliogenerado'] != 'N')
					{
						$s = "SELECT cap.tipogastodesc, cap.prefijosucursal, cap.id, 
							  ...
							  LEFT JOIN foliosgastoscajachica fol ON (det.keyfoliosgastoscajachica = fol.folio AND fol.keytipopagoindex = '3')
							  WHERE tipopagoindex = '3' 
					...
					else
					{
						$s = "SELECT tipogastodesc, prefijosucursal, folio as id, DATE_FORMAT(fecha,'%d-%m-%Y') as fecha, factura, 
						  ...
						  FROM capturagastoscajachica WHERE tipopagoindex = '3' ";
                    ...
                    <tr class="<? if ($line % 2 ==0){ echo 'Balance2' ;}else{ echo 'Balance' ;} ?>"  <? //if ($line==0){ echo "style='visibility:hidden;display:none'" ;} ?>  >
					  ...
					  <td width="32" align="center" class="style31"  ><input name="total<?=$row->id ?>" type="text" class="style2" id="total<?=$row->id ?>" readonly="" style="font-size:8px; font:tahoma; font-weight:bold" size="10" value="<?=$row->total ?>" />
                        <?
							if($row->autorizado == 'S')
							{
        ...
        <tr>
          <td><table width="610" height="21" border="0" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td width="87"><div class="ebtn_guardar" onClick="<? if($_POST['consultarfolio'] > 0 && $_POST['foliogenerado'] != 'N') echo "mensajeNoGuardar()"; else echo "enviar()"; ?>"></div></td>
              <td width="121"><div class="ebtn_Cerrarreporte" onClick="<? if($_POST['consultarfolio'] > 0 && $_POST['foliogenerado'] != 'N') echo "mensajeNoGenerar()"; else echo "generar()"; ?>"></div></td>
			  <td width="70"><div class="ebtn_imprimir" onClick="imprimir()"></div></td>
            </tr>
          </table>
          </td>
        </tr>
...
<script language="javascript">
	...
	{
		abrirVentanaFija('buscarFolioGastos.php?tipopago=3&idsucursal='+document.form1.idsucursal.value+'', 550, 450, 'ventana', 'Busqueda');
	}
	...
	function imprimir(){		
		...
		+"&titulo=GASTOS CREDITO&sucursal="+u.idsucursal.value);
