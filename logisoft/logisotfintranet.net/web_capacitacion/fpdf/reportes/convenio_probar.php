<?	
	...
	cs.estado eest, cs.municipio emun,
	...
	if($f->precioporcaja==1){
		...
		WHERE tipo='CONVENIO' AND idconvenio = '$_GET[folio]' GROUP BY zona";
		$rx = mysql_query($s,$l) or die($s);
		...
		for($i=0;$i<count($limites);$i++){
			$pdf->MultiCell(100,3,$limites[$i],0,'C',0);
		}
	...
	if($f->consignacioncaja==1){
		...
		$s = "SELECT CONCAT(kmi,'-',kmf,' KM') zonas FROM cconvenio_configurador_caja WHERE tipo='CONSIGNACION' AND idconvenio = '$_GET[folio]' GROUP BY zona";
		$rx = mysql_query($s,$l) or die($s);
		$columnasde = floor(mysql_num_rows($rx)/2);
