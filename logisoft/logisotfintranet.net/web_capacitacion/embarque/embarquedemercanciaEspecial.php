...
<script>
	...
	/*function moverAlaDerecha(){
		if(tabla1.getRecordCount()==""){
			...
			if(tabla1.getSelectedIdRow()!=""){
				...
				consultaTexto("resMovADerC","embarquedemercanciaEspecial_con.php?accion=4&idpagina="+u.idpagina.value+
							  "&folio="+tabla1.getSelectedRow().guia+"&sid="+Math.random());
			}else if(tabla3.getSelectedIdRow()!=""){
				document.getElementById('aderecha').style.visibility='hidden';
				consultaTexto("resMovADerU","embarquedemercanciaEspecial_con.php?accion=5&idpagina="+u.idpagina.value+
	...
	function moverAlaizquierda(){
		if(tabla2.getRecordCount()==""){
			...
			if(tabla2.getSelectedIdRow()!=""){
				consultaTexto("resMovAIzqC","embarquedemercanciaEspecial_con.php?accion=6&idpagina="+u.idpagina.value+
							  "&folio="+tabla2.getSelectedRow().guia+"&sid="+Math.random());
			}else if(tabla4.getSelectedIdRow()!=""){
				consultaTexto("resMovAIzqU","embarquedemercanciaEspecial_con.php?accion=7&idpagina="+u.idpagina.value+
	...
	function moverAlaDerecha(){
		if(tabla1.getRecordCount()==""){
			...
			if(tabla1.getSelectedIdRow()!=""){
				consultaTexto("resMovADerC","embarquedemercanciaEspecial_con.php?accion=4&idpagina="+u.idpagina.value+"&folio="+tabla1.getSelectedRow().guia
				+"&sid="+Math.random());
			}else if(tabla3.getSelectedIdRow()!=""){
				consultaTexto("resMovADerU","embarquedemercanciaEspecial_con.php?accion=5&idpagina="+u.idpagina.value+"&folio="+tabla3.getSelectedRow().guia
					+"&registro="+tabla3.getSelectedRow().registro+"&random="+Math.random());
			}else{
				alerta3('Debe seleccionar la fila que desea mover a la derecha','¡Atención!');	
			}
	...
	function resMovADerC(datos){
		...
		if(datos.indexOf("guardado")>-1){
			if(!DS2.buscar(tabla1.getSelectedRow().guia,'guia'))
				DS2.agregarRegistro(tabla1.getSelectedRow());
	...
	function resMovADerU(datos){
		barra.setValue(parseFloat(datos.split(",")[1]));
		if(datos.indexOf("guardado")>-1){
			if(!DS2.buscar(tabla3.getSelectedRow().guia,'guia')){
				DS1.buscarYMostrar(tabla3.getSelectedRow().guia,'guia');
				DS2.agregarRegistro(tabla1.getSelectedRow());
			}
			DS4.agregarRegistro(tabla3.getSelectedRow());
	...
	function resMovAIzqC(datos){
		barra.setValue(parseFloat(datos.split(",")[1]));
		if(datos.indexOf("guardado")>-1){
			if(!DS1.buscar(tabla2.getSelectedRow().guia,'guia'))
				DS1.agregarRegistro(tabla2.getSelectedRow());
	...
	function resMovAIzqU(datos){
		...
		if(datos.indexOf("guardado")>-1){
			if(!DS1.buscar(tabla4.getSelectedRow().guia,'guia')){
				DS2.buscarYMostrar(tabla4.getSelectedRow().guia,'guia');
				DS1.agregarRegistro(tabla2.getSelectedRow());
			}
			DS3.agregarRegistro(tabla4.getSelectedRow());
...
/*******************/
	function ObtDetalleIzq(){
		...
		if(tabla1.getValSelFromField('guia','No_GUIA')!=""){
			consultaTexto("mostrarIzqAbajo","embarquedemercanciaEspecial_con.php?accion=2&idpagina="+u.idpagina.value+
...
/****************/
	function ObtDetalleDer(){
		...
		if(tabla2.getValSelFromField('guia','No_GUIA')!=""){
			consultaTexto("mostrarDerAbajo","embarquedemercanciaEspecial_con.php?accion=3&idpagina="+u.idpagina.value+
...
/********************/
	function obtenerUnidadBusqueda(unidad,bitacora){
		consulta("mostrarUnidadBusqueda","consultasembarqueEspecial.php?bitacora="+bitacora+"&unidad="+unidad+"&accion=1&sid="+Math.random());
	}
...
/********************/
	function CargarGuias(){
		consultaTexto("mostrarCargarGuias","embarquedemercanciaEspecial_con.php?accion=1&idpagina="+u.idpagina.value+
			"&foliobitacora="+document.all.foliobitacora.value+"&unidad="+document.all.unidad.value+"&sid="+Math.random());
	}
...
/********************/
	...
	function limpiarDatos(){
		...
		//consultaTexto("ponerIniciado","embarquedemercanciaEspecial_con.php?accion=11");
	}
	...
	function buscarFolio(){
		if(u.buscar.value==0){
			...
			if(DS2.buscarYMostrar(u.guia.value,'guia')){
				moverAlaizquierda();
				u.guia.value = "";
			}else{
	...
	function guardarValores(){
		...
		if(tabla2.getRecordCount()>0 && u.guardado.value == 0){
			...
			consultaTexto("resValidar","embarquedemercanciaEspecial_con.php?accion=8&idpagina="+u.idpagina.value+
	...
	function guardarFinal(){
		...
		/*$.ajax({
		   type: "GET",
		   url: "embarquedemercanciaEspecial_con.php",
		...
		consultaTexto("resGuardar","embarquedemercanciaEspecial_con.php?accion=9&idpagina="+u.idpagina.value+"&unidad="+unidad
	...
	function imprimirEmbarque(){
		if(u.guardado.value == 1){		
			if(document.URL.indexOf("web/")>-1){		
				window.open("https://www.pmmintranet.net/web/fpdf/reportes/relacionEmbarque.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+u.folio.value+"&unidad="+u.unidad.value+"&fechaembarque="+u.fecha.value+"&destino="+u.destino.value);
			
			}else if(document.URL.indexOf("web_capacitacion/")>-1){
			window.open("https://www.pmmintranet.net/web_capacitacion/fpdf/reportes/relacionEmbarque.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+u.folio.value+"&unidad="+u.unidad.value+"&fechaembarque="+u.fecha.value+"&destino="+u.destino.value);
			
			}else if(document.URL.indexOf("web_pruebas/")>-1){
			window.open("https://www.pmmintranet.net/web_pruebas/fpdf/reportes/relacionEmbarque.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+u.folio.value+"&unidad="+u.unidad.value+"&fechaembarque="+u.fecha.value+"&destino="+u.destino.value);
			}
			/*consultaTexto("resFaltantes","embarquedemercanciaEspecial_con.php?accion=13&folio="+
	...
	function cargarEmbarque(folio){
		consultaTexto("resCargarEmbarque","embarquedemercanciaEspecial_con.php?accion=10&idpagina="+u.idpagina.value+
...
<table width="619" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
  <tr>
    <td width="619" class="FondoTabla Estilo4">EMBARQUE DE MERCANC&Iacute;A DE CONTINGENCIAS</td>
  </tr>
  <tr>
        ...
        <table width="618" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td width="100"><span class="Tablas">
              <input name="unidad" type="text" class="Tablas" id="unidad" readonly="readonly" style="width:100px;background:#FFFF99" value="<?=$unidad ?>" onFocus="foco(this.name)" onBlur="document.getElementById('oculto').value=''" onkeydown="if(event.keyCode==8){document.all.ruta.value='';document.all.recorrido.value='';}"  onkeypress="if(event.keyCode==13){/*obtenerUnidadBusqueda(this.value)*/}" />
            </span></td>
            <td width="483">
            	<input type="hidden" name="guardado" />
              	<input name="foliobitacora" type="hidden" id="foliobitacora" />
                <div class="ebtn_buscar" onclick="abrirVentanaFija('buscarUnidadEmbEspecial.php?funcion=obtenerUnidadBusqueda&tipo=embarques', 600, 500, 'ventana', 'Busqueda');"></div>
                </td>
          </tr>
        </table></td>
      </tr>
