...
INNER JOIN configuradorservicios con ON cs.id=con.servicio WHERE cs.id=$id";
	...
	$s="SELECT IFNULL(SUM(peso),0) As peso, IFNULL(SUM(volumen),0) As volumen FROM evaluacionmercanciadetalletmp WHERE usuario='$usuario' AND fecha='$fechahora'";			
	$r = mysql_query($s,$link) or die("error en linea ".__LINE__);
		if(mysql_num_rows($r)>0){
			$f = mysql_fetch_object($r);
			$cant = mysql_num_rows($r);
	...
	if($tipo=="emplaye"){
		//$sql=@mysql_query("SELECT emplaye,costoextra FROM configuradorservicios",$link);
		//$row=@mysql_fetch_array($sql);
		//$costoemplaye=$row[0];
		//$costoemplayeextra=$row[1];
		$sqlemplaye=@mysql_query("SELECT IFNULL(SUM(peso),0) As peso, IFNULL(SUM(volumen),0) As volumen FROM evaluacionmercanciadetalletmp WHERE usuario='$usuario' AND fecha='$fechahora'",$link);
	...
	$lista=split($coma,$miArray);		
		...
		$sqltmp=@mysql_query("INSERT INTO evaluacionmercanciadetalletmp (id,cantidad, descripcion, contenido, peso, largo, ancho, alto, volumen, usuario, fecha) VALUES ('null','$arre[0]', UCASE('$arre[1]'), UCASE('$arre[2]'), '$arre[3]', '$arre[4]', '$arre[5]', '$arre[6]', '$arre[7]','$usuario','$fechahora')",$link);
...
$sqlins=@mysql_query("INSERT INTO contenidos(descripcion)VALUES('$arre[2]')",$link); 		}
	...
	$sqlupd=mysql_query("UPDATE evaluacionmercanciadetalletmp SET cantidad='$arre[0]', descripcion='$arre[1]', concepto='$arre[2]', peso='$arre[3]', largo='$arre[4]', ancho='$arre[5]', alto='$arre[6]', volumen='$arre[7]' WHERE id='$id'",$link);
	}
?>
		...
		$sql=mysql_query("SELECT * FROM evaluacionmercanciadetalletmp WHERE usuario='$usuario' AND fecha='$fechahora'",$link);
			...
			while($row=mysql_fetch_array($sql)){?>
                    <tr class="<? if ($line % 2 ==0){ echo 'Balance2' ;}else{ echo 'Balance' ;} ?>" ondblclick="ObtenerEvaluacion('<?=$row[id] ?>','<?=$usuario ?>');" >
                      ...
                      <td width="32" align="center" class="style31"  ><input name="cantidad2" type="text" class="style2" id="cantidad2" readonly="" style="font-size:8px; font:tahoma; font-weight:bold" value="<?=htmlentities($row[cantidad])?>" size="8" /></td>
                      <td width="95" align="center" class="style31"><input name="descripcion2" type="text" class="style2" id="descripcion2" style="font-size:8px; font:tahoma;font-weight:bold" value="<?=htmlentities($row[descripcion]) ?>" readonly="" size="20" /></td>
                      <td width="128" align="center" class="style31"><input name="contenido2" type="text" class="style2" id="contenido2" style="font-size:8px; font:tahoma;font-weight:bold" value="<?=htmlentities($row[contenido]) ?>" readonly="" size="20" /></td>
                      <td width="119" class="style31" align="center"><input name="peso2" type="text" class="style2" id="peso2" style="font-size:8px; font:tahoma;font-weight:bold" value="<?=htmlentities($row[peso]) ?>" readonly="" size="8" /></td>
                      <td width="43" class="style31" align="center"><input name="largo2" type="text" readonly="" class="style2" id="largo2" style="font-size:8px; font:tahoma;font-weight:bold" value="<?=htmlentities($row[largo]) ?>" size="5" /></td>
                      <td width="29" class="style31" align="center"><input name="ancho2" type="text" readonly="" class="style2" id="ancho2" style="font-size:8px; font:tahoma;font-weight:bold" value="<?=htmlentities($row[ancho]) ?>" size="5" />
                      </td>
                      <td width="22" align="center" class="style31" ><input name="alto2" type="text" class="style2" id="alto2" readonly="" style="font-size:8px; font:tahoma;font-weight:bold" value="<?=htmlentities($row[alto]) ?>" size="5" /></td>
                      <td width="40" align="center" class="style31"><input name="volumen2" type="text" class="style2" id="volumen2" readonly="" style="font-size:8px; font:tahoma;font-weight:bold" value="<?=$row[volumen] ?>" size="8" /></td>
                    </tr>
                    <?
		$line ++ ; }	
