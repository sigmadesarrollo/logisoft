...
if($_GET['accion']==1){//  embarquedemercancia.php
	...
	WHERE bitacorasalida.unidad='$_GET[unidad]' and bitacorasalida.status = 0 and bitacorasalida.cancelada = 0";
		...
		if(mysql_num_rows($r)>0){
			...
			$s = "SELECT IFNULL(SUM(encontrados),0) AS total FROM(
				(SELECT COUNT(*) encontrados FROM 
				guiaventanilla_unidades gu
				INNER JOIN guiasventanilla gv ON gu.idguia = gv.id
				WHERE gu.proceso = 'POR RECIBIR' AND gu.unidad = '$_GET[unidad]' AND 
				gv.idsucursaldestino = '$_SESSION[IDSUCURSAL]')
				UNION
				(SELECT COUNT(*) encontrados 
				FROM guiasempresariales_unidades gu 
				INNER JOIN guiasempresariales ge ON gu.idguia = ge.id
				WHERE gu.proceso = 'POR RECIBIR' AND gu.unidad = '$_GET[unidad]' AND 
				ge.idsucursaldestino = '$_SESSION[IDSUCURSAL]')
			) AS t1";
