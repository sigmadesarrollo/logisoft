<?
	...
	if(!$_SESSION[IDUSUARIO]!=""){
		postError("La sesión ha terminado favor de loguearse de nuevo");
	}
	...
	if($_GET[accion]==1){
		...
		from catalogoempleado where id = '$_GET[idempleado]' AND sucursal=$_SESSION[IDSUCURSAL] AND puesto between 4 and 57";
	...
	if($_GET[accion]==2){
		//borrar los del mismo id 
		$s = "delete from repartomercanciaead_temp 
		where idusuario='$_GET[idpagina]'
		AND sucursal = ".$_SESSION[IDSUCURSAL]."";
		mysql_query($s,$l) or postError($s);
		
		/*$s = "SELECT CONCAT(DATE_FORMAT(ADDDATE(CURRENT_DATE, INTERVAL -2 DAY), '%y%m%d'),'000000000') valor";
		$r = mysql_query($s,$l) or postError($s);
		...
		$s = "delete from repartomercanciaead_temp 
		where 
		SUBSTRING(idusuario,LOCATE('.',idusuario)+1,15)<=$valorlimite
		AND sucursal = ".$_SESSION[IDSUCURSAL]."";
		mysql_query($s,$l) or postError($s);*/
		...
		gvu.codigobarras, gvu.estado, gvu.peso, 'NO', '$_GET[idpagina]', '$_SESSION[IDSUCURSAL]'
		...
		inner join guiaventanilla_unidades as gvu on gv.id = gvu.idguia and (gv.estado='ALMACEN DESTINO' or gv.estado='PARCIALMENTE EN REPARTO EAD')
			...
			$filtro1 
			and gvu.proceso='ALMACEN DESTINO'
		UNION
		SELECT null, ge.id, cs.descripcion, ge.fecha, geu.paquete, geu.depaquetes, 
		geu.codigobarras, geu.estado, geu.peso, 'NO', '$_GET[idpagina]', '$_SESSION[IDSUCURSAL]'
		...
		inner join guiasempresariales_unidades as geu on ge.id = geu.idguia and (ge.estado='ALMACEN DESTINO' or ge.estado='PARCIALMENTE EN REPARTO EAD')
			...
			$filtro2 
			and geu.proceso='ALMACEN DESTINO'";
		mysql_query($s,$l) or postError($s);
		...
		$s = "select t2.* from (
			select t.*, ifnull(ead.cerro,1) as cerro from (
				...
				where rmeadt.idusuario = '$_GET[idpagina]' and rmeadt.sucursal = '$_SESSION[IDSUCURSAL]'
	...
	if($_GET[accion]==3){
		...
		and idusuario = '$_GET[idpagina]' and sucursal=".$_SESSION[IDSUCURSAL]."";
		$r = mysql_query($s,$l) or postError($s);
		$arre = array();
		while($f = mysql_fetch_object($r)){
	...
	if($_GET[accion]==6){
		$s = "UPDATE repartomercanciaead_temp SET enunidad = 'SI' WHERE folioguia = '$_GET[folio]' 
		and paquete = '$_GET[registro]' and idusuario = '$_GET[idpagina]' and sucursal=".$_SESSION[IDSUCURSAL]."";
		mysql_query($s,$l) or postError($s);
		echo "guardado";
	}
	...
	if($_GET[accion]==9){
		...
		from repartomercanciaead_temp
		where enunidad = 'SI' and idusuario = '$_GET[idpagina]' and sucursal=".$_SESSION[IDSUCURSAL].") as t1
		inner join 
		(select folioguia, enunidad 
		from repartomercanciaead_temp
		where enunidad = 'NO' and idusuario = '$_GET[idpagina]' and sucursal=".$_SESSION[IDSUCURSAL].") as t2 
	...
	if($_GET[accion]==10){		
		...
		if($guiasYaGuardadas!=""){
			//postError("Las siguientes guias: $guiasYaGuardadas ya fueron asignadas a un reparto");
		}
		#***********************************************************
		...
		WHERE e.idusuario = '$_GET[idpagina]' AND e.sucursal=".$_SESSION[IDSUCURSAL]."
		...
		from repartomercanciaead_temp
		where repartomercanciaead_temp.idusuario = '$_GET[idpagina]' AND repartomercanciaead_temp.enunidad = 'SI' ";
		mysql_query($s,$l) or postError($s);
		...
		WHERE repartomercanciaead_temp.idusuario = '$_GET[idpagina]'
		...
		SELECT gv.id, IF(gv.totalpaquetes = COUNT(gvu.id),'EN REPARTO EAD','EN REPARTO EAD') AS estado
		FROM guiasventanilla AS gv
		INNER JOIN guiaventanilla_unidades AS gvu ON gv.id = gvu.idguia
		INNER JOIN tmp_seleccionadas tp ON gv.id = tp.folioguia
		AND gvu.proceso = 'EN REPARTO EAD' AND gv.estado = 'ALMACEN DESTINO'
		GROUP BY gv.id";
	...
	if($_GET[accion]==11){//Buscar Folio repartoMercanciaEad.php
		$s = "delete from repartomercanciaead_temp where idusuario = '$_GET[idpagina]'";
		mysql_query($s,$l) or postError($s);
		...
		$r=mysql_query($s,$l)or postError($s."<BR>".mysql_error($l));
	...
	WHERE rmd.idreparto='$_GET[folio]' and rmd.sucursal = $_SESSION[IDSUCURSAL]";
			...
			while($f = mysql_fetch_object($r)){
					...
					$w="INSERT INTO repartomercanciaead_temp
						...
						mysql_query($w,$l)or postError($w."<BR>".mysql_error($l));	
			}
