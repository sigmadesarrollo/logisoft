   ...
   <Row>
	...
	if($_GET[accion] == 1){//STD-ProcedimientoReporteVentas_PM
	   ...
	   <Row ss:Height="13.5">
		...
		$s = "SELECT rv.prefijosucursal sucursal,CONCAT('#',sg.idcliente) idcliente,CONCAT_WS(' ',sg.nombre,sg.apepat,sg.apemat) AS cliente,
		sg.cantidad,CONCAT('#',sg.factura) factura, SUM(rv.total) importe, 0 servicios, SUM(rv.total) total
