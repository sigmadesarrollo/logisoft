...
INNER JOIN configuradorservicios con ON cs.id=con.servicio WHERE cs.id=$id";
	...
	$lista=split($coma,$miArray);		
		...
		$sqltmp=@mysql_query("INSERT INTO evaluacionmercanciadetalletmp (id,cantidad, descripcion, contenido, peso, largo, ancho, alto, volumen, pesototal, pesounit, usuario, fecha) VALUES ('null','$arre[0]', UCASE('$arre[1]'), UCASE('$arre[2]'), '$arre[3]', '$arre[4]', '$arre[5]', '$arre[6]', '$arre[7]', '$arre[8]','$arre[9]', '$usuario','$fechahora')",$link);
...
$sqlins=@mysql_query("INSERT INTO contenidos(descripcion)VALUES('$arre[2]')",$link); 		}
	...
	$sqlupd=mysql_query("UPDATE evaluacionmercanciadetalletmp SET cantidad='$arre[0]', descripcion='$arre[1]', concepto='$arre[2]', peso='$arre[3]', largo='$arre[4]', ancho='$arre[5]', alto='$arre[6]', volumen='$arre[7]',pesototal='$arre[8]', pesounit='$arre[9]' WHERE id='$id'",$link);
	}
?>
		...
		$sql=mysql_query("SELECT * FROM evaluacionmercanciadetalletmp WHERE usuario='$usuario' AND fecha='$fechahora'",$link);
			...
			while($row=mysql_fetch_array($sql)){?>
                    <tr class="<? if ($line % 2 ==0){ echo 'Balance2' ;}else{ echo 'Balance' ;} ?>" ondblclick="ObtenerEvaluacion('<?=$row[id] ?>','<?=$usuario ?>');" >
                      ...
                      <td width="119" class="style31" align="center"><input name="peso" type="text" class="style2" id="peso" style="font-size:8px; font:tahoma;font-weight:bold" value="<?=htmlentities($row[peso]) ?>" readonly="" size="8" /></td>
