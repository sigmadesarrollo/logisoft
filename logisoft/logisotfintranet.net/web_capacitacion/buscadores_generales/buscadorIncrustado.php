...
<div style="position:absolute; display:none; background-image:url(<?=$raiz?>img/fondo_nuevoBuscar.jpg); left: 15px; width: 810px; top: 131px; height: 361px; z-index:999" id="<?=$nombreBuscador?>">
    ...
    <tr>
	  ...
	  <td  class="FondoTabla">Cr&eacute;dito</td>
	  <td  class="FondoTabla">Convenio</td>
    </tr>
    <tr>
      <td colspan="7">
      <div id="txtHint" style="width:100%; height:300px; overflow: scroll;">
      <table width="762" id="tablaclientes" border="0" align="left" alagregar="" alborrar="" cellpadding="0" cellspacing="0">
      	  <tr>
...
var valt = agregar_una_tabla("tablaclientes", "fil_", 1, "fondoFila0└fondoFila1", "");
var latabla= "";
var u = document.all;
...
function ponerOnClick(valor,valordevuelto,nombre){
	return	"<label style='padding-right:4px; width:100%;'><a style='cursor:hand;' onclick='<?=$funcion?>("+valordevuelto+",\""+nombre+"\");'>"+valor+"</a></label>";
}
...
function resultado(datos){
	...
	if(con>0){
		...
		for(m=0;m<con;m++){
			...
			insertar_en_tabla(valt,ponerOnClick(nick,idcliente,nombre)+"└"+ponerOnClick(rfc,idcliente,nombre)+"└"+
			ponerOnClick(idcliente,idcliente,nombre)+"└"+ponerOnClick(nombre,idcliente,nombre)+"└"+
			ponerOnClick(sucursal,idcliente,nombre)+"└"+ponerOnClick(credito,idcliente,nombre)+"└"+
			ponerOnClick(convenio,idcliente,nombre));
		}	
	}else{
...
function <?=$nombreBuscador?>_limpiarCampos(){
	if(latabla=="")
		latabla=document.all.txtHint.innerHTML;
	document.all.txtHint.innerHTML=latabla;

	document.all.buscarnick.value = "";
	document.all.buscarrfc.value = "";
	document.all.buscarid.value = "";
	document.all.buscarnombre.value = "";
	document.all.buscarciudad.value = "";
}
</script>
