<?
	...
	if($_GET[accion] == 1){
		...
		if(mysql_num_rows($r)>0){
			...
			$s = "SELECT IF(@varpeso<=limite, costo, costo+(((@varpeso-limite)/porcada)*costoextra)) costoemplaye
			FROM configuradorservicios WHERE servicio = 2;";
	...
	if($_GET[accion] == 4){
		...
		******************************************* ERRRORRRRR ********************************************
		...
		if(mysql_num_rows($r)>1){
			...
			INNER JOIN catalogodescripcion cd ON ed.descripcion = cd.id
	...
	if($_GET[accion] == 5){
		...
		WHERE guia = '$_GET[folio]' AND (accion='CANCELADO' OR sustitucion='SUSTITUCION REALIZADA')";
		$r = mysql_query($s,$l) or die($s);
		if(mysql_num_rows($r)>0){
			$f = mysql_fetch_object($r);
