<?	session_start();
	...
	if($_GET[accion]=="0"){
		...
		$s = "SELECT sucursal FROM catalogodestino WHERE id = ".$row[1]."";
		$r = mysql_query($s,$l) or die($s);
		$fd= mysql_fetch_object($r);
		$row[4] = $fd->sucursal;
		...
		idsucursaldestino='".$row[4]."', idremitente='".$row[2]."', ocurre = 1,
		...
		$s = "SELECT CONCAT(cso.calle,'|', cso.colonia,'  ', cso.numero,'  CP: ', cso.cp,'|',
		cso.poblacion,'|',cso.telefono) dirremitente,
		CONCAT(csd.calle,'|', csd.colonia,'  ', csd.numero,'  CP: ', csd.cp,'|',
		csd.poblacion,'|',csd.telefono) dirdestinatario
		FROM catalogosucursal cso
		INNER JOIN catalogosucursal csd
		WHERE cso.id = $_SESSION[IDSUCURSAL] AND csd.id = $row[4]";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
		
		$f->estado = "guardado";
		$f->folio = $folio;
		$f->nuevoFolio = $newfolio;
		...
		re.rfc AS remrfc, de.rfc AS desrfc, ci.guia, ci.sucorigen
		...
		if(mysql_num_rows($r)>0){
			...
			FROM catalogosucursal cso
			INNER JOIN catalogosucursal csd
			WHERE cso.id = $f->sucorigen AND csd.id = $f->sucdestino";
			$r = mysql_query($s,$l) or die($s);
			$tt = mysql_fetch_object($r);
			
			$f->dirremitente = cambio_texto($tt->dirremitente);
			$f->dirdestinatario = cambio_texto($tt->dirdestinatario);
			
			$s = "select estado from guiasventanilla where id='".$f->guia."'";
			$rgv = mysql_query($s,$l) or die($s);
			$fgv = mysql_fetch_object($rgv);
			
			$f->estadoguia = $fgv->estado;
		...
		$principal = "";
		$s="SELECT ci.folio, DATE_FORMAT(ci.fechacorreo,'%d/%m/%Y') AS fecha, ci.estado, ci.destino,
		ci.sucdestino, ci.remitente, ci.sucorigen,
		...
		if(mysql_num_rows($r)>0){
			...
			$s = "select estado from guiasventanilla where id='".$_GET[guia]."'";
			$rgv = mysql_query($s,$l) or die($s);
			$fgv = mysql_fetch_object($rgv);
