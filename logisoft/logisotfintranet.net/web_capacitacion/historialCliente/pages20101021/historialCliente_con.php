<?	//session_start();
	...
	if($_GET[accion]==1){
		...
		if(mysql_num_rows($r)>0){
			...
			$s = "SELECT 
			(SELECT COUNT(*) FROM propuestaconvenio p WHERE p.idprospecto = ".$_GET[cliente]." AND p.tipo = 'CLI') AS tpropuesta,
			(SELECT COUNT(*) FROM generacionconvenio g WHERE idcliente = ".$_GET[cliente]." AND 
			DATEDIFF(vigencia,CURDATE()) <= (SELECT diasvencimientoconvenio FROM configuradorgeneral)) AS tconveniovencimiento,
			(SELECT COUNT(*) FROM solicitudtelefonica WHERE cliente = ".$_GET[cliente]." AND estado = 'POR SOLUCIONAR') AS tcat,
			(SELECT COUNT(*) FROM solicitudguiasempresarialesnw s WHERE s.ncliente = ".$_GET[cliente]." AND
			(s.status IS NULL OR s.status='')) AS tsolicitud,
			(SELECT COUNT(*) FROM entregasespecialesead 
			WHERE IF(opcion2 = 0,remitente,destinatario) =  ".$_GET[cliente].") AS tespeciales";
			$r = mysql_query($s,$l) or die($s);
			$f = mysql_fetch_object($r);
			...
			(SELECT FORMAT(consumomensual,2) FROM generacionconvenio WHERE idcliente = ".$_GET[cliente]." AND estadoconvenio = 'ACTIVADO') AS compromisomensual,
			(SELECT FORMAT(IFNULL(SUM(total),0),2) FROM pagoguias WHERE cliente = ".$_GET[cliente]." AND MONTH(fechacreo) = MONTH(CURDATE()) 
			AND fechacancelacion IS NULL) AS consumido";
			$r = mysql_query($s,$l) or die($s);
			$f = mysql_fetch_object($r);
		...
		END AS mes, 
		SUM(ventas) AS ventas, consumomensual, CONCAT(porc,'%') AS porc, 0 AS entregas, 0 AS recoleccion, 0 AS saldo, 0 AS consumo
		FROM
		(SELECT UPPER(MONTHNAME(rv.fecharealizacion)) AS mes,
		SUM(rv.total) AS ventas, FORMAT(g.consumomensual,2) AS consumomensual,
		FORMAT((100 * SUM(rv.total) / g.consumomensual),2) AS porc
		FROM reportes_ventas rv
		INNER JOIN generacionconvenio g ON rv.convenio = g.folio
		WHERE rv.idcliente = ".$_GET[cliente]." AND rv.activo = 'S' AND rv.fecharealizacion>=DATE_ADD(CAST(CONCAT(YEAR(CURRENT_DATE()),'/',MONTH(CURRENT_DATE()),'/01') AS DATE), INTERVAL -11 MONTH)
		GROUP BY UPPER(MONTHNAME(rv.fecharealizacion))) R1
		GROUP BY mes";
			$arr = array();
			$r = mysql_query($s,$l) or die($s);
		...
		$s = "SELECT gc.folio, DATE_FORMAT(gc.fecha,'%d/%m/%Y') AS fecha,
		CONCAT(IF(gc.descuentosobreflete=1 OR gc.consignaciondescuento=1,'DESCUENTO,', ''), 
		IF(gc.precioporkg=1 OR gc.consignacionkg=1,'KILOGRAMO,',''), IF(gc.precioporcaja=1 OR 
		gc.consignacioncaja=1,'PAQUETE,',''), IF(gc.prepagadas=1,'PREPAGADA,','')) AS tipo,
		estadopropuesta, tipoautorizacion, DATE_FORMAT(gc.vigencia,'%d/%m/%Y') AS vigencia,
		cs.prefijo AS sucursal FROM propuestaconvenio gc
		INNER JOIN catalogosucursal cs ON gc.sucursal = cs.id
		WHERE gc.idprospecto = ".$_GET[cliente]." AND tipo = 'CLI'";
		$r = mysql_query($s,$l) or die($s);
		if(mysql_num_rows($r)>0){	
			$arr = array();
			while($f = mysql_fetch_object($r)){
		...
		$s = "SELECT s.folio, DATE_FORMAT(s.fechaqueja,'%d/%m/%Y') AS fechaqueja,
		cs.descripcion AS sucursal, s.queja, IFNULL(s.guia,'') AS guia,
		IFNULL(s.folioatencion,'') AS folioatencion, IFNULL(s.recoleccion,'') AS recoleccion,
		...
		/*$s = "SELECT guia AS referencia, credito, 
		IF(tipo='NORMAL','G. VENTANILLA',IF(tipo='EMPRESARIAL','G. EMPRESARIAL',IF(tipo='FACT','FACTURA',''))) AS tipo,
		total FROM pagoguias
		WHERE pagado = '".((!empty($_GET[tipo]))?'S':'N')."' AND cliente = ".$_GET[cliente]."";
		$r = mysql_query($s,$l) or die($s);
		if(mysql_num_rows($r)>0){	
			$arr = array();
			while($f = mysql_fetch_object($r)){
		...
		$s = "SELECT CASE mes
			...
			WHEN 'DECEMBER' THEN 'Diciembre'
		END AS mes, 
		SUM(ventas) AS ventas, consumomensual, CONCAT(porc,'%') AS porc
		FROM
		(SELECT UPPER(MONTHNAME(rv.fecharealizacion)) AS mes,
		SUM(rv.total) AS ventas, FORMAT(g.consumomensual,2) AS consumomensual,
