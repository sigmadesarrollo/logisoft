...
<?
		...
		$cantidad = 1;
		if($_GET[tipo]==1){
			$s = "SELECT SUM(cantidad) as total
			FROM guiaventanilla_detalle WHERE idguia = '$_GET[codigo]'";
			$rd = mysql_query($s,$l) or die($s);
			$fd = mysql_fetch_object($rd);
			$f->totalpaquetes = $fd->total;
			
			$s = "SELECT cantidad, descripcion, contenido, peso, volumen, largo, alto, ancho
			FROM guiaventanilla_detalle WHERE idguia = '$_GET[codigo]'";
		}elseif($_GET[tipo]==2){
			$s = "SELECT SUM(cantidad) as total
			FROM guiaventanilla_detalle WHERE idguia = '$_GET[codigo]'";
			...
			$s = "SELECT cantidad, descripcion, contenido, peso, volumen, largo, alto, ancho
			FROM guiasempresariales_detalle WHERE id = '$_GET[codigo]'";
		}else if($_GET[tipo]==3){
