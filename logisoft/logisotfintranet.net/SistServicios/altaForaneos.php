...
<title>Alta Vehiculos Foraneos</title>
...
<script>
	jQuery(document).ready(function(){
		...
		if('<?php echo $_GET[Folio]; ?>'!=''){
			jQuery.post("consultas.php",{accion:'Fora',eFolio:'<?php echo $_GET[Folio]; ?>',val:Math.random()},function(datos){				
		...
		jQuery("#btnGuardar").click(function(){
			...
			jQuery.ajax({type:"POST",url:"registrar.php",
				   data:"accion=Fora&"+$("form").serialize()+"&val="+Math.random(),
	...
	function Limpiar(Bool){
		...
		if(Bool){
			...
			jQuery.post("consultas.php",{accion:'',tabla:'foraneos',val:Math.random()},function(datos){
...
<form id="form1" name="form1" method="post" action="">
	<table width="100%" border="0" cellpadding="3" cellspacing="1">
      <tr>
      	<td colspan="2">
        	<span class="sLink" id="s0">Inicio</span> || <span class="sLink" id="s1">Mantenimiento Vehicular</span> || <span class="sLink" id="s2">Mobiliario y Equipo</span> || <span class="sLink" id="s3">Papeleria</span>        </td>
      </tr>
      <tr>
      	<td colspan="2">
        <span style="color:#006; font-size:25px; font-weight:bold">
        	Alta Vehiculos Foraneos        </span><br />
