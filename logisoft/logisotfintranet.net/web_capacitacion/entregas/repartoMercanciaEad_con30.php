<?
	...
	if($_GET[accion]==2){
		...
		if($_GET[sector]=="0"){
			...
			$filtro1 = "and gv.sector = '$_GET[ssector]' and gv.idsucursaldestino = $_SESSION[IDSUCURSAL]";
			$filtro2 = "and ge.sector = '$_GET[ssector]' and ge.idsucursaldestino = $_SESSION[IDSUCURSAL] ";
		}
...
/*		$arre = array();
		...
		if(mysql_num_rows($r)>0){
			...
			while($f = mysql_fetch_object($r)){				
				...
				if($cont==30){
					$datosIzq .= "{'guia':'$f->guia','origen':'$f->origen','fecha':'$f->fecha','codigobarra':'$f->codigobarra'}";
					if($contr!=$registros){
						$datosIzq .= "]}";
					}
					$cont = 0;
	...
	if($_GET[accion]==3){
		...
		if(mysql_num_rows($r)>0){
			...
			while($f = mysql_fetch_object($r)){				
				...
				$f->guia = cambio_texto($f->guia);
				if($cont==30){
					$datosIzq .= "{'registro':'$f->registro','paquete':'$f->paquete','codigobarra':'$f->codigobarra','estado':'$f->estado','guia':'$f->guia'}";
					if($contr!=$registros){
						$datosIzq .= "]}";
					}
					$cont = 0;
		...
		$datosIzq = (($datosIzq=="")? "{'datos':[]}" : $datosIzq);
		echo "[".$datosIzq."]";
...
/*		$arre = array();
	...
	if($_GET[accion]==111){
		$s = "SELECT guia, origen, DATE_FORMAT(fecha,'%d/%m/%Y')AS fecha, codigobarras FROM repartomercanciadetalle
		WHERE idreparto = ".$_GET[reparto]." AND sucursal = ".$_SESSION[IDSUCURSAL]."";
		$r = mysql_query($s,$l) or die($s);
		$cont = 0;
		$contr = 0;
		$datos = "";
		...
		if(mysql_num_rows($r)>0){
			...
			while($f = mysql_fetch_object($r)){
				...
				$f->guia = cambio_texto($f->guia);
				if($cont==30){
					$datosIzq .= "{'guia':'$f->guia','origen':'$f->origen','fecha':'$f->fecha','codigobarra':'$f->codigobarras'}";
					if($contr!=$registros){
						$datosIzq .= "]}";
					}
					$cont = 0;
		...
		$datosIzq = (($datosIzq=="")? "{'derecha':[]}" : $datosIzq);
		echo "[".$datosIzq."]";
	}
