...
<script>
	...
	tabla1.setAttributes({
		...
		campos:[
			...
			{nombre:"F. RECEPCION", medida:80, alineacion:"center",  datos:"fechareferencia"},
			{nombre:"ENTREGA", medida:80, alineacion:"center",  datos:"entrega"},
			{nombre:"FACTURA", medida:8, tipo:"oculto", alineacion:"left", datos:"factura"},
			...
			{nombre:"COMPROMISO", medida:80, alineacion:"center", datos:"compromiso"},
			...
			{nombre:"INVENTARIO", medida:4, alineacion:"left", tipo:"oculto", datos:"inventario"},
			{nombre:"INVENTARIO2", medida:4, alineacion:"left", tipo:"oculto", datos:"inventario2"}
		],
		filasInicial:15,
		alto:250,
		seleccion:true,
	...
	function mostrarGeneral(datos){		
		...
		u.fecha.value = row[1];
		consultaTexto("eliminarTemporal","carteraMorosa_con.php?accion=0&tabla=inventariomorosodetalle_tmp");
	}
	...
	function generarDetalle(){
		...
		consultaTexto("mostrarDetalle","carteraMorosa_con.php?accion=5&cliente="+((u.criterio[0].checked==true)?u.cliente.value:0)+"&fecha="+u.fecha.value+"&ead="+((u.chkguias.checked==true)?1:0));
	}
	...
	function guardar(){
		...
		if(cant == 0){
			alerta3("No existen datos en el Detalle del Inventario Moroso","¡Atención!");
			return false;
		}
	...
	function guardarConfirmacion(){
		...
		for(var i=0; i<cant; i++){
			if(u["detalle_INVENTARIO"][i].value != u["detalle_INVENTARIO2"][i].value){				
				u.registros.value += u["detalle_ID"][i].value+","+u["detalle_ASIGNADO"][i].value
				+","+u["detalle_CAUSA"][i].value+","+u["detalle_IDUSUARIO"][i].value
				+","+u["detalle_COMPROMISO"][i].value+","+u["detalle_INVENTARIO"][i].value
				+","+u["detalle_INVENTARIO2"][i].value+";";
		...
		if(si == cant){
			...
			//alerta3("registrarCartera","carteraMorosa_con.php?accion=6&noseleccion=si&folios="+u.registros.value);
			consultaTexto("registrarCartera","carteraMorosa_con.php?accion=6&noseleccion=si&folios="+u.registros.value);
		}
	}
	...
	function confirmarAsignacion(){
		consultaTexto("registrarCartera","carteraMorosa_con.php?accion=6&noseleccion=no&empleado="+u.empleado_hidden.value
	...
	function agregarResponsable(){
		...
		var fi = u.h_fecha.value.split("/");
		var ff = obj.compromiso.split("/");
		var f1 = new Date(fi[2],fi[1],fi[0]);
		...
		for(var i=0;i<tabla1.getRecordCount();i++){
			if(u["detalle_SEL"][i].checked == true && u["detalle_ASIGNADO"][i].value=="NO"){
				...
				obj.id 			= arr.id;
				obj.inventario	= arr.inventario;
				obj.inventario2	= arr.inventario2;
				obj.guardado	= arr.guardado;
				obj.fechaasignada = ((arr.fechaasignada!="" || arr.fechaasignada!="0000-00-00")?arr.fechaasignada:'<?=date('Y/m/d'); ?>');
			...
			/*if(u["detalle_SEL"][i].checked == true && u["detalle_ASIGNADO"][i].value=="NO"){
				...
				obj.asignado 	= "SI";
				obj.causa		= u.compromiso.options[u.compromiso.selectedIndex].text;			
				obj.compromiso 	= ((f1 > f2)?"":arr.compromiso);
				obj.empleado	= u.empleado.value;
				obj.idempleado	= u.empleado_hidden.value;
				obj.id 			= arr.id;
				obj.inventario	= arr.inventario;
...
<form id="form1" name="form1" method="post" action="">
  <table width="800" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
    <tr>
      <td class="FondoTabla">Inventario Moroso  </td>
    </tr>
    <tr>
        ...
        <tr>
          <td colspan="4"><table width="549" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
              ...
              <td colspan="3"><label>
                <input name="cliente" type="text" id="cliente" class="Tablas" onkeypress="if(event.keyCode==13){obtenerClienteEnter(this.value)}" onkeyup="validarCaja(event,this.name,2)" style="width:70px" />
                <img src="../img/Buscar_24.gif" width="24" height="23" align="absbottom" style="cursor:pointer" onclick="if(document.all.cliente.readOnly==false){abrirVentanaFija('../buscadores_generales/buscarClienteGen.php?funcion=obtenerCliente', 600, 450, 'ventana', 'Busqueda')}" /></label>                <input name="nombre" type="text" class="Tablas" id="nombre" readonly="" style="width:300px; background-color:#FFFF99" /></td>
              </tr>
            
            <tr>
              <td width="170">Fecha al:
                <input name="fecha" type="text" class="Tablas" style="width:75px" id="fecha" maxlength="10"/>
                <img src="../img/calendario.gif" alt="Baja" width="20" height="20" align="absbottom" style="cursor:pointer" title="Calendario" onclick="displayCalendar(document.forms[0].fecha,'dd/mm/yyyy',this)" /></td>
              <td width="147"><input type="checkbox" name="chkguias" value="1" style="width:13px" />
Mostrar Gu&iacute;as EAD </td>
              <td width="149"><img src="../img/Boton_Generar.gif" id="btnGenerar" onclick="generarDetalle()" style="cursor:pointer" /></td>
            </tr>
          </table></td>
        </tr>
        <tr>
        ...
        <tr>
          <td>Compromiso:</td>
          <td colspan="2"><select name="compromiso" class="Tablas" id="compromiso" style="width:250px">
            <option value="0" selected="selected">SELECCIONAR COMPROMISO</option>
            <option value="1" >CONTACTAR REMITENTE O SUCURSAL ORIGEN</option>
            <option value="2" >ENTREGAR EN EL TRANSCURSO DEL D&Iacute;A</option>
            <option value="3" >ENTREGAR A LAS 09:00</option>
            <option value="4" >ENTREGAR A LAS 10:00</option>
            <option value="5" >ENTREGAR A LAS 11:00</option>
            <option value="6" >ENTREGAR A LAS 12:00</option>
            <option value="7" >ENTREGAR A LAS 13:00</option>
            <option value="8" >ENTREGAR A LAS 14:00</option>
            <option value="9" >ENTREGAR A LAS 15:00</option>
            <option value="10" >ENTREGAR A LAS 16:00</option>
            <option value="11" >ENTREGAR A LAS 17:00</option>
            <option value="12" >ENTREGAR A LAS 18:00</option>
            <option value="13" >REEXPEDIR MERCANC&Iacute;A</option>
          </select></td>
          <td>&nbsp;</td>
        </tr>
        ...
        <tr>
          <td colspan="4"><table width="100%" border="0" cellpadding="0" cellspacing="0">
            ...
            <tr>
              ...
              <td width="14%"><input name="empleado_hidden" class="Tablas" type="text" id="empleado_hidden" style="width:70px" onkeyup="validarCaja(event,this.name,1)" onkeypress="if(event.keyCode==13){obtenerEmpleado(this.value)}"/>
              ...
              <td><input name="empleado" type="text" class="Tablas" id="empleado" readonly="" style="width:250px; background-color:#FFFF99" />                <label></label></td>
              <td width="43%" valign="middle"><img src="../img/Boton_AsignarAct.gif" width="117" height="20" style="cursor:pointer" onclick="agregarResponsable()" /></td>
              </tr>
          </table></td>
          </tr>
        <tr>
          <td width="86">&nbsp;</td>
          <td width="207"><label></label></td>
          <td width="65">&nbsp;</td>
          <td width="191"><label>
            ...
            <input name="h_fecha" type="hidden" id="h_fecha" value="<?=$fecha ?>" />
          </label></td>
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="4"><div style=" height:270px; width:799px; overflow:auto">
        ...
        <tr>
          <td colspan="4"><table width="534" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td width="135">Guias Pend. de Entrega: </td>
              <td width="132"><input name="guias" type="text" class="Tablas" readonly="true" style="background-color:#FFFF99; width:100px" /></td>
              <td width="140" align="right">Importe Total:</td>
              <td width="127"><input name="total" type="text" class="Tablas" readonly="true" style="background-color:#FFFF99" /></td>
            </tr>
          </table></td>
        </tr>
