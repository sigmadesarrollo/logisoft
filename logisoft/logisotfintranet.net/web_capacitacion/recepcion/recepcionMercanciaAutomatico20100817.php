	...
	function moverAlaDerecha(){
		if(tabla1.getRecordCount()==""){
			...
			if(tabla1.getSelectedIdRow()!=""){
				consultaTexto("resMovADerC","recepcionMercanciaAutomatico_con.php?accion=4&folio="+tabla1.getSelectedRow().guia);
			}else if(tabla3.getSelectedIdRow()!=""){
				consultaTexto("resMovADerU","recepcionMercanciaAutomatico_con.php?accion=5&folio="+tabla3.getSelectedRow().guia
	...
	function moverAlaizquierda(){
		if(tabla2.getRecordCount()==""){
			...
			if(tabla2.getSelectedIdRow()!=""){
				consultaTexto("resMovAIzqC","recepcionMercanciaAutomatico_con.php?accion=6&folio="+tabla2.getSelectedRow().guia);
			}else if(tabla4.getSelectedIdRow()!=""){
				consultaTexto("resMovAIzqU","recepcionMercanciaAutomatico_con.php?accion=7&folio="+tabla4.getSelectedRow().guia
...
/*******************/
	function ObtDetalleIzq(){
		...
		if(tabla1.getValSelFromField('guia','No_GUIA')!=""){
			consultaTexto("mostrarIzqAbajo","recepcionMercanciaAutomatico_con.php?accion=2&folio="+tabla1.getValSelFromField('guia','No_GUIA'));
		}
	}
...
/****************/
	function ObtDetalleDer(){
		...
		consultaTexto("mostrarDerAbajo","recepcionMercanciaAutomatico_con.php?accion=3&folio="+tabla2.getValSelFromField('guia','No_GUIA'));
		}
	}
...
/********************/
	function CargarGuias(){
		consultaTexto("mostrarCargarGuias","recepcionMercanciaAutomatico_con.php?accion=1&unidad="+u.unidad.value+"&sucursal="+'<?=$_SESSION[IDSUCURSAL]?>'+"&sid="+Math.random());
	}
...
/********************/
	...
	function guardarValores(){
		<?=$cpermiso->verificarPermiso("390",$_SESSION[IDUSUARIO]);?>
		if(u.unidad.value==""){
		...
		if(tabla2.getRecordCount()>0 && u.guardado.value == ""){
			consultaTexto("resValidar","recepcionMercanciaAutomatico_con.php?accion=8&mathrand="+Math.random());
		}else{
	...
	function guardarFinal(){
		...
		consultaTexto("resGuardar","recepcionMercanciaAutomatico_con.php?accion=9&unidad="+unidad
	...
	function mostrarFaltante(){
		if(u.guardado.value == ""){
			...
			if(tabla1.getRecordCount()>0){
			abrirVentanaFija('reporteDanoFaltante.php?guia='+arr.guia+'&ruta='+u.ruta.value
			+'&unidad='+u.unidad.value
			+'&tipo=faltantedbl&sucursal=<?=$_SESSION[IDSUCURSAL]?>', 600, 480, 'ventana', 'REPORTE DE DAÑOS Y FALTANTES');
			}
		}
	...
	function estadoGuia(estado,observaciones){
		...
		if(estado != "BUEN ESTADO"){
			consultaTexto("mostrarDerAbajo","recepcionMercanciaAutomatico_con.php?accion=11&estado="+estado+"&folio="+arr.guia);
			abrirVentanaFija('reporteDanoFaltante.php?guia='+arr.guia+'&ruta='+u.ruta.value
			+'&unidad='+u.unidad.value
	...
	function cargarRecepcion(folio){
		u.folio.value = folio;
		consultaTexto("resCargarRecepcion","recepcionMercanciaAutomatico_con.php?accion=12&folio="+folio)
	}
...
<table width="621" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
  <tr>
    <td width="619" class="FondoTabla Estilo4">RECEPCI&Oacute;N DE MERCANCIA AUTOMATICA </td>
  </tr>
  <tr>
