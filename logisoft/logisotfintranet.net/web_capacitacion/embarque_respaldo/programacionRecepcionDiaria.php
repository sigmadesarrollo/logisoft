...
<script>
	...
	campos:[
		...
		seleccion:true,
		ordenable:false,
		eventoClickFila:"obtenerDetalleAbajo()",
		nombrevar:"tabla1"
	});
	...
	function mostrarDatosPrecintos(){
		if(tabla1.getSelectedIdRow()==""){
			...
			abrirVentanaFija('registroPrecintosRecepcion.php?sucursal='+'<?=$_SESSION[IDSUCURSAL]?>'+'&unidad='+arr.unidad+'&tipo=llegada', 550, 450, 'ventana', 'Busqueda','checarAsignacionPrecinto();');
		}
	}
	...
	function obtenerHoraLlegadaSalida(datos){
		...
		if(objeto[0].tipo == "llegada"){			
			u.horallegada.value = "";
			u.horallegada.value = objeto[0].hora;
			u.tipo.value = objeto[0].tipo;
			actualizarFila('llegada');
		}else{
			u.horasalida.value  = "";
			u.horasalida.value = objeto[0].hora;
			u.tipo.value = objeto[0].tipo;
...
<table width="450" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
  ...
  <tr>
    <td><table width="449" border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td colspan="5" align="center"><table width="448" id="detalle2" border="0" cellspacing="0" cellpadding="0"></table>          </td>
      </tr>
	  <tr>
