<?php
	...
	class pdf extends FPDF{
		...
		function Header(){
			...
			$this->Cell(70,10,'REPORTE:  CONVENIO DE PRESTACION DE SERVICIOS % DESCUENTO                            		FECHA IMPRESO:'.date('d/m/Y').'',0,0,'L');
			$this->Ln(5);
	...
	$pdf->MultiCell(0,10,'9.- El cliente acepta que la forma de cobro para sus envíos es (Seleccionar una opción):',0,'J');
	$pdf->MultiCell(0,10,'		Descuento sobra Flete ________%',0,'J');
		
	$pdf->MultiCell(0,10,'			9.1.- Servicios sin costo para el cliente',0,'J');
	$pdf->MultiCell(0,10,'				Recolección a Domicilio				(Sobre lo capturado el Grid)',0,'J');
		
	$pdf->MultiCell(0,10,'El Descuento sobre Flete no aplica a las tarifas mínimas establecidas por la Prestadora de Servicios, ni a los servicios adicionales.',0,'J');
	...
	$pdf->MultiCell(0,5,'11.- Si El Cliente solicita Guías Electrónicas se le asignarán un password, nip y los folios de las guías, los cuales podrá utilizar a través de nuestra página web, el uso del nip y los folios es responsabilidad de El Cliente.  La elaboración de una guía electrónica no implicará una responsabilidad de su contenido para la Prestadora de Servicios hasta que esta confirme que ha recibido las mercancías.',0,'J');
			$pdf->MultiCell(0,5,'',0,'J');
	$pdf->MultiCell(0,5,'12.- El Cliente solicita que se consideren las siguientes Restricciones en sus envíos siempre y cuando el valor del envío este a su cargo.',0,'J');
				$pdf->MultiCell(0,5,'',0,'J');
	...
	$pdf->MultiCell(0,5,'',0,'J');
		...
		if($f->consignaciondescuento==1 || $fx->c1>0 || $fx->c2>0 || $fx->c3>0){
