...
<script>
	...
	tabla1.setAttributes({
		...
		campos:[
			...
			{nombre:"No_GUIA", medida:80, alineacion:"center", datos:"guia",onDblClick:"mostrarDetalle"},
	...
	function obtenerInicio(){
		consultaTexto("mostrarInicio", "liquidaciondemercancia_con.php?accion=1&and="+Math.random());
	}
	...
	function obtenerGuia(idreparto){
		...
		consultaTexto("resObtenerGuia","liquidaciondemercancia_con.php?accion=2&folio="+u.folio.value);
	}
	...
	function validacheck(){
		...
		consultaTexto('respuestaCambioSeleccionado',"liquidaciondemercancia_con.php?accion=5&guiasseleccionadas="+guias);
	...
	function facturar(){
		...
		for(var i=0; i<tabla1.getRecordCount();i++){
			if(u["tablalista_CHECK"][i].checked==true && u["tablalista_PAGO"][i].value == 'CREDITO' && u["tablalista_FACTURA"][i].value=='0'){
				...
				guias += ((guias!="")?",":"")+u["tablalista_No_GUIA"][i].value;
			}
		}
		if(guias==""){
	...
	function obtenerConductor(idconductor){
		consultaTexto("mostrarConductor","liquidaciondemercancia_con.php?accion=3&idempleado="+idconductor+"&and="+Math.random());
	}
	...
	function ValidarGuardar(tipo){
		...
		var recibidos = "";
		for(var i=0;i<tabla1.getRecordCount();i++){
			if(u["tablalista_CHECK"][i].checked==true && u["tablalista_FACTURA"][i].value=='0' && u["tablalista_PAGO"][i].value=='CREDITO'){
				 guias += ((guias!="")?",":"")+u["tablalista_No_GUIA"][i].value;
			}
		}
		if(guias!=""){
		...
		for(var i=0;i<tabla1.getRecordCount();i++){
			if(u["tablalista_NOMBRE"][i].value==""){
				 recibidos += ((recibidos!="")?",":"")+u["tablalista_No_GUIA"][i].value;
			}
		}
		if(recibidos!=""){
		...
		if(u.folio.value==""){
			...
			consultaTexto("MostrarGuardar", "liquidaciondemercancia_con.php?accion=4&idreparto="+u.folio.value+"&entregadas="+u.entregadas.value+
	...
	function obtenerFolio(folio){
		...
		consultaTexto("mostrarDatosEncabezados","liquidaciondemercancia_con.php?accion=6&folio="+folio+"&enc="+Math.random());		
	}
...
cobContad += parseFloat((obj.detalle[i].tipoflete != "PAGADO" && obj.detalle[i].condicionpago == "CONTADO" && obj.detalle[i].estado == "ENTREGADA POR LIQUIDAR")?1:0);
	...
	function actualizarFila(datos){
		...
		consultaTexto("MostrarGuardarDatosPersonales","liquidaciondemercancia_con.php?accion=7&nombre="+
      ...
      <tr>
        <td width="615"><table width="615" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td width="75"><input name="folio" type="text" class="Tablas" id="folio" value="<?=$folio ?>" style="width:70px" onKeyPress="if(event.keyCode==13){obtenerGuia(this.value)}"/></td>
            <td width="30">
            <img src="../img/Buscar_24.gif" width="24" height="23" align="absbottom" style="cursor:pointer" onclick="abrirVentanaFija('../buscadores_generales/buscarFolioRepartos.php?funcion=obtenerGuia&tipo=preliquidacion',600,475,'ventana','BUSCAR LIQUIDACION MERCANCIA','');" />
            </td>
            ...
            <td width="86"><input name="folio2" type="text" class="Tablas" id="folio2" style="width:80px" value="<?=$folio2 ?>" onkeypress="if(event.keyCode==13){obtenerFolio(this.value);}"/></td>
            <td width="36"><div class="ebtn_buscar" onclick="abrirVentanaFija('../buscadores_generales/buscarliquidacionmercancia.php?funcion=obtenerFolio',600,475,'ventana','BUSCAR LIQUIDACION MERCANCIA','');"></div></td>
      ...
      <tr>
        <td><table width="615" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td width="116">Por Cobrar Contado </td>
      ...
      <tr>
        <td><table width="615" border="0" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <td width="35">&nbsp;</td>
            <td width="116">&nbsp;</td>
            <td width="197">T. a Liquidar: <span class="Tablas">
              <input name="total" type="text" class="Tablas" id="total" style="width:100px;background:#FFFF99; text-align:right"  readonly=""/>
            </span></td>
            <td colspan="3">Total Entregado: <span class="Tablas">
              <input name="entregado" type="text" class="Tablas" readonly="readonly" id="entregado" onfocus="this.select()" style="text-align:right;width:100px;background:#FFFF99;" value="0" onkeypress="solonumeros(event); if(event.keyCode==13){diferencia(this.value);}" onkeydown="if(event.keyCode==9){diferencia(this.value);}" />
            </span></td>
            </tr>
        ...
        <td><table width="615" border="0" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td colspan="4" align="center"><table width="276">
                <tr>
                  <td ><div id="guardar" style=":<? if($_POST[accion]=='grabar'){?>visibility:hidden<? }?>" class="ebtn_guardar" onclick="ValidarGuardar(0)"></div></td>
                  <td><div id="cerrar" class="ebtn_cerrarliquidacion" onclick="confirmar('¿Desea cerrar la liquidación del folio de reparto EAD: '+ u.folio.value +'?', '', 'ValidarGuardar(1)', '');"></div></td>
                  <td><div id="limpiar" class="ebtn_nuevo" onclick="limpiarTodo()"></div></td>
                  <td><img src="../img/Boton_Facturar.gif" onclick="facturar()"></td>
                </tr>
              </table>
              </td>
            </tr>
