<?	session_start();
	...
	if($_GET[accion]=="0"){//BORRAR TEMPORALES
		...
		WHERE id = ".$_GET[sucursal]." -- AND subdestinos=1";
		...
		WHERE cs.id = ".$_GET[sucursal]." AND cd.subdestinos=1";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
			$f->descripcion = cambio_texto($f->descripcion);			
		...
		$origen = "";	
		$s = "SELECT id, descripcion AS destino FROM catalogodestino WHERE sucursal=".$_GET[sucursal]."";
		$registros = array();
		$r = mysql_query($s,$l) or die($s);
		...
		if($_GET[tip]=="grabar"){
			...
			$s = "INSERT INTO recoleccion SET
			folio = '$folio', sucursal = '".$_GET[idsucursal]."',
			...
			SELECT 0 AS id, '$folio' AS recoleccion, '".$_GET[idsucursal]."' AS sucursal,cantidad,
