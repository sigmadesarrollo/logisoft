...
if ($_GET[accion]==1){
	...
	UNION 
		(SELECT cs.id AS sucursal,ge.id AS guia
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturadetalle fd ON a.factura=fd.factura
	...
	UNION 
		(SELECT cs.id AS sucursal,sge.id AS guia
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturadetalle fd ON a.factura=fd.factura
	...
	UNION 
		(SELECT cs.id AS sucursal,f.folio AS guia
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
	...
	$s="SELECT FORMAT(SUM(importe),2) AS importe FROM (
		...
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' AND '".cambiaf_a_mysql($_GET[fecha2])."'
		AND fp.procedencia='C' AND cs.id='".$_GET[sucursal]."' AND ISNULL(fp.fechacancelacion) GROUP BY cs.id
		UNION 
		SELECT cs.id AS sucursal,SUM(fp.total) AS importe 
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id 
		INNER JOIN catalogocliente cc ON fp.cliente=cc.id
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' AND '".cambiaf_a_mysql($_GET[fecha2])."'
		AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY cs.id
	)t";
	$r = mysql_query($s,$l) or die($s);
	...
	UNION 
		...
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' AND '".cambiaf_a_mysql($_GET[fecha2])."'  
		AND f.sobmontoafacturar>0 AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.id)
	)t GROUP BY guia ORDER BY fecha,guia $limite"; 
