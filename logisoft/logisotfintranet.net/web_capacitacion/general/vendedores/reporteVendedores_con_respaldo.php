<?	session_start();
	...
	if($_GET[accion]==1){//PRINCIPAL
		...
		AND activo = 'S'
		GROUP BY prefijoorigen";
		$r = mysql_query($s,$l) or die($s);
		$totalregistros = mysql_num_rows($r);
		
		$s = "SELECT COUNT(DISTINCT(idvendedor)) AS tvendedor, FORMAT(SUM(flete),2) AS flete, FORMAT(SUM(IF(cobrado='S',flete,0)),2) AS vtascobradas 
		...
		IF(cobrado='S',SUM(flete),0) AS vtascobradas 
		FROM reporte_vendedores_ventas
		WHERE fecha BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql($_GET[fechafin])."'
		AND activo = 'S'
		...
		v.idcliente, v.cliente, v.flete, IF(v.guia=t.guia,t.estado,IF(v.guia=s.id,s.estado,'')) AS estado
		FROM reporte_vendedores_ventas v
		LEFT JOIN (SELECT id AS guia, estado FROM guiasventanilla
		UNION
		SELECT id AS guia, estado FROM guiasempresariales) t ON v.guia = t.guia
