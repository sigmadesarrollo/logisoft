<?	session_start();
	...
	if($_GET[accion]==1){
		...
		$s = "SELECT IF(guiaempresarial = '',1,0) gv, IF(guiaempresarial <> '',1,0)  ge 
			FROM evaluacionmercancia WHERE folio=$fx->folio AND sucursal=".$_SESSION[IDSUCURSAL].";";
		$r = mysql_query($s,$l) or die($s); $f = mysql_fetch_object($r);
		$s = "UPDATE lasalertas SET evpegeguve=evpegeguve+$f->gv,evpegeguem=evpegeguem+$f->ge
