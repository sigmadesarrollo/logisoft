<? 
	...
	/*$sql="SELECT cs.prefijo as sucursal,cc.id AS cliente, CONCAT_WS(' ',cc.nombre,cc.paterno,cc.materno) AS nombrecliente,
		...
		IFNULL(IF(ge.factura IS NOT NULL OR ge.factura<>0,ge.total+ge.texcedente+ge.tseguro,0),0) AS importe,
		IFNULL(s.total-(ge.total+ge.texcedente+ge.tseguro),0) AS porfacturar
		...
		/*$sql="SELECT cs.prefijo as sucursal,cc.id AS cliente, CONCAT_WS(' ',cc.nombre,cc.paterno,cc.materno) AS nombrecliente,
		s.foliotipo AS venta, IFNULL(CONCAT_WS(' - ',s.desdefolio,s.hastafolio),0) AS folios, ifnull(f.folio,0)as folio,
		IFNULL(IF(ge.factura IS NOT NULL OR ge.factura<>0,0,s.total),0) AS importe,
		...
		$sql="	SELECT cs.prefijo AS sucursal,rv.idcliente AS cliente, rv.nombrecliente,
		s.foliotipo AS venta, IFNULL(CONCAT_WS(' - ',s.desdefolio,s.hastafolio),0) AS folios, IFNULL(rv.factura,0)AS folio,
		IFNULL(s.total,0) AS importe,
		0 AS porfacturar 
		FROM reportes_ventas rv
		INNER JOIN catalogosucursal cs ON rv.idsucorigen = cs.id
		LEFT JOIN solicitudguiasempresariales s ON rv.folio = s.id	
		WHERE rv.fecharealizacion BETWEEN '".cambiaf_a_mysql($_GET[fechaini])."' AND '".cambiaf_a_mysql($_GET[fechafin])."'
		AND cs.prefijo = '".$_GET[sucursal]."' AND rv.tipoventa='SOLICITUD DE FOLIOS' AND rv.tipoempresarial='PREPAGADA' AND rv.activo='S' GROUP BY s.id LIMIT 0,30";
