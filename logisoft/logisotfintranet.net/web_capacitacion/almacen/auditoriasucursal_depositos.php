...
<script>
	...
	tabla1.setAttributes({
		nombre:"detalle",
		campos:[
			{nombre:"FECHA", medida:100, alineacion:"left",  datos:"fecha"},
			{nombre:"IMPORTE", medida:80, alineacion:"right", tipo:"moneda", datos:"importe"},
			{nombre:"FECHA CORTE",medida:100, alineacion:"left", datos:"fechacorte"}		
		],
		filasInicial:30,
		alto:250,
		seleccion:true,
	...
	function obtenerDetalle(){
		consultaTexto("resTabla1","auditoriasucursalnuevo_con.php?sucursalseleccionada=<?=$_GET[sucursalseleccionada]?>&accion=4<?=(($_GET[folio]!="")?"&folio=$_GET[folio]":"")?>&contador="+u.pag1_contador.value
		+"&s="+Math.random());
	}
	...
	function paginacion(movimiento){
		switch(movimiento){
			case 'primero':
				consultaTexto("resTabla1","auditoriasucursalnuevo_con.php?sucursalseleccionada=<?=$_GET[sucursalseleccionada]?>&accion=4&contador=0&s="+Math.random());
				break;
			case 'adelante':
				if(u.pag1_adelante.value==1){
					consultaTexto("resTabla1","auditoriasucursalnuevo_con.php?sucursalseleccionada=<?=$_GET[sucursalseleccionada]?>&accion=4&contador="+(parseFloat(u.pag1_contador.value)+1)					
					+"&s="+Math.random());
				}
				break;
			case 'atras':
			...
			case 'ultimo':
				...
				consultaTexto("resTabla1","auditoriasucursalnuevo_con.php?sucursalseleccionada=<?=$_GET[sucursalseleccionada]?>&accion=4&contador="+contador
				+"&s="+Math.random());
				break;
		}
	}
