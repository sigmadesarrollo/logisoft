...
+(SELECT COUNT(*) FROM guiasempresariales '.$condicion.' )');*/
	...
	$s = "select count(id) from (
	    SELECT distinct count(bs.folio) as id  
		FROM cartaporte cp 
		...
		where bs.status = 1 and IDEntregaOcurre= 0
		group by p.fechasalida,p.hrsalida,p.fechallegada,p.hrllegada
		order by 1 
	) as t1";
...
UNION (SELECT id,DATE_FORMAT(fecha, "%d/%m/%Y") AS fecha FROM guiasempresariales '.$condicion.' ) limit '.$st.','.$pp,$link);*/
		...
		/*$get =@mysql_query("SELECT id,fecha, destinatario FROM (
			...
			$get =@mysql_query("SELECT id,fecha, destinatario FROM (
			SELECT  cp.Folio as id,cp.OrigenNombre as origen , cp.DestinoNombre,p.fechasalida,p.hrsalida,p.fechallegada as fecha ,p.hrllegada,Nombre_Cliente as destinatario
			FROM cartaporte cp 
			inner join recoleccion r on cp.IDRecoleccion = r.Folio
			inner join bitacorasalida bs on r.foliobitacora = bs.folio and r.folio = bs.Foliorecoleccion
			inner join programacionrecepciondiaria p on bs.folio = p.idbitacora
...
<span onClick="parent.<?=$_GET[funcion]?>('<?=$row[id];?>');try{parent.VentanaModal.cerrar();}catch(e){parent.mens.cerrar();}" style="cursor: pointer; color:#0000FF"><?=$row[id];?></span></td>
    ...
    <tr>
      ...
      <td colspan="3" align="center"><font color="#FF0000" size="-1"><? echo paginacion($total, $pp, $st, "buscarGuiasEmpresariales_VentanillaGen2.php?funcion=$_GET[funcion]&cliente=$_GET[cliente]&tipo=$_GET[tipo]&st="); ?></font></td>
