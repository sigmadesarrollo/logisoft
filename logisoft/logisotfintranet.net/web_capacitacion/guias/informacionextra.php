...
<?
	...
	if($_GET[tipo]==1){
		...
		gv.tipoidentificacion, IF(gv.tipoflete = 1,'POR COBRAR','PAGADA') tipoflete,
		date_format(gv.fechaentrega,'%d/%m/%Y') fechaentrega
		...
		CONCAT_WS(' ',ce.nombre, ce.apellidopaterno, ce.apellidomaterno) AS capturista,
		gv.idsucursalorigen, gv.idsucursaldestino, gv.recibio, gv.numeroidentificacion,
		gv.tipoidentificacion, 'PAGADA' tipoflete,
		date_format(gv.fechaentrega,'%d/%m/%Y') fechaentrega
        ...
        onblur="devolverDestino()" value="<?=$f->recepcion?>" /></td>
          ...
          <tr>
            ...
            <?
				...
				if($fechapago==""){
					if(($f->estado=='ENTREGADA' || $f->estado=='POR ENTREGAR') && $f->condicion=='CONTADO' && $f->tipoflete=='POR COBRAR'){
						$fechapago = $f->fechaentrega;
					}
				}
