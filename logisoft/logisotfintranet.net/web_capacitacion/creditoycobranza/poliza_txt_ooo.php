<?
	...
	$totalcargos = 0;
		...
		#GUIAS HECHAS
		$s = "DROP TABLE IF EXISTS movimientos_xax;";
		mysql_query($s,$l) or die(mysql_error($l)."-".$s);

		$s = "CREATE TABLE `movimientos_xax` (
		...
		UNION
		SELECT DISTINCT 1, 'G', f.folio, concat('LIQ. DEL $diainicioarriba AL $diafinarriba $mesarriba/".substr($dias[0],2,4)." ',cs.prefijo), SUM(IFNULL(f.sobsubtotal,0)) adicional, 
		CONVERT(CONCAT_WS('',$inicuecom,cs.idsucursal,'000600') USING utf8) cuenta, cs.prefijo
		FROM facturacion f
		INNER JOIN catalogosucursal_tmp_".$var_itabla." cs ON f.idsucursal = cs.id
		WHERE f.idsucursal = $idsucursalorigen
