...
<script>
	...
	function validarTotales(){
		...
		if(u.cheque.value!=""){
			if(u.cheque.value.replace("$ ","").replace(/,/g,"")!=u.efectivo.value.replace("$ ","").replace(/,/g,"")){
				alerta("Si va a pagar con cheque la cantidad debe ser exactamente el total","Atencion","banco");
				return false;
			}
			...
			var comparacion	= total - acumulado;
			 ...
			 if(comparacion>0){
				...
				if (cambio>0){
					...
					up.efectivo.value				= u.efectivo.value.replace("$ ","").replace(/,/,"");
				}
				up.cheque.value					= u.cheque.value.replace("$ ","").replace(/,/,"");
				up.banco.value					= u.banco.value;
				up.ncheque.value				= u.ncheque.value;
				up.tarjeta.value				= u.tarjeta.value.replace("$ ","").replace(/,/,"");
				up.transferencia.value			= u.transferencia.value.replace("$ ","").replace(/,/,"");
				up.nc.value						= u.nc.value.replace("$ ","").replace(/,/,"");
    ...
    <table width="405" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
          ...
          <td height="147"><br><table width="362" border="0" align="center" cellpadding="0" cellspacing="0">
                  ...
                  <tr>
                    ...
                    <td width="100"><input name="cheque" tabindex="4" type="text" class="Tablas"  style="font:tahoma; font-size:9px; text-align:right;<? if($s<=0){echo "background:#FFFF99;";}else{echo "background:'';";} ?> text-align:right" onFocus="enfocar(this)" onBlur="desenfocar(this)" onKeyPress="return Numeros(event)" onKeyDown="if(event.keyCode==13){validarCheque(this); calcular(); return tabular(event);}" value="<?=$cheque ?>" size="15" maxlength="15" <? if($s<=0){echo "readonly=''";} ?> ></td>
                  </tr>
                  <tr>
