<? 	session_start();
	...
	if(mysql_num_rows($r)==0){
		...
		#para actualizar las alertas de convenios
		$s = "UPDATE lasalertas SET prpeau=(SELECT SUM(IF(estadopropuesta LIKE '%EN AUTORIZACION%',1,0)) FROM propuestaconvenio),
		prau=(SELECT SUM(IF(estadopropuesta LIKE '%AUTORIZADA%',1,0)) FROM propuestaconvenio),
		prauco=(SELECT SUM(IF(estadopropuesta LIKE 'AUTORIZADA',1,0)) FROM propuestaconvenio),
		copeim=(SELECT SUM(IF(estadoconvenio = 'AUTORIZADO',1,0)) FROM generacionconvenio),
		copeac=(SELECT SUM(IF(estadoconvenio = 'IMPRESO',1,0)) FROM generacionconvenio);";
		mysql_query($s,$l) or die($s);
		
		$s = "CREATE TEMPORARY TABLE `alertas` (
		  `tcopeau` BIGINT(21) DEFAULT '0',
		  `idsucursal` DOUBLE DEFAULT '0'
		) ENGINE=MYISAM DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
		mysql_query($s,$l) or die($s);
		
		$s = "INSERT INTO alertas 
		SELECT COUNT(*) AS tcopeau,sucursal AS idsucursal FROM generacionconvenio 
		WHERE DATEDIFF(vigencia,CURDATE()) <= (SELECT diasvencimientoconvenio FROM configuradorgeneral) GROUP BY sucursal;";
		mysql_query($s,$l) or die($s);
		//$s = "UPDATE lasalertas INNER JOIN alertas ON sucursal=idsucursal SET copeau=tcopeau;";
		//mysql_query($s,$l) or die($s);
...
<body>
    <div id="layout" style="height:1100px">
    	...
    	<div class="header"><img src="img_nm/logo.jpg" alt="PMM - Paqueteria y mensajeria en movimiento" width="128" height="63"/>
        <span class="noticias" style="text-align:right" onclick="verNoticias()">News....</span>
...
<ul>
	 ...
	 <? }else{
			...
			$r = mysql_query($s,$l) or die($s);
			$f = mysql_fetch_object($r);
			echo strtoupper(utf8_encode($f->descripcion))."<input type='hidden' name='sucursal' value='".$_SESSION[IDSUCURSAL]."'/>";
		}
     ...
     <?
      ...
      <iframe onblur="document.all.modificando.value=1;" name="pagina4" id="pagina4" scrolling="auto" 
        align="top" width="850" height="770" src="<?=$f->pestana5?>" frameborder="0" style="display:none"></iframe>
         ...
         <p style="text-align:center; color:#fff; font-size:10px;"> &copy; 2010 Powered by Empresaria</p>
</div>
