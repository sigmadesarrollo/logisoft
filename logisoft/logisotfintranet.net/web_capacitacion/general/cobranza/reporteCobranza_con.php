<?
	...
	if($_GET[accion]==1){
		...
		if($_GET[sucursal]!=1){
			$suc="WHERE temp.sucursal = $_GET[sucursal]";
		}
		...
		AND ISNULL(ge.factura) ".(($_GET[sucursal]==1)?"":" AND ge.idsucursalorigen = $_GET[sucursal]")." GROUP BY ge.idsucursalorigen
		UNION
		...
		".(($_GET[sucursal]==1)?"":" AND gv.idsucursalorigen = $_GET[sucursal]")." GROUP BY gv.idsucursalorigen
		UNION
		SELECT f.idsucursal AS id FROM facturacion f WHERE f.credito='SI' AND ISNULL(f.fechacancelacion) 
		".(($_GET[sucursal]==1)?"":" AND f.idsucursal = $_GET[sucursal]")." GROUP BY f.idsucursal
		UNION
		SELECT temp.sucursal AS id FROM tmp_clientes$x temp $suc GROUP BY temp.sucursal
		...
		WHERE ge.tipopago='CREDITO' AND (ge.tipoguia='CONSIGNACION' OR (ge.tipoguia='PREPAGADA' AND (ge.texcedente>0 OR ge.tseguro>0))) 
		AND ISNULL(ge.factura) ".(($_GET[sucursal]==1)?"":" AND tc.idsucursal = $_GET[sucursal]")."
		UNION
		...
		WHERE gv.condicionpago=1 AND ISNULL(gv.factura) AND gv.estado!='CANCELADO' ".(($_GET[sucursal]==1)?"":" AND pg.sucursalacobrar = $_GET[sucursal]")."
		UNION
		...
		".(($_GET[sucursal]==1)?"":" AND f.idsucursal = $_GET[sucursal]")."
		UNION
		...
		FROM tmp_clientes$x temp INNER JOIN catalogosucursal cs ON temp.sucursal=cs.id $suc
		)t1";
		...
		INNER JOIN tmp_clientes$x temp ON tc.idcliente=temp.idcliente
		WHERE ge.tipopago='CREDITO' AND (ge.tipoguia='CONSIGNACION' OR (ge.tipoguia='PREPAGADA' AND (ge.texcedente>0 OR ge.tseguro>0))) 
		AND ISNULL(ge.factura) ".(($_GET[sucursal]==1)?"":" AND tc.idsucursal = $_GET[sucursal]")." GROUP BY tc.idsucursal
		UNION
		SELECT cs.id,cs.prefijo,0 AS clientes,
		...
		INNER JOIN tmp_clientes$x temp ON pg.cliente=temp.idcliente
		WHERE gv.condicionpago=1 AND ISNULL(gv.factura) AND gv.estado!='CANCELADO'
		".(($_GET[sucursal]==1)?"":" AND pg.sucursalacobrar = $_GET[sucursal]")."
	...
	if($_GET[accion]==5){
		...
		UNION
		SELECT f.fecha,cs.prefijo AS sucursal,f.folio AS refcargo,0 AS refabono,SUM(IFNULL(f.total,0)) AS cargos,0 AS abonos,
		SUM(IFNULL(f.total,0)) AS saldo,'' AS descripcion
		FROM facturacion f 
		INNER JOIN catalogosucursal cs ON f.idsucursal=cs.id
		WHERE f.credito='SI' AND ISNULL(f.fechacancelacion) AND f.cliente=$_GET[idcliente] AND f.idsucursal='$_GET[prefijosucursal]' 
		AND f.fecha BETWEEN '$fecha1' AND '$fecha2' AND f.tipoguia!='empresarial' GROUP BY f.folio
