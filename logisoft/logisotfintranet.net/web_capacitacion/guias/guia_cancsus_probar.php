...
<script>
	...
	var botonesnuevo = '<img id="btnguardar" src="../img/Boton_Guardar.gif" style="cursor:hand" onClick="if(validarDatos() && valcreditodisponible()){ ejecutarSubmit(); };">&nbsp;&nbsp;<img src="../img/Boton_Nuevo.gif" style="cursor:hand" onClick="limpiar_evaluacion();limpiar_destinatario();limpiar_remitente();">';
	...
	var botonesguardar = '<img id="btnguardar" src="../img/Boton_Guardar.gif" style="cursor:hand" onClick="if(validarDatos() && valcreditodisponible()){ ejecutarSubmit(); };">';
	...
	function solicitarDatosConv(){
		if(document.all.sucdestino_hidden.value!=""){
			...
			+"&idsucursalorigen="+u.idsucursalorigen.value+"&valran="+Math.random());
		}
	}
	...
	function obtenerMecanciaConvenio(datos){
		...
		if(u.lstflete.value==1){
			...
			if(u.desporcobrar.value!="1" && u.desconvenio.value!="1" && valCon.restringRXCDestinatario()){
				...
				alerta3("El destinatario tiene restringido el servicio RECIBO POR COBRAR<br>");
				document.getElementById('btnguardar').style.visibility="hidden";
				return false;
				var entroRestringPorCobrar = 1;
			}
		}
		//de la sucursal
		...
		if(valCon.checarCredito(u.lstflete.value) && entroRestringPorCobrar==0){
		...
		for(var i=0; i<mensajescambios.length;i++){
			switch(mensajescambios[i][2]){
				...
				case 3:
					...
					break;
				case 4:
					consultaTexto("respuestaCredito", "guia_consulta_conv.php?accion=2&idpagina="+u.idpagina.value+"&idcliente="+((u.lstflete.value==0)?u.idremitente.value:u.iddestinatario.value));
					break;
				case 5:
					u.txtead.value = 0;
					u.txtrestrinccion.value = "";
	...
	function solicitarGuiaCan(folio){
		u = document.all;
		//window.open("guiacs_consulta.php?accion=5&idpagina="+u.idpagina.value+"&folio="+folio+"&rand="+Math.random());
		consulta("respuestaGuia","guiacs_consulta.php?accion=5&idpagina="+u.idpagina.value+"&folio="+folio+"&rand="+Math.random());
	}
	...
	function respuestaGuia(datos){
		...
		if(encon>0){
			...
			var iddirdestinatario		= datos.getElementsByTagName('iddirecciondestinatario').item(0).firstChild.data;
			var personamoral			= datos.getElementsByTagName('personamoral').item(0).firstChild.data;
			...
			u.des_direcciones.value = iddirdestinatario;
			u.des_personamoral.value= personamoral;
			...
			u.txtrecoleccion.value = (recoleccion=="0")?"":recoleccion;
	...
	function guardarCancelacion(){
		consulta("respuestaCancelacion","guiacs_consulta.php?accion=6&idpagina="+u.idpagina.value+"&folio="+document.all.folioSeleccionado.innerHTML
	...
	function cancelarFinal(){
		consulta("respuestaCancelarFinal","guiacs_consulta.php?accion=7&idpagina="+u.idpagina.value+"&folio="+document.all.folioSeleccionado.innerHTML
		+"&rand="+Math.random());
	}
	...
	function calcularservicios(){
		...
		var excedente = parseFloat(u.t_txtexcedente.value.replace("$ ","").replace(/,/g,""));
			...
			u.txtrecoleccion.value = u.txtrecoleccion.value.replace(/ /g,"");
			
			if(u.txtrecoleccion.value!='' && u.t_txtrecoleccionh.value=="0" && valCon.validaRecsucursal(valCon.validarConvenioAUsar(u.lstflete.value),u.idsucursalorigen.value)){
		...
		var txtotrosAgregado= parseFloat((u.txtotrosAgregado.value=="")?0:u.txtotrosAgregado.value.replace("$ ","").replace(/,/g,""));
		
		u.t_txtotros.value 	= templaye+tbolsaempaque+tavisocelular+tacuserecibo+tcod+txtotrosAgregado;
	...
	function pedirDatosEvaluacion(idevaluacion,idguia){
		if('<?=$_SESSION[IDSUCURSAL]?>'!=""){
			...
			consulta("devolverDatosEvaluacion", "guiacs_consulta.php?accion=1&idpagina="+u.idpagina.value+"&folio="+idevaluacion+
					 "&folioguia="+idguia+
					 "&idsucorigen="+sucursalorigen+"&idsucdestino="+u.sucdestino_hidden.value+
					 "&iddestino="+u.destino_hidden.value+"&valrandom="+Math.random());
		}else{
	...
	function devolverRemitente(valor){
		...
		if(u.folioevaluacion.value==""){
			...
			consulta("mostrarRemitente", "guiacs_consulta.php?accion=2&idpagina="+u.idpagina.value+"&idcliente="+valor+"&valrandom="+Math.random());
		}
	}
	...
	function devolverDestinatario(valor){
		var u = document.all;
		var bguia=document.getElementById('folioSeleccionado').innerHTML;
		limpiar_destinatario();
		document.all.iddestinatario.value = valor;
		
		window.open("guiacs_consulta.php?accion=2&idpagina="+u.idpagina.value+"&idcliente="+valor
		+"&poblacion="+u.npobdes.value+"&guia="+bguia+"&valrandom="+Math.random());
		
		consulta("mostrarDestinatario", "guiacs_consulta.php?accion=2&idpagina="+u.idpagina.value+"&idcliente="+valor
		+"&poblacion="+u.npobdes.value+"&guia="+bguia+"&valrandom="+Math.random());
	}
	
	function mostrarDestinatario2(datos){
		var u = document.all;
		var encon = datos.getElementsByTagName('encontro').item(0).firstChild.data;
		if(encon>0){
			var endir = datos.getElementsByTagName('encontrodirecciones').item(0).firstChild.data;
			u.iddestinatario.value		= datos.getElementsByTagName('codigo').item(0).firstChild.data;
	...
	function mostrarDestinatario(datos){
		...
		var encon = datos.getElementsByTagName('encontro').item(0).firstChild.data;
		if(encon=='a'){
			alerta("Cambio de destinatario no permitido","Destinatario","iddestinatario");
			consulta("mostrarDestinatario2", "guiacs_consulta.php?accion=2&idpagina="+u.idpagina.value+"&poblacion="+u.npobdes.value+"&guia="+bguia
	...
	function devolverDestino(){
		...
		if(u.destino_hidden.value==""){
			...
			consulta("mostrarDestino", "guiacs_consulta.php?accion=3&idpagina="+u.idpagina.value+"&iddestino="+u.destino_hidden.value);
		}
	}
	...
	function validarDatos(){
		...
		if(u.folioevaluacion.value==""){
			...
			return false;
		}else if(document.all.destino_hidden.value=='undefined' || document.all.destino_hidden.value=='' || document.all.destino_hidden.value=='0'){
			alerta('Proporcione un destino valido','¡Atención!',"destino");
			return false;
	...
	function registrarGuia(){
		...
		var pagina = "guiacs_consulta.php?accion=4&evaluacion="+evaluacion+"&fecha="+fecha+"&tipoflete="+tipoflete
		+"&idpagina="+u.idpagina.value+"&ocurre="+ocurre+"&idsucursalorigen="+idsucursalorigen+"&iddestino="+iddestino
		+"&idsucursaldestino="+idsucursaldestino+"&condicionpago="+condicionpago+"&idremitente="+idremitente+"&iddireccionremitente="+iddireccionremitente
	...
	function resGuiaGuardada(datos){
		...
		if(guardado==1){
			...
			alerta3("La guia ya fue enviada a sustitucion","¡Atencion!");
			return false;
		}else{
      ...
      <tr>
        <td valign="top"><table width="100%" height="76" border="0" cellpadding="0" cellspacing="0">
            ...
            <tr>
              ...
              <td class="Tablas">
              Otros:
      <input name="txtotrosAgregado" type="text" style="font:tahoma; font-size:9px; text-align:right; text-align:right" value="<?=$txtotros ?>" size="8" onkeypress="if(event.keyCode==13){calculartotales();}else{return tiposMoneda(event,this.value);}" onblur="calculartotales();" />
              </td>
              <td class="Tablas">&nbsp;</td>
              <td class="Tablas">&nbsp;</td>
              <td class="Tablas" align="right">&nbsp;</td>
            </tr>
        </table></td>
      </tr>
    </table>
      ...
      <tr>
        ...
        <td width="284"><table width="282" border="0" align="left" cellpadding="0" cellspacing="0">
            <tr>
			  ...
			  <img id="btnguardar" src="../img/Boton_Guardar.gif" style="cursor:hand" onClick="if(validarDatos() && valcreditodisponible()){ ejecutarSubmit(); };">
			  </td>
</tr>
        </table></td>
      </tr>
