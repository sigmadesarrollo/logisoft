<?
	...
	if($_GET[accion] == 4){
		...
		$rb->prepararDatosGuiaV($newfolio, $_GET[evaluacion], $_SESSION[IDSUCURSAL], $_SESSION[IDUSUARIO], $_GET[recoleccion]);
		...
		evaluacion='$_GET[evaluacion]', fecha=current_date, tipoflete='$_GET[tipoflete]', 
		...
		$r = @mysql_query(str_replace("''","null",$s),$l) or $rb->rollBackGuiaventanilla(0,$s,$l);
		
		
		$verifG->verificaGuiaVentanilla($newfolio);
		
		$s = "update evaluacionmercancia set estado = 'ENGUIA' 
		where folio = $_GET[evaluacion] and sucursal = ".$_SESSION[IDSUCURSAL]."";
		mysql_query($s,$l) or $rb->rollBackGuiaventanilla(2,$s,$l);
		
		
		$s = "INSERT INTO guias_valorotros
		SET guia = '$newfolio', otros='$_GET[nuevootros]', fecha=CURRENT_DATE";
		$r = @mysql_query($s,$l) or $rb->rollBackGuiaventanilla(3,$s,$l);
		...
		$r = @mysql_query(str_replace("''","null",$s),$l) or $rb->rollBackGuiaventanilla(4,$s,$l);
		
		//SE GUARDA LA EVALUACION Y LA GUIA EN EL HISTORIAL DE MOVIMIENTOS
		$s = "
		insert into historialmovimientos(modulo,folio,estado,idusuario,fechamodificacion) 
		values ('guiasventanilla','".$newfolio.",".$_GET[evaluacion]."','$estado','$_SESSION[IDUSUARIO]',CURRENT_TIMESTAMP);";
		mysql_query($s,$l) or $rb->rollBackGuiaventanilla(5,$s,$l);
		...
		if($_GET[recoleccion]!="" && $_GET[recoleccion]!="0"){
			...
			$r = @mysql_query(str_replace("''","null",$s),$l) or $rb->rollBackGuiaventanilla(6,$s,$l);
		}
		...
		$s = "call proc_RegistroAuditorias('LG','$newfolio',$_SESSION[IDSUCURSAL])";
			mysql_query($s, $l) or $rb->rollBackGuiaventanilla(7,$s,$l);
		
		
		$s = "call proc_RegistroVentas('VENTA_GUIAS_V','$newfolio',0)";
		$r = @mysql_query($s,$l) or $rb->rollBackGuiaventanilla(8,$s,$l);
		
		/*
		******************************************* ERRRORRRRR EN PROC VENTAA ********************************************
		or postError("<xml version=\"1.0\" standalone=\"yes\" encoding=\"utf-8\"> 
				<datos>
				<guardado>0</guardado>
				<consulta>".str_replace("''","null",$s)."</consulta>
				</datos>
		...
		@mysql_query(str_replace("''","null",$s),$l) or $rb->rollBackGuiaventanilla(9,$s,$l);
		}
		...
		@mysql_query(str_replace("''","null",$s),$l) or $rb->rollBackGuiaventanilla(10,$s,$l);
		...
		".(($_GET[condicionpago]==0 && $_GET[tipoflete]==0)?", pagado='S', fechapago = CURRENT_DATE, 
		usuariocobro = $_SESSION[IDUSUARIO], sucursalcobro = $_SESSION[IDSUCURSAL]":"");
		$r = @mysql_query(str_replace("''","null",$s),$l) or $rb->rollBackGuiaventanilla(11,$s,$l);
		...
		if(mysql_num_rows($r)>1){
			...
			$r = @mysql_query($s,$l) or $rb->rollBackGuiaventanilla(12,$s,$l);
		}else{
			$s = "INSERT INTO guiaventanilla_detalle 			
		...
		while($f = mysql_fetch_object($r)){
			...
			for($i=0;$i<$f->cantidad;$i++){
				...
				@mysql_query($s,$l) or $rb->rollBackGuiaventanilla(13,$s,$l);
				$cantidad++;
			}
		}
		...
		@mysql_query($s,$l) or $rb->rollBackGuiaventanilla(15,$s,$l);
		...
		if($estado == 'ALMACEN ORIGEN'){
			...
			$s = "UPDATE lasalertas SET evpegeguve=evpegeguve-1 WHERE sucursal=$_SESSION[IDSUCURSAL];";
			mysql_query($s,$l) or postError(mysql_error($l).$s);
		}
	...
	if($_GET[accion] == 6){
		...
		$s = "UPDATE lasalertas SET guloaupaca=guloaupaca+1 WHERE sucursal=$_SESSION[IDSUCURSAL];";
