...
function ObtenerConsulta(e,nombrecaja,valor){
	 ...
	 */
		...
		consulta("resultado","buscarClienteGen_con.php?accion=1"+valor+"<?=($_GET[tiposol])?"&tiposol=$_GET[tiposol]":""?>&suert="+Math.random());
		
	}
}

function ponerOnClick(valor,valordevuelto){
	//return	"<a style='cursor:hand;' onclick='pedirCliente(1); parent.VentanaModal.cerrar(); '>"+valor+"</a>";
	
	return	"<a style='cursor:hand;' onclick='parent.<?=$_GET[funcion]?>("+valordevuelto+"); if(parent.mens!=null && parent.mens.getMensaje()>-1){ parent.mens.cerrar();}else{parent.VentanaModal.cerrar();}'>"+valor+"</a>";
}
...
function resultado(datos){
	...
	if(con>0){
		for(m=0;m<con;m++){	
			...
			ponerOnClick(paterno,idcliente)+"└"+ponerOnClick(materno,idcliente)+"└"+ponerOnClick(sucursal,idcliente));
		}	
	}
}
</script>
...
<body>
  <table width="700" class="Tablas"  border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
    <tr>
    ...
    <tr>
      ...
      <table width="657" id="tablaclientes" border="0" align="left" alagregar="" alborrar="" cellpadding="0" cellspacing="0">
      	  <tr>
