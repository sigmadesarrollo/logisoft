...
<script>
	...
	function validar(){
		...
		if(u.nombrequeja.value == ""){
			...
			arr[1] = u.sucursal_hidden.value.replace(/,/g,'');
			arr[2] = u.queja.value.replace(/,/g,'');
			arr[3] = u.folioatencion.value.replace(/,/g,'');
			arr[4] = u.guia.value.replace(/,/g,'');
			arr[5] = u.recoleccion.value.replace(/,/g,'');
			arr[6] = u.nombrequeja.value.replace(/,/g,'');
			arr[7] = u.telefonoqueja.value.replace(/,/g,'');
			arr[8] = u.emailqueja.value.replace(/,/g,'');
			arr[9] = u.empresaqueja.value.replace(/,/g,'');
			arr[10] = u.observaciones.value.replace(/,/g,'').toUpperCase();
			arr[11] = u.responsable.value.replace(/,/g,'');
			arr[12] = u.supervisor.value.replace(/,/g,'');
			arr[13] = u.cliente.value.replace(/,/g,'');
			arr[14] = ((u.foliodano.value=="")?0:u.foliodano.value.replace(/,/g,''));
			arr[15] = u.iddireccion.value.replace(/,/g,'');
			if(u.accion.value==""){
				consultaTexto("registroQueja","centroAtencionTelefonica_con.php?accion=5&arre="+arr.join(",").replace(/Ñ/g,"%C3%91")
				+"&direccion2="+u.emailresponsable.value+"&direccion3="+u.emailsupervisor.value
				+"&queja="+u.queja.value+"&val="+Math.random());
			}else{
				consultaTexto("cambiosQueja","centroAtencionTelefonica_con.php?accion=6&arre="+arr.join(",").replace(/Ñ/g,"%C3%91")
				+"&folio="+u.folio.value
	...
	function mostrarFolioAtencion(datos){
		...
		if(datos.indexOf("no encontro")<0){
			...
			u.iddireccion.value = obj.principal.iddireccion;
	...
	function obtenerFolioQuejaTel2(folio){
		folio=u.folio.value;
		consultaTexto("mostrarDatosQueja","centroAtencionTelefonica_con.php?accion=9&folio="+folio);		
	}
	function mostrarDatosQueja(datos){
		...
		consultaTexto("mostrarCliente","centroAtencionTelefonica_con.php?accion=15&folio="+u.folio.value+"&valor="+Math.random());
			...
			alerta3("No existe el folio","¡Atención!");
			limpiar();
		}
	}
            ...
            <tr>
              <td width="400" height="11"><table width="599" border="0" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td width="168"><input name="folio" type="text" class="Tablas" id="folio" style="width:100px;" value="<?=$folio ?>" 
				  onkeypress="if(event.keyCode=='13'){obtenerFolioQuejaTel2(this.value);}"/>
                    ...
                    <td colspan="6"><table width="599" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        ...
                        <td width="190"><span class="Tablas">
                          <input name="cliente" type="text" class="Tablas" id="cliente" style="background:#FFFF99;width:60px" readonly="" value="<?=$_POST[cliente] ?>" onKeyPress="if(document.all.queja.value=='OTROS SERVICIOS'){obtenerCliente(event,this.value);}" onKeyUp="return validarCliente(event,this.name)" />
