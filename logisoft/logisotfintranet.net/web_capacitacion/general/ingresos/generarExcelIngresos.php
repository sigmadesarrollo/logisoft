 ...
 <Worksheet ss:Name="Hoja1">';
	...
	if($_GET[accion]==1){//REPORTE INGRESOS
		...
		FROM ( 
			...
			AND fp.procedencia='C' AND ISNULL(fp.fechacancelacion) 
			".(($_GET[sucursal]==1)?"":" AND fp.sucursal = $_GET[sucursal]")."
		...
		UNION 
			...
			WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' 
			AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) 
			".(($_GET[sucursal]==1)?"":" AND fp.sucursal = $_GET[sucursal]")."
			GROUP BY fp.sucursal
		UNION	
			SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,IFNULL(SUM(fp.efectivo),0) efectivo,
			...
			INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id
			WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' AND '".cambiaf_a_mysql($_GET[fecha2])."' 
			AND (fp.procedencia='G' AND fp.tipo='V') AND ISNULL(fp.fechacancelacion) 
			".(($_GET[sucursal]==1)?"":" AND fp.sucursal = $_GET[sucursal]")."
			GROUP BY fp.sucursal
		UNION
			SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,IFNULL(SUM(fp.efectivo),0) efectivo,
		...
		UNION	 
			...
			INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id		
			WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' AND '".cambiaf_a_mysql($_GET[fecha2])."' 
			AND (fp.procedencia='M' OR fp.procedencia='O') AND ISNULL(fp.fechacancelacion) 
			".(($_GET[sucursal]==1)?"":" AND fp.sucursal = $_GET[sucursal]")."
			GROUP BY fp.sucursal
	...
	UNION 
		...
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) 
		".(($_GET[sucursal]==1)?"":" AND fp.sucursal = $_GET[sucursal]")." GROUP BY fp.sucursal)
	UNION	/* contado */
	...
	UNION
		...
		INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND fp.procedencia='F' AND f.facturaestado='GUARDADO' AND f.credito='NO' 
		".(($_GET[sucursal]==1)?"":" AND fp.sucursal = $_GET[sucursal]")." GROUP BY fp.sucursal)
	UNION	/* entregadas */
		(SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,0 AS contado,
	...
	$s="SELECT nombresucursal,DATE_FORMAT(fecha,'%d/%m/%Y')AS fecha,guia,cliente,importe,CONCAT('#',caja) caja,CONCAT('#',factura) factura,CONCAT('#%',total) total
	FROM (
		(SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,fp.guia,
		CONCAT(cc.nombre,' ',cc.paterno,' ',cc.materno)AS cliente,fp.total AS importe,lcd.idusuario AS caja,'' AS factura,'' AS total
		FROM formapago fp
		INNER JOIN liquidacioncobranzadetalle lcd ON fp.guia=lcd.factura
		INNER JOIN liquidacioncobranza lc ON lcd.folioliquidacion=lc.folio
		INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id
	...
	UNION 
		(SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,gv.id AS guia,CONCAT(cc.nombre,' ',cc.paterno,' ',cc.materno)AS cliente,
		gv.total AS importe,gv.idusuario AS caja,a.factura,(f.total+f.sobmontoafacturar+f.otrosmontofacturar) AS total
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
	...
	UNION 
		...
		CONCAT(cc.nombre,' ',cc.paterno,' ',cc.materno)AS cliente,ge.total AS importe,ge.idusuario AS caja,'' AS factura,'' AS total 
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturadetalle fd ON a.factura=fd.factura
	...
	UNION 
		...
		CONCAT(cc.nombre,' ',cc.paterno,' ',cc.materno)AS cliente,sge.total AS importe,sge.idusuario AS caja,'' AS factura,'' AS total
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturadetalle fd ON a.factura=fd.factura
	...
	UNION 
		...
		CONCAT(cc.nombre,' ',cc.paterno,' ',cc.materno)AS cliente,f.otrosmontofacturar AS importe,f.idusuario AS caja,'' AS factura,'' AS total		
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturacion f ON a.factura=f.folio
	...
	UNION 
		(SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,f.folio AS guia,
		CONCAT(cc.nombre,' ',cc.paterno,' ',cc.materno)AS cliente,f.sobmontoafacturar AS importe,f.idusuario AS caja,'' AS factura,'' AS total	
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturacion f ON a.factura=f.folio
		...
		<Cell ss:StyleID="s24"><Data ss:Type="String">TOTAL FACT</Data></Cell>
		</Row>';
	...
	}else if($_GET[accion]==10){//CONCILIACION DE INGRESOS
	$s="SELECT nombresucursal,fecha,SUM(contado),SUM(cobranza),SUM(entregadas),(SUM(contado)+SUM(cobranza)+SUM(entregadas)) total  
	FROM ( /* cobranza */
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,0 AS contado,SUM(fp.total) AS cobranza,0 AS entregadas 
		FROM formapago fp
		INNER JOIN liquidacioncobranzadetalle lcd ON fp.guia=lcd.factura
		INNER JOIN liquidacioncobranza lc ON lcd.folioliquidacion=lc.folio
		INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id		
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND fp.procedencia='C' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.fecha
	UNION 
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,0 AS contado,SUM(gv.total) AS cobranza,0 AS entregadas
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturadetalle fd ON a.factura=fd.factura
		...
		INNER JOIN catalogocliente cc ON fp.cliente=cc.id
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' AND '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.fecha
	UNION 
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,0 AS contado,SUM(ge.total) AS cobranza,0 AS entregadas
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturadetalle fd ON a.factura=fd.factura
		...
		AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.fecha
	UNION 
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,0 AS contado,SUM(sge.total) AS cobranza,0 AS entregadas
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturadetalle fd ON a.factura=fd.factura
		...
		AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.fecha
	UNION 
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,0 AS contado,SUM(f.otrosmontofacturar) AS cobranza,0 AS entregadas
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturacion f ON a.factura=f.folio
		...
		INNER JOIN catalogocliente cc ON fp.cliente=cc.id
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' AND '".cambiaf_a_mysql($_GET[fecha2])."'  
		AND f.otrosmontofacturar>0 AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.fecha
	UNION 
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,0 AS contado,SUM(f.sobmontoafacturar) AS cobranza,0 AS entregadas
		FROM formapago fp 
		INNER JOIN abonodecliente ac ON fp.guia=ac.folio AND ac.idsucursal=fp.sucursal
		INNER JOIN abonodecliente_facturas a ON ac.id=a.folioabono
		INNER JOIN facturacion f ON a.factura=f.folio
		...
		INNER JOIN catalogocliente cc ON fp.cliente=cc.id
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' AND '".cambiaf_a_mysql($_GET[fecha2])."'  
		AND f.sobmontoafacturar>0 AND fp.procedencia='A' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.fecha
	UNION	/* contado */
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,SUM(gv.total) AS contado,0 AS cobranza,0 AS entregadas 
		FROM formapago fp
		INNER JOIN guiasventanilla gv ON fp.guia=gv.id
		INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND (fp.procedencia='G' AND fp.tipo='V') AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.fecha
	UNION
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,SUM(fp.total) AS contado,0 AS cobranza,0 AS entregadas 
		FROM formapago fp 
		INNER JOIN facturacion f ON fp.guia=f.folio
		INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' and '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND fp.procedencia='F' AND f.facturaestado='GUARDADO' AND f.credito='NO' AND cs.id='".$_GET[sucursal]."' GROUP BY fp.fecha
	UNION	/* entregadas */
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,0 AS contado,0 AS cobranza,SUM(fp.total) AS entregadas 
		FROM formapago fp 
		INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id		
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' AND '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND fp.procedencia='M' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.fecha
	UNION
		SELECT cs.id AS sucursal,cs.prefijo AS nombresucursal,fp.fecha,0 AS contado,0 AS cobranza,SUM(fp.total) AS entregadas
		FROM formapago fp
		INNER JOIN catalogosucursal cs ON fp.sucursal=cs.id
		WHERE fp.fecha BETWEEN '" .cambiaf_a_mysql($_GET[fecha])."' AND '".cambiaf_a_mysql($_GET[fecha2])."' 
		AND fp.procedencia='O' AND ISNULL(fp.fechacancelacion) AND cs.id='".$_GET[sucursal]."' GROUP BY fp.fecha	
