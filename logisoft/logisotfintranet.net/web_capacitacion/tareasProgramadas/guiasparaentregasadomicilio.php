...
<script>
	...
	tabla1.setAttributes({
		...
		campos:[
			...
			{nombre:"SUC. DEST.", medida:50, alineacion:"center",  datos:"sucursaldestino"},
			{nombre:"REMITENTE", medida:170, alineacion:"left", datos:"remitente"},
			{nombre:"DESTINATARIO", medida:170, alineacion:"left", datos:"destinatario"},
			{nombre:"DIRECCION DEST", medida:150,  alineacion:"left", datos:"direcciondest"},
			{nombre:"SECTOR", medida:50,  alineacion:"center", datos:"sector"},
			{nombre:"TIPO PLETE", medida:60, alineacion:"left", datos:"tipoflete"},
			{nombre:"IMPORTE TOTAL", medida:70, alineacion:"right", tipo:"moneda", datos:"importetotal"}
	...
	function obtenerDetalle(){
		consultaTexto("resTabla1","consultas_con.php?accion=3&contador="+u.pag1_contador.value
		+"&s="+Math.random());
	}
	...
	function paginacion(movimiento){
		switch(movimiento){
			case 'primero':
				consultaTexto("resTabla1","consultas_con.php?accion=3&contador=0&s="+Math.random());
				break;
			case 'adelante':
				if(u.pag1_adelante.value==1){
					consultaTexto("resTabla1","consultas_con.php?accion=3&contador="+(parseFloat(u.pag1_contador.value)+1)					
					+"&s="+Math.random());
				}
				break;
			case 'atras':
			...
			case 'ultimo':
				...
				consultaTexto("resTabla1","consultas_con.php?accion=3&contador="+contador
				+"&s="+Math.random());
				break;
		}
	}
	...
	function imprimirReporte(){
		if(tabla1.getRecordCount()>0){
			if(document.URL.indexOf("web_capacitacionPruebas/")>-1){		
				var v_dir = "https://www.pmmintranet.net/web_capacitacionPruebas/tareasProgramadas/";
			}else if(document.URL.indexOf("web_capacitacion/")>-1){
				var v_dir = "https://www.pmmintranet.net/web_capacitacion/tareasProgramadas/";
			}else if(document.URL.indexOf("web_pruebas/")>-1){
				var v_dir = "https://www.pmmintranet.net/web_pruebas/tareasProgramadas/";
			}
			window.open(v_dir+"guiaseadExcel.php");
		}else{
...
<form id="form1" name="form1" method="post" action="">
  <table width="800" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
    <tr>
      <td class="FondoTabla"> GUIAS PARA EAD </td>
    </tr>
    <tr>
      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td><div id="txtDir" style=" height:280px; width:799px; overflow:auto" align="left">
				<table id="detalle" width="100%" border="0" cellspacing="0" cellpadding="0"></table>
			</div>
			</td>
          </tr>
          <tr>
          ...
          <tr>
            <td align="right"><div class="ebtn_imprimir" onclick="imprimirReporte()" style="margin-right:5px; margin-bottom:5px"></div></td>
          </tr>
      </table></td>
    </tr>
  </table>
