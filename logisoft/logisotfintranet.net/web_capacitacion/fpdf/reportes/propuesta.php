<?	
	...
	class pdf extends FPDF{
		...
		function Header(){
			...
			$this->Cell(200,10,'PROPUESTA DE PRESTACION DE SERVICIOS',0,0,'C');			
			$this->Ln(5);
	...
	$s = "SELECT gc.folio, 0 consumomensual, DATE_FORMAT(gc.fecha, '%d/%m/%Y') AS fecha, 
	DATE_FORMAT(gc.vigencia, '%d/%m/%Y') AS vigencia,
	cs.descripcion AS sucursal, gc.nvendedor, gc.idprospecto idcliente, gc.rfc, 
	...
	if($f->precioporcaja==1){
		...
		$s = "SELECT CONCAT('ZONA ',kmi,'-',kmf) zonas FROM convenio_configurador_caja WHERE tipo='CONVENIO' AND idconvenio = '$_GET[folio]' GROUP BY zona";
		$rx = mysql_query($s,$l) or die($s);
		...
		$s = "SELECT descripcion FROM convenio_configurador_caja 
		WHERE tipo='CONVENIO' AND idconvenio =  '$_GET[folio]' 
		...
		while($fz = mysql_fetch_object($rz)){
			...
			$data2[$filas] = array();
			$s = "SELECT precio
			FROM convenio_configurador_caja WHERE tipo='CONVENIO' 
	...
	$s = "SELECT servicio, cobro, precio FROM convenio_servicios WHERE idconvenio = $_GET[folio] and tipo = 'CONVENIO'";
	$rx = mysql_query($s,$l) or die($s);
	...
	if($f->consignacioncaja==1){
		...
		$s = "SELECT CONCAT('ZONA ',kmi,'-',kmf) zonas FROM convenio_configurador_caja WHERE tipo='CONSIGNACION' AND idconvenio = '$_GET[folio]' GROUP BY zona";
		$rx = mysql_query($s,$l) or die($s);
		$columnasde = mysql_num_rows($rx)/2;
		$contacolum = 0;
		...
		while($fz = mysql_fetch_object($rz)){
			...
			$data2[$filas] = array();
			$s = "SELECT precio
			FROM convenio_configurador_caja WHERE tipo='CONSIGNACION' 
			AND idconvenio = '$_GET[folio]' and descripcion = '$fz->descripcion'";
			$rx = mysql_query($s,$l) or die($s);
			$columnasde = mysql_num_rows($rx)/2;
			$contacolum = 0;
	...
	$s = "SELECT servicio, cobro, precio FROM convenio_servicios WHERE idconvenio = $_GET[folio] and tipo = 'CONSIGNACION'";
	$rx = mysql_query($s,$l) or die($s);
	if(mysql_num_rows($rx)>-1){
		$numpunto++;
		$pdf -> SetFont("Arial","",10);
	...
	Este convenio se celebra en la ciudad de '.$f->poblacion.','.$f->estado.'  el '.$f->fecha.' , con vencimiento el '.$f->vigencia.'. Sujeto a revisión y aprobación del Departamento de Ventas corporativas en un lapso 72 horas hábiles.',0,'J');
