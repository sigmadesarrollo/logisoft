<?	session_start();
	/*if(!$_SESSION[IDUSUARIO]!=""){
		...
		if($accion == ""){
			$row	=folio('catalogobanco','webpmm');
			$codigo	=$row[0];
		}
		
		if($accion == "grabar"){
			$s	= "INSERT INTO catalogobanco SET
			descripcion = UCASE('$descripcion'), usuario=UCASE('".$_SESSION[NOMBREUSUARIO]."'),
			...
			$sqlupd	=mysql_query("UPDATE catalogobanco SET descripcion=UCASE('$descripcion'), 
			usuario=UCASE('".$_SESSION[NOMBREUSUARIO]."'), fecha=current_timestamp() 
			WHERE id='$codigo'",$link);
...
abrirVentanaFija('CatalogoBanco_Buscar.php', 550, 450, 'ventana', 'Busqueda');
	}
});
</script>
  ...
  <table width="100%" border="0">
    ...
    <tr>
      <td><table width="310" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
        <tr>
          <td class="FondoTabla">CAT&Aacute;LOGO DE BANCOS</td>
        </tr>
        <tr>
          <td><br><table width="309" border="0" align="center">
            ...
            <tr>
              ...
              <td width="159">
                ...
                &nbsp;<img src="../../img/Buscar_24.gif" alt="Buscar" width="24" height="23" align="absbottom" style="cursor:pointer" onClick="abrirVentanaFija('CatalogoBanco_Buscar.php', 550, 450, 'ventana', 'Busqueda')"></td>
              <td width="137">&nbsp;</td>
              </tr>
            <tr>
...
<script>
	parent.frames[1].document.getElementById('titulo').innerHTML = 'CATÁLOGO BANCOS';
</script>
<? 
if ($mensaje!=""){
