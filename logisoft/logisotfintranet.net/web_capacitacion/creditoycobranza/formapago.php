<?
	...
	if(ereg(",",$_GET[factura]))
		$where = "WHERE folio in(".$_GET[factura].")";
	else
		$where = "WHERE folio = ".$_GET[factura]."";
	...
	$rpc = mysql_query("SELECT pagocheque FROM catalogocliente WHERE id = '".$_GET[cliente]."'",$l);
	$spc = mysql_fetch_object($rpc);
	...
	$sq = "SELECT sum(total + sobmontoafacturar + otrosmontofacturar) AS total
...
<script>
	...
	function verificarCheque(valor){
		u.efectivo.readOnly = valor
		u.efectivo.style.backgroundColor=(valor)?"#FFFF99":"";
		u.tarjeta.readOnly = valor
		u.tarjeta.style.backgroundColor=(valor)?"#FFFF99":"";
		u.transferencia.readOnly = valor
		u.transferencia.style.backgroundColor=(valor)?"#FFFF99":"";
		document.getElementById('imgnota').style.visibility=(valor)?"hidden":"";
		u.banco.value = "";
		u.ncheque.value = "";
		u.banco.disabled = !valor;
		u.ncheque.readOnly = !valor;
		u.ncheque.style.backgroundColor=(!valor)?"#FFFF99":"";
		if(valor){
			u.cheque.value = u.total.value;
			u.efectivo.value = "";
			u.tarjeta.value = "";
	...
	function calcular(){
		...
		if(acumulado>total && document.getElementById('chk_cheque').checked==false){
			u.cambio.value 	=   "$ "+numcredvar((Math.round((acumulado-total)*100)/100).toLocaleString());	
		}else{
			u.cambio.value 	=   "$ 0.00";
		}
	}	
	...
	function validarTotales(){
		...
		if(u.total.value != u.acumulado.value){
			...
			up = parent.document.all;	
				...
				parent.<?=$_GET[funcion] ?>(<?=$_GET[factura]?>);
    ...
    <table width="405" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#016193">
          ...
          <td height="147"><br><table width="362" border="0" align="center" cellpadding="0" cellspacing="0">
                  <tr>
                    ...
                    <td><input tabindex="0" onKeyPress="return Numeros(event)" onKeyDown="return tabular(event)" name="total" type="text" id="total" class="Tablas"  style="font:tahoma; font-size:9px; background:#FFFF99; text-align:right" readonly="true" value="<?=$f->total ?>" size="15"   ></td>
                    <td class="Tablas">Pagar Cheque:</td>
                    <td><input type="checkbox" onClick="verificarCheque(this.checked)" name="chk_cheque" id="chk_cheque" value="" <? if($spc->pagocheque==1){echo "disabled='true'";} ?>></td>
                  </tr>
                  <tr>
                    <td class="Tablas">Efectivo:</td>
                    <td width="120"><input name="efectivo" tabindex="1" type="text" class="Tablas"  style="font:tahoma; font-size:9px; text-align:right" onFocus="enfocar(this)" onBlur="desenfocar(this)" onKeyPress="return Numeros(event)" onKeyDown="if(event.keyCode==13){calcular(); return tabular(event);}" value="<?=$efectivo ?>" size="15" maxlength="15"  ></td>
                    <td width="84" class="Tablas">Cheque:</td>
                    <td width="100"><input name="cheque" readonly tabindex="4" type="text" class="Tablas"  style="font:tahoma; font-size:9px; text-align:right;background:#FFFF99; text-align:right" onFocus="enfocar(this)" onBlur="desenfocar(this)" onKeyPress="return Numeros(event)" onKeyDown="if(event.keyCode==13){validarCheque(this); calcular(); return tabular(event);}" value="<?=$cheque ?>" size="15" maxlength="15" ></td>
                  </tr>
                  <tr>
                    ...
                    <td><select name="banco" id="banco" tabindex="5" disabled style="font:tahoma; font-size:9px; text-align:right;text-align:right;width:100px;" onKeyDown="if(event.keyCode==13){return tabular(event);}">
                  ...
                  <tr>
                  	...
                  	<td><input name="folio" tabindex="7" type="text" class="Tablas" id="folio" onKeyDown="if(event.keyCode==13){return tabular(event);}" style="width:70px;background:#FFFF99" value="<?=$folio ?>" readonly=""/>
               	    <img src="../img/Buscar_24.gif" id="imgnota" width="24" height="23" align="absbottom" onClick="abrirVentanaFija('../buscadores_generales/buscarFolionotacredito.php?cliente=<?=$_GET[cliente] ?>&funcion=obtenerFolionotascredito', 300, 300, 'ventana', 'Busqueda')" /></td>
                  ...
                  <input name="accion" type="hidden" id="accion" value="<?=$accion ?>">
                        ...
                        <table width="83" border="0" align="right">
                          <tr>
                            <td width="77"><img src="../img/Boton_Aceptar.gif" alt="E" width="70" height="20" style="cursor:pointer" onClick="validarTotales()"></td>
</tr>
                    </table></td>
                  </tr>
              </table><br></td>
