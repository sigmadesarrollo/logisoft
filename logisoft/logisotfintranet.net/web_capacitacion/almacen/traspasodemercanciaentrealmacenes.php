	...
	for($i=0;$i<$registros;$i++){
		...
		if($valor==0 && $cobrar=='si'){		
			...
			if($f->ead==0){
				...
				INNER JOIN ".((substr($_POST["tabladetalle_No_Guia"][$i],0,3)=="999")?"guiasempresariales":"guiasventanilla")." AS gv ON '".$_POST["tabladetalle_No_Guia"][$i]."' = gv.id 
				...
				$arre_cliente['informacion']['name'] = $fdfw->clientefacturacion;
				$arre_cliente['informacion']['rfc'] = $fdfw->rfc;
				$arre_cliente['informacion']['street'] = $fdfw->calle;
				//$arre_cliente['informacion']["inside_number"] = "";
				$arre_cliente['informacion']["outside_number"] = $fdfw->numero;
				$arre_cliente['informacion']["col"] = $fdfw->colonia;
				$arre_cliente['informacion']["cp"] = $fdfw->codigo;
				# de la facturacion
				$arre_cliente['informacion']["municipio"] = $fdfw->municipio;
				$arre_cliente['informacion']["state"] = $fdfw->estado;
				...
				if($tipoguia=='V'){
					$s = "insert into facturacion (tipoguia, idsucursal, facturaestado, credito, cliente, nombrecliente, apellidopaternocliente,
					...
					select 'empresarial','$_SESSION[IDSUCURSAL]','GUARDADO', '$credito', cc.id,cc.nombre, cc.paterno,
				...
				$sx = "SELECT otroscantidad, otrosdescripcion, otrosimporte, otrossubtotal, otrosiva, otrosivaretenido, otrosmontofacturar
				FROM facturacion WHERE folio = $nfact";
				$rmo = mysql_query($sx,$link) or die($sx);
				$fmo = mysql_fetch_object($rmo);
				
				
				$var_subtotal 			+= $fmo->otrossubtotal;
				$var_iva 				+= $fmo->otrosiva;
				$var_ivaretenido		+= $fmo->otrosivaretenido;
				$var_total				+= $fmo->otrosmontofacturar;
				
				$arre_cliente['producto'][$var_contadordetalle]['preciounitario'] = $fmo->otrosimporte;
				$arre_cliente['producto'][$var_contadordetalle]['descripcion'] = "TRASPASO OCURRE A EAD DE GUIA ".$_POST["tabladetalle_No_Guia"][$i];
				$arre_cliente['producto'][$var_contadordetalle]['cantidad'] = 1;
				$arre_cliente['producto'][$var_contadordetalle]['importe'] = $fmo->otrossubtotal;
