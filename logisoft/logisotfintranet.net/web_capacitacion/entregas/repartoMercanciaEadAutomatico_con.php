<?
	...
	if($_GET[accion]==1){
		...
		from catalogoempleado where id = $_GET[idempleado] AND (puesto = 4 or puesto = 57) and sucursal=".$_SESSION[IDSUCURSAL]."";
	...
	if($_GET[accion]==2){
		...
		gvu.codigobarras, gvu.estado, gvu.peso, if(gvu.proceso='ENRUTADA','SI','NO'), '$_SESSION[IDUSUARIO]', '$_SESSION[IDSUCURSAL]'
		...
		inner join guiaventanilla_unidades as gvu on gv.id = gvu.idguia
			 where gvu.unidad = '$_GET[unidad]'
		UNION
		SELECT null, ge.id, cs.descripcion, ge.fecha, geu.paquete, geu.depaquetes, 
		geu.codigobarras, geu.estado, geu.peso, if(geu.proceso='ENRUTADA','SI','NO'), '$_SESSION[IDUSUARIO]', '$_SESSION[IDSUCURSAL]'
	...
	if($_GET[accion]==10){		
		...
		CONCAT('EN REPARTO EAD A ',IF(g.incompleta='S',' INCOM',''),IF(g.danos='S',' DAÑO','')), 
		...
		INNER JOIN tmp_seleccionadas tp ON g.id = tp.folioguia
		...
		$s="insert into repartomercanciadetalle 
			...
			where rg.idusuario = $_SESSION[IDUSUARIO]
	...
	if($_GET[accion]==11){//Buscar Folio repartoMercanciaEad.php
		...
		$principal = "";
		$s="SELECT rmead.folio AS id,DATE_FORMAT(rmead.fecha,'%d/%m/%Y')AS fecha,cu.numeroeconomico unidad,rmead.conductor1,rmead.conductor2,rmead.sector,
			...
			INNER JOIN catalogounidad cu ON rmead.unidad = cu.id
