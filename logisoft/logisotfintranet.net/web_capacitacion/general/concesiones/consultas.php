<?	session_start();
	...
	if($_GET[accion]=="0"){
		...
		".((!empty($_GET[fechainicio]))? " AND h.fecha BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' 
		AND '".cambiaf_a_mysql($_GET[fechafin])."'" : " AND h.fecha <= '".cambiaf_a_mysql($_GET[fechafin])."' ")." AND gv.idsucursalorigen!=gv.idsucursaldestino
		AND cs.concesion!=0 AND NOT ISNULL(cs.concesion) AND h.sucursal=".$_GET[sucursal]." GROUP BY cs.id) AS gv";
		mysql_query($s,$l) or die($s);
		...
		GROUP BY ge.id 
		UNION /*canceladas*/
		SELECT NULL,NULL,$_GET[sucursal],'R' AS tipo,(gv.id) guia,(gv.fecha) fechaguia,(gv.tflete*-1) flete,(gv.ttotaldescuento*-1) descuento,
		((gv.tflete+gv.texcedente)-gv.ttotaldescuento)*-1 fleteneto,((((gv.tflete+gv.texcedente)-gv.ttotaldescuento)*(cs.recibido/100))*-1) comision,
		(gv.trecoleccion)*-1 recoleccion,0 AS comisionrad,(gv.tcostoead)*-1 entrega,((gv.tcostoead*(cs.porcead/100))*-1) comisionead,(gv.texcedente)*-1 sobrepeso,
		...
		GROUP BY ge.id 
		UNION
		SELECT NULL,NULL,$_GET[sucursal],'V' AS tipo,(gv.id) guia,(gv.fecha) fechaguia,(gv.tflete*-1) flete,(gv.ttotaldescuento*-1) descuento,
		((gv.tflete+gv.texcedente)-gv.ttotaldescuento)*-1 fleteneto,((((gv.tflete+gv.texcedente)-gv.ttotaldescuento)*(cs.recibido/100))*-1) comision,
		(gv.trecoleccion*-1) recoleccion,0 AS comisionrad,(gv.tcostoead*-1) entrega,((gv.tcostoead*(cs.porcead/100))*-1) comisionead,(gv.texcedente*-1) sobrepeso,
		(((((gv.tflete+gv.texcedente)-gv.ttotaldescuento)*(cs.recibido/100))+(gv.tcostoead*(cs.porcead/100)))*-1) total,(gv.total)*-1 tgral,
		CONCAT(IF(gv.tipoflete=0,'PAGADA','POR COBRAR'),'-',IF(gv.condicionpago=0,'CONTADO','CREDITO')) condicion,gv.estado,$_SESSION[IDUSUARIO]
		...
		if(mysql_num_rows($r)>0){
			...
			$r = mysql_query($s,$l) or die($s);
			$f = mysql_fetch_object($r);
			$totales = str_replace('null','""',json_encode($f));
				
			echo "({principal:$principal,tabla1:$tabla1,totales:$totales})";
		}else{
			echo "no encontro";
		}	
