...
<script>
	...
	function ValidarGuardar(tipo){
		...
		if(u.folio.value==""){
			...
			var datosenviados = "accion=4&idreparto="+u.folio.value+"&entregadas="+u.entregadas.value+
...
cobContad += parseFloat((obj.detalle[i].tipoflete != "PAGADO" && obj.detalle[i].condicionpago == "CONTADO" && obj.detalle[i].estado == "ENTREGADA POR LIQUIDAR")?1:0);
	...
	function actualizarDatosFormaPago(efectivo,cheque,ncheque,banco,nnotacredito,notacredito,guia){
		consultaTexto("MostrarGuardarDatosFormasPago","liquidaciondemercancia_con.php?accion=8&efectivo="+efectivo+"&cheque="+cheque+"&ncheque="+ncheque+"&banco="+banco+"&nnotacredito="+nnotacredito+"&notacredito="+notacredito+"&guia="+guia+"&valram="+Math.random());
	}
