<? 	session_start();
	...
	$sql=mysql_query("SELECT CONCAT('$prefijo','-',LPAD((SELECT ifnull(MAX(folio),0)+1 FROM embarquedemercancia),5,'0'))",$link);
	...
	function moverAlaDerecha(){
		...
		if(tabla1.getRecordCount()==""){
			...
			if(tabla1.getSelectedIdRow()!=""){
				...
				consultaTexto("resMovADerC","embarquedemercancia_con.php?accion=4&folio="+tabla1.getSelectedRow().guia+"&sid="+Math.random());
	...
	function moverAlaizquierda(){
		...
		if(tabla2.getRecordCount()==""){
			...
			if(tabla2.getSelectedIdRow()!=""){
				...
				consultaTexto("resMovAIzqC","embarquedemercancia_con.php?accion=6&folio="+tabla2.getSelectedRow().guia+"&sid="+Math.random());
...
/*******************/
	...
	function ObtDetalleIzq(){
		...
		if(tabla1.getValSelFromField('guia','No_GUIA')!=""){
			...
			consultaTexto("mostrarIzqAbajo","embarquedemercancia_con.php?accion=2&folio="+tabla1.getValSelFromField('guia','No_GUIA')+"&sid="+Math.random());
...
/****************/
	...
	function ObtDetalleDer(){
		...
		if(tabla2.getValSelFromField('guia','No_GUIA')!=""){
			...
			consultaTexto("mostrarDerAbajo","embarquedemercancia_con.php?accion=3&folio="+tabla2.getValSelFromField('guia','No_GUIA')+"&sid="+Math.random());
...
/********************/
		...
		//alerta3("embarquedemercancia_con.php?accion=9&unidad="+unidad+"&recorrido="+recorrido
			...
			//	+"&ruta="+ruta+"&folios="+tabla2.getValuesFromField('guia')+"&mathrand="+Math.random(),"");
					...
					consultaTexto("resGuardar","embarquedemercancia_con.php?accion=9&unidad="+unidad
					 ...
					 +"&ruta="+ruta+"&tipoembarque=NORMAL&folios="+tabla2.getValuesFromField('guia')+"&foliobitacora="+foliobitacora+"&mathrand="+Math.random());
	...
	function imprimirEmbarque(){
		window.open("http://www.pmmintranet.net/web/embarque/reporteEmbarque.php?sucursal=<?=$_SESSION[IDSUCURSAL]?>&folio="+
		+"&unidad="+u.unidad.value);
	}

	function cargarEmbarque(folio){
		consultaTexto("resCargarEmbarque","embarquedemercancia_con.php?accion=10&folio="+folio);
	}
    ...
    <td><table width="618" border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <table width="618" border="0" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <td width="483"><div class="ebtn_buscar" onclick="abrirVentanaFija('../buscadores_generales/buscarUnidadGen.php?funcion=obtenerUnidadBusqueda&validarconbitacora=1', 600, 500, 'ventana', 'Busqueda');"></div><input type="hidden" name="guardado" />
      ...
      <tr>
        ...
        <td align="center"><input name="accion" type="hidden" id="accion" />
          ...
          <td width="79" id="celdaGuardar"><div id="d_guardar" class="ebtn_guardar" onClick="guardarValores()"></div> <div id="d_imprimir" class="ebtn_imprimir" onClick="imprimirEmbarque()" style="visibility:hidden"></div></td>
