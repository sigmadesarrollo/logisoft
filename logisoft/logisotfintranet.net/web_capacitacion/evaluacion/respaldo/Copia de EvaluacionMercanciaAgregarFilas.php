<?	session_start();
	...
	$usuario=$_GET['usuario']; $fechahora=$_GET['fechahora']; $cantidad=$_POST['cantidad']; $descripcion=$_POST['descripcion']; $contenido=$_POST['contenido']; $peso=$_POST['peso']; $largo=$_POST['largo']; $ancho=$_POST['ancho']; $alto=$_POST['alto']; $volumen=$_POST['volumen']; $peso=$_POST['peso']; $pesototal=$_POST['pesototal']; $pesobascula=$_GET['pesobascula'];

	if($tipo=="modificar"){

		$id=$_GET['id'];

		$sql=mysql_query("SELECT * FROM evaluacionmercanciadetalletmp WHERE id='$id'",$link);

		$row=mysql_fetch_array($sql);

		$cantidad=$row[1]; $descripcion=$row[2]; $contenido=$row[3]; $peso=$row[4]; $largo=$row[5]; $ancho=$row[6]; $alto=$row[7]; $volumen=$row[8];		
...
if(document.getElementById('cantidad').value==""){
	...
	alerta2('','Los datos han sido agregados satisfactoriamente','cantidad');	
...
function conectar() {
	...
	//form1.texto.value += "Conectado\n"

}

function desconectar() { objSerie.PortOpen = false; }

function enviar() {

	var cadena = form1.cadena.value;

	objSerie.Output = cadena;

	//form1.texto.value += "local: " + cadena + "\n";
...
function comunicacion() {
	...
	if ( objSerie.CommEvent == 2 ) {//si estamos recibiendo datos

		var cadena = objSerie.Input;

		if(cadena.substr(0,2)!="kg"){

		//form1.texto.value += "remoto:"+ cadena + "\n";

		peso=cadena; alert(peso);		
...
<script language="javascript" for="objSerie" event="OnComm">comunicacion()</script>

</head>



<BODY onLoad="document.all.cantidad.focus();" onUnload="desconectar();" style="margin:1px 0 0 10">



<OBJECT id="objSerie" classid="clsid:648A5600-2C6E-101B-82B6-000000000014" VIEWASTEXT>
    ...
    <tr bgcolor="dee3d5">
      ...
      <td ><table width="300" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
          ...
          <td width="67" class="Tablas">Cantidad:</td>

          <td colspan="3" class="Tablas"><label>

            <input name="cantidad" type="text" class="Tablas" id="cantidad" value="<?=$cantidad ?>" size="5" onKeyPress="return Numeros(event)" onKeyDown="return tabular(event,this)" />
        ...
        <tr>
          ...
          <td colspan="3" class="Tablas"><input name="descripcion" type="text" class="Tablas" id="descripcion" style="font-size:9px; text-transform:uppercase" onFocus="foco(this.name)" onKeyUp="ajax_showOptions(this,'getCountriesByLetters',event,'ajax-list-descripcion.php')" onBlur="document.getElementById('oculto').value=''" onKeyPress="return tabular(event,this)" value="<?=$descripcion ?>" size="30" maxlength="50" />
        ...
        <tr>
          ...
          <td colspan="3" class="Tablas"><input name="contenido" type="text" class="Tablas" id="contenido" value="<?=$contenido ?>" size="38" onKeyPress="return tabular(event,this)" style="text-transform:uppercase; font:tahoma" autocomplete="array:concep" />           </td>
        ...
        <tr>
          ...
          <td width="83" class="Tablas"><input name="peso" type="text" class="Tablas" id="peso" value="<?=$peso ?>" size="10" onKeyPress="return Numeros(event)" onKeyDown="CalcularUnitario(event); return tabular(event,this)" /></td>

          <td width="48" class="Tablas">Largo:</td>

          <td width="77" class="Tablas"><input name="largo" type="text" class="Tablas" id="largo" value="<?=$largo ?>" size="7" onKeyPress="return Numeros(event)" onKeyDown="return tabular(event,this)" />
        ...
        <tr>
          ...
          <td class="Tablas"><input name="ancho" type="text" class="Tablas" id="ancho" value="<?=$ancho ?>" size="8" onKeyPress="return Numeros(event)" onKeyDown="return tabular(event,this)" />

          cm</td>

          <td class="Tablas">Alto:</td>

          <td class="Tablas"><input name="alto" type="text" class="Tablas" id="alto" value="<?=$alto ?>" size="7" onKeyPress="return Numeros(event)" onKeyDown="return tabular(event,this)" onBlur="CalcularVolumen()" />
        ...
        <tr>
          ...
          <td colspan="4" class="Tablas">Peso Total: 

            <input name="pesototal" type="text" class="Tablas" id="pesototal" value="<?=$pesototal ?>" size="7" readonly="" style="background:#FFFF99" />

            &nbsp;

            Peso Volumétrico:

            <input name="volumen" type="text" class="Tablas" id="volumen" value="<?=$volumen ?>" size="9" readonly="" style="background:#FFFF99" /></td>

          </tr>

        <tr>

          <td colspan="3" class="Tablas"><input name="text" type="text" id="cadena" onKeyPress="if (event.keyCode==13) enviar();" value=""></td>
        ...
        <tr>
          ...
          <td colspan="4"><input name="fechahora" type="hidden" id="fechahora" value="<?=$fechahora ?>">
