<?	session_start();
	...
	$result=mysql_query("SELECT descripcion FROM contenidos",$link) or die("error");
...
?>
<OBJECT ID="clsBalanza" style="display:none"
CLASSID="CLSID:9FFC4EAB-D1CB-4E7B-8582-A8DD90AD57E5">
</OBJECT>

<OBJECT ID="cPeso" style="display:none"
...
<script src="../evaluacion/js/ajax.js"></script> 
...
<script>
	...
	function Validar(){
		if(document.getElementById('cantidad').value==""){
		...
		}else if(document.getElementById('iddescripcion').value==undefined || document.getElementById('descripcion').value==""){ 
			...
			arr[3] = u.contenido.value;
			arr[4] = u.peso.value;
			arr[5] = u.largo.value;
			arr[6] = u.alto.value;
			arr[7] = u.ancho.value;
			arr[8] = u.volumen.value;
			arr[9] = u.pesototal.value;
			arr[10] = ((u.pesounit.checked==true)? 1 : 0);			
			if(u.esmodificar.value == ""){
				consultaTexto("registroEvaluacion","correointerno_con.php?accion=2&arre="+arr+"&contenido="+arr[3]);
			}else{
				consultaTexto("modificarEvaluacion","correointerno_con.php?accion=6&arre="+arr+"&contenido="+arr[3]+"&fecha="+u.fechahora.value);
			}
...
<script type="text/javascript" src="../evaluacion/js/ajax-dynamic-list.js"></script>
<link href="../evaluacion/FondoTabla.css" rel="stylesheet" type="text/css" />
      ...
      <td ><table width="330" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td colspan="4" class="Tablas"><label>
            <input name="cantidad" type="text" class="Tablas" id="cantidad" onKeyPress="if(event.keyCode==13){ document.all.descripcion.focus();} return solonumeros(event);" value="<?=$cantidad ?>" size="5" maxlength="5" />
