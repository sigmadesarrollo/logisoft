<?
	...
	if($_GET[accion] == 2){
		...
		$s = "delete from guiasventanilla_cs WHERE id='$_GET[folio]'; ";
		mysql_query($s,$l) or die("<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
				<datos>
				<guardado>0</guardado>
				<consulta>$s</consulta>
		...
		$s = "delete from guiaventanilla_detalle_cs WHERE idguia='$_GET[folio]'; ";
		mysql_query($s,$l) or die("<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
				<datos>
				<guardado>0</guardado>
				<consulta>$s</consulta>
	...
	if($_GET[accion] == 3){
		...
		mysql_query($s,$l) or die("<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
			...
			$s = "DELETE FROM guiasventanilla_cs WHERE id='$_GET[idgcys]';";
			mysql_query($s,$l) or die("<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
			<datos>
			<guardado>0</guardado>
			<consulta>$s</consulta>
			...
			$s = "DELETE FROM guiaventanilla_detalle_cs WHERE idguia='$_GET[idgcys]';";
			mysql_query($s,$l) or die("<xml version=\"1.0\" standalone=\"yes\" encoding=\"iso-8859-1\"> 
			<datos>
			<guardado>0</guardado>
			<consulta>$s</consulta>
		...
		$s = "SELECT sucursal FROM guiasventanilla_cs WHERE id=$_GET[folio];";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
		$s = "UPDATE lasalertas SET gufoaupasu=gufoaupasu+1 WHERE sucursal=$f->sucursal;";
		mysql_query($s,$l) OR die(mysql_error($l).$s);
	}
	
	if($_GET[accion] == 4){
		$s = "select folioguia, idsucursalorigen, idsucursaldestino, condicionpago from guiasventanilla_cs where id = $_GET[folio]";
		$r = mysql_query($s,$l) or die($s);
		$f = mysql_fetch_object($r);
		$condicionpago = $f->condicionpago;
		
		$var_sucursalorigen = $f->idsucursalorigen;
		$var_sucursaldestino = $f->idsucursaldestino;
		...
		if($condicionpago==1){
			$s = "CALL proc_RegistroCobranza('CANCELARVENTA', '$f->folioguia', 'VENTANILLA', '', 0, 0);";
			$r = mysql_query($s,$l) or die("<xml version=\"1.0\" standalone=\"yes\" encoding=\"utf-8\"> 
					<datos>
					<guardado>0</guardado>
					<guardado>$s</guardado>
		...
		(SELECT idsucursal FROM catalogosucursal WHERE id = '$var_sucursalorigen')) AS refe), 
		noguia = '$newfolio', tipoguia = 'V', origen = '$var_sucursalorigen', destino = '$var_sucursaldestino'";
		$r = @mysql_query($s,$l) or die("<xml version=\"1.0\" standalone=\"yes\" encoding=\"utf-8\"> 
					<datos>
					<guardado>0</guardado>
					<consulta>".str_replace("''","null",$s)."</consulta>
		...
		SELECT null,'$newfolio',descripcion,contenido,peso,paquete,depaquetes,estado,'$estado',unidad,ubicacion,codigobarras
