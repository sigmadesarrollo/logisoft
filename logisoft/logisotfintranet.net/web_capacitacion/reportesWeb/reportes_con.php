<?	session_start();
	...
	if($_GET[accion]==1){//OBTENER ESTADO DE CUENTA	
		...
		FROM( /* cargos */
		SELECT ge.fecha,SUM($if) AS cargos,0 AS abonos,SUM($if) AS saldo
		FROM guiasempresariales ge 
		INNER JOIN tmp_convenio$x tc ON ge.idremitente = tc.idcliente
		...
		AND '".cambiaf_a_mysql($_GET[fechafin])."' $sucursal_filtro
		UNION
		SELECT gv.fecha,SUM(gv.total) AS cargos,0 AS abonos,SUM(gv.total) AS saldo
		FROM guiasventanilla gv	
		INNER JOIN pagoguias pg ON gv.id = pg.guia
		...
		UNION
		SELECT f.fecha,SUM(f.total + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)) AS cargos,0 AS abonos,
		...
		UNION
		SELECT f.fecha,SUM(fd.total) AS cargos,0 AS abonos,SUM(fd.total) AS saldo
		FROM facturacion f 
		INNER JOIN facturadetalle fd ON f.folio=fd.factura
		INNER JOIN tmp_clientes$x temp ON f.cliente=temp.idcliente
		WHERE f.credito='SI' AND ISNULL(f.fechacancelacion) AND temp.idcliente=$_GET[cliente] AND f.tipoguia!='empresarial'
		AND f.fecha BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql($_GET[fechafin])."' $sucursal_filtro3
		UNION	/* abonos */
		SELECT fp.fecha,0 AS cargos,SUM(fp.total) AS abonos,SUM(fp.total) AS saldo
		FROM formapago fp 
		...
		UNION
		SELECT f.fecha,cs.prefijo AS sucursal,f.folio AS refcargo,0 AS refabono,SUM(IFNULL(f.total,0) + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)) AS cargos,
		0 AS abonos,SUM(IFNULL(f.total,0) + IFNULL(f.sobmontoafacturar,0) + IFNULL(f.otrosmontofacturar,0)) AS saldo,'' AS descripcion
		FROM facturacion f 
		...
		WHERE (fp.procedencia='A' OR fp.procedencia='C') AND temp.idcliente=$_GET[cliente]
		AND fp.fecha BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql($_GET[fechafin])."' $sucursal_filtro4  
		GROUP BY fp.guia)t1 ORDER BY fecha $limite";
		$r = mysql_query($s,$l) or die($s."\n".mysql_error($l));
		$ar = array();
		...
		IFNULL(f.otrosmontofacturar,0)),0)) AS alcorriente, SUM(IFNULL(f.total,0) + IFNULL(f.sobmontoafacturar,0) +
		...
		SUM(ge.texcedente) AS sobrepeso,SUM($if) AS saldo,0 AS factura,IFNULL(ge.acuserecibo,0)AS contrarecibo,tc.idcliente, '' AS tcargo 
		FROM guiasempresariales ge 
		INNER JOIN tmp_convenio$x tc ON ge.idremitente = tc.idcliente
		INNER JOIN catalogosucursal cs ON tc.idsucursal=cs.id
		INNER JOIN tmp_clientes$x temp ON tc.idcliente=temp.idcliente
		WHERE ge.tipopago='CREDITO' AND (ge.tipoguia='CONSIGNACION' OR (ge.tipoguia='PREPAGADA' AND (ge.texcedente>0 OR ge.tseguro>0))) 
		AND ISNULL(ge.factura) $andsucursal1 $folioCliente GROUP BY ge.id
		UNION
		...
		SUM(gv.texcedente) AS sobrepeso,SUM(gv.total) AS saldo,0 AS factura,IFNULL(gv.acuserecibo,0)AS contrarecibo,pg.cliente,
		...
		SUM(fd.excedente) AS sobrepeso,SUM(fd.total) AS saldo,IFNULL(f.folio,'')AS factura,0 AS contrarecibo,f.cliente, '' AS tcargo
		FROM facturacion f 
		INNER JOIN facturadetalle fd ON f.folio=fd.factura
		INNER JOIN guiasventanilla gv ON fd.folio=gv.id
		INNER JOIN catalogosucursal cs ON f.idsucursal=cs.id
		INNER JOIN tmp_clientes$x temp ON f.cliente=temp.idcliente
		WHERE f.credito='SI' AND ISNULL(f.fechacancelacion) AND f.estadocobranza <> 'C' AND f.tipoguia!='empresarial' $andsucursal3 $folioCliente
		GROUP BY fd.folio)t ORDER BY cliente,idcliente,fechaguia $limite";
		$r = mysql_query($s,$l) or die($s);
		$ar = array();
		while($f = mysql_fetch_object($r)){
			$f->cliente = cambio_texto($f->cliente);
