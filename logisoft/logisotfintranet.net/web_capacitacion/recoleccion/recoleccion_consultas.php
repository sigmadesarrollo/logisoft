<?	session_start();
	...
	if($_GET[accion]=="0"){//BORRAR TEMPORALES
		...
		if($_GET[tip]=="grabar"){
			...
			(FolioBitacora,folio,sucursal,fecharegistro,estado,origen,
			 destino,npedidos,dirigido,chnombre,llama,telefono,
			 comentarios,cliente,calle,numero,crucecalles,cp,
			 colonia,poblacion,municipio,telefono2,sector,horario,
			 horario2,hrcomida,hrcomida2,unidad,usuario,fecha)
			values 
			('".$_GET[foliobitacora]."','".$_GET[folio]."','".$_GET[idsucursal]."',
			'".cambiaf_a_mysql($_GET[fecha])."',
			'".$_GET[estado_hidden]."','".trim($_GET[origen_hidden])."',
			'".trim($_GET[destino_hidden])."','".$_GET[npedidos]."',
			UCASE('".$_GET[dirigido]."'),'".$_GET[chNombre]."',
			UCASE('".$_GET[llama]."'),'".$_GET[telefono]."',
			UCASE('".$_GET[comentarios]."'),'".$_GET[cliente]."',
			UCASE('".$_GET[calle]."'),UCASE('".$_GET[numero]."'),
			UCASE('".$_GET[crucecalles]."'),'".$_GET[cp]."',
			UCASE('".$_GET[colonia]."'),UCASE('".$_GET[poblacion]."'),
			UCASE('".$_GET[municipio]."'),'".$_GET[telefono2]."',
			UCASE('".$_GET[sector]."'),'".$_GET[horario_hidden]."',
			'".$_GET[horario2_hidden]."','".$_GET[comida_hidden]."',
			'".$_GET[comida2_hidden]."',UCASE('".$_GET[unidad]."'),
			'".$_SESSION[NOMBREUSUARIO]."',CURRENT_TIMESTAMP())";
			mysql_query($s,$l) or die($s);
			$id = mysql_insert_id($l);

			$s = "UPDATE bitacorasalida SET FolioRecoleccion='".$_GET[folio]."'
			WHERE folio= ".$_GET[foliobitacora]."";		
			mysql_query($s,$l) or die($s);
			
			$s = "select * from recoleccion where folio =".$_GET[folio];
			$r = mysql_query($s,$l) or die($s);
			$f = mysql_fetch_object($r);
			$folio = $f->folio;
			
			$s = "INSERT INTO recolecciondetalle
			SELECT 0 AS id, '".$_GET[folio]."' AS recoleccion, '".$_GET[idsucursal]."' AS sucursal,cantidad,
			iddescripcion,descripcion,contenido,peso,largo,ancho,alto,volumen,
			...
			$s = "call proc_VentasVsPresupuesto('OP REPARTO','$folio',$_SESSION[IDSUCURSAL]);";
			$r = mysql_query($s,$l) or die("$s");
			
			$s = "CALL proc_ReporteProductividad('RECOLECCION','".$_GET['unidad']."',
			'".$folio."', current_timestamp(), ".$_GET[idsucursal].")";
			mysql_query($s,$l) or die($s);
			...
			estado = '".$_GET[estado_hidden]."',origen = '".trim($_GET[origen_hidden])."',
			destino = '".trim($_GET[destino_hidden])."', npedidos = '".$_GET[npedidos]."',
			dirigido= UCASE('".$_GET[dirigido]."'), chnombre='".$_GET[chNombre]."',
			llama = UCASE('".$_GET[llama]."'), telefono = '".$_GET[telefono]."',
			comentarios = UCASE('".$_GET[comentarios]."'),cliente='".$_GET[cliente]."',
			calle=UCASE('".$_GET[calle]."'), numero=UCASE('".$_GET[numero]."'),
			crucecalles=UCASE('".$_GET[crucecalles]."'), cp='".$_GET[cp]."',
			colonia = UCASE('".$_GET[colonia]."'), poblacion=UCASE('".$_GET[poblacion]."'),
			municipio=UCASE('".$_GET[municipio]."'), telefono2='".$_GET[telefono2]."',
			sector=UCASE('".$_GET[sector]."'),horario='".$_GET[horario_hidden]."',
			horario2='".$_GET[horario2_hidden]."',hrcomida='".$_GET[comida_hidden]."',
			hrcomida2='".$_GET[comida2_hidden]."', unidad=UCASE('".$_GET[unidad]."'), 
			usuario='".$_SESSION[NOMBREUSUARIO]."',fecha=current_timestamp()
			WHERE folio='".$_GET[folioant]."' AND sucursal=".$_GET[sucursalant]."";
			mysql_query($s,$l) or die($s);

			$s = "UPDATE bitacorasalida SET FolioRecoleccion='".$_GET[folio]."'
			WHERE FolioRecoleccion= '".$_GET[folio]."'";		
			mysql_query($s,$l) or die($s);
			...
			echo "modifico,".(($_GET[idsucursal]!=$_GET[sucursalant])?$folio:$_GET[folioant]);
		}
		...
		$s = "UPDATE recoleccion SET estado='CANCELADO' and FolioBitacora = 0
		WHERE folio='".$_GET['folio']."' AND sucursal=".$_GET[idsucursal];
		mysql_query($s,$l) or die($s);
		
			$s = "UPDATE bitacorasalida SET FolioRecoleccion= NULL
			WHERE FolioRecoleccion='".$_GET['folio']."'";		
			mysql_query($s,$l) or die($s);
