<?
	...
	if($_GET[accion] == 1){
		...
		if(mysql_num_rows($r)>0){
			...
			$cliente = "{
				'nombre':'".cambio_texto(strtoupper($f->nombre))."',
				'paterno':'".cambio_texto(strtoupper($f->paterno))."',
				'materno':'".cambio_texto(strtoupper($f->materno))."',
				'rfc':'".cambio_texto(strtoupper($f->rfc))."',
				'celular':'".cambio_texto($f->celular)."',
				'foliocredrito':'".cambio_texto($f->foliocredrito)."',
			...
			if(mysql_num_rows($rx)>0){
				...
				while($fx = mysql_fetch_object($rx)){
					$fx->id = cambio_texto(strtoupper($fx->id));
					...
					$fx->crucecalles = cambio_texto(strtoupper($fx->crucecalles));
					$fx->municipio = cambio_texto(strtoupper($fx->municipio));
					$fx->pais = cambio_texto(strtoupper($fx->pais));
					$fx->estado = cambio_texto(strtoupper($fx->estado));
					$fx->fax = cambio_texto(strtoupper($fx->fax));
					$fx->facturacion = cambio_texto(strtoupper($fx->facturacion));
					$fx->municipio = cambio_texto(strtoupper($fx->municipio));
	...
	if($_GET[accion] == 2){
		...
		AND sge.estado = 'GUARDADA'  AND sge.sucursalacobrar=$_GET[sucorigen]
