...
<script>
	...
	function calcularservicios(){
		...
		var templaye 		= parseFloat((u.txtemplaye.valuex=="")?0:u.txtemplaye.valuex);

		var tbolsaempaque	= parseFloat((u.txtbolsaempaque2.valuex=="")?0:u.txtbolsaempaque2.valuex);

		var tavisocelular	= parseFloat((u.txtavisocelular1.valuex=="")?0:u.txtavisocelular1.valuex);

		var tdeclarado		= parseFloat((u.txtdeclarado.valuex=="")?0:u.txtdeclarado.valuex);

		var tacuserecibo	= parseFloat((u.txtacuserecibo.valuex=="")?0:u.txtacuserecibo.valuex);

		var tcod			= parseFloat((u.txtcod.valuex=="")?0:u.txtcod.valuex);

		

		valorx					= templaye+tbolsaempaque+tavisocelular+tdeclarado+tacuserecibo+tcod;



		u.t_txtotros.value		= numcredvar(valorx.toLocaleString());

		u.t_txtotros.valuex		= valorx;
	...
	function calculartotales(){
		...
		var ptflete 			= parseFloat((u.flete.valuex=="")?0:u.flete.valuex);

		var ptdescuento 		= parseFloat((u.t_txtdescuento2.valuex=="")?0:u.t_txtdescuento2.valuex);

		var ptead 				= parseFloat((u.t_txtead.valuex=="")?0:u.t_txtead.valuex);

		var ptrecoleccion 		= parseFloat((u.t_txtrecoleccion.valuex=="")?0:u.t_txtrecoleccion.valuex);

		var ptseguro	 		= parseFloat((u.t_txtseguro.valuex=="")?0:u.t_txtseguro.valuex);

		var ptotros		 		= parseFloat((u.t_txtotros.valuex=="")?0:u.t_txtotros.valuex);

		var ptexcedente			= parseFloat((u.t_txtexcedente.valuex=="")?0:u.t_txtexcedente.valuex);

		var ptcombustible		= parseFloat((u.t_txtcombustible.valuex=="")?0:u.t_txtcombustible.valuex);

		

		var ptsubtotal			= ptflete+ptdescuento+ptead+ptrecoleccion+ptseguro+ptotros+ptexcedente+ptcombustible;

		valorx					= Math.round(ptsubtotal*100)/100;

		u.t_txtsubtotal.value	= numcredvar(valorx.toLocaleString());		

		valory					= Math.round((ptsubtotal*(parseFloat(u.pc_iva.value)/100))*100)/100;

		u.t_txtiva.value		= numcredvar(valory.toLocaleString());

		valorz					= Math.round( (ptsubtotal*(parseFloat(u.pc_ivaretenido.value)/100))*100) /100;

		u.t_txtivaretenido.value= numcredvar(valorz.toLocaleString());

		u.t_txttotal.value		= Math.round((ptsubtotal+valory+valorz )*100)/100;
	...
	function devolverDatosEvaluacion(datos){
		...
		if(encon>0){
			...
			u.txtemplayeh.value = numcredvar(totalemplaye.toLocaleString());

			u.txtbolsaempaque1h.value = cantidadbolsa;

			u.txtbolsaempaque2h.value = numcredvar(totalbolsaempaque.toLocaleString());
			...
			if(emplaye==1){
				...
				u.txtemplaye.value = numcredvar(totalemplaye.toLocaleString());

				u.txtemplaye.valuex = totalemplaye;
			...
			if(bolsaempaque==1){
				...
				u.txtbolsaempaque2.value = numcredvar(totalbolsaempaque.toLocaleString());

				u.txtbolsaempaque2.valuex= totalbolsaempaque;
			...
			if(erecoleccion!=0){
				...
				u.txtrecoleccionh.value		= numcredvar(erecoleccion.toLocaleString());	

				u.txtrecoleccion.value		= numcredvar(erecoleccion.toLocaleString());

				u.txtrecoleccion.valuex		= erecoleccion;
			...
			u.txtavisocelular1h.value 	= numcredvar(avisocelular.toLocaleString());

			u.txtavisocelular1.valuex 	= avisocelular;

			u.txtacusereciboh.value 	= numcredvar(acuserecibo.toLocaleString());

			u.txtacuserecibo.valuex 	= acuserecibo;

			u.txtcodh.value 			= numcredvar(cod.toLocaleString());
			...
			if(enconeva>0){
				...
				u.totalpaquetes.valuex 	= tpaquetes;

				u.totalpeso.valuex 		= tpeso;

				u.totalvolumen.valuex 	= tvolumen;

				u.flete.valuex			= timporte;

				u.flete.value			= numcredvar(timporte.toLocaleString());

				u.totalpaquetes.value 	= numcredvar(tpaquetes.toLocaleString());

				u.totalpeso.value 		= numcredvar(tpeso.toLocaleString());

				u.totalvolumen.value 	= numcredvar(tvolumen.toLocaleString());
			...
			if((parseFloat(u.flete.value)*0.10)<parseFloat(u.pc_ead.value)){
				...
				u.t_txtead.value = numcredvar(u.pc_ead.value.toLocaleString());

				u.t_txtead.valuex = u.pc_ead.value;

			}else{

				valorx = Math.round(((parseFloat(u.flete.value)-parseFloat((u.t_txtdescuento2.value=="")?0:u.t_txtdescuento2.value))*.10)*100)/100;

				u.t_txtead.value = numcredvar(valorx.toLocaleString());

				u.t_txtead.valuex = Math.round(((parseFloat(u.flete.value)-parseFloat((u.t_txtdescuento2.value=="")?0:u.t_txtdescuento2.value))*.10)*100)/100;
			...
			if((parseFloat(u.flete.value)*0.10)<parseFloat(u.pc_recoleccion.value)){
				...
				u.t_txtrecoleccion.value = numcredvar(u.pc_recoleccion.value.toLocaleString());

				u.t_txtrecoleccion.valuex = u.pc_recoleccion.value;

			}else{

				valorx = Math.round(((parseFloat(u.flete.value)-parseFloat((u.t_txtdescuento2.value=="")?0:u.t_txtdescuento2.value))*.10)*100)/100;

				u.t_txtrecoleccion.value = numcredvar(valorx.toLocaleString());

				u.t_txtrecoleccion.valuex = Math.round(((parseFloat(u.flete.value)-parseFloat((u.t_txtdescuento2.value=="")?0:u.t_txtdescuento2.value))*.10)*100)/100;
			...
			if(u.txtdeclarado.value!="" && u.txtdeclarado.value!="0"){
				...
				if(parseFloat(u.txtdeclarado.value)<parseFloat(u.pc_porcada.value)){
					...
					u.t_txtseguro.valuex = u.pc_costo.value;

				}else{

					valorx = Math.round(((parseFloat(u.txtdeclarado.value)/parseFloat(u.pc_porcada.value))*parseFloat(u.pc_costo.value))*100)/100;

					u.t_txtseguro.value = numcredvar(valorx.toLocaleString());

					u.t_txtseguro.valuex = Math.round(((parseFloat(u.txtdeclarado.value)/parseFloat(u.pc_porcada.value))*parseFloat(u.pc_costo.value))*100)/100;
			...
			valorx = Math.round(((parseFloat(u.flete.value)-parseFloat((u.t_txtdescuento2.value=="")?0:u.t_txtdescuento2.value))*(parseFloat(u.pc_tarifacombustible.value)/100))*100)/100;

			u.t_txtcombustible.value = numcredvar(valorx.toLocaleString());

			u.t_txtcombustible.value = valorx;
              ...
              <td><table width="185" height="0" border="0" align="right" cellpadding="0" cellspacing="0" bordercolor="#016193">
                  ...
                  <tr>
                    ...
                    <td><label>
                      ...
                      <textarea name="txtrestrinccion" style="width:180px; font-size:9px; text-transform:uppercase"></textarea>
        ...
        <td valign="top"><table width="100%" height="76" border="0" cellpadding="0" cellspacing="0">
            ...
            <tr>
              ...
              <td class="Tablas">Valor Declarado
                ...
                <input name="txtdeclarado" type="text" readonly="true" onBlur=" calculartotales();" onKeyPress="if(event.keyCode==13){ calculartotales();}" style="background:#FFFF99;font:tahoma; font-size:9px; text-align:right" valuex="0" value="<?=$rrfc ?>" size="10" /></td>
      ...
      <tr>
        ...
        <td width="400"><table width="400" border="0" cellpadding="0" cellspacing="0" bordercolor="#016193">
            ...
            <tr>
              ...
              <td><textarea name="textarea2" style="width:400px; font-size:9px; font:tahoma; text-transform:uppercase"></textarea></td>
