<?
	...
	if($_POST[guardar_hidden]==1){
		...
		for($i=1; $i<$_POST[cantidadzonas]; $i++){
			...
			where zona = $i and tipo='CONVENIO' and idusuario = $_SESSION[IDUSUARIO] and isnull(idconvenio)";

			mysql_query(str_replace("''","null",$s),$l);

			

		}

		$_POST[tienedatoskg]=1;
	...
	if($_GET[idconvenio]){
		...
		if($_GET[tienedatoskg]!=1 && $_POST[tienedatoskg]!= 1){

			$s = "delete from convenio_configurador_preciokg where idusuario = $_SESSION[IDUSUARIO] and tipo = 'CONVENIO' and isnull(idconvenio)";
		...
		$s = "select * from convenio_configurador_preciokg where idusuario = $_SESSION[IDUSUARIO] and tipo='CONVENIO' and isnull(idconvenio) order by zona";
		...
		if($sihaytabla<1){
			...
			SELECT NULL, cd.columna, zoi, zof, null, 'CONVENIO', '$_SESSION[NOMBREUSUARIO]', $_SESSION[IDUSUARIO], CURRENT_DATE
      ...
      <tr>
        ...
        <td colspan="2"><table width="535" border="0" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <input type="hidden" name="tienedatoskg" value="<?=($_GET[tienedatoskg]!="")?$_GET[tienedatoskg]:$_POST[tienedatoskg]?>">
...
<script>
	...
	parent.document.all.div_preciokg.innerHTML = document.all.divdatos.innerHTML;

	parent.document.all.tienedatoskg.value = 1;

	parent.document.all.tienedatoskg_excedio.value = <?=$excedio?>;
