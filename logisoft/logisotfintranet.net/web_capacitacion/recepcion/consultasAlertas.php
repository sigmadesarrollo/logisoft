<?	session_start();
	...
	if($_GET[accion]==1){//Atrasos Embarques
		...
		INNER JOIN catalogocliente r ON ge.idremitente = r.id
		INNER JOIN catalogocliente de ON ge.idremitente = de.id
		WHERE ge.estado = 'ALMACEN ORIGEN' AND ge.idsucursaldestino = ".$_SESSION[IDSUCURSAL]."";		
		if($_GET[tipo]=="0"){
			$r = mysql_query($s,$l) or die($s);
			echo mysql_num_rows($r);
		}else if($_GET[tipo]=="1"){
	...
	}else if($_GET[accion]=="ultimomercancia"){
		$s = "SELECT COUNT(*) AS total FROM guiasventanilla		
		WHERE estado = 'ALMACEN ORIGEN' AND idsucursaldestino = ".$_SESSION[IDSUCURSAL]."
		UNION
		...
		$s = "SELECT * FROM (
		SELECT gv.id AS guia, DATE_FORMAT(gv.fecha,'%d/%m/%Y') AS fecha, o.descripcion AS origen,
		CONCAT_WS(' ',r.nombre,r.paterno,r.materno) AS remitente,
		CONCAT_WS(' ',de.nombre,de.paterno,de.materno) AS destinatario,
		IF(gv.tipoflete=0,'PAGADO','POR COBRAR') AS tipoflete, gv.total,
		IF(gv.tipoflete=0,'EAD','OCURRE') AS tipoentrega
		...
		$s = "SELECT folio, DATE_FORMAT(fecha,'%d/%m/%Y') AS fechaemision, 
		CONCAT_WS(' ',nombrecliente,apellidopaternocliente,apellidopaternocliente) AS cliente,
		SUM(total + sobmontoafacturar + otrosmontofacturar) AS importe FROM facturacion 
		WHERE credito = 'SI' AND tipofactura = 'NORMAL'
		".(($_SESSION[IDSUCURSAL]!=1)? " AND idsucursal = $_SESSION[IDSUCURSAL]" : "")."
		HAVING folio IS NOT NULL";
		$r = mysql_query($s,$l) or die($s);
		$totalregistros = mysql_num_rows($r);		
		
		$totales = 0;
		
		$s = "SELECT folio AS factura, DATE_FORMAT(fecha,'%d/%m/%Y') AS fechaemision, 
		CONCAT_WS(' ',nombrecliente,apellidopaternocliente,apellidopaternocliente) AS cliente,
		SUM(total + sobmontoafacturar + otrosmontofacturar) AS importe FROM facturacion 
		WHERE credito = 'SI' AND tipofactura = 'NORMAL'
		".(($_SESSION[IDSUCURSAL]!=1)? " AND idsucursal = $_SESSION[IDSUCURSAL]" : "")."		
		HAVING factura IS NOT NULL
		$limite";
		$r = mysql_query($s,$l) or die($s);
		$arr = array();
		while($f = mysql_fetch_object($r)){
		...
		AND g.fecha BETWEEN '".cambiaf_a_mysql($_GET[fechainicio])."' AND '".cambiaf_a_mysql($_GET[fechafin])."'
