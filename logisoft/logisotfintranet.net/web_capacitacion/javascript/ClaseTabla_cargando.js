...
function ClaseTabla(nombre, campos, tamano){
	...
	this.showCargando		= 	f_showCargando;
	...
	function f_setJsonData(datos, vobjeto){
		...
		try{
			...
			for(var i=0; i<cantidad; i++){
					for(var j=0; j<objeto.campos.length; j++){
						...
						if(this.attributes.campos[j].tipo=="checkbox"){
							...
							if(datos[i][objeto.campos[j].datos.toLocaleString()]=='S'){
								document.all[objeto.nombre + "_" + objeto.campos[j].nombre][this.indiceAgregados].style.visibility='visible';
								document.all[objeto.nombre + "_" + objeto.campos[j].nombre][this.indiceAgregados].checked = true;
							}
						}else{
			...
			alert("Error Clase Tabla f_setJsonData: " + e.description);
		}
	}
			...
			if(tbody.rows.length<objeto.filasInicial){
				...
				for(var i=0; i<objeto.campos.length; i++){
					...
					if(objeto.campos[i].onClick != undefined){
						onclick = "onclick='"+objeto.campos[i].onClick+((objeto.campos[i].tipo=="checkbox")?"(this.checked,"+j+")'":"(this.value,"+j+")'");
					}
					var ondblclick = "";
					if(objeto.campos[i].onDblClick != undefined){
						ondblclick = "ondblclick='"+objeto.campos[i].onDblClick+((objeto.campos[i].tipo=="checkbox")?"(this.checked,"+j+")'":"(this.value,"+j+")'");
					}
		...
		if(indice<=this.jsonData.datos.length){
			for(var i=0; i<objeto.campos.length; i++){
				if(objeto.campos[i].tipo=="checkbox"){
					...
					if(fila[objeto.campos[i].datos]=='S'){
						document.all[objeto.nombre + "_" + objeto.campos[i].nombre][indice].style.visibility = "visible";
						document.all[objeto.nombre + "_" + objeto.campos[i].nombre][indice].checked = true;
					}
				}else{
	...
	function f_clear(valor){
		...
		div.innerHTML = "<div id='"+objeto.nombre+"_cargando_div' style='float:left; position:absolute; border:1px solid #1C9BB7; background-color:#CCC; font-weight:bold; color:#29578F; width:80px; height:20px; padding-top:8px; padding-left:5px; visibility:hidden'>CARGANDO...</div>"
		+"<table id='"+objeto.nombre+"_tabla' border='0' cellpadding='0' cellspacing='0'></table>";
		try{
		if(valor == undefined){
			this.jsonData.datos = new Array();
			this.indiceAgregados = 0;
	...
	function f_showCargando(valor){
		var objeto = this.attributes;
		document.getElementById(objeto.nombre+"_cargando_div").style.visibility=((valor)?'visible':'hidden');
	}
}