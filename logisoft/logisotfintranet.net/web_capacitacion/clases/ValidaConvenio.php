<?
	...
	class ValidaConvenio{
		...
		var $remitente_scredito			= "";
		var $destinatario_scredito		= "";
		...
		function ValidaConvenio($idremitente, $iddestinatario, $iddestino, $idsucdestino, $idsucursalorigen=null){	
			...
			if($idsucursalorigen==null){
				$this->idsucursalorigen = $_SESSION[IDSUCURSAL];
			}
			...
			if($this->idremitente!=""){
				...
				consignacionkg, consignacioncaja,consignaciondescuento, consignaciondescantidad, 
				valordeclarado, porcada, limite, costoextra
				...
				if(mysql_num_rows($r)>0){
					...
					$this->rem_consignacionkg = $f->consignacionkg;
					$this->rem_consignacioncaja = $f->consignacioncaja;
					$this->rem_consignaciondescuento = $f->consignaciondescuento;
					$this->rem_consignaciondescantidad = $f->consignaciondescantidad;
					
					$this->rem_valordeclarado = $f->valordeclarado;
					$this->rem_limite = $f->limite;
					$this->rem_porcada = $f->porcada;
					$this->rem_costoextra = $f->costoextra;
					...
					$this->rem_consignacionkg = 0;
					$this->rem_consignacioncaja = 0;
					$this->rem_consignaciondescuento = 0;
					$this->rem_consignaciondescantidad = 0;
					
					$this->rem_valordeclarado = 0;
					$this->rem_limite = 0;
					$this->rem_porcada = 0;
					$this->rem_costoextra = 0;
				...
				$s = "SELECT scd.idsucursal, scd.sucursal 
				FROM solicitudcreditosucursaldetalle scd
				INNER JOIN solicitudcredito sc ON sc.folio = scd.solicitud
				WHERE sc.cliente = $this->idremitente AND sc.estado = 'ACTIVADO'";
				$r = mysql_query($s,$l) or die($s);
				$arre = array();
				if(mysql_num_rows($r)>0){
					...
					$this->remitente_scredito = json_encode($arre);
				}else{
					$this->remitente_scredito = "''";
				}
			}
			...
			if($this->iddestinatario!=""){	
				...
				consignacionkg, consignacioncaja,consignaciondescuento, consignaciondescantidad,  
				valordeclarado, porcada, limite, costoextra
				FROM generacionconvenio WHERE idcliente = $this->iddestinatario AND 
				vigencia > CURRENT_DATE AND generacionconvenio.estadoconvenio = 'ACTIVADO'";
				$r = mysql_query($s,$l) or die($s);
				if(mysql_num_rows($r)>0){
					$f = mysql_fetch_object($r);
					...
					$this->des_consignacionkg = $f->consignacionkg;
					$this->des_consignacioncaja = $f->consignacioncaja;
					$this->des_consignaciondescuento = $f->consignaciondescuento;
					$this->des_consignaciondescantidad = $f->consignaciondescantidad;
					
					$this->des_valordeclarado = $f->valordeclarado;
					$this->des_limite = $f->limite;
					$this->des_porcada = $f->porcada;
					$this->des_costoextra = $f->costoextra;
					...
					$this->des_consignacionkg = 0;
					$this->des_consignacioncaja = 0;
					$this->des_consignaciondescuento = 0;
					$this->des_consignaciondescantidad = 0;
					
					$this->des_valordeclarado = 0;
					$this->des_limite = 0;
					$this->des_porcada = 0;
					$this->des_costoextra = 0;
				...
				FROM solicitudcreditosucursaldetalle scd
				INNER JOIN solicitudcredito sc ON sc.folio = scd.solicitud
				WHERE sc.cliente = $this->iddestinatario AND sc.estado = 'ACTIVADO'";
				$r = mysql_query($s,$l) or die($s);
				$arre = array();
				if(mysql_num_rows($r)>0){
					while($f = mysql_fetch_object($r)){
					...
					$this->destinatario_scredito = json_encode($arre);
				}else{
					$this->destinatario_scredito = "''";
				}
			}
			...
			if($this->iddestino!=""){
				...
				FROM catalogodistancias WHERE (idorigen=".$this->idsucursalorigen." AND iddestino=".$this->idsucdestino.") 
				OR (iddestino=".$this->idsucursalorigen." AND idorigen=".$this->idsucdestino.")) BETWEEN zoi AND zof
		...
		function getJsonDataVentanilla(){
			...
			if( $this->idremitente !=""){
				...
				$res .= "datosremitente:{
					   ...
					   consignacionkg:'$this->rem_consignacionkg',
					   consignacioncaja:'$this->rem_consignacioncaja',
					   consignaciondescuento:'$this->rem_consignaciondescuento',
					   consignaciondescantidad:'$this->rem_consignaciondescantidad',
					   
					   vendedorconvenio:'$this->rem_nvendedor',
					   
					   vdcosto: '$this->rem_valordeclarado',
					   vdporcada: '$this->rem_porcada',
					   vdlimite: '$this->rem_limite',
					   vdcostoextra: '$this->rem_costoextra',
					   ...
					   sucursalescredito:$this->remitente_scredito,
			...
			if( $this->iddestinatario !=""){
				...
				$res .= "
				  datosdestinatario:{
					   ...
					   consignacionkg:'$this->des_consignacionkg',
					   consignacioncaja:'$this->des_consignacioncaja',
					   consignaciondescuento:'$this->des_consignaciondescuento',
					   consignaciondescantidad:'$this->des_consignaciondescantidad',
					   
					   vendedorconvenio:'$this->des_nvendedor',
					   
					   vdcosto: '$this->des_valordeclarado',
					   vdporcada: '$this->des_porcada',
					   vdlimite: '$this->des_limite',
					   vdcostoextra: '$this->des_costoextra',
					   ...
					   sucursalescredito:$this->destinatario_scredito,
		...
		function obtenerExcedenteEmp($convenio, $idorigen, $iddestino, $descripcion, $peso, $cant_merc, $folio=null){
			$this->convenio 		= $convenio;
			$this->idorigen			= $idorigen;
			$this->iddestino		= $iddestino;
			$this->descripcion		= $descripcion;
			...
			$excedente = 0;	
				...
				$s = "SELECT IF(prepagada='SI', 'P', 'C') AS tipo 
				FROM solicitudguiasempresariales WHERE '$folio' 
				BETWEEN desdefolio AND hastafolio";
				$rfe = mysql_query($s,$l) or die($s);
				$fse = mysql_fetch_object($rfe); 
				if($fse->tipo == 'P'){
					$excedekg=0;
					//se borro no es asi
		...
		function ObtenerFleteEmp($convenio, $idorigen, $iddestino, $descripcion, $peso, $cant_merc, $folio=null){
			$this->convenio 		= $convenio;
			$this->idorigen			= $idorigen;
			$this->iddestino		= $iddestino;
			$this->descripcion		= $descripcion;
