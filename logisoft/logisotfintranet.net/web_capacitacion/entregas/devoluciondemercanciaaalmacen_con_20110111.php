<?	session_start();
	...
	if($_GET[accion]==3){
		...
		$s = "UPDATE guiasventanilla SET fechaentrega = CURRENT_DATE, estado = 'ENTREGADA POR LIQUIDAR' WHERE id in($cadenas_folios)";
		mysql_query($s,$l) or die($s);

		$s = "UPDATE guiasempresariales SET fechaentrega = CURRENT_DATE, estado = 'ENTREGADA POR LIQUIDAR'  WHERE id in($cadenas_folios)";
		mysql_query($s,$l) or die($s);
		
		$s = "UPDATE guiasventanilla SET fechaentrega = CURRENT_DATE, estado = 'ALMACEN DESTINO', entradasalida = 'ENTRADA'  WHERE id in($cadenas_folios2)";
		mysql_query($s,$l) or die($s);

		$s = "UPDATE guiasempresariales SET fechaentrega = CURRENT_DATE, estado = 'ALMACEN DESTINO', entradasalida = 'ENTRADA'  WHERE id in($cadenas_folios2)";
		mysql_query($s,$l) or die($s);
