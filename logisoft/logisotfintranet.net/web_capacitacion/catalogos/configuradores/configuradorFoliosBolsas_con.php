<?	session_start();
	...
	if($_GET[accion]==1){
		$s = "SELECT folioinicial as finicial, foliofinal as ffinal, cantidad, 'x' AS existe FROM configuradorfoliosbolsas";
		...
		/*$s = "SELECT * FROM asignacionbolsas";
		$r = mysql_query($s,$l) or die($s);
		if(mysql_num_rows($r)>0){
			$s = "SELECT IFNULL(MAX(foliofinal),0) + 1 as foliofinal FROM asignacionbolsas";
		...
		$s = "SELECT * FROM asignacionbolsas WHERE folioinicial BETWEEN '".$_GET[finicial]."' AND '".$_GET[ffinal]."'";
		$r = mysql_query($s,$l) or die($s);
		if(mysql_num_rows($r)>0){
			die('folio inicial entre');
		}
		
		$s = "SELECT * FROM asignacionbolsas WHERE foliofinal BETWEEN '".$_GET[finicial]."' AND '".$_GET[ffinal]."'";
		$r = mysql_query($s,$l) or die($s);
		if(mysql_num_rows($r)>0){
		...
		while($_GET[cantidad]>0){
			$s = "SELECT id, folioinicial, foliofinal, restante FROM configuradorfoliosbolsas WHERE restante > 0";
			$r = mysql_query($s,$l) or die($s);
			if(mysql_num_rows($r)>0){
					$f = mysql_fetch_object($r);
				if($f->restante >= $_GET[cantidad]){
					$upd = mysql_query("UPDATE configuradorfoliosbolsas SET
					restante= restante - $_GET[cantidad] WHERE id=$f->id",$l);					
					$_GET[cantidad] = 0;
				}else{
					$upd = mysql_query("UPDATE configuradorfoliosbolsas SET restante = 0 WHERE id=$f->id",$l);					
		...
		if($_GET[cantidad]==0){
		$s = "INSERT INTO asignacionbolsas (fechaasignacion,sucursal,folioinicial,foliofinal,cantidad,usuario,fecha)
				 VALUES
		...
		for($i=$_GET[finicial];$i<=$_GET[ffinal];$i++){
			$s = "INSERT INTO asignacionbolsasdetalle
			...
			IFNULL(MAX(foliofinal),0) AS foliofinal FROM asignacionbolsas";
			$sq = mysql_query($s, $l) or die($s); $fol = mysql_fetch_object($sq);
		...
		$s = "SELECT IFNULL(SUM(restante),0) AS cantidad FROM configuradorfoliosbolsas";
		$r = mysql_query($s,$l) or die($s);
		$registros = array();	
		while($f = mysql_fetch_object($r)){
			$s = "SELECT IFNULL(MAX(folioinicial),0) AS folioinicial, 
		...
		FROM asignacionbolsas WHERE sucursal=".$_GET[sucursal]." ORDER BY fechaasignacion ASC";
		$r = mysql_query($s,$l) or die($s);
		$registros = array();
		if(mysql_num_rows($r)>0){
			while($f = mysql_fetch_object($r)){
